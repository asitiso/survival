...
export function heroGroundContactOwnershipPresentation(input, reducedMotion = false) {
    ...
    if (owner === 'locomotion') {
        locomotionGroundScale = 1;
        actionFollowScale = .58;
        maxX = 2.4;
        maxY = 1.8;
    }
    else if (owner === 'cast') {
        locomotionGroundScale = .58;
        actionFollowScale = .36;
        maxX = 3.1;
        maxY = 2;
    }
    else if (owner === 'evade') {
        locomotionGroundScale = .42;
        actionFollowScale = .18;
        alphaScale = .9;
        maxX = 2.2;
        maxY = 1.7;
    }
    else if (owner === 'ultimate') {
        locomotionGroundScale = .28;
        actionFollowScale = .12;
        maxX = 1.8;
        maxY = 1.5;
    }
    const liftWeight = clamp(lift / 10, 0, 1);
    if (owner === 'ultimate') {
        widthScale = 1 - liftWeight * .15;
        heightScale = 1 - liftWeight * .22;
        alphaScale *= 1 - liftWeight * .34;
    }
    else if (owner === 'cast') {
        widthScale = 1 - liftWeight * .055;
        heightScale = 1 - liftWeight * .08;
        alphaScale *= 1 - liftWeight * .1;
    }
    else if (owner === 'evade') {
        widthScale = 1 - liftWeight * .04;
        heightScale = 1 - liftWeight * .06;
        alphaScale *= 1 - liftWeight * .08;
    }
    if (reducedMotion) {
        locomotionGroundScale *= .82;
        actionFollowScale *= .62;
        maxX *= .72;
        maxY *= .72;
        widthScale = 1 + (widthScale - 1) * .7;
        heightScale = 1 + (heightScale - 1) * .7;
        alphaScale = Math.max(.62, alphaScale * .92);
    }
