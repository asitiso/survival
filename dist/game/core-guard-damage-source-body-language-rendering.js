...
export function coreGuardDamageSourceBodyLanguagePresentation(input, reducedFlash = false) {
    const sourceClass = input.source === 'projectile' ? 'projectile' : input.source === 'contact' ? 'contact' : 'other';
    ...
    return { sourceClass, visible, bodyScaleX, bodyScaleY, deflectAlpha: visible && sourceClass === 'projectile' ? .5 * accentScale : 0, contactRingAlpha: visible && sourceClass === 'contact' ? .42 * accentScale : 0, damageOwner: input.owner, duplicatesDamageCue: false, presentationOnly: true };
}
