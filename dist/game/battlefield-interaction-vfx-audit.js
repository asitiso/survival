...
import { BATTLEFIELD_INTERACTION_VFX_ATLAS, auditBattlefieldInteractionVfxAtlas, battlefieldCoreVisualState, battlefieldInteractionSprite } from './battlefield-interaction-vfx-assets.js';
function add(samples, id, expected, actual) { samples.push({ id, expected, actual, passed: Object.is(expected, actual) }); }
function inBounds(r) { return r.sx >= 0 && r.sy >= 0 && r.sw === 128 && r.sh === 128 && r.sx + r.sw <= BATTLEFIELD_INTERACTION_VFX_ATLAS.width && r.sy + r.sh <= BATTLEFIELD_INTERACTION_VFX_ATLAS.height; }
const ENTRIES = [
...
export function auditBattlefieldInteractionVfx() {
    ...
    for (const [group, id] of ENTRIES) {
        const r = battlefieldInteractionSprite(group, id);
        const key = `${r.sx}:${r.sy}`;
        add(samples, `${group}-${id}-bounds`, true, inBounds(r));
        add(samples, `${group}-${id}-unique`, false, seen.has(key));
        seen.add(key);
    }
    const thresholds = [[1, 'normal'], [.61, 'normal'], [.60, 'warning'], [.31, 'warning'], [.30, 'critical'], [0, 'critical']];
    for (const [ratio, expected] of thresholds)
        add(samples, `core-state-${ratio}`, expected, battlefieldCoreVisualState(ratio));
    const counts = new Map();
    for (const [group] of ENTRIES)
        counts.set(group, (counts.get(group) ?? 0) + 1);
    for (const [group, expected] of [['core', 3], ['pickup', 2], ['supply', 1], ['objective', 3], ['field-node', 5], ['spawn-portal', 2]])
        add(samples, `group-${group}-count`, expected, counts.get(group) ?? 0);
    ...
    return { samples, actionCount: ACTION_BUTTONS.length, presentationOnly: true, loadFailureBlocksGameplay: false, snapshotSchemaMutation: false, gameplayFormulaMutation: false, newAtlasCount: 1, passed: samples.length === 64 && samples.every(sample => sample.passed) };
}
