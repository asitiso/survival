...
import { DEFAULT_RASTER_BASELINE_SPECS, type RasterBaselineThresholds } from './render-raster-baseline.js';
...
export interface RasterBaselineChangeReport {
  id:string;
  status:'unchanged'|'review-required';
  baselineSignature:string;
  currentSignature:string;
  similarity:number;
  criticalSimilarity:number;
  changedCells:number;
  criticalChangedCells:number;
  thresholds:RasterBaselineThresholds;
  approvalToken:string;
  issues:string[];
}
export interface RasterBaselineApproval { approved:boolean;id:string;signature:string;token:string; }
export interface DefaultRasterBaselineReportEntry { id:string;status:'unchanged'|'review-required';expectedSignature:string;currentSignature:string;approvalToken:string; }
export interface DefaultRasterBaselineReport { ok:boolean;issues:string[];entries:DefaultRasterBaselineReportEntry[]; }
...
function thresholds(input?:Partial<RasterBaselineThresholds>):RasterBaselineThresholds{return{minSimilarity:Math.max(.5,Math.min(1,input?.minSimilarity??.985)),minCriticalSimilarity:Math.max(.5,Math.min(1,input?.minCriticalSimilarity??.995))};}
function compareCells(baseline:RasterRenderContract,current:RasterRenderContract):{changed:number;criticalChanged:number;criticalSimilarity:number}{
  ...
  for(const baseFrame of baseline.frames){
    ...
    for(let i=0;i<n;i++){
      ...
      if(weight>=4){
        ...
        if(a!==b)criticalChanged+=1;
      }
    }
  }
  return{changed,criticalChanged,criticalSimilarity:criticalTotal<=0?1:Math.max(0,Math.min(1,criticalSame/criticalTotal))};
}
function tokenFor(id:string,baselineSignature:string,currentSignature:string,changedCells:number,criticalChangedCells:number):string{
  return`RB-${fnv([id,baselineSignature,currentSignature,changedCells,criticalChangedCells].join('|'))}`;
}

export function rasterBaselineChangeReport(id:string,baseline:RasterRenderContract,current:RasterRenderContract,input?:Partial<RasterBaselineThresholds>):RasterBaselineChangeReport{
  ...
  const similarity=rasterSimilarity(baseline,current),cells=compareCells(baseline,current),issues:string[]=[];
  if(similarity<t.minSimilarity)issues.push(`similarity:${similarity.toFixed(6)}<${t.minSimilarity.toFixed(6)}`);
  if(cells.criticalSimilarity<t.minCriticalSimilarity)issues.push(`critical-similarity:${cells.criticalSimilarity.toFixed(6)}<${t.minCriticalSimilarity.toFixed(6)}`);
  if(currentSignature!==baselineSignature&&issues.length===0)issues.push(`signature-change:${baselineSignature}->${currentSignature}`);
...
export function approveRasterBaselineChange(report:RasterBaselineChangeReport,token:string):RasterBaselineApproval{
...
export function defaultRasterBaselineReport():DefaultRasterBaselineReport{
  const entries=DEFAULT_RASTER_BASELINE_SPECS.map((spec)=>{
    ...
    const status=currentSignature===spec.signature?'unchanged' as const:'review-required' as const;
