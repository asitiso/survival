...
export class EnemyManager {
  ...
  renderProjectiles(ctx: CanvasRenderingContext2D, bossSpecialVfxAtlasImage: CanvasImageSource | null = null, bossSpecialVfxAtlasReady = false, battlefieldEnvironmentReactionVfxAtlasImage: CanvasImageSource | null = null, battlefieldEnvironmentReactionVfxAtlasReady = false, bossProjectileLifecycleVfxAtlasImage: CanvasImageSource | null = null, bossProjectileLifecycleVfxAtlasReady = false, presentationQuality: PresentationQuality = 'high', reducedFlash = false, reducedMotion = false): void {
    ...
    for (const projectile of this.projectiles) {
      ...
      if(bossTravelRelease&&bossTravelRelease.visible&&bossBridgeBudget.visible){ctx.save();ctx.globalAlpha=Math.min(bossTravel?.alpha??0,bossTravelRelease.alpha)*bossBridgeBudget.alphaScale*threatOwnership.travelAlphaScale;ctx.strokeStyle='#ffb26f';ctx.lineWidth=Math.max(1.2,projectile.radius*.15);ctx.beginPath();ctx.moveTo(bossTravelRelease.start.x,bossTravelRelease.start.y);ctx.lineTo(bossTravelRelease.end.x,bossTravelRelease.end.y);ctx.stroke();ctx.restore();}
      if(trail.owner==='launch'&&projectileResolutionBudget.visible){ctx.save();ctx.globalAlpha=trail.alpha*threatOwnership.launchAlphaScale*directionCarryRecovery.tailAlphaScale*projectileResolution.transitionAlphaScale*projectileResolutionBudget.effectStrength;ctx.strokeStyle=projectile.bossArchetype?'#ffb26f':'#ff7c86';ctx.lineWidth=Math.max(1.3,projectile.radius*.2);ctx.beginPath();ctx.moveTo(trail.tail.x,trail.tail.y);ctx.lineTo(trail.head.x,trail.head.y);ctx.stroke();ctx.restore();}
      else if(travelThreatCarry.visible&&projectileResolutionBudget.visible){const speed=Math.hypot(projectile.vel.x,projectile.vel.y)||1,dx=projectile.vel.x/speed,dy=projectile.vel.y/speed,len=Math.max(travelThreatCarry.minLength,trail.length*travelThreatCarry.lengthScale*directionCarryRecovery.tailLengthScale*projectileResolution.trailLengthScale);ctx.save();ctx.globalAlpha=travelThreatCarry.alphaScale*threatOwnership.travelAlphaScale*directionCarryRecovery.tailAlphaScale*projectileResolution.transitionAlphaScale*projectileResolutionBudget.effectStrength;ctx.strokeStyle=projectile.bossArchetype?'#ffb26f':'#ff7c86';ctx.lineWidth=Math.max(1.1,projectile.radius*.14);ctx.beginPath();ctx.moveTo(visualPos.x-dx*len,visualPos.y-dy*len);ctx.lineTo(visualPos.x,visualPos.y);ctx.stroke();ctx.restore();}
      ...
      if (hasBossVisual && bossSpecialVfxAtlasImage && projectile.bossArchetype) {
        ...
        ctx.save(); ctx.translate(visualPos.x, visualPos.y); ctx.rotate(angle); ctx.globalAlpha = 0.92*projectileResolution.bodyAlphaScale;
      ...
      if (projectile.bossArchetype && bossProjectileLifecycleVfxAtlasReady && bossProjectileLifecycleVfxAtlasImage) {
        ...
        ctx.save(); ctx.translate(visualPos.x, visualPos.y); ctx.rotate(angle); ctx.globalAlpha = 0.78*projectileResolution.bodyAlphaScale;
      ...
      if (projectile.sourceType === 'archer' && battlefieldEnvironmentReactionVfxAtlasReady && battlefieldEnvironmentReactionVfxAtlasImage) {
        ...
        ctx.save(); ctx.translate(visualPos.x, visualPos.y); ctx.rotate(angle); ctx.globalAlpha = 0.94*projectileResolution.bodyAlphaScale;
