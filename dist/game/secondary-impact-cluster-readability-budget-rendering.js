...
function lifeOf(entry) { return entry.maxTtl > 0 ? Math.max(0, Math.min(1, entry.ttl / entry.maxTtl)) : 0; }
export function secondaryImpactClusterReadabilityBudgetPresentation(impacts, quality, reducedFlash = false) {
    const groups = [];
    for (let index = 0; index < impacts.length; index++) {
        ...
        for (let gIndex = 0; gIndex < groups.length; gIndex++) {
            ...
            if (impact.stableClusterKey && group.stableKey) {
                if (impact.stableClusterKey === group.stableKey) {
                    best = gIndex;
                    bestDistance = 0;
                }
        ...
        if (best >= 0) {
            ...
            group.sumX += impact.pos.x;
            group.sumY += impact.pos.y;
            group.maxLife = Math.max(group.maxLife, lifeOf(impact));
        }
        else
            groups.push({ indices: [index], sumX: impact.pos.x, sumY: impact.pos.y, maxLife: lifeOf(impact), ...(impact.stableClusterKey ? { stableKey: impact.stableClusterKey } : {}) });
    }
    ...
    const output = impacts.map(() => ({ visible: false, alphaScale: 0, sizeScale: 0, clusterCount: 0, clusterIndex: -1, presentationOnly: true }));
