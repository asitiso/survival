...
import { createDefaultWorldState, evolveWorld } from './endless/world-evolution.js';
...
export function auditFieldNodeIdentityAssets() {
    const atlas = auditFieldNodeIdentityAtlas();
    const samples = [];
    const push = (caseId, passed, kind) => { samples.push({ caseId, passed, ...(kind ? { kind } : {}) }); };
    const body = new Set(), presentation = new Set(), fallback = new Set();
    ...
    for (const kind of FIELD_NODE_IDENTITY_KINDS) {
        ...
        if (presentationOk)
            presentation.add(kind);
        push(`${kind}:presentation`, presentationOk, kind);
        const fallbackOk = icon.textFallbackPreserved;
        if (fallbackOk)
            fallback.add(kind);
        push(`${kind}:fallback`, fallbackOk, kind);
    ...
    const defaultWorld = createDefaultWorldState();
    push('world:default-state', defaultWorld.current === 'calm' && defaultWorld.nodes.length === 0 && defaultWorld.evolutionCount === 0);
    const legacy = { heroId: 'arkan', elapsedMs: 480000, level: 20, threat: 3, kills: 500, bossesDefeated: 2, elitesDefeated: 10, gold: 2000, xp: 9000, guardianCoreHp: 900, guardianCoreMaxHp: 1000, fate: 'none', spellFusionCount: 1, mapEvolutionRank: 1, masteryLevel: 8, deviceClass: 'mid' };
    ...
    if (atlas.coverage !== 1 || atlas.uniqueCellCount !== 5 || atlas.outOfBounds.length)
        issues.push('atlas');
    if (bodyCoverage !== 1)
        issues.push('body-coverage');
    if (presentationCoverage !== 1)
        issues.push('presentation-coverage');
    ...
    if (iconMotionAmplitude !== 0)
        issues.push('motion');
    if (fieldNodeGameplayMutation)
        issues.push('gameplay-mutation');
    if (worldEvolutionMutation)
        issues.push('world-evolution-mutation');
    if (actionCount !== 9)
