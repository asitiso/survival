...
export type HeroCastRenderLayer = 'cast' | 'recover';
...
const ROW_BY_LAYER: Readonly<Record<HeroCastRenderLayer, number>> = {
  cast: 0,
  recover: 1,
};

const SCALE_BY_LAYER: Readonly<Record<HeroCastRenderLayer, number>> = {
  cast: 6.65,
  recover: 6.15,
};

const ALPHA_BY_LAYER: Readonly<Record<HeroCastRenderLayer, number>> = {
  cast: 0.46,
  recover: 0.34,
};

export interface HeroCastRenderSpriteRect {
  sx: number;
  sy: number;
  sw: number;
  sh: number;
  heroId: HeroId;
  layer: HeroCastRenderLayer;
}

export interface HeroCastRenderPresentation {
  visible: boolean;
  size: number;
  alpha: number;
  focusOffset: number;
...
export function heroCastRenderSprite(heroId: HeroId, layer: HeroCastRenderLayer): HeroCastRenderSpriteRect {
...
export function heroCastRenderPresentation(layer: HeroCastRenderLayer, radius: number, atlasReady: boolean, emphasis = 0): HeroCastRenderPresentation {
...
export interface HeroCastRenderAtlasAudit {
  itemCount: number;
  coverage: number;
  uniqueCellCount: number;
  outOfBounds: string[];
  heroes: HeroId[];
  layers: HeroCastRenderLayer[];
  passed: boolean;
}

export function auditHeroCastRenderAtlas(heroIds: readonly HeroId[]): HeroCastRenderAtlasAudit {
  const layers: HeroCastRenderLayer[] = ['cast', 'recover'];
