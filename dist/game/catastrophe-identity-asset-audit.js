...
export function auditCatastropheIdentityAssets() {
    const atlas = auditCatastropheIdentityAtlas();
    const samples = [];
    const push = (caseId, passed, id) => { samples.push({ caseId, passed, ...(id ? { id } : {}) }); };
    const iconSet = new Set(), rotationSet = new Set(), fallbackSet = new Set();
    ...
    CATASTROPHE_IDENTITY_IDS.forEach((id, index) => {
        const icon = catastropheIdentityIcon(id);
        const expected = EXPECTED[id];
        const active = catastropheAt(1200 + 180 * index);
        const rectOk = icon.sx >= 0 && icon.sy >= 0 && icon.sx + icon.sw <= 288 && icon.sy + icon.sh <= 192;
        if (rectOk)
            iconSet.add(id);
        push(`${id}:body`, rectOk, id);
        ...
        const rotationOk = active?.id === id;
        if (rotationOk)
            rotationSet.add(id);
        push(`${id}:rotation`, rotationOk, id);
        ...
        catastropheModifierMutation = catastropheModifierMutation || !modifierOk;
    ...
    const timingStartOk = catastropheAt(1199) === null && catastropheAt(1200)?.id === 'goldenNight';
    push('timing:start', timingStartOk);
    const order = CATASTROPHE_IDENTITY_IDS.map((_, i) => catastropheAt(1200 + 180 * i)?.id);
    const rotationOrderOk = order.every((id, i) => id === CATASTROPHE_IDENTITY_IDS[i]);
    push('timing:rotation-order', rotationOrderOk);
    const loopOk = catastropheAt(1200 + 180 * 5)?.id === 'goldenNight';
    push('timing:loop', loopOk);
    const neutralOk = sameModifiers(catastropheModifiers(null), { goldMultiplier: 1, enemySpeedMultiplier: 1, cooldownMultiplier: 1, spawnPressureMultiplier: 1, eliteIntervalMultiplier: 1, coreDamageMultiplier: 1 });
    push('modifiers:neutral', neutralOk);
    ...
    if (atlas.coverage !== 1 || atlas.uniqueCellCount !== 5 || atlas.outOfBounds.length)
        issues.push('atlas');
    if (iconCoverage !== 1)
        issues.push('icon-coverage');
    if (rotationCoverage !== 1)
    ...
    if (iconMotionAmplitude !== 0)
        issues.push('motion');
    if (catastropheTimingMutation)
        issues.push('timing-mutation');
    if (catastropheModifierMutation || !neutralOk)
