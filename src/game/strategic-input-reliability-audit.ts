import { StrategicActionReleaseTracker, strategicActionReleaseRadius, type StrategicActionId } from '../core/strategic-action-release.js';
...
export interface StrategicInputReliabilityAudit {
  samples:number;
  releaseSamples:number;
  jitterSamples:number;
  boundarySamples:number;
  ownershipSamples:number;
...
const strategicButtons=ACTION_BUTTONS.filter((button): button is (typeof ACTION_BUTTONS)[number] & { id: StrategicActionId } => button.id==='shop'||button.id==='auto');
...
function boundaryChecks():boolean[]{
...
function ownershipChecks():boolean[]{
  const autoSingle=(()=>{
    const tracker=new StrategicActionReleaseTracker();
...
export function auditStrategicInputReliability():StrategicInputReliabilityAudit{
  ...
  const issues:string[]=[];
  if(!releasePassed)issues.push('release-commit');
  if(!jitterPassed)issues.push('release-jitter');
  if(!boundaryPassed)issues.push('release-boundary');
  if(!ownershipPassed)issues.push('single-owner');
  if(!cancelPassed)issues.push('cancel-safety');
  if(!foldablePassed)issues.push('foldable-release');
  if(!invariants.every(Boolean))issues.push('frozen-invariants');
