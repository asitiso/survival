...
export const SPECIALIST_INTENT_TYPES = ['bomber', 'shaman', 'shieldbearer', 'assassin', 'siegeGolem', 'nullifier'];
...
export function isSpecialistIntentType(type) {
    return SPECIALIST_INTENT_TYPES.includes(type);
}
export function specialistIntentIcon(type) {
...
export function specialistIntentOnBodyLayout(radius, world) {
...
export function specialistIntentEmphasis(type, state) {
    if (type === 'bomber' || type === 'shaman')
        return 1;
    if (type === 'shieldbearer')
        return state.guardHp > 0 ? 1 : 0;
    if (type === 'assassin')
        return state.specialistTimer <= 1.2 ? 1 : 0;
    if (type === 'siegeGolem')
        return state.target === 'core' ? 1 : 0;
    return state.heroInsideNullifier ? 1 : 0;
}
export function auditSpecialistIntentAtlas() {
    const cells = new Set();
    const outOfBounds = [];
    for (const type of SPECIALIST_INTENT_TYPES) {
        const [column, row] = CELL_BY_TYPE[type];
        cells.add(`${column}:${row}`);
        if (column < 0 || row < 0 || column >= SPECIALIST_INTENT_ATLAS.columns || row >= SPECIALIST_INTENT_ATLAS.rows)
