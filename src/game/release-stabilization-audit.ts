...
import { clearRunSnapshot, loadRunSnapshot, type RunSnapshot } from '../domain/run-snapshot.js';

const sample:RunSnapshot={version:1,savedAt:80,heroId:'arkan',traitId:'destruction',threatLevel:3,elapsed:60,hero:{level:10,xp:10,xpNext:20,hp:100,maxHp:100,coins:20,kills:30},coreHp:100,spellLevels:{fireBolt:1,chainLightning:1,frostNova:1,flameField:1,meteorStorm:1,blackHole:1},equipment:{coins:20,weapon:null,armor:null,healingPotions:1,inventory:[],discoveredRecipes:[]},relic:null,fusions:[],fateChoices:[],map:{id:'ruinedGate',evolutionStage:0},progression:{bossesKilled:1,goldEarned:20,shopTokens:0}};
export interface ReleaseStabilizationAudit{sessionStorageFallbackPassed:boolean;snapshotRecoveryPassed:boolean;viewportLifecyclePassed:boolean;surfaceFreezePassed:boolean;passed:boolean;}
export function auditReleaseStabilization():ReleaseStabilizationAudit{
  const storage=createResilientStorage(()=>{throw new Error('SecurityError');});storage.setItem('probe','latest');
  const sessionStorageFallbackPassed=storage.getItem('probe')==='latest';
  const good=JSON.stringify(sample);const recovery={getItem:(key:string)=>{if(key==='arcane-last-stand.run-snapshot')throw new Error('blocked');return key.endsWith('.backup')?good:null;},setItem(){},removeItem(){}};
  const snapshotRecoveryPassed=loadRunSnapshot(recovery)?.savedAt===80;
  const calls:string[]=[];clearRunSnapshot({getItem(){return null;},setItem(){},removeItem(key){calls.push(key);if(key==='arcane-last-stand.run-snapshot')throw new Error('blocked');}});
