export type HeroActionLaunchOwner='movement'|'cast'|'ultimate'|'recovery';
export type HeroActionLaunchKind='normal'|'ultimate';
export interface HeroActionLaunchOriginCoherenceInput{owner:HeroActionLaunchOwner;bodyOffsetX:number;bodyOffsetY:number;bodyFacingX:number;bodyFacingY:number;movementFacingX:number;movementFacingY:number;radius:number;poseStrength:number;kind:HeroActionLaunchKind}
export interface HeroActionLaunchOriginCoherencePresentation{owner:'action-pose'|'movement';facingX:number;facingY:number;originOffsetX:number;originOffsetY:number;forwardDistance:number;bodyFollow:number;convergeSeconds:number;presentationOnly:true}
const clamp=(v:number,min=0,max=1)=>Math.max(min,Math.min(max,Number.isFinite(v)?v:0));
const norm=(x:number,y:number,fallbackX=1,fallbackY=0)=>{const m=Math.hypot(x,y);if(m>.0001)return{x:x/m,y:y/m};const fm=Math.hypot(fallbackX,fallbackY)||1;return{x:fallbackX/fm,y:fallbackY/fm};};
const bounded=(x:number,y:number,max:number)=>{const m=Math.hypot(x,y);if(m<=max||m<=.0001)return{x,y};const s=max/m;return{x:x*s,y:y*s};};
export function heroActionLaunchOriginCoherencePresentation(input:HeroActionLaunchOriginCoherenceInput,reducedMotion=false):HeroActionLaunchOriginCoherencePresentation{
