...
export interface ActionCueClarityAuditSample {
  actionId: ActionId | 'global';
  caseId: string;
...
export interface ActionCueClarityAudit {
  passed: boolean;
  samples: ActionCueClarityAuditSample[];
  maxAnimatedOuterCues: number;
  queuedDuplicateTextCount: number;
  staleReadyReplayCount: number;
...
const COMBAT_ACTIONS: ActionId[]=['spell1','spell2','spell3','spell4','ultimate1','ultimate2'];
const ULTIMATES: ActionId[]=['ultimate1','ultimate2'];
const add=(samples:ActionCueClarityAuditSample[],actionId:ActionId|'global',caseId:string,expected:ActionCueClarityAuditSample['expected'],actual:ActionCueClarityAuditSample['actual'])=>samples.push({actionId,caseId,expected,actual,passed:expected===actual});

export function auditActionCueClarity(): ActionCueClarityAudit {
  const samples:ActionCueClarityAuditSample[]=[];
  ...
  if(maxAnimatedOuterCues>1)issues.push('outer-cue-overlap');
  if(queuedDuplicateTextCount!==0)issues.push('queued-duplicate-text');
  if(staleReadyReplayCount!==0)issues.push('stale-ready-replay');
