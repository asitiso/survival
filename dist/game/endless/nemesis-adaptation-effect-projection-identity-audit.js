...
const KINDS = ['spell_guard', 'blink_hunt', 'core_siege', 'enrage_clock', 'mirror_affinity'];
const expected = (kind, rank) => kind === 'spell_guard' ? Math.max(.72, 1 - rank * .035) : kind === 'blink_hunt' ? Math.min(1.45, 1 + rank * .05) : kind === 'core_siege' ? Math.min(1.5, 1 + rank * .05) : kind === 'enrage_clock' ? Math.max(.7, 1 - rank * .04) : .94;
const close = (a, b) => Math.abs(a - b) < 1e-9;
export function auditNemesisAdaptationEffectProjectionIdentityAssets() {
    const samples = [], issues = [], atlas = auditNemesisAdaptationEffectIdentityAtlas();
    const kindCoverage = new Set(), rankCoverage = new Set(), identityCoverage = new Set();
    for (const kind of KINDS)
        for (const rank of [1, 2, 3]) {
            const adaptation = { kind, rank, ...(kind === 'mirror_affinity' ? { affinity: 'frost' } : {}) };
            const p = projectNemesisAdaptationEffect(adaptation);
            kindCoverage.add(kind);
            rankCoverage.add(rank);
            identityCoverage.add(p.effectId);
    ...
    for (const id of NEMESIS_ADAPTATION_EFFECT_IDENTITY_IDS) {
        const icon = nemesisAdaptationEffectIdentityIcon(id);
        samples.push({ id: `identity:${id}:static`, passed: icon.animated === false && icon.motionAmplitude === 0 });
        samples.push({ id: `identity:${id}:safe`, passed: icon.textFallbackPreserved && !icon.loadFailureBlocksGameplay && icon.maxVisibleHelperIcons === 2 });
    }
    ...
    if (!adaptationCoverageComplete)
        issues.push('adaptation-coverage');
    if (!rankCoverageComplete)
        issues.push('rank-coverage');
    if (!identityCoverageComplete)
        issues.push('identity-coverage');
    if (!mirrorAffinityPreserved)
        issues.push('mirror-affinity');
