...
export interface SoakAudit {
  minutes:number[];
  ascensionTier:number;
  maxEnemyBudget:number;
  maxProjectileBudget:number;
  maxEffectBudget:number;
  caps:{enemyBudget:number;projectileBudget:number;effectBudget:number};
  passed:boolean;
}

export function auditSixHourSoak(deviceClass:DeviceClass='low', threat=5):SoakAudit {
