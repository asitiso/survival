...
export function thumbFatigueAudit() {
    const profile = thumbComfortProfile();
    const origin = { x: 180, y: 720 };
    ...
    const issues = [];
    if (reduction < .25)
        issues.push('insufficient-reach-relief');
    if (maxSoftReach > profile.maxReach + .001)
        issues.push('comfort-radius-exceeded');
    if (maxAnchorShift > 96)
        issues.push('anchor-drift');
    return {
