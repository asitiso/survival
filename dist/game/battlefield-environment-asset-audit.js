...
import { BATTLEFIELD_ENVIRONMENT_ATLAS, BATTLEFIELD_ENVIRONMENT_MAP_IDS, auditBattlefieldEnvironmentAtlas, battlefieldEnvironmentIconStyle, battlefieldEnvironmentSprite, } from './battlefield-environment-assets.js';
export function auditBattlefieldEnvironmentAssets() {
    const atlas = auditBattlefieldEnvironmentAtlas();
    const surfaces = ['combat', 'hud', 'result', 'lobby-resume', 'replay'];
    const stages = [0, 1, 2];
    const samples = [];
    ...
    if (atlas.coverage !== 1 || atlas.uniqueCellCount !== 9 || atlas.outOfBounds.length)
        issues.push('atlas');
