...
export interface CombatInputReliabilityAudit {
  samples: number;
  timingSamples: number;
  castActionCount: number;
  actionCount: number;
  windowSeconds: number;
  timingPassed: boolean;
  duplicateCoalescingPassed: boolean;
  exactlyOncePassed: boolean;
  manualPriorityPassed: boolean;
  lifecycleClearPassed: boolean;
...
export function auditCombatInputReliability(): CombatInputReliabilityAudit {
  ...
  const arbitration: Array<'manual' | 'auto'> = [];
  if (priority.consumeIfReady('spell1', 0)) arbitration.push('manual');
  if (arbitration.length === 0) arbitration.push('auto');
  ...
  const issues: string[] = [];
  if (!timingPassed) issues.push('timing-window');
  if (!duplicateCoalescingPassed) issues.push('duplicate-coalescing');
  if (!exactlyOncePassed) issues.push('exactly-once');
  if (!manualPriorityPassed) issues.push('manual-priority');
  if (!lifecycleClearPassed) issues.push('lifecycle-clear');
  if (castActionCount !== 6) issues.push('cast-action-surface');
