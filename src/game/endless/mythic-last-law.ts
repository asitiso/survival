...
export interface MythicLastLawProfile {
  active: boolean;
  label: string;
...
const NEUTRAL: MythicLastLawProfile = {
  active:false,
  label:'',
...
export function mythicLastLawProfile(profile: MythicBossProfile, hpRatio: number, weakpointRatio: number): MythicLastLawProfile {
  if (!profile.active || clamp(hpRatio, 0, 1) > .15) return NEUTRAL;
