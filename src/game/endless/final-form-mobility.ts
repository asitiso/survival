import { clampMagnitude, normalize, type Vec2 } from '../../core/math.js';
import type { HeroFinalFormId } from './final-form.js';

export type FinalFormMobilityFamily = 'surge'|'flow'|'drift'|'anchor';

export interface FinalFormMobilityProfile {
  formId: HeroFinalFormId;
  family: FinalFormMobilityFamily;
  moveSpeedMultiplier: number;
  response: number;
  signatureImpulse: number;
}

const PROFILES: Record<HeroFinalFormId,FinalFormMobilityProfile> = {
...
export function finalFormMobilityProfile(formId:HeroFinalFormId):FinalFormMobilityProfile { return PROFILES[formId]; }

export function advanceFinalFormMotion(current:Vec2, desired:Vec2, dt:number, formId:HeroFinalFormId|null):Vec2 {
  ...
  if (Math.hypot(target.x,target.y)<.001 && Math.hypot(next.x,next.y)<.015) return {x:0,y:0};
  return clampMagnitude(next,1);
}

export function signatureMobilityImpulse(formId:HeroFinalFormId|null,facing:Vec2):Vec2 {
  if (!formId) return {x:0,y:0};
