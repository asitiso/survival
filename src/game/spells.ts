...
import { impactRhythmRecoveryPresentation, rhythmRecoveryBudgetPresentation } from './threat-rhythm-recovery-rendering.js';
import { bossTelegraphTemporalPriorityPresentation, secondaryRhythmTemporalRetirementPresentation } from './threat-rhythm-temporal-arbitration-rendering.js';
  ...
  render(ctx: CanvasRenderingContext2D, motion?: ResidualCombatMotionPolicy, propVfxAtlasImage?: HTMLImageElement | null, propVfxAtlasReady = false, heroProjectileAtlasImage?: HTMLImageElement | null, heroProjectileAtlasReady = false, heroSpellSignatureAtlasImage?: HTMLImageElement | null, heroSpellSignatureAtlasReady = false, ultimateSignatureAtlasImage?: HTMLImageElement | null, ultimateSignatureAtlasReady = false, persistentSpellZoneVfxAtlasImage?: HTMLImageElement | null, persistentSpellZoneVfxAtlasReady = false, crowdControlPropagationVfxAtlasImage?: HTMLImageElement | null, crowdControlPropagationVfxAtlasReady = false, reducedFlash = false, ultimatePostImpactResidueVfxAtlasImage?: HTMLImageElement | null, ultimatePostImpactResidueVfxAtlasReady = false, reducedMotion = false, presentationQuality: PresentationQuality = 'high', primaryProjectileLabelBlockers:readonly Vec2[]=[], heroPos:Vec2|null=null, bossTelegraphZones:readonly {pos:Vec2;radius:number}[]=[]): void {
    ...
    if (heroProjectileAtlasReady && heroProjectileAtlasImage) {
      ...
      for (const impact of this.projectileImpactVisuals) {
        ...
        const impactRhythmRecovery=impactRhythmRecoveryPresentation({suppression:1-impactPeakSeparation.impactInteriorScale,release:impactSecondaryRecoveryGate.release,stress:Math.max(impactPeakBudget.load,impactSecondaryRecoveryGateBudget.hold),critical:impactCritical},reducedMotion,reducedFlash),impactRhythmRecoveryBudget=rhythmRecoveryBudgetPresentation({recoveringFamilies:1+(persistentHazardOverlap?1:0)+(bossTelegraphOverlap?1:0),stress:Math.max(impactPeakBudget.load,impactSecondaryRecoveryGateBudget.hold),criticalCount:impactCritical?1:0,safeLaneVisible:false,bossActive:bossTelegraphOverlap},reducedMotion,reducedFlash);
        const impactBossTemporal=bossTelegraphTemporalPriorityPresentation({charge:bossTelegraphOverlap?1:0,hazardPressure:persistentHazardOverlap?1:0,secondaryPeak:impactRhythmRecovery.interiorScale,critical:impactCritical},reducedMotion,reducedFlash),impactTemporalRetirement=secondaryRhythmTemporalRetirementPresentation({life:impactLife,churn:Math.min(1,impactNeighborCount/8),pressure:impactRhythmRecoveryBudget.load,critical:impactCritical},reducedMotion,reducedFlash);
