...
const LEVELS = [1, 5, 10];
export function auditVisualEffectsSafety() {
    let samples = 0, maxFlashAlpha = 0, maxWaveCount = 0, maxRayCount = 0;
    const shapes = new Set();
    for (const hero of HERO_PROFILES)
        for (const spell of SPELLS)
            for (const level of LEVELS) {
                const d = spellVfxDescriptor(hero.id, spell, level);
                samples++;
                shapes.add(d.shape);
                maxFlashAlpha = Math.max(maxFlashAlpha, d.flashAlpha);
                maxWaveCount = Math.max(maxWaveCount, d.waveCount);
                maxRayCount = Math.max(maxRayCount, d.rayCount);
            }
    const low = new PresentationRuntime('low');
    for (let i = 0; i < 8; i++)
        low.emitScreenEffect({ kind: 'shockwave', x: 0, y: 0, radius: 20, color: '#fff', ttl: .2, alpha: .2 });
    ...
    const bossArchetypes = ['inferno', 'summoner', 'juggernaut', 'abyssWitch', 'twinMaw', 'timeEater'];
    const bossProfiles = bossArchetypes.flatMap((a) => [bossPhaseCinematicProfile(a, 2), bossPhaseCinematicProfile(a, 3)]);
    const cameraKinds = ['meteor', 'vortex', 'bossPhase2', 'bossPhase3', 'killChain'];
    const cameraProfiles = cameraKinds.map(cameraPressureProfile);
    const killProfiles = [1, 2, 3].map(killChainVfxProfile);
    const maps = ['ruinedGate', 'frozenFen', 'crystalQuarry'];
    const mapProfiles = maps.flatMap((m) => [0, 1, 2].map((stage) => mapEnvironmentVfxDescriptor(m, stage, 'high')));
    const advancedVisualEffectsPassed = new Set(bossProfiles.map((p) => p.motif)).size === 6 && bossProfiles.every((p) => p.shockwaveCount <= 3 && p.edgePulseAlpha <= 0.34 && p.vignetteAlpha <= 0.28) &&
    ...
    const enemyTypes = ['grunt', 'hound', 'brute', 'archer', 'bomber', 'shaman', 'shieldbearer', 'assassin', 'siegeGolem', 'nullifier', 'golden', 'elite', 'boss'];
    const enemyProfiles = enemyTypes.map((type) => ({ hit: enemyImpactVfxDescriptor(type, 'heavy'), death: enemyDeathCue(type) }));
    const ultimateProfiles = ['meteorStorm', 'blackHole'].flatMap((spell) => [1, 5, 10].map((level) => ultimateChoreographyDescriptor(spell, level)));
    ...
    const destruction = maps.flatMap((m) => [0, 1, 2].map((stage) => environmentDestructionVfxDescriptor(m, 'evolutionCollapse', stage, 'high'))).concat(maps.map((m) => environmentDestructionVfxDescriptor(m, 'crystalBlast', 2, 'high')));
    const glowKinds = ['impact', 'ultimate', 'boss', 'environment'];
    const glowProfiles = glowKinds.flatMap((kind) => ['high', 'medium', 'low'].map((q) => screenGlowProfile(kind, q, false)));
    const edgeRhythm = ['watch', 'danger', 'critical'].flatMap((level) => ['hero', 'core'].map((target) => edgeThreatVfxProfile(level, target)));
    const deathRhythm = enemyTypes.flatMap((type) => ['high', 'medium', 'low'].map((q) => deathAfterglowProfile(type, q)));
    const aftermathRhythm = ['meteorStorm', 'blackHole'].flatMap((spell) => [1, 5, 10].flatMap((level) => ['high', 'medium', 'low'].map((q) => ultimateAftermathProfile(spell, level, q))));
    const settleRhythm = bossArchetypes.flatMap((boss) => ['high', 'medium', 'low'].map((q) => bossSettleProfile(boss, q)));
    let transition = createVfxQualityTransition('low');
    transition = advanceVfxQualityTransition(transition, 'high', 1.21);
    transition = advanceVfxQualityTransition(transition, 'high', 1.21);
    ...
    const directionalPresence = ['normal', 'heavy', 'critical'].flatMap((tier) => qualities.map((q) => directionalHitVfxProfile(tier, q)));
    ...
    const ambientPresence = maps.flatMap((map) => [0, 1, 2].flatMap((stage) => qualities.map((q) => mapAmbientDepthProfile(map, stage, q, false))));
    ...
    const hitTiming = ['normal', 'heavy', 'critical'].flatMap((tier) => qualities.flatMap((q) => [12, 80, 240].map((distance) => hitApproachProfile({ x: 0, y: 0 }, { x: distance, y: 0 }, tier, q))));
    ...
    const readabilityTiming = qualities.flatMap((q) => ['normal', 'danger', 'critical'].map((threat) => visualReadabilityBudget(q, threat)));
    ...
    const recoilCoherence = ['normal', 'heavy', 'critical'].flatMap((tier) => qualities.flatMap((q) => [18, 86, 260].map((distance) => directionalImpactRecoilProfile({ x: 0, y: 0 }, { x: distance, y: 0 }, tier, q))));
    ...
    const flowCoherence = maps.flatMap((map) => [0, 1, 2].flatMap((stage) => [false, true].map((critical) => mapAmbientFlowProfile(map, stage, 1.71, critical))));
    const focusCoherence = qualities.flatMap((q) => ['normal', 'danger', 'critical'].flatMap((threat) => ['none', 'strained', 'desperate'].map((boss) => visualFocusBudget(q, threat, boss))));
