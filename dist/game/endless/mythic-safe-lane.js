...
function candidateScore(hazards, point, heroRadius, preferred) {
    let score = 0;
    for (const hazard of hazards) {
        if (!hazard.geometryShape)
            continue;
        const contact = mythicArenaHazardContact(hazard, point, heroRadius);
        if (contact.hit)
            return -1_000_000 - contact.penetration;
        ...
        if (hazard.geometryShape === 'ring') {
            ...
            if (d < inner)
                score += 70 + (inner - d) * .12;
            else if (d > hazard.radius + heroRadius)
                score += 12;
        }
    }
    if (preferred) {
        const d = distance(point, preferred.target);
        if (d <= preferred.radius)
            score += Math.max(0, preferred.weight) * (1 - d / Math.max(1, preferred.radius));
    }
    return score;
}
export function mythicSafeLaneHint(hazards, heroPos, heroRadius, width, height, preferred = null) {
    ...
    for (const hazard of relevant) {
        if (hazard.geometryShape === 'ring')
            candidates.push({ x: clamp(hazard.pos.x, 56, w - 56), y: clamp(hazard.pos.y, 96, h - 56) });
    }
    if (preferred)
        candidates.push({ x: clamp(preferred.target.x, 56, w - 56), y: clamp(preferred.target.y, 96, h - 56) });
    let best = candidates[0], bestScore = Number.NEGATIVE_INFINITY;
    let worst = Number.POSITIVE_INFINITY;
    for (const point of candidates) {
        const score = candidateScore(relevant, point, heroRadius, preferred);
        if (score > bestScore) {
            best = point;
            bestScore = score;
        }
