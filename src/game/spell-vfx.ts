...
export type SpellVfxFamily = 'fire' | 'frost' | 'lightning' | 'holy';
export type SpellVfxTier = 0 | 1 | 2;
export type SpellVfxShape = 'bolt' | 'fork' | 'nova' | 'field' | 'meteor' | 'vortex';

export interface SpellVfxDescriptor {
  family: SpellVfxFamily;
  tier: SpellVfxTier;
  ultimate: boolean;
  shape: SpellVfxShape;
  primary: string;
...
const PALETTE: Record<HeroId, { family: SpellVfxFamily; primary: string; secondary: string }> = {
...
const SHAPE: Record<SpellId, SpellVfxShape> = {
  fireBolt: 'bolt',
  chainLightning: 'fork',
  frostNova: 'nova',
  flameField: 'field',
...
const STRUCTURE: Record<SpellId, { waves: [number, number, number]; rays: [number, number, number] }> = {
...
export function spellVfxDescriptor(heroId: HeroId, spellId: SpellId, level: number): SpellVfxDescriptor {
  const palette = PALETTE[heroId];
  const tier: SpellVfxTier = level >= 10 ? 2 : level >= 5 ? 1 : 0;
...
export type UltimateChoreographyMotion='descent'|'orbit';
export interface UltimateChoreographyDescriptor { motion:UltimateChoreographyMotion; trailCount:number; ringCount:number; orbitCount:number; glowAlpha:number; afterglow:number; streakLength:number; }
export function ultimateChoreographyDescriptor(spellId:Extract<SpellId,'meteorStorm'|'blackHole'>,level:number):UltimateChoreographyDescriptor {
  const tier=level>=10?2:level>=5?1:0;
  if(spellId==='meteorStorm') return {motion:'descent',trailCount:[4,6,8][tier]!,ringCount:[2,3,4][tier]!,orbitCount:0,glowAlpha:[.20,.27,.34][tier]!,afterglow:[.16,.22,.28][tier]!,streakLength:[110,145,180][tier]!};
