...
export function criticalDepthLatchPresentation(input, reducedFlash = false) {
    ...
    return { criticalAlphaScale: 1, secondaryAlphaScale: secondaryAlphaScale * (reducedFlash ? .9 : 1), presentationOnly: true };
}
export function bossTelegraphStackOrderPresentation(input, reducedFlash = false) {
    ...
    return { edgeAlphaScale, decorationAlphaScale, primary, presentationOnly: true };
}
export function safeLaneEdgeClutterProtectionPresentation(input, reducedFlash = false) {
    ...
    return { focus, pathAlphaFloor, safeLaneAlphaScale, secondaryAlphaScale, presentationOnly: true };
}
export function canonicalBodyDepthReclaimPresentation(input, reducedMotion = false) {
    const release = clamp01(input.release), pressure = clamp01(input.pressure);
    if (input.owner === 'canonical')
        return { bodyAlphaScale: 1, overlayAlphaScale: 1, trailAlphaScale: reducedMotion ? .92 : 1, presentationOnly: true };
    const floor = input.owner === 'critical' ? .82 : .52, reclaim = clamp01(release * (1 - pressure * .22));
    return { bodyAlphaScale: 1, overlayAlphaScale: floor + (1 - floor) * reclaim, trailAlphaScale: (.48 + .52 * reclaim) * (reducedMotion ? .88 : 1), presentationOnly: true };
}
export function impactEdgeGhostRetirementPresentation(input, reducedFlash = false) {
    ...
    return { edgeAlphaScale, fillAlphaScale, retired: life <= .01, presentationOnly: true };
}
export function unifiedDepthStackBudgetPresentation(input, reducedMotion = false, reducedFlash = false) {
    ...
    return { stress, criticalAlphaScale: 1, canonicalBodyAlphaScale: 1, safeLaneAlphaScale: input.safeLaneVisible ? 1 + stress * .12 * (reducedFlash ? .72 : 1) : 1, bossTelegraphEdgeAlphaScale: input.bossTelegraphCount > 0 ? Math.max(.94, .98 * (reducedFlash ? .98 : 1)) : 1, secondaryAlphaScale: (1 - stress * .46) * motion * flash, presentationOnly: true };
}
