...
import { LONG_RUN_OATH_MILESTONES, advanceLongRunOaths, longRunOathModifiers } from './endless/long-run-oaths.js';
...
function legacy(elapsedMs) { return { heroId: 'arkan', elapsedMs, level: 80, threat: 5, kills: 9000, bossesDefeated: 18, elitesDefeated: 300, gold: 50000, xp: 100000, guardianCoreHp: 1000, guardianCoreMaxHp: 1000, fate: 'none', spellFusionCount: 2, mapEvolutionRank: 4, masteryLevel: 30, deviceClass: 'high' }; }
function completedState(kind, milestone) { const target = kind === 'boss_hunt' ? 2 : kind === 'core_guard' ? 240_000 : kind === 'endure' ? 300_000 : 100; return { completedMilestones: [], failedMilestones: [], expiredMilestones: [], history: [kind], active: { id: `audit-${kind}`, milestone, kind, title: kind, startedAtMs: milestone * 60_000, deadlineMs: (milestone + 60) * 60_000, target, progress: target, baselineCoreHp: 1000, coreDamage: 0 }, boon: null }; }
function runtimeBoonMatches(kind, milestone) { const now = milestone * 60_000 + 1000, result = advanceLongRunOaths(completedState(kind, milestone), legacy(now), [], 0, 1), expected = oathRequirementBoonIdentity(kind).boonId; return result.state.boon?.kind === expected && result.state.boon.expiresAtMs - now === 90_000 && result.effects.some(effect => effect.type === 'oath_completed'); }
function modifierStable(kind) { const boon = oathRequirementBoonIdentity(kind).boonId, state = { completedMilestones: [], failedMilestones: [], expiredMilestones: [], history: [], active: null, boon: { kind: boon, expiresAtMs: 100_000 } }, m = longRunOathModifiers(state, 1); if (boon === 'prosperity')
    return m.goldMultiplier === 1.16; if (boon === 'power')
    return m.spellPowerMultiplier === 1.09; if (boon === 'guard')
    return m.coreDamageTakenMultiplier === .88; return m.bossDamageMultiplier === 1.1; }
export function auditOathRequirementBoonIdentityAssets() {
    const req = auditOathRequirementIdentityAtlas(), boon = auditOathBoonOutcomeIdentityAtlas(), samples = [];
    let gameplayMutation = false;
    const push = (caseId, kind, passed) => { samples.push({ caseId, kind, passed }); if (!passed)
        gameplayMutation = true; };
    KINDS.forEach((kind, index) => {
        const profile = oathRequirementBoonIdentity(kind), reqIcon = oathRequirementIdentityIcon(profile.requirementId), boonIcon = oathBoonOutcomeIdentityIcon(profile.boonId), milestone = LONG_RUN_OATH_MILESTONES[index];
    ...
    if (ACTION_BUTTONS.length !== 9)
        issues.push('actions');
    if (JSON.stringify(LONG_RUN_OATH_MILESTONES) !== JSON.stringify([120, 150, 180, 240, 300, 360]))
        issues.push('milestones');
