export const HERO_ASCENSION_MODIFIER_IDENTITY_IDS = ['spell-power', 'cooldown', 'area', 'move-speed', 'hero-guard', 'core-guard', 'fusion-power', 'boss-damage'];
const CELL = {
...
export const HERO_ASCENSION_MODIFIER_IDENTITY_ATLAS = { src: './assets/ui/hero-ascension-modifier-icons.png', columns: 4, rows: 2, cellSize: 96, width: 384, height: 192 };
const pct = (index, count) => count <= 1 ? 0 : (index / (count - 1)) * 100;
export function heroAscensionModifierIdentityIcon(id) { const [column, row] = CELL[id], meta = META[id]; return { id, label: meta.label, accent: meta.accent, sx: column * 96, sy: row * 96, sw: 96, sh: 96, animated: false, motionAmplitude: 0, textFallbackPreserved: true, loadFailureBlocksGameplay: false }; }
export function heroAscensionModifierIdentityStyle(id) { const [column, row] = CELL[id]; return `--secondary-icon-image:url('${HERO_ASCENSION_MODIFIER_IDENTITY_ATLAS.src}');--secondary-icon-bg-size:400% 200%;--secondary-icon-bg-position:${pct(column, 4)}% ${pct(row, 2)}%`; }
