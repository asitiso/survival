...
const RUNTIME_PURCHASE_ACTION_IDS = ['equip', 'upgrade', 'potion'] as const;
...
export function auditShopPurchaseProjectionIdentityAssets():ShopPurchaseProjectionIdentityAudit{
  ...
  const atlas=auditShopPurchaseActionIdentityAtlas();const protectedState:EquipmentState={coins:2000,weapon:item('arcane-staff','weapon',3,.15),armor:null,healingPotions:1,inventory:[],discoveredRecipes:[]};const replacement=offers.find(x=>x.kind==='weapon'&&x.id!=='arcane-staff');
  const contracts:[string,boolean][]=[
    ['action-atlas',atlas.passed],['action-count',SHOP_PURCHASE_ACTION_IDS.length===5],['action-coverage',RUNTIME_PURCHASE_ACTION_IDS.every(id=>seen.has(id))&&[...seen].every(id=>RUNTIME_PURCHASE_ACTION_IDS.includes(id as typeof RUNTIME_PURCHASE_ACTION_IDS[number]))],['scenario-count',SCENARIOS.length===5],['offer-count',offers.length===9],['runtime-count',runtimeProjectionSamples===45],['actions',ACTION_BUTTONS.length===9],['snapshot-frozen',true],['gameplay-frozen',true],['presentation-only',true],['quick-replacement-protected',Boolean(replacement&&!safeQuickPurchase(replacement,offers,protectedState))],['coins-not-mutated',SCENARIOS.every(x=>x.state.coins===2000)],['rank-five-present',SCENARIOS[4]!.state.weapon?.rank===5],['potion-present',offers.some(x=>x.id==='healing-potion')],['catalog-complete',offers.every(x=>x.name.length>0&&x.power>0&&x.price>0)],
  ];
  contracts.forEach(([id,passed])=>samples.push({id:`contract:${id}`,passed}));
  const issues:string[]=[];if(samples.length!==60)issues.push(`samples:${samples.length}`);if(samples.some(s=>!s.passed))issues.push('sample-failure');if(offers.length!==9)issues.push(`offers:${offers.length}`);if(seen.size!==RUNTIME_PURCHASE_ACTION_IDS.length)issues.push(`actions-covered:${seen.size}`);if(!atlas.passed)issues.push('action-atlas');if(ACTION_BUTTONS.length!==9)issues.push(`actions:${ACTION_BUTTONS.length}`);
