...
function effectiveRadius(id) { const t = spellTuning(id, 10); if (id === 'chainLightning')
    return 0; if (id === 'fireBolt')
    return t.radius; return t.radius; }
function contact(offset, spellRadius, nodeRadius = 24) {
    if (offset <= spellRadius + nodeRadius)
        return 1;
...
export function auditAutoWeakpointEffect() {
    ...
    if (averageContactGain < .18)
        issues.push('weakpoint-gain-too-small');
    if (minAutoContact < .92)
        issues.push('auto-contact-floor');
