...
export function auditBossEffectivePressureMultiThreatPriority() {
    const samples = [];
    ...
    const twoProfiles = [[.04, .08], [.08, .04], [.12, .20], [.20, .12]];
    for (let a = 0; a < EFFECTS.length; a++)
        for (let b = a + 1; b < EFFECTS.length; b++)
            for (const [ma, mb] of twoProfiles) {
                const first = EFFECTS[a], second = EFFECTS[b], input = neutral();
                setMultiplier(input, first, threatValue(first, ma));
                setMultiplier(input, second, threatValue(second, mb));
                const opportunity = EFFECTS.find(id => id !== first && id !== second);
                setMultiplier(input, opportunity, opportunityValue(opportunity, .45));
                ...
                twoThreatCases++;
                dualThreatRetentionPassed &&= retained;
                strongestThreatOrderingPassed &&= ordered;
                maxTwoPassed &&= p.primaryEffects.length <= 2;
                push(`two:${first}:${second}:${ma}:${mb}`, retained && ordered);
            }
    const threeProfiles = [[.04, .08, .12], [.18, .06, .10], [.12, .12, .05]];
    for (let excluded = 0; excluded < EFFECTS.length; excluded++)
        for (const profile of threeProfiles) {
            ...
            threeThreatCases++;
            dualThreatRetentionPassed &&= ok;
            strongestThreatOrderingPassed &&= ok;
            maxTwoPassed &&= p.primaryEffects.length <= 2;
            push(`three:${excluded}:${profile.join('-')}`, ok);
        }
    for (const id of EFFECTS) {
        const input = neutral();
        setMultiplier(input, id, threatValue(id, .02));
        const opportunity = EFFECTS.find(other => other !== id);
        setMultiplier(input, opportunity, opportunityValue(opportunity, .40));
        ...
        compatibilityCases++;
        oneThreatCompatibilityPassed &&= ok;
        push(`compat-one-threat:${id}`, ok);
    }
    for (const id of EFFECTS) {
        const input = neutral();
        setMultiplier(input, id, opportunityValue(id, .20));
        ...
        push(`compat-no-threat:${id}`, ok);
    }
    for (const id of EFFECTS) {
        const input = neutral();
        setMultiplier(input, id, opportunityValue(id, .005));
        const p = projectBossEffectivePressure(input), ok = p.primaryEffects.length === 0;
        compatibilityCases++;
        noThreatCompatibilityPassed &&= ok;
        push(`compat-threshold:${id}`, ok);
    }
    ...
    if (twoThreatCases !== 24)
        issues.push(`two:${twoThreatCases}`);
    if (threeThreatCases !== 12)
        issues.push(`three:${threeThreatCases}`);
    if (compatibilityCases !== 12)
        issues.push(`compat:${compatibilityCases}`);
    if (invariantCases !== 16)
        issues.push(`invariants:${invariantCases}`);
    if (!dualThreatRetentionPassed)
        issues.push('dual-threat-retention');
    if (!strongestThreatOrderingPassed)
        issues.push('strongest-threat-ordering');
    if (!oneThreatCompatibilityPassed)
        issues.push('one-threat-compatibility');
