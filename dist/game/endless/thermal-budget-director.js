...
function elapsedPressure(seconds) {
    if (seconds < 1800)
        return 0;
    if (seconds < 7200)
        return .04;
...
const DEVICE_PRESSURE = { low: .16, mid: .08, high: 0 };
const GOVERNOR_PRESSURE = { full: 0, reduced: .12, minimal: .24 };
export function thermalBudgetPolicy(input) {
    ...
    const tier = pressure >= .76 ? 'hot' : pressure >= .36 ? 'warm' : 'cool';
    if (tier === 'hot')
        return { tier, pressure, visualDensityMultiplier: .72, particleCapMultiplier: .62, trailCapMultiplier: .56, audioVoiceMultiplier: .72, telegraphMultiplier: 1, enemyLogicMultiplier: 1 };
    if (tier === 'warm')
        return { tier, pressure, visualDensityMultiplier: .88, particleCapMultiplier: .82, trailCapMultiplier: .78, audioVoiceMultiplier: .88, telegraphMultiplier: 1, enemyLogicMultiplier: 1 };
...
export function auditThermalBudget() {
    const devices = ['low', 'mid', 'high'].map((deviceClass) => {
