...
function legendaryWeapon(build, id) {
    return build.equipment.weapon?.legendary === true && build.equipment.weapon.id === id;
}
function legendaryArmor(build, id) {
    return build.equipment.armor?.legendary === true && build.equipment.armor.id === id;
}
function def(id) { return { id, ...NAMES[id] }; }
export function activeSynergies(build) {
    const out = [];
    if (build.relicId === 'abyss-eye' && legendaryWeapon(build, 'arcane-staff'))
        out.push(def('forbidden-arcana'));
    if (build.relicId === 'chrono-shard' && legendaryWeapon(build, 'rapid-wand'))
        out.push(def('broken-time'));
    if (build.relicId === 'guardian-heart' && legendaryArmor(build, 'guardian-plate'))
        out.push(def('last-bastion'));
    if (build.traitId === 'destruction' && legendaryWeapon(build, 'blast-rod'))
        out.push(def('starbreaker'));
    if (build.traitId === 'goldSense' && legendaryWeapon(build, 'golden-wand'))
        out.push(def('golden-fever'));
    if (build.traitId === 'rapidCasting' && legendaryWeapon(build, 'rapid-wand'))
        out.push(def('overclock'));
    if (build.heroId === 'arkan' && build.relicId === 'ember-crown' && legendaryWeapon(build, 'arcane-staff'))
        out.push(def('ember-dominion'));
    if (build.heroId === 'seria' && build.relicId === 'winter-heart' && legendaryWeapon(build, 'blast-rod'))
        out.push(def('winter-dominion'));
    if (build.heroId === 'kain' && build.relicId === 'storm-core' && legendaryWeapon(build, 'rapid-wand'))
        out.push(def('storm-dominion'));
    if (build.heroId === 'edric' && build.relicId === 'oath-seal' && legendaryArmor(build, 'guardian-plate'))
        out.push(def('oath-dominion'));
    return out;
}
export function synergyHudNames(build, limit = 2) {
    return activeSynergies(build).slice(0, Math.max(0, Math.floor(limit))).map((entry) => entry.name);
}
export function synergyModifiers(build) {
    const out = { ...NEUTRAL };
