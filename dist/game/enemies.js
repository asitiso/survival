...
import { impactTierForDamage } from './combat-feedback.js';
import { bossArchetypeForOrdinal, bossArchetypeTuning, bossArchetypeSpecial, bossPatternTuning, bossPhaseForRatio, bossVariantTierForOrdinal, bossVariantTuning, bossVariantLabel } from './boss-patterns.js';
import { eliteAffixModifiers, selectEliteAffixes, eliteAffixLabel } from './elite-affixes.js';
import { SPECIALIST_COMBAT_CONTRACT, assassinBlinkPosition, selectSpecialistEnemyType, specialistTarget } from './enemy-specialists.js';
...
import { projectileImpactLabelPlacements } from './projectile-impact-label-placement-arbitration.js';
...
import { projectileImpactIdentityKeys, updateProjectileImpactIdentityCoherence } from './projectile-impact-identity-coherence.js';
import { advanceSpawnLaneMemory, rememberSpawnLanePortal } from './spawn-lane-presentation-memory.js';
...
import { spawnPressureVfxSprite } from './spawn-pressure-vfx-assets.js';
import { regularEnemyActionVfxSprite } from './regular-enemy-action-vfx-assets.js';
...
import { enemyHitStaggerPresentation } from './enemy-hit-death-transition-rendering.js';
...
import { projectileThreatRhythmPresentation, specialistThreatRhythmPresentation, threatRhythmBudgetPresentation } from './threat-impact-rhythm-stagger-rendering.js';
import { projectileThreatRhythmRecoveryPresentation, specialistThreatRhythmRecoveryPresentation, threatRhythmRecoveryBudgetPresentation } from './threat-impact-rhythm-recovery-rendering.js';
import { denseProjectileThreatRhythmPresentation, denseSpecialistThreatRhythmPresentation, denseThreatRhythmBudgetPresentation } from './threat-impact-dense-rhythm-rendering.js';
...
function pointSegmentProximity(point, a, b, band) { if (!a || !b)
    ...
    return Math.max(0, 1 - distance(point, a) / safeBand); const t = Math.max(0, Math.min(1, ((point.x - a.x) * dx + (point.y - a.y) * dy) / len2)), nearest = { x: a.x + dx * t, y: a.y + dy * t }; return Math.max(0, 1 - distance(point, nearest) / safeBand); }
const BASE = {
...
const LATE_BASE_WEIGHTS = {
    grunt: 0.32, hound: 0.16, brute: 0.16, archer: 0.14, bomber: 0.12, shaman: 0.10,
};
export function selectRegularEnemyType(seconds, roll, weights) {
    const r = Math.max(0, Math.min(0.999999, roll));
    if (seconds < 480 || !weights) {
        if (seconds > 45 && r < 0.22)
            return 'hound';
        if (seconds > 90 && r < 0.32)
            return 'bomber';
        if (seconds > 120 && r < 0.48)
            return 'brute';
        if (seconds > 150 && r < 0.60)
            return 'archer';
        if (seconds > 210 && r < 0.68)
            return 'shaman';
        return 'grunt';
    }
    const order = ['grunt', 'hound', 'brute', 'archer', 'bomber', 'shaman'];
    let total = 0;
    for (const type of order)
        total += LATE_BASE_WEIGHTS[type] * Math.max(0, weights[type]);
    if (total <= 0)
        return 'grunt';
...
export class EnemyManager {
    ...
    spawnPortalVfx = [];
    spawnLaneMemory = [];
    archerProjectileImpactVfx = [];
    bossProjectileImpactVfx = [];
    projectileGuardImpactVfx = [];
    projectileCoreGuardImpactVfx = [];
    coreContactGuardImpactVfx = [];
    coreMixedPressureGuardArbitration = createCoreMixedPressureGuardArbitrationState();
    projectileImpactCountHold = [];
    projectileImpactLabelAnchorHold = [];
    projectileImpactIdentityCoherence = [];
    regularEnemyActionVfx = [];
    eliteAffixResponseVfx = [];
    specialistReactionVfx = [];
    specialistStrikeOriginVfx = [];
    nullifierHeroInside = new Set();
    nextId = 1;
    spawnTimer = 0;
    eliteTimer = 30;
    bossTimer = 120;
    ...
    endlessEliteHealthMultiplier = 1;
    get bossCountdown() { return Math.max(0, this.bossTimer); }
    get activeProjectileCount() { return this.projectiles.length; }
    projectileThreatViews() { return this.projectiles.map((p) => { const threat = projectileThreatPositionHandoff({ gameplayPos: p.pos, launchOffset: p.visualLaunchOffset, launchTtl: p.visualLaunchTtl, launchMaxTtl: p.visualLaunchMaxTtl }, this.activeReducedMotion); return { ...p, pos: { ...p.pos }, vel: { ...p.vel }, visualPos: { ...threat.pos } }; }); }
    coreWorldGuardPresentationState() { const projectileStrength = this.projectileCoreGuardImpactVfx.reduce((best, cue) => Math.max(best, cue.preventionRatio * Math.max(0, Math.min(1, cue.ttl / Math.max(.001, cue.maxTtl)))), 0), contactStrength = this.coreContactGuardImpactVfx.reduce((best, cue) => Math.max(best, cue.preventionRatio * Math.max(0, Math.min(1, cue.ttl / Math.max(.001, cue.maxTtl)))), 0), state = advanceCoreMixedPressureGuardArbitration(this.coreMixedPressureGuardArbitration, { projectileStrength, contactStrength }, 0); return { owner: state.owner, strength: Math.max(projectileStrength, contactStrength) }; }
    spawnPortalViews() { return this.spawnPortalVfx.map((portal) => ({ ...portal, pos: { ...portal.pos } })); }
    spawnLanePresentationViews() { return this.spawnLaneMemory.map((portal) => ({ ...portal, pos: { ...portal.pos } })); }
    reset() {
        this.enemies = [];
        this.projectiles = [];
        this.deaths = [];
    ...
    setEndlessScaling(healthMultiplier, damageMultiplier, projectileSpeedMultiplier, eliteHealthMultiplier = 1) {
    ...
    update(dt, ctx) {
        ...
        for (const cue of this.specialistReactionVfx)
            cue.ttl -= Math.max(0, dt);
        for (const cue of this.specialistStrikeOriginVfx)
            cue.ttl -= Math.max(0, dt);
        ...
        if (this.spawnTimer <= 0 && this.enemies.length < director.enemyBudget) {
            ...
            for (let i = 0; i < burst && this.enemies.length < director.enemyBudget; i++)
                this.spawnRegular(ctx.elapsed, director.danger, ctx.regularWeights);
        ...
        for (const enemy of this.enemies) {
            ...
            const commitRenderMotion = () => {
                ...
                if (enemy.type === 'boss')
                    enemy.bossGroundOriginRebase = advanceBossGroundOriginRebaseState(enemy.bossGroundOriginRebase, { x: enemy.pos.x, y: enemy.pos.y, phase: bossPhaseForRatio(enemy.hp / Math.max(1, enemy.maxHp)), cycle: enemy.bossCycle ?? 0 }, dt, enemy.radius, false);
                renderMotionCommitted = true;
            };
            const commitAttackResolve = () => {
                if (attackResolveCommitted)
                    return;
            ...
            if (enemy.spawnGroundMaterialize)
            ...
            if (enemy.type === 'boss') {
                const recoveryArchetype = enemy.bossArchetype ?? bossArchetypeForOrdinal(enemy.bossOrdinal ?? 0);
                enemy.bossSpecialRecovery = advanceBossSpecialRecoveryState(enemy.bossSpecialRecovery, false, dt, recoveryArchetype);
                enemy.bossHeavyHitStagger = advanceBossHeavyHitStaggerState(enemy.bossHeavyHitStagger, null, dt);
                enemy.bossVariantTier = bossVariantTierForOrdinal(enemy.bossOrdinal ?? 0, ctx.bossVariantBonus ?? 0);
            }
            ...
            if (enemy.type === 'nullifier') {
                ...
                if (inside && !wasInside) {
                    this.nullifierHeroInside.add(enemy.id);
                    this.queueSpecialistReactionVfx(enemy, 'nullifier', enemy.pos, ctx.hero.pos, 0.64);
                }
                else if (!inside && wasInside)
                    this.nullifierHeroInside.delete(enemy.id);
            }
            ...
            if (enemy.type === 'archer' && dist <= enemy.preferredRange) {
                ...
                didAttackThisFrame = true;
                if (isSpecialistEnemyType(enemy.type) && enemy.type !== 'nullifier') {
                    const dir = normalize({ x: targetObj.pos.x - enemy.pos.x, y: targetObj.pos.y - enemy.pos.y });
                    const strike = specialistStrikeOriginCoherencePresentation({ type: enemy.type, radius: enemy.radius, facingX: dir.x, facingY: dir.y, pullback: 0, lunge: 1, resolve: 0, silhouetteForward: enemy.radius * .42, silhouetteLateral: 0 }, ctx.reducedMotion ?? false);
                    const maxTtl = .18;
                    this.specialistStrikeOriginVfx.push({ pos: { ...enemy.pos }, origin: { x: enemy.pos.x + strike.originOffsetX, y: enemy.pos.y + strike.originOffsetY }, target: { ...targetObj.pos }, recoveryFacing: { x: enemy.renderMotion?.facingX ?? dir.x, y: enemy.renderMotion?.facingY ?? dir.y }, enemyId: enemy.id, type: enemy.type, ttl: maxTtl, maxTtl });
                    if (this.specialistStrikeOriginVfx.length > 24)
                ...
                if (enemy.target === 'core') {
                    ...
                    if (contactGuard.owner === 'contact-guard') {
                        this.coreContactGuardImpactVfx.push({ pos: { ...targetObj.pos }, incoming: { x: targetObj.pos.x - enemy.pos.x, y: targetObj.pos.y - enemy.pos.y }, preventionRatio, ttl: maxTtl, maxTtl });
                        if (this.coreContactGuardImpactVfx.length > 12)
                            this.coreContactGuardImpactVfx.splice(0, this.coreContactGuardImpactVfx.length - 12);
                    }
                }
                else
                    ctx.onHeroDamage(enemy.damage * frenzyDamage, 'contact');
                if (enemy.type === 'siegeGolem') {
        ...
        const impactHoldInputs = [...this.archerProjectileImpactVfx.map((cue) => ({ impact: cue.pos, incoming: cue.incoming, sourceClass: 'archer' })), ...this.bossProjectileImpactVfx.map((cue) => ({ impact: cue.pos, incoming: cue.incoming, sourceClass: 'boss' }))];
        const impactHoldClusters = projectileImpactClusters({ impacts: impactHoldInputs, quality: 'high', reducedFlash: false });
        const impactIdentity = updateProjectileImpactIdentityCoherence(this.projectileImpactIdentityCoherence, impactHoldClusters, dt);
        this.projectileImpactIdentityCoherence = impactIdentity.memory;
        if (impactIdentity.retiredIdentityIds.length) {
            this.projectileImpactCountHold = retireProjectileImpactCountHoldIdentities(this.projectileImpactCountHold, impactIdentity.retiredIdentityIds);
            this.projectileImpactLabelAnchorHold = retireProjectileImpactLabelAnchorIdentities(this.projectileImpactLabelAnchorHold, impactIdentity.retiredIdentityIds);
        }
        ...
        for (const id of this.nullifierHeroInside)
            if (!liveNullifiers.has(id))
                this.nullifierHeroInside.delete(id);
    }
    damage(enemy, amount, source, visualSource = 'normal') {
        ...
        enemy.hitImpactTier = impactTier;
        enemy.hitDirectionX = hitVector.x;
    ...
    pushAway(enemy, origin, distanceAmount) {
        if (!enemy.alive || distanceAmount <= 0)
            return;
    ...
    markLastDeathVisualSource(source) {
        const death = this.deaths[this.deaths.length - 1];
        if (death)
            death.visualSource = source;
    }
    drainDeaths() {
    ...
    spawnEventEnemy(type, danger, target = 'hero', pos) {
    ...
    nearestEnemy(pos, predicate = () => true) {
        let best = null;
        let bestD = Number.POSITIVE_INFINITY;
        for (const enemy of this.enemies) {
            if (!enemy.alive || !predicate(enemy))
                continue;
    ...
    drawProjectileImpactSourceContinuity(ctx, pos, incoming, sourceClass, quality, reducedFlash) {
        const segment = projectileImpactSourceContinuity({ impact: pos, incoming, sourceClass, quality, reducedFlash });
        if (!segment)
            return;
        ctx.save();
        ctx.globalAlpha = segment.alpha;
        ctx.strokeStyle = segment.accent;
        ctx.lineWidth = sourceClass === 'boss' ? 3 : 2;
        ctx.setLineDash([7, 5]);
        ctx.beginPath();
    ...
    drawProjectileImpactSourceCluster(ctx, cluster, heldCount, labelPlacement) {
        ctx.save();
        ctx.globalAlpha = cluster.alpha;
        ctx.strokeStyle = cluster.accent;
        ctx.fillStyle = cluster.accent;
        ctx.lineWidth = cluster.sourceClass === 'boss' ? 3 : 2;
        ctx.setLineDash([7, 5]);
        ctx.beginPath();
        ctx.moveTo(cluster.start.x, cluster.start.y);
        ctx.lineTo(cluster.end.x, cluster.end.y);
        ctx.stroke();
        ctx.setLineDash([]);
        if (heldCount > 1 && labelPlacement?.visible) {
            ctx.font = '700 10px system-ui';
            ctx.textAlign = 'center';
            ctx.fillText(`×${heldCount}`, labelPlacement.pos.x, labelPlacement.pos.y);
        }
        ctx.restore();
    }
    projectileImpactLabelBlockers(presentationQuality = 'high', reducedFlash = false) {
        const impactClusterInputs = [...this.archerProjectileImpactVfx.map((cue) => ({ impact: cue.pos, incoming: cue.incoming, sourceClass: 'archer' })), ...this.bossProjectileImpactVfx.map((cue) => ({ impact: cue.pos, incoming: cue.incoming, sourceClass: 'boss' }))];
    ...
    renderProjectiles(ctx, bossSpecialVfxAtlasImage = null, bossSpecialVfxAtlasReady = false, battlefieldEnvironmentReactionVfxAtlasImage = null, battlefieldEnvironmentReactionVfxAtlasReady = false, bossProjectileLifecycleVfxAtlasImage = null, bossProjectileLifecycleVfxAtlasReady = false, presentationQuality = 'high', reducedFlash = false, reducedMotion = false, safeLaneOrigin = null, safeLaneTarget = null) {
        ...
        for (const projectile of this.projectiles) {
            ...
            if (bossTravelRelease && bossTravelRelease.visible && bossBridgeBudget.visible) {
                ctx.save();
                ...
                ctx.lineWidth = Math.max(1.2, projectile.radius * .15) * projectileReadabilityContrast.strokeWidthScale * (projectile.bossArchetype ? 1 : projectileFinalSettle.strokeWidthScale);
                ctx.beginPath();
            ...
            if (trail.owner === 'launch' && projectileResolutionBudget.visible) {
                ctx.save();
                ...
                ctx.lineWidth = Math.max(1.3, projectile.radius * .2) * projectileReadabilityContrast.strokeWidthScale * (projectile.bossArchetype ? 1 : projectileFinalSettle.strokeWidthScale);
                ctx.beginPath();
            ...
            else if (travelThreatCarry.visible && projectileResolutionBudget.visible) {
                const speed = Math.hypot(projectile.vel.x, projectile.vel.y) || 1, dx = projectile.vel.x / speed, dy = projectile.vel.y / speed, len = Math.max(travelThreatCarry.minLength, trail.length * travelThreatCarry.lengthScale * directionCarryRecovery.tailLengthScale * projectileResolution.trailLengthScale);
                ctx.save();
                ...
                ctx.translate(projectileNormalX * projectileSpatial.lateralOffset, projectileNormalY * projectileSpatial.lateralOffset);
                ctx.strokeStyle = projectile.bossArchetype ? '#ffb26f' : '#ff7c86';
                ctx.lineWidth = Math.max(1.1, projectile.radius * .14) * projectileReadabilityContrast.strokeWidthScale * (projectile.bossArchetype ? 1 : projectileFinalSettle.strokeWidthScale);
                ctx.beginPath();
            ...
            if (hasBossVisual && bossSpecialVfxAtlasImage && projectile.bossArchetype) {
                ...
                ctx.rotate(angle);
                ctx.globalAlpha = 0.92 * projectileResolution.bodyAlphaScale * projectileDepth.bodyAlphaScale * projectileLaneDepth.bodyAlphaScale * projectileDepthBudget.canonicalBodyAlphaScale * projectileDepthRecovery.bodyAlphaScale * safeLaneDepthRecovery.bodyAlphaScale * projectileRecoveryBudget.canonicalBodyAlphaScale * projectileCriticalLatch.criticalAlphaScale * projectileCanonicalStack.bodyAlphaScale * projectileUnifiedStack.canonicalBodyAlphaScale * projectileCorridorSeparation.bodyAlphaScale * projectileSpatialSeparationBudget.canonicalBodyAlphaScale * projectileCorridorRelease.bodyAlphaScale * projectileSpatialRecoveryBudget.canonicalBodyAlphaScale * projectileDenseArbitration.bodyAlphaScale * projectileDenseBattlefield.canonicalBodyAlphaScale * projectileDepthPlane.bodyScale * projectileDepthPlaneBudget.canonicalScale * projectileDepthReentry.bodyScale * projectileDepthReentryBudget.canonicalScale * projectileBossFocus.bodyScale * projectileBossFocusBudget.canonicalScale * projectileCanonicalReacquisition.bodyScale * projectileCanonicalReacquisitionBudget.canonicalScale * projectileCriticalReengagement.bodyScale * projectileCriticalReengagementBudget.canonicalScale;
            ...
            if (projectile.sourceType === 'archer' && battlefieldEnvironmentReactionVfxAtlasReady && battlefieldEnvironmentReactionVfxAtlasImage) {
                ...
                ctx.translate(visualPos.x, visualPos.y);
                ctx.rotate(angle);
                ctx.globalAlpha = 0.94 * projectileResolution.bodyAlphaScale * projectileDepth.bodyAlphaScale * projectileLaneDepth.bodyAlphaScale * projectileDepthBudget.canonicalBodyAlphaScale * projectileDepthRecovery.bodyAlphaScale * safeLaneDepthRecovery.bodyAlphaScale * projectileRecoveryBudget.canonicalBodyAlphaScale * projectileCriticalLatch.criticalAlphaScale * projectileCanonicalStack.bodyAlphaScale * projectileUnifiedStack.canonicalBodyAlphaScale * projectileCorridorSeparation.bodyAlphaScale * projectileSpatialSeparationBudget.canonicalBodyAlphaScale * projectileCorridorRelease.bodyAlphaScale * projectileSpatialRecoveryBudget.canonicalBodyAlphaScale * projectileDenseArbitration.bodyAlphaScale * projectileDenseBattlefield.canonicalBodyAlphaScale * projectileDepthPlane.bodyScale * projectileDepthPlaneBudget.canonicalScale * projectileDepthReentry.bodyScale * projectileDepthReentryBudget.canonicalScale * projectileBossFocus.bodyScale * projectileBossFocusBudget.canonicalScale * projectileCanonicalReacquisition.bodyScale * projectileCanonicalReacquisitionBudget.canonicalScale * projectileCriticalReengagement.bodyScale * projectileCriticalReengagementBudget.canonicalScale;
                ctx.drawImage(battlefieldEnvironmentReactionVfxAtlasImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, -size / 2, -size / 2, size, size);
                ctx.restore();
            }
        }
        const impactClusterInputs = [
            ...this.archerProjectileImpactVfx.map((cue) => ({ impact: cue.pos, incoming: cue.incoming, sourceClass: 'archer' })),
            ...this.bossProjectileImpactVfx.map((cue) => ({ impact: cue.pos, incoming: cue.incoming, sourceClass: 'boss' })),
        ];
        ...
        for (let i = 0; i < impactClusters.length; i++) {
            const cluster = impactClusters[i], heldCount = heldCounts[i] ?? cluster.count, labelPlacement = impactLabelPlacements[i];
            this.drawProjectileImpactSourceCluster(ctx, cluster, heldCount, labelPlacement);
        }
        if (battlefieldEnvironmentReactionVfxAtlasReady && battlefieldEnvironmentReactionVfxAtlasImage) {
            ...
            for (const cue of this.archerProjectileImpactVfx) {
                const t = Math.max(0, Math.min(1, cue.ttl / .34));
                const size = 64 + (1 - t) * 18;
                const impactEntryOffset = cue.entryOffset ?? { x: 0, y: 0 };
                const ordinaryImpactAlphaScale = cue.ordinaryImpactAlphaScale ?? 1;
                const impactVisualPos = projectileImpactVisualPosition(cue.pos, impactEntryOffset, cue.ttl, .34);
                ctx.save();
                ctx.globalAlpha = t * .78 * ordinaryImpactAlphaScale;
                ctx.drawImage(battlefieldEnvironmentReactionVfxAtlasImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, impactVisualPos.x - size / 2, impactVisualPos.y - size / 2, size, size);
                ctx.restore();
            }
        }
        for (const cue of this.projectileGuardImpactVfx) {
            const guard = projectileGuardImpactHandoffPresentation({ preventedRatio: cue.preventionRatio, impactTtl: cue.ttl, impactMaxTtl: cue.maxTtl }, reducedFlash);
            if (guard.owner !== 'guard' || guard.guardImpactAlpha <= 0)
                continue;
            ...
            ctx.save();
            ctx.globalAlpha = guard.guardImpactAlpha;
            ctx.strokeStyle = '#8fffd3';
            ctx.lineWidth = 3;
            ctx.beginPath();
            ctx.moveTo(cue.pos.x - nx * half, cue.pos.y - ny * half);
            ctx.lineTo(cue.pos.x + nx * half, cue.pos.y + ny * half);
            ctx.stroke();
            ctx.restore();
        }
        ...
        for (const cue of this.projectileCoreGuardImpactVfx) {
            const guard = coreProjectileGuardImpactHandoffPresentation({ preventedRatio: cue.preventionRatio, impactTtl: cue.ttl, impactMaxTtl: cue.maxTtl }, reducedFlash);
            if (guard.owner !== 'core-guard' || guard.coreGuardImpactAlpha <= 0)
                continue;
            const mag = Math.hypot(cue.incoming.x, cue.incoming.y) || 1, nx = -cue.incoming.y / mag, ny = cue.incoming.x / mag, half = guard.deflectDistance * .5;
            ctx.save();
            ctx.globalAlpha = guard.coreGuardImpactAlpha * mixedCoreGuard.projectileAlphaScale;
            ctx.strokeStyle = '#7fd9ff';
            ctx.lineWidth = 3;
            ctx.beginPath();
            ctx.arc(cue.pos.x, cue.pos.y, guard.arcRadius, Math.atan2(ny, nx) - .9, Math.atan2(ny, nx) + .9);
            ctx.stroke();
            ctx.beginPath();
            ctx.moveTo(cue.pos.x - nx * half, cue.pos.y - ny * half);
            ctx.lineTo(cue.pos.x + nx * half, cue.pos.y + ny * half);
        ...
        for (const cue of this.coreContactGuardImpactVfx) {
            const guard = coreContactGuardMemoryPresentation({ preventedRatio: cue.preventionRatio, ttl: cue.ttl, maxTtl: cue.maxTtl }, reducedFlash);
            if ((guard.owner !== 'contact-guard' && guard.owner !== 'memory') || (guard.contactAlpha <= 0 && guard.memoryAlpha <= 0))
                continue;
            ...
            ctx.lineWidth = guard.owner === 'memory' ? 1.5 : 2.5;
            ctx.setLineDash(guard.owner === 'memory' ? [5, 7] : []);
            ctx.beginPath();
            ctx.ellipse(0, 8, guard.braceWidth * .5, guard.braceHeight * .5, 0, 0, Math.PI * 2);
            ctx.stroke();
            ctx.setLineDash([]);
            ctx.restore();
        }
        if (bossProjectileLifecycleVfxAtlasReady && bossProjectileLifecycleVfxAtlasImage) {
            for (const cue of this.bossProjectileImpactVfx) {
                ...
                const impactEntryOffset = cue.entryOffset ?? { x: 0, y: 0 };
                const impactVisualPos = projectileImpactVisualPosition(cue.pos, impactEntryOffset, cue.ttl, cue.maxTtl);
                const ordinaryImpactAlphaScale = cue.ordinaryImpactAlphaScale ?? 1;
                ctx.save();
                ctx.globalAlpha = Math.min(0.82, t * 0.9) * ordinaryImpactAlphaScale;
    ...
    renderEnemies(ctx, spriteAtlasImage = null, spriteAtlasReady = false, bossSpriteAtlasImage = null, bossSpriteAtlasReady = false, residualMotion, eliteAffixAtlasImage = null, eliteAffixAtlasReady = false, specialistIntentAtlasImage = null, specialistIntentAtlasReady = false, heroPos = null, specialistCombatVfxAtlasImage = null, specialistCombatVfxAtlasReady = false, bossPhaseOverlayVfxAtlasImage = null, bossPhaseOverlayVfxAtlasReady = false, interactionAtlasImage = null, interactionAtlasReady = false, spawnPressureVfxAtlasImage = null, spawnPressureVfxAtlasReady = false, regularEnemyActionVfxAtlasImage = null, regularEnemyActionVfxAtlasReady = false, eliteAffixLifecycleVfxAtlasImage = null, eliteAffixLifecycleVfxAtlasReady = false, enemyTargetPressureVfxAtlasImage = null, enemyTargetPressureVfxAtlasReady = false, corePos = null, specialistReactionLifecycleVfxAtlasImage = null, specialistReactionLifecycleVfxAtlasReady = false, reducedFlash = false, reducedMotion = false, hazardPressure = 0) {
        for (const portal of this.spawnPortalVfx) {
        ...
        for (const [strikeCueIndex, cue] of this.specialistStrikeOriginVfx.entries()) {
            ...
            ctx.save();
            ctx.globalAlpha = (.28 + .5 * life) * strikeCueBudget.alphaScale;
            ctx.strokeStyle = cue.type === 'assassin' ? '#e4b5ff' : cue.type === 'siegeGolem' ? '#ffcb7a' : '#9fd7ff';
            ctx.fillStyle = ctx.strokeStyle;
            ctx.lineWidth = 2.4 * strikeCueBudget.lineWidthScale;
            ctx.beginPath();
            ctx.moveTo(cue.pos.x, cue.pos.y);
            ctx.lineTo(arrival.marker.x, arrival.marker.y);
            ctx.stroke();
            ctx.beginPath();
            ctx.arc(arrival.marker.x, arrival.marker.y, 2.8 + 1.8 * life, 0, Math.PI * 2);
            ctx.fill();
            ctx.restore();
            if (impactFinish.visible) {
                const responseStrength = this.coreContactGuardImpactVfx.some((g) => g.ttl > 0 && distance(g.pos, cue.target) <= 28) ? 1 : 0, impactResponse = specialistImpactFinishResponseArbitrationPresentation({ finishAlpha: impactFinish.alpha, responseStrength }, reducedFlash), impactFinishBudget = specialistImpactFinishDensityBudgetPresentation({ activeCount: activeImpactFinishCount, indexFromNewest: Math.max(0, activeStrikeCueCount - 1 - strikeCueIndex), type: cue.type, life: life }, reducedMotion, reducedFlash), recoveryDensityBudget = specialistImpactRecoveryDensityBudgetPresentation({ activeCount: activeImpactFinishCount, indexFromNewest: Math.max(0, activeStrikeCueCount - 1 - strikeCueIndex), owner: impactRecovery.owner, type: cue.type, recoveryBlend: impactRecovery.recoveryBlend }, reducedMotion);
                if (impactFinishBudget.visible && recoveryDensityBudget.visible) {
                    const recoveryEnd = { x: impactRecovery.start.x + (impactRecovery.end.x - impactRecovery.start.x) * recoveryDensityBudget.lengthScale, y: impactRecovery.start.y + (impactRecovery.end.y - impactRecovery.start.y) * recoveryDensityBudget.lengthScale };
                    ctx.save();
                    ctx.globalAlpha = impactFinish.alpha * impactRecovery.alphaScale * recoveryDensityBudget.alphaScale * strikeCueBudget.alphaScale * impactResponse.alphaScale * impactFinishBudget.alphaScale;
                    ctx.strokeStyle = cue.type === 'assassin' ? '#f0cfff' : cue.type === 'siegeGolem' ? '#ffd48f' : '#b9e4ff';
                    ctx.lineWidth = 2.2 * strikeCueBudget.lineWidthScale;
                    ctx.beginPath();
                    ctx.moveTo(impactRecovery.start.x, impactRecovery.start.y);
                    ctx.lineTo(recoveryEnd.x, recoveryEnd.y);
                    ctx.stroke();
                    ctx.restore();
                }
            }
        ...
        for (const enemy of this.enemies) {
            ...
            if (nextAttackAnticipation?.visible && (anticipationHandoff?.alphaScale ?? 0) > 0 && (anticipationDensityBudget?.visible ?? true)) {
                const start = Math.max(4, enemy.radius * .72), end = start + nextAttackAnticipation.reach * (anticipationDensityBudget?.reachScale ?? 1);
                ctx.save();
                ctx.globalAlpha = nextAttackAnticipation.alpha * (anticipationHandoff?.alphaScale ?? 1) * (anticipationSilhouetteContinuity?.anticipationAlphaScale ?? 1) * (anticipationSilhouetteHandoff?.cueAlphaScale ?? 1) * (anticipationSilhouetteDensityBudget?.cueAlphaScale ?? 1) * (anticipationDensityBudget?.alphaScale ?? 1);
                ctx.strokeStyle = enemy.type === 'assassin' ? '#e4b5ff' : enemy.type === 'siegeGolem' ? '#ffcb7a' : enemy.type === 'nullifier' ? '#87b8ff' : '#b9e4ff';
                ctx.lineWidth = 1.8;
                ctx.beginPath();
                ctx.moveTo(nextAttackAnticipation.facingX * start, nextAttackAnticipation.facingY * start);
                ctx.lineTo(nextAttackAnticipation.facingX * end, nextAttackAnticipation.facingY * end);
                ctx.stroke();
                ctx.restore();
            }
            ...
            if (enemy.type === 'elite' || enemy.type === 'boss') {
                ctx.save();
                if (enemy.type === 'boss' && bossSpecialCueBudget)
                    ctx.globalAlpha = bossSpecialCueBudget.baseOutlineScale * bossSpecialCueBudget.alphaScale;
            ...
            if (enemy.type === 'boss' && (enemy.specialTimer ?? 99) <= 1.2) {
                ctx.save();
                if (bossAnticipationOriginLock)
                    ctx.translate(bossAnticipationOriginLock.offsetX, bossAnticipationOriginLock.offsetY);
                else if (bossAnticipationOrigin)
                    ctx.translate(bossAnticipationOrigin.ringOffsetX, bossAnticipationOrigin.ringOffsetY);
                ...
                ctx.beginPath();
                ctx.arc(0, 0, (enemy.radius + 22 + charge * 22 + (bossSpecialBodyLanguage?.auraRadiusBoost ?? 0)) * (bossSpecialAnticipation?.ringRadiusScale ?? 1), 0, Math.PI * 2);
                ctx.stroke();
                if (charge > 0.55) {
                    ...
                    ctx.beginPath();
                    ctx.arc(0, 0, (enemy.radius + 42 + charge * 34) * (bossSpecialAnticipation?.ringRadiusScale ?? 1), 0, Math.PI * 2);
                    ctx.stroke();
                }
                ctx.globalAlpha = 1;
                ctx.restore();
            ...
            if (dynamicSilhouette) {
                ...
                ctx.beginPath();
                ctx.ellipse(0, 0, enemy.radius * .92 * specialistSilhouetteWidthScale, enemy.radius * .92 * specialistSilhouetteHeightScale, 0, 0, Math.PI * 2);
                ctx.fill();
                ctx.restore();
            }
            ctx.save();
            ...
            ctx.fill();
            ctx.strokeStyle = enemy.target === 'core' ? '#76dbff' : 'rgba(18,23,31,.9)';
            ctx.lineWidth = 3;
            ctx.stroke();
            ctx.restore();
            if (spritePresentation.visible && spriteAtlasImage && isEnemySpriteType(enemy.type)) {
                ...
                if (enemy.hitFlash > 0) {
                    ...
                    ctx.beginPath();
                    ctx.arc(0, 0, enemy.radius * 0.82, 0, Math.PI * 2);
                    ctx.fill();
                    ctx.restore();
                }
            }
            if (bossPresentation?.visible && bossSpriteAtlasImage && bossArchetype) {
                ...
                if (enemy.hitFlash > 0) {
                    ...
                    ctx.fillStyle = '#ffffff';
                    ctx.beginPath();
                    ctx.arc(0, 0, enemy.radius * 0.86, 0, Math.PI * 2);
                    ctx.fill();
                    ctx.restore();
                }
                ...
                if (phase >= 2 && bossPhaseOverlayVfxAtlasReady && bossPhaseOverlayVfxAtlasImage) {
                    ...
                    ctx.save();
                    ctx.globalAlpha = (phase === 3 ? 0.76 : 0.56) * (bossSpecialCueBudget ? bossSpecialCueBudget.phaseOverlayScale : 1) * (bossSpecialCueBudget?.alphaScale ?? 1);
            ...
            if (enemyTargetPressureVfxAtlasReady && enemyTargetPressureVfxAtlasImage && enemyTargetPressureVisible(enemy.type, enemy.target)) {
                ...
                ctx.save();
                ctx.globalAlpha = reducedFlash ? 0.56 : 0.88;
            ...
            if (enemy.type === 'golden') {
                ctx.strokeStyle = 'rgba(255,232,111,.92)';
                ctx.lineWidth = 4;
                ctx.beginPath();
                ctx.arc(0, 0, enemy.radius + 8 + Math.sin(enemy.attackTimer * 3) * (residualMotion?.goldenEnemyMotionAmplitude ?? 0.05) * 40, 0, Math.PI * 2);
                ctx.stroke();
                ...
                ctx.strokeStyle = 'rgba(255,106,61,.88)';
                ctx.lineWidth = 3;
                ctx.beginPath();
                ctx.arc(0, 0, enemy.radius + 7 + Math.sin(enemy.attackTimer * 8) * (residualMotion?.bomberBodyMotionAmplitude ?? 0.05) * 40, 0, Math.PI * 2);
                ctx.stroke();
                ctx.fillStyle = '#40150c';
                ctx.beginPath();
                ctx.arc(0, 0, 5, 0, Math.PI * 2);
                ctx.fill();
            }
            else if (enemy.type === 'shaman') {
                ctx.strokeStyle = 'rgba(126,255,172,.72)';
                ctx.lineWidth = 3;
                ctx.beginPath();
                ctx.arc(0, 0, enemy.radius + 10, 0, Math.PI * 2);
                ctx.stroke();
            ...
            if (enemy.type === 'shieldbearer' && (enemy.guardHp ?? 0) > 0) {
                ctx.strokeStyle = 'rgba(152,190,255,.88)';
                ctx.lineWidth = 5;
                ctx.beginPath();
                ctx.arc(0, 0, enemy.radius + 7, Math.PI * 0.65, Math.PI * 1.35);
                ctx.stroke();
            }
            else if (enemy.type === 'assassin') {
                ctx.strokeStyle = 'rgba(214,126,255,.75)';
                ctx.lineWidth = 2;
                ctx.setLineDash([5, 4]);
                ctx.beginPath();
                ...
                ctx.fillStyle = '#ffe0a5';
                ctx.font = '900 9px system-ui';
                ctx.textAlign = 'center';
                ctx.fillText('CORE', 0, 2);
            }
            else if (enemy.type === 'nullifier') {
                ctx.strokeStyle = 'rgba(111,168,255,.55)';
                ctx.lineWidth = 2;
                ctx.beginPath();
                ctx.arc(0, 0, enemy.preferredRange * 0.32, 0, Math.PI * 2);
                ctx.stroke();
            }
            if (isSpecialistCombatVfxType(enemy.type) && specialistCombatVfxAtlasReady && specialistCombatVfxAtlasImage) {
                ...
                ctx.save();
                ctx.globalAlpha = emphasis ? 0.74 : 0.34;
            ...
            if (isSpecialistIntentType(enemy.type) && specialistIntentAtlasReady && specialistIntentAtlasImage) {
                ...
                if (emphasis) {
                    ...
                    ctx.strokeStyle = 'rgba(205,245,255,.84)';
                    ctx.lineWidth = 1.25;
            ...
            if (enemy.type !== 'grunt' && enemy.type !== 'hound') {
                ...
                ctx.fillStyle = '#2b1015';
                ctx.fillRect(-enemy.radius, -enemy.radius - 12, w, 5);
                ctx.fillStyle = '#f45b68';
                ctx.fillRect(-enemy.radius, -enemy.radius - 12, w * Math.max(0, enemy.hp / enemy.maxHp), 5);
            }
            ctx.restore();
        }
        if (regularEnemyActionVfxAtlasReady && regularEnemyActionVfxAtlasImage) {
            for (const cue of this.regularEnemyActionVfx) {
                ...
                ctx.save();
                ctx.globalAlpha = Math.min(reducedFlash ? 0.48 : 0.84, t * 0.92);
        ...
        if (specialistReactionLifecycleVfxAtlasReady && specialistReactionLifecycleVfxAtlasImage) {
            for (const cue of this.specialistReactionVfx) {
                ...
                const state = progress < 0.38 ? 'trigger' : 'afterglow';
                const sprite = specialistReactionLifecycleVfxSprite(cue.type, state);
                const drawPos = cue.type === 'assassin' && state === 'afterglow' && cue.targetPos ? cue.targetPos : cue.type === 'siegeGolem' && cue.targetPos ? cue.targetPos : cue.pos;
                const base = cue.type === 'siegeGolem' ? 132 : cue.type === 'nullifier' ? 124 : cue.type === 'shieldbearer' ? 104 : 96;
                const size = base * (1 + progress * .18);
                ctx.save();
                ctx.globalAlpha = Math.min(reducedFlash ? 0.42 : 0.76, (1 - progress) * 0.82 + 0.08);
                ctx.drawImage(specialistReactionLifecycleVfxAtlasImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, drawPos.x - size / 2, drawPos.y - size / 2, size, size);
                ctx.restore();
            }
        }
    }
    spawnRegular(seconds, danger, weights) {
        const specialist = selectSpecialistEnemyType(seconds, Math.random());
        const type = specialist ?? selectRegularEnemyType(seconds, Math.random(), weights);
        const coreChance = Math.min(0.34, 0.20 + danger * 0.008);
        const target = specialist ? specialistTarget(specialist) : (Math.random() < coreChance ? 'core' : 'hero');
    ...
    spawn(type, danger, target, explicitPos) {
        ...
        const portalKind = type === 'boss' ? 'boss' : type === 'elite' ? 'elite' : isSpecialistEnemyType(type) ? 'specialist' : 'regular';
        this.enemies.push({
            ...stats,
        ...
        if (this.spawnPortalVfx.length > 28)
            this.spawnPortalVfx.splice(0, this.spawnPortalVfx.length - 28);
        return id;
    }
    commandAuraBoost(enemy) {
        if (enemy.type === 'boss' || enemy.type === 'golden')
            return 1;
        for (const candidate of this.enemies) {
            if (!candidate.alive || candidate.id === enemy.id || (candidate.commandAuraMultiplier ?? 1) <= 1)
                continue;
    ...
    spawnPointOutsideArena() {
        const side = Math.floor(Math.random() * 4);
        const pad = 45;
        if (side === 0)
            return { x: ARENA_MARGIN + Math.random() * (LOGICAL_WIDTH - ARENA_MARGIN * 2), y: ARENA_MARGIN - pad };
        if (side === 1)
            return { x: LOGICAL_WIDTH - ARENA_MARGIN + pad, y: ARENA_MARGIN + Math.random() * (LOGICAL_HEIGHT - ARENA_MARGIN * 2) };
        if (side === 2)
            return { x: ARENA_MARGIN + Math.random() * (LOGICAL_WIDTH - ARENA_MARGIN * 2), y: LOGICAL_HEIGHT - ARENA_MARGIN + pad };
        return { x: ARENA_MARGIN - pad, y: ARENA_MARGIN + Math.random() * (LOGICAL_HEIGHT - ARENA_MARGIN * 2) };
    }
    updateBossSpecial(enemy, dt, ctx, danger, enemyBudget, bossVariantBonus = 0) {
        const phase = bossPhaseForRatio(enemy.hp / enemy.maxHp);
        if (!enemy.bossArchetype && enemy.bossOrdinal === undefined) {
            ...
            if (phase === 1)
                ...
                if (cycle % 2 === 0)
                    ...
                    this.summonBossAdds(enemy, legacy.summonCount, danger, enemyBudget);
            }
            else {
                this.fireEnemyFan(enemy, ctx.hero.pos, legacy.fanProjectiles, legacy.fanSpread);
                if (cycle % 2 === 1)
        ...
        const projectileCount = (count) => Math.max(1, Math.round(count * (tacticLink?.projectileCountMultiplier ?? 1)));
        const summonCount = (count) => Math.max(0, Math.round(count * (tacticLink?.summonCountMultiplier ?? 1)));
        const dashDistance = (amount) => amount * (tacticLink?.dashDistanceMultiplier ?? 1);
        if (archetype === 'inferno') {
            ...
            if (tuning.ringProjectiles > 0 && (phase === 3 || cycle % 2 === 1))
                this.fireEnemyRing(enemy, projectileCount(tuning.ringProjectiles), tuning.projectileSpeedMultiplier);
            if (tuning.summonCount > 0 && cycle % 3 === 2)
            ...
            if (cycle % 2 === 0)
                this.fireEnemyFan(enemy, ctx.hero.pos, projectileCount(tuning.fanProjectiles), tuning.fanSpread, tuning.projectileSpeedMultiplier);
            if (phase === 3 && tuning.ringProjectiles > 0 && cycle % 3 === 2)
                this.fireEnemyRing(enemy, projectileCount(tuning.ringProjectiles), tuning.projectileSpeedMultiplier);
        }
            ...
            this.fireEnemyFan(enemy, ctx.hero.pos, projectileCount(tuning.fanProjectiles), tuning.fanSpread, tuning.projectileSpeedMultiplier);
            if (tuning.ringProjectiles > 0 && phase >= 2)
                this.fireEnemyRing(enemy, projectileCount(tuning.ringProjectiles), tuning.projectileSpeedMultiplier);
            if (tuning.summonCount > 0 && phase === 3 && cycle % 3 === 2)
                this.summonBossAdds(enemy, summonCount(tuning.summonCount * this.bossEncounterModifiers.summonCountMultiplier), danger, enemyBudget, archetype);
        }
            ...
            if (phase >= 2)
                this.dashBossToward(enemy, ctx.hero.pos, dashDistance(tuning.dashDistance * 0.55));
        }
        else {
            const special = bossArchetypeSpecial(archetype, phase);
            this.fireEnemyFan(enemy, ctx.hero.pos, projectileCount(tuning.fanProjectiles), tuning.fanSpread, tuning.projectileSpeedMultiplier);
            if (tuning.ringProjectiles > 0)
                this.fireEnemyRing(enemy, projectileCount(tuning.ringProjectiles), tuning.projectileSpeedMultiplier);
        ...
        if (enemy.isApex && enemy.apexSecondaryArchetype)
            this.executeApexSecondary(enemy, enemy.apexSecondaryArchetype, ctx, danger, enemyBudget, phase, tuning.projectileSpeedMultiplier);
        ...
        if (mythicPhase) {
            ...
            for (const channel of mythicPhase.channels.slice(1, 3))
                this.executeApexSecondary(enemy, channel, ctx, danger, enemyBudget, phase, tuning.projectileSpeedMultiplier, phasePressure);
        }
        ...
        if (tacticLink)
            ctx.onMythicTacticAttackLinkConsumed?.(archetype);
    }
    executeApexSecondary(enemy, archetype, ctx, danger, enemyBudget, phase, projectileSpeedMultiplier, pressure) {
    ...
    fireEnemyFan(enemy, target, count, spread, speedMultiplier = 1) {
    ...
    fireEnemyRing(enemy, count, speedMultiplier = 1) {
        for (let i = 0; i < count && this.projectiles.length < 150; i++) {
    ...
    summonBossAdds(boss, count, danger, enemyBudget, archetype = 'inferno') {
        if (count > 0)
            boss.bossSpecialOriginHandoff = advanceBossSpecialOriginHandoffState(boss.bossSpecialOriginHandoff, { kind: 'summon', offsetX: 0, offsetY: 0 }, 0, boss.radius, false);
        for (let i = 0; i < count && this.enemies.length < enemyBudget; i++) {
            let type;
            if (archetype === 'summoner')
                type = i % 4 === 3 ? 'shaman' : i % 3 === 2 ? 'brute' : 'hound';
            else if (archetype === 'abyssWitch')
                type = i % 2 === 0 ? 'nullifier' : 'shaman';
            else if (archetype === 'timeEater')
    ...
    dashBossToward(boss, target, distanceAmount) {
        if (distanceAmount <= 0)
            return;
    ...
    detonateBomber(enemy, ctx) {
        ...
        if (distance(enemy.pos, ctx.hero.pos) <= blastRadius + ctx.hero.radius)
            ctx.onHeroDamage(enemy.damage, 'explosion');
        if (distance(enemy.pos, ctx.core.pos) <= blastRadius + ctx.core.radius)
            ctx.onCoreDamage(enemy.damage, 'explosion', enemy.pos);
        enemy.alive = false;
    }
    healNearby(shaman) {
        let healedCount = 0;
        for (const ally of this.enemies) {
            if (!ally.alive || ally.id === shaman.id || ally.hp >= ally.maxHp)
                continue;
        ...
        if (healedCount > 0)
            this.queueRegularEnemyActionVfx('shaman', 'resolve', shaman.pos, 0.44);
    }
    fireEnemyProjectile(enemy, target, reducedMotion = false) {
        const dir = normalize({ x: target.x - enemy.pos.x, y: target.y - enemy.pos.y });
        if (enemy.type === 'archer')
            this.queueRegularEnemyActionVfx('archer', 'resolve', enemy.pos, 0.34);
    ...
    queueRegularEnemyActionVfx(kind, state, pos, maxTtl = 0.36) {
    ...
    queueSpecialistReactionVfx(enemy, type, pos = enemy.pos, targetPos, maxTtl = 0.52) {
        this.specialistReactionVfx.push({ pos: { ...pos }, ...(targetPos ? { targetPos: { ...targetPos } } : {}), enemyId: enemy.id, type, ttl: maxTtl, maxTtl });
        if (this.specialistReactionVfx.length > 24)
            this.specialistReactionVfx.splice(0, this.specialistReactionVfx.length - 24);
    }
        ...
        for (const p of this.projectiles) {
            ...
            if (distance(p.pos, target.pos) <= p.radius + target.radius) {
                ...
                if (p.target === 'core') {
                    ...
                    if (guard.owner === 'core-guard') {
                        this.projectileCoreGuardImpactVfx.push({ pos: { ...p.pos }, incoming: { ...p.vel }, preventionRatio, ttl: maxTtl, maxTtl });
                        if (this.projectileCoreGuardImpactVfx.length > 16)
                            this.projectileCoreGuardImpactVfx.splice(0, this.projectileCoreGuardImpactVfx.length - 16);
                    }
                }
                else {
                    ...
                    if (guard.owner === 'guard') {
                        this.projectileGuardImpactVfx.push({ pos: { ...p.pos }, incoming: { ...p.vel }, preventionRatio, ttl: maxTtl, maxTtl });
                        if (this.projectileGuardImpactVfx.length > 16)
                            this.projectileGuardImpactVfx.splice(0, this.projectileGuardImpactVfx.length - 16);
                    }
                }
                if (p.sourceType === 'archer') {
                    this.archerProjectileImpactVfx.push({ pos: { ...p.pos }, incoming: { ...p.vel }, entryOffset, ordinaryImpactAlphaScale, ttl: .34 });
                    if (this.archerProjectileImpactVfx.length > 24)
                        this.archerProjectileImpactVfx.splice(0, this.archerProjectileImpactVfx.length - 24);
                }
                if (p.bossArchetype) {
                    const maxTtl = .38;
                    this.bossProjectileImpactVfx.push({ pos: { ...p.pos }, archetype: p.bossArchetype, incoming: { ...p.vel }, entryOffset, ordinaryImpactAlphaScale, ttl: maxTtl, maxTtl });
                    if (this.bossProjectileImpactVfx.length > 24)
                        this.bossProjectileImpactVfx.splice(0, this.bossProjectileImpactVfx.length - 24);
                }
                p.ttl = -1;
            }
        }
