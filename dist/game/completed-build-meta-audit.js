import { overdriveModifiers, createDefaultOverdriveState } from './endless/build-overdrive.js';
import { finalFormCatalog, finalFormModifiers } from './endless/final-form.js';
...
import { relicCandidates, relicModifiers } from './relics.js';
import { FUSION_IDS, fusionModifiers } from './spell-fusions.js';
...
function legalRelics(heroId) {
    const ids = new Set();
    for (let ordinal = 0; ordinal < 6; ordinal += 1) {
        for (const id of relicCandidates(heroId, null, () => 0, bossArchetypeForOrdinal(ordinal), 15))
            ids.add(id);
    }
    return [...ids].sort();
}
function fusionPairs() {
    const pairs = [];
    for (let i = 0; i < FUSION_IDS.length; i += 1)
        for (let j = i + 1; j < FUSION_IDS.length; j += 1)
            pairs.push([FUSION_IDS[i], FUSION_IDS[j]]);
    return pairs;
}
function pairProfile(ids, heroId) {
...
function archetypeProfile(archetype) {
...
function identityRelicOffense(heroId, relicId) {
    const relic = relicModifiers(relicId, heroId);
    let value = 1;
    if (heroId === 'arkan')
        value *= 1 + relic.arkanExplosionChanceBonus * .28 + Math.max(0, relic.arkanExplosionRadiusMultiplier - 1) * .08;
    if (heroId === 'kain')
        value *= 1 + Math.max(0, relic.kainOverloadGainMultiplier - 1) * .10 + Math.max(0, relic.kainOverloadMaxCooldownReduction - .20) * .40;
    if (heroId === 'edric')
        value *= 1 + Math.max(0, relic.edricAuraRadiusBonus) / 1000;
    return value;
}
export function completedBuildMetaSamples() {
    const samples = [];
            ...
            for (const fusionIds of pairs) {
                ...
                for (const form of finalFormCatalog(hero.id)) {
                    ...
                    for (const archetype of ARCHETYPES) {
                        ...
                        if (hero.id === 'edric')
                            core *= 1 + Math.max(0, relic.edricAuraRadiusBonus) / 900 + Math.max(0, .78 - relic.edricCoreAuraMultiplier) * .35;
...
export function auditCompletedBuildMeta() {
    ...
    for (const group of keyed.values()) {
        ...
        if (group.length !== 3 || !(group[0].releaseMargin > group[1].releaseMargin && group[1].releaseMargin > group[2].releaseMargin))
            threatMonotonic = false;
    }
    maxHeroTopSpread = round(maxHeroTopSpread);
    maxWithinHeroSpread = round(maxWithinHeroSpread);
