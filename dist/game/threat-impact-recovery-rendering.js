...
export function projectileDirectionCarryRecoveryPresentation(input, reducedMotion = false, reducedFlash = false) {
    ...
    return { owner, tailAlphaScale: (ownerAlpha + .14 * speed) * (.76 + .24 * life) * flash, tailLengthScale: ownerLength * (.86 + .14 * speed) * (reducedMotion ? .72 : 1), headAlphaScale: owner === 'canonical' ? .88 : 1, presentationOnly: true };
}
export function projectileArrivalSettleRecoveryPresentation(input, reducedFlash = false) {
    ...
    return { directionAlphaScale, footprintAlphaScale, spriteAlphaScale, presentationOnly: true };
}
export function hazardResidueReleasePresentation(input, reducedFlash = false) {
    ...
    if (life > .16)
        return { owner: 'hazard', hazardEdgeScale: 1, clearedGroundAlphaScale: memory * .16 * flash, presentationOnly: true };
    const release = clamp01(life / .16);
    return { owner: 'residue', hazardEdgeScale: (.72 + .28 * release) * flash, clearedGroundAlphaScale: (.56 + .34 * (1 - release) + memory * .1) * flash, presentationOnly: true };
}
export function safeLaneHazardReclaimPresentation(input, reducedMotion = false, reducedFlash = false) {
    ...
    return { release, safeLaneAlphaScale, hazardAlphaScale, presentationOnly: true };
}
export function silhouetteRecoveryReentryPresentation(input, reducedMotion = false) {
    ...
    if (input.owner === 'locomotion')
        return { locomotionWeight: 1, trailDistanceScale: 1, alphaScale: 1, presentationOnly: true };
    ...
    return { locomotionWeight, trailDistanceScale, alphaScale, presentationOnly: true };
}
export function continuityCrowdBudgetPresentation(input, reducedMotion = false) {
