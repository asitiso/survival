import type { EnemySpawnLaneCue, EnemySpawnLaneEdge, EnemySpawnLaneKind, EnemySpawnLaneTarget } from './enemy-spawn-lane-readability.js';

export const SPAWN_LANE_EDGE_STACK_LABEL_SEPARATION=48;
export const SPAWN_LANE_EDGE_STACK_SCREEN_INSET=22;
export interface SpawnLaneEdgeStackCue extends EnemySpawnLaneCue{
  stackSlot:number;
...
const TARGET_RANK:Record<EnemySpawnLaneTarget,number>={hero:0,core:1};
const clamp=(v:number,min:number,max:number)=>Math.max(min,Math.min(max,v));
function baseLabel(cue:EnemySpawnLaneCue,width:number,height:number){
  const inset=SPAWN_LANE_EDGE_STACK_SCREEN_INSET;
  if(cue.edge==='north')return{x:clamp(cue.start.x,inset,width-inset),y:inset};
  if(cue.edge==='south')return{x:clamp(cue.start.x,inset,width-inset),y:height-inset};
  if(cue.edge==='west')return{x:inset,y:clamp(cue.start.y,inset,height-inset)};
  return{x:width-inset,y:clamp(cue.start.y,inset,height-inset)};
}
function identitySort(a:{cue:EnemySpawnLaneCue;index:number},b:{cue:EnemySpawnLaneCue;index:number}){
...
function along(edge:EnemySpawnLaneEdge,pos:{x:number;y:number},amount:number,width:number,height:number){
  const inset=SPAWN_LANE_EDGE_STACK_SCREEN_INSET;
...
export function spawnLaneEdgeStackArbitration(input:{cues:readonly EnemySpawnLaneCue[];width:number;height:number;}):SpawnLaneEdgeStackCue[]{
  ...
  const output:SpawnLaneEdgeStackCue[]=input.cues.map((cue)=>({...cue,start:{...cue.start},end:{...cue.end},stackSlot:0,labelPos:baseLabel(cue,width,height),labelVisible:cue.count>1,presentationOnly:true}));
  const edges:EnemySpawnLaneEdge[]=['north','east','south','west'];
  for(const edge of edges){
    ...
    for(let slot=0;slot<entries.length;slot++){
      const entry=entries[slot]!,base=baseLabel(entry.cue,width,height);let chosen=base;
      if(placed.some((p)=>distance(p,chosen)<SPAWN_LANE_EDGE_STACK_LABEL_SEPARATION)){
        ...
        if(clean)chosen=clean;
      }
      placed.push(chosen);
      const out=output[entry.index]!;out.stackSlot=slot;out.labelPos=chosen;out.labelVisible=out.count>1;
    }
  }
  return output;
}
