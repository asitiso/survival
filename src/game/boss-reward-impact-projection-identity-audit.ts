...
import { buildBossRewardChoices, type BossRewardChoice } from './upgrades.js';
import { FUSION_IDS, MAX_FUSIONS_PER_RUN, type FusionId } from './spell-fusions.js';
...
import { projectBossRewardImpact, type BossRewardImpactContext } from './boss-reward-impact-projection.js';

export interface BossRewardImpactProjectionIdentitySample{id:string;passed:boolean;}
export interface BossRewardImpactProjectionIdentityAudit{
  samples:BossRewardImpactProjectionIdentitySample[];roleIdentityCount:number;roleCoverage:number;roleUniqueCellCount:number;heroCount:number;scenarioCount:number;generatedChoiceSamples:number;roleIds:readonly string[];actionCount:number;snapshotSchemaMutation:false;gameplayMutation:false;issues:string[];passed:boolean;
}
...
type Scenario={id:string;levels:Record<SpellId,number>;activeRelic:RelicId|null;activeFusions:readonly FusionId[];boss:'inferno'|'summoner'|'juggernaut';mastery:number;};
const levels=(value:number):Record<SpellId,number>=>Object.fromEntries(SPELLS.map(id=>[id,value])) as Record<SpellId,number>;
...
function context(heroId:HeroId,scenario:Scenario):BossRewardImpactContext{return{heroId,activeRelic:scenario.activeRelic,activeFusions:scenario.activeFusions,spellLevels:scenario.levels};}
const choice=(id:'spellPower'|'cooldown'|'maxHp'):BossRewardChoice=>({kind:'upgrade',id,title:id,description:id,accent:'#fff'});

export function auditBossRewardImpactProjectionIdentityAssets():BossRewardImpactProjectionIdentityAudit{
  const samples:BossRewardImpactProjectionIdentitySample[]=[];
  let generatedChoiceSamples=0;
  for(const heroId of HEROES)for(const scenario of SCENARIOS){
    const spells={levels:{...scenario.levels}} as SpellSystem;
    ...
    for(const reward of choices){const frozen=JSON.stringify(reward),projection=projectBossRewardImpact(reward,context(heroId,scenario));generatedChoiceSamples++;samples.push({id:`${heroId}:${scenario.id}:${reward.id}`,passed:choices.length===3&&BOSS_REWARD_IMPACT_ROLE_IDS.includes(projection.roleId)&&projection.roleLabel.length>0&&projection.summary.length>0&&JSON.stringify(reward)===frozen});}
  }
  ...
  const issues:string[]=[];if(samples.length!==60)issues.push(`samples:${samples.length}`);if(samples.some(sample=>!sample.passed))issues.push('sample-failure');if(!atlas.passed)issues.push('role-atlas');if(generatedChoiceSamples!==48)issues.push(`generated:${generatedChoiceSamples}`);if(ACTION_BUTTONS.length!==9)issues.push(`actions:${ACTION_BUTTONS.length}`);
