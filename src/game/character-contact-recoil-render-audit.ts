...
export interface CharacterContactRecoilRenderAuditSample { id:string; expected:boolean|number|string; actual:boolean|number|string; passed:boolean; }
const add=(samples:CharacterContactRecoilRenderAuditSample[],id:string,expected:boolean|number|string,actual:boolean|number|string)=>samples.push({id,expected,actual,passed:Object.is(expected,actual)});

export function runCharacterContactRecoilRenderAudit(){
  const samples:CharacterContactRecoilRenderAuditSample[]=[];
