...
import { advanceContract, createContractOffer, shouldOfferContract, } from './contracts.js';
import { recordBossEncounter } from './nemesis.js';
import { createDefaultExtensionState } from './snapshot.js';
...
function contractOutcomeTelemetry(before, after, telemetry) {
    let next = telemetry;
    if (after.completedCount > before.completedCount)
        next = recordContractTelemetry(next, 'completed');
    if (after.failedCount > before.failedCount)
        next = recordContractTelemetry(next, 'failed');
    return next;
}
function worldCatchUp(legacy, world, rng) {
    ...
    while (shouldEvolveWorld(nextWorld, legacy.elapsedMs) && safety < 16) {
        ...
        for (const effect of evolved.effects) {
            if (effect.type !== 'spawn_field_node' || effect.expiresAtMs > legacy.elapsedMs)
                effects.push(effect);
        }
        safety += 1;
    }
...
function heroIdForSignature(value) {
    return value === 'seria' || value === 'kain' || value === 'edric' ? value : 'arkan';
}
export function advanceEndlessRuntime(input) {
    ...
    for (const event of events)
        extension.telemetry = recordTelemetryEvent(extension.telemetry, event);
    ...
    for (const event of events) {
        if (event.type !== 'boss_defeated' && event.type !== 'boss_encounter_end')
            continue;
    ...
    const heroId = heroIdForSignature(legacy.heroId);
    ...
    if (heroAscensionResult.offered && extension.heroAscension.pendingOffer)
        effects.push({ type: 'show_hero_ascension_offer', milestone: extension.heroAscension.pendingOffer.milestone });
    ...
    for (const milestone of chronicleResult.unlocked)
        effects.push({ type: 'chronicle_milestone', minute: milestone.minute, title: milestone.title, rewardGold: milestone.rewardGold, coreHealPercent: milestone.coreHealPercent });
    ...
    if (checkpointResult.reached)
        effects.push({ type: 'run_checkpoint', minute: checkpointResult.reached.minute, title: checkpointResult.reached.title });
    ...
    if (recapResult.reached)
        effects.push({ type: 'run_milestone_recap', ...recapResult.reached });
