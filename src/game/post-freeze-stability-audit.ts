...
import { loadRunSnapshot, sanitizeRunSnapshot, saveRunSnapshot, type RunSnapshot, type SnapshotStorage } from '../domain/run-snapshot.js';

export interface PostFreezeStabilityAudit {
...
function memoryStorage():SnapshotStorage&{map:Map<string,string>} {
  const map=new Map<string,string>();
  return {map,getItem:(key)=>map.get(key)??null,setItem:(key,value)=>{map.set(key,value);},removeItem:(key)=>{map.delete(key);}};
}

function sample(savedAt:number,elapsed:number):RunSnapshot {
...
function auditBlockedStorageContinuity():boolean {
...
function auditJournalClockRollback():boolean {
...
function auditMultiDayPersistence():boolean {
  const storage=memoryStorage();
...
function auditSnapshotSchemaGuard():boolean {
...
export function auditPostFreezeStability():PostFreezeStabilityAudit {
