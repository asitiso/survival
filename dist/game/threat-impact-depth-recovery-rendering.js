...
export function projectileDepthRecoveryPresentation(input, reducedMotion = false) {
    ...
    return { bodyAlphaScale: 1, trailAlphaScale: Math.max(floor, Math.min(1, 1 - suppression)) * (reducedMotion ? .94 : 1), recovery: clamp01(release * (1 - occlusion)), presentationOnly: true };
}
export function safeLaneDepthRecoveryPresentation(input, reducedMotion = false, reducedFlash = false) {
    ...
    return { bodyAlphaScale: 1, trailAlphaScale: Math.max(floor, 1 - protection * .48) * (reducedMotion ? .94 : 1), safeLaneAlphaScale: 1 + protection * .12 * (reducedFlash ? .72 : 1), release, presentationOnly: true };
}
export function impactDepthHandoffPresentation(input, reducedFlash = false) {
...
export function bossTelegraphDepthReleasePresentation(input, reducedFlash = false) {
...
export function specialistDepthRecoveryPresentation(input, reducedMotion = false) {
    ...
    return { bodyAlphaScale: 1, directionAlphaScale, trailAlphaScale, presentationOnly: true };
}
export function depthRecoveryBudgetPresentation(input, reducedMotion = false, reducedFlash = false) {
    ...
    return { stress, criticalAlphaScale: 1, canonicalBodyAlphaScale: 1, secondaryRecoveryScale: (1 - stress * .4) * motion * flash, presentationOnly: true };
}
