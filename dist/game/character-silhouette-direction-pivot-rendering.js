...
export function characterSilhouetteDirectionPivotPresentation(input, reducedMotion = false) {
    const locomotion = unit(input.locomotion.x, input.locomotion.y), owned = unit(input.owned.facingX, input.owned.facingY);
    if (input.owned.owner === 'locomotion')
        return { owner: input.owned.owner, facingX: locomotion.x, facingY: locomotion.y, pivotWeight: 0, trailDistanceScale: input.owned.trailDistanceScale, presentationOnly: true };
    ...
    return { owner: input.owned.owner, facingX: Math.cos(a), facingY: Math.sin(a), pivotWeight, trailDistanceScale: input.owned.trailDistanceScale, presentationOnly: true };
}
