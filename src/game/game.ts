  ...
  private drawBossArenaHazards(ctx: CanvasRenderingContext2D, motion: SecondaryCombatMotionPolicy): void {
    ...
    if (safeLane) {
      ...
      if(forecast&&forecast.urgency>=.65&&safeLaneVisual.bridgeVisible){
        ctx.globalAlpha=(.18+.22*forecast.urgency)*safeLaneVisual.bridgeAlphaScale*safeLaneAttention.bridgeAlphaScale*safeLaneAttentionRecovery.bridgeRecoveryScale*safeLaneHazardRecovery.bridgeAlphaScale;ctx.strokeStyle='#7fd9ff';ctx.setLineDash([4,8*forecastDashGapScale]);
