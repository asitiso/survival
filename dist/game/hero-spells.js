...
export function heroSpellIdentity(heroId, spellId) {
    const identity = { ...BASE, ...HERO_COLORS[heroId] };
    if (heroId === 'arkan') {
        if (spellId === 'fireBolt')
            return { ...identity, damageMultiplier: 1.08, splashRadius: 44, splashDamageMultiplier: 0.42 };
        if (spellId === 'chainLightning')
            return { ...identity, damageMultiplier: 1.10 };
        if (spellId === 'frostNova')
            return { ...identity, novaDamageMultiplier: 1.22, areaMultiplier: 1.06 };
        if (spellId === 'flameField')
            return { ...identity, fieldDamageMultiplier: 1.22, areaMultiplier: 1.08 };
        if (spellId === 'meteorStorm')
            return { ...identity, meteorDamageMultiplier: 1.18, areaMultiplier: 1.08 };
        return { ...identity, holeDamageMultiplier: 1.18 };
    }
    if (heroId === 'seria') {
        if (spellId === 'fireBolt')
            return { ...identity, projectileSlowFactor: 0.70, projectileSlowDuration: 1.25 };
        if (spellId === 'chainLightning')
            return { ...identity, chainSlowFactor: 0.66, chainSlowDuration: 1.15, chainJumpBonus: 1 };
        if (spellId === 'frostNova')
            return { ...identity, novaDamageMultiplier: 1.20, areaMultiplier: 1.10 };
        if (spellId === 'flameField')
            return { ...identity, fieldSlowFactor: 0.62, fieldSlowDuration: 0.55, areaMultiplier: 1.08 };
        if (spellId === 'meteorStorm')
            return { ...identity, meteorSlowFactor: 0.58, meteorSlowDuration: 2.4, meteorDelayMultiplier: 0.92 };
        return { ...identity, holeSlowFactor: 0.48, holeSlowDuration: 0.55, areaMultiplier: 1.08 };
    }
    if (heroId === 'kain') {
        if (spellId === 'fireBolt')
            return { ...identity, projectileSpeedMultiplier: 1.35, pierceBonus: 1 };
        if (spellId === 'chainLightning')
            return { ...identity, chainJumpBonus: 2, damageMultiplier: 1.04 };
        if (spellId === 'frostNova')
            return { ...identity, areaMultiplier: 0.92, novaDamageMultiplier: 1.10 };
        if (spellId === 'flameField')
            return { ...identity, fieldTickMultiplier: 1.35, fieldDamageMultiplier: 0.92 };
        if (spellId === 'meteorStorm')
            return { ...identity, meteorDelayMultiplier: 0.68, meteorDamageMultiplier: 0.92 };
        return { ...identity, holeTickMultiplier: 1.35, holeDamageMultiplier: 0.92 };
    }
    if (spellId === 'fireBolt')
        return { ...identity, pierceBonus: 2, projectileSpeedMultiplier: 1.06 };
    if (spellId === 'chainLightning')
        return { ...identity, chainJumpBonus: 1 };
    if (spellId === 'frostNova')
        return { ...identity, knockback: 82, areaMultiplier: 1.08 };
    if (spellId === 'flameField')
        return { ...identity, fieldAtCore: true, fieldSlowFactor: 0.76, fieldSlowDuration: 0.45, areaMultiplier: 1.10 };
    if (spellId === 'meteorStorm')
        return { ...identity, areaMultiplier: 1.12 };
...
export function heroActionLabel(heroId, actionId) {
    if (actionId === 'potion')
        return '물약';
    if (actionId === 'shop')
        return '상점';
...
export function heroSpellName(heroId, spellId) {
