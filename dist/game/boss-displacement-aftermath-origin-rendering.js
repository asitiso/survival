...
export function bossDisplacementAftermathOriginPresentation(input, reducedMotion = false) {
    const rebase = clamp(input.rebase, 0, 1), special = clamp(input.specialStrength, 0, 1), settle = clamp(input.settle, 0, 1), owner = rebase > .04 ? 'rebase' : 'body';
    if (owner === 'body')
        return { owner, originOffsetX: 0, originOffsetY: 0, aftermathAlphaScale: 1, contactPulseScale: 1 };
    ...
    if (reducedMotion) {
        aftermathAlphaScale = Math.min(aftermathAlphaScale, .72);
        contactPulseScale = Math.min(contactPulseScale, .58);
    }
