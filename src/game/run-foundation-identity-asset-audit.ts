...
export interface RunFoundationIdentityAuditSample { id:string; passed:boolean; }
export interface RunFoundationIdentityAssetAudit {
  samples:RunFoundationIdentityAuditSample[]; traitCount:number; relicCount:number; ascensionCount:number;
  traitRecallCoverage:number; relicToastCoverage:number; ascensionToastCoverage:number;
  textFallbackPreserved:boolean; imageLoadFailureNonBlocking:boolean; actionCount:number; snapshotSchemaMutation:false;
  issues:string[]; passed:boolean;
}
const sample=(id:string,passed:boolean):RunFoundationIdentityAuditSample=>({id,passed});
export function auditRunFoundationIdentityAssets():RunFoundationIdentityAssetAudit{
  const samples:RunFoundationIdentityAuditSample[]=[];
  for(const id of RUN_FOUNDATION_TRAIT_IDS){const x=runTraitIdentity(id);samples.push(sample(`trait:${id}`,x.atlasSrc.endsWith('decision-path-icons.png')&&x.persistentRecallSupported===true&&x.motionAmplitude===0));}
  for(const id of RUN_FOUNDATION_RELIC_IDS){const x=relicAcquisitionIdentity(id);samples.push(sample(`relic:${id}`,x.atlasSrc.endsWith('build-identity-icons.png')&&x.acquisitionToastSupported===true&&x.motionAmplitude===0));}
  for(const id of RUN_FOUNDATION_ASCENSION_IDS){const x=ascensionSelectionIdentity(id);samples.push(sample(`ascension:${id}`,x.atlasSrc.endsWith('deep-run-decision-icons.png')&&x.selectionToastSupported===true&&x.motionAmplitude===0));}
  const destruction=runTraitBonuses('destruction');
  const winter=relicModifiers('winter-heart','seria');
  const base=createDefaultHeroAscensionState();const offered=advanceHeroAscension('arkan',35*60_000,base);const selected=offered.state.pendingOffer?selectHeroAscension(offered.state,offered.state.pendingOffer.options[0]!.optionId):base;
  ...
  for(const [id,passed] of checks)samples.push(sample(id,passed));
