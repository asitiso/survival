...
export interface EightTwelveHourFlowHealthAudit {
  samples:number;
  childAuditCount:4;
  shopDormancyCoverage:number;
  rewardFocusCoverage:number;
  toastSilenceCoverage:number;
  priorityPreservationCoverage:number;
  ...
  finalFormIdentityPreserved:true;
...
export function auditEightTwelveHourFlowHealth():EightTwelveHourFlowHealthAudit {
  ...
  for(const elapsedSeconds of checkpoints)for(const mythicActive of [false,true])for(const finalFormActive of [false,true]){
    samples+=4;
    const s=eightTwelveHourShopFocus(elapsedSeconds,equipment);if(s.dormant&&s.keepClickable&&!s.showTokenCount)shop++;
    const r=focusEightTwelveHourBossRewards(rewards,{elapsedSeconds,completeBuild:true,finalFormActive:false});if(r.compact&&r.choices.filter(choice=>choice.best).length===1&&!r.autoSelect)reward++;
    ...
    if(routineHidden&&criticalShown)toast++;
    ...
    if((!mythicActive||p.preserveBossDanger)&&(!finalFormActive||labels.length===1))priority++;
  }
