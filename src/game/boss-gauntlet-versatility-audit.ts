import { bossBuildMatchupSamples, type BossBuildMatchupSample } from './boss-build-matchup-audit.js';
import type { BossArchetype } from './boss-patterns.js';
import { HERO_PROFILES, type HeroId } from './hero-profiles.js';

export interface BossGauntletVersatilitySample{
  heroId:HeroId;
  buildKey:string;
  bossCount:number;
  gauntletIndex:number;
  versatilityFloor:number;
  versatilityAverage:number;
}
export interface BossGauntletHeroSummary{
  heroId:HeroId;
  best:BossGauntletVersatilitySample;
  specialistGain:number;
}
export interface BossGauntletVersatilityAudit{
  samples:BossGauntletVersatilitySample[];
  heroes:BossGauntletHeroSummary[];
  minTopBuildVersatilityFloor:number;
  maxSpecialistGain:number;
  maxHeroGauntletSpread:number;
  catastrophicTopBuildCount:number;
  passed:boolean;
}
function round(value:number):number{return Math.round(value*10000)/10000;}
function key(sample:BossBuildMatchupSample):string{return [sample.heroId,sample.relicId,[...sample.fusionIds].sort().join('+'),sample.finalFormId,sample.archetype].join('|');}
function geometric(values:readonly number[]):number{return Math.exp(values.reduce((sum,value)=>sum+Math.log(Math.max(.0001,value)),0)/Math.max(1,values.length));}
export function bossGauntletVersatilitySamples():BossGauntletVersatilitySample[]{
  const matchups=bossBuildMatchupSamples();
  const heroBossTop=new Map<string,number>();
  ...
  const groups=new Map<string,BossBuildMatchupSample[]>();
  for(const sample of matchups){const k=key(sample);const list=groups.get(k)??[];list.push(sample);groups.set(k,list);}
  const out:BossGauntletVersatilitySample[]=[];
...
export function auditBossGauntletVersatility():BossGauntletVersatilityAudit{
  const samples=bossGauntletVersatilitySamples();
  const matchups=bossBuildMatchupSamples();
  const heroes:BossGauntletHeroSummary[]=[];
  for(const hero of HERO_PROFILES){
    ...
    for(const boss of bosses as Set<BossArchetype>){
