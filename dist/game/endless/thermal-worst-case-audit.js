...
import { thermalBudgetPolicy } from './thermal-budget-director.js';
import { thermalPolicyForEffectiveTier } from './thermal-recovery-hysteresis.js';
const DEVICES = ['low', 'mid', 'high'];
const HOURS = [2, 8, 12];
const FPS = { low: 26, mid: 32, high: 38 };
function round(value) { return Math.round(value * 10000) / 10000; }
export function thermalWorstCaseCheckpoints() {
...
export function auditThermalWorstCase() {
    ...
    for (const deviceClass of DEVICES) {
        ...
        if (group[2].visualDensity > group[0].visualDensity || group[2].projectileVisualDensity > group[0].projectileVisualDensity)
            longRunDegradesPresentationFirst = false;
        if (group[2].visualDensity < group[0].visualDensity || group[2].projectileVisualDensity < group[0].projectileVisualDensity)
            sawDecorativeReduction = true;
    }
