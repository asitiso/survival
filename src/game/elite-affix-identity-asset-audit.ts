...
import { eliteAffixCount, eliteAffixModifiers, type EliteAffixId } from './elite-affixes.js';
...
export type EliteAffixIdentityAuditSurface = 'single' | 'double' | 'other';

export interface EliteAffixIdentityAssetSample {
  caseId: string;
  affixId?: EliteAffixId;
  surface: EliteAffixIdentityAuditSurface;
  passed: boolean;
}

export interface EliteAffixIdentityAssetAudit {
  samples: EliteAffixIdentityAssetSample[];
  affixCount: number;
  coverage: number;
  uniqueCellCount: number;
  outOfBounds: EliteAffixId[];
  singleAffixCoverage: number;
  doubleAffixCoverage: number;
...
const EXPECTED_MODIFIERS: Readonly<Record<EliteAffixId, ReturnType<typeof eliteAffixModifiers>>> = {
...
export function auditEliteAffixIdentityAssets(): EliteAffixIdentityAssetAudit {
  const atlas = auditEliteAffixIdentityAtlas();
  const samples: EliteAffixIdentityAssetSample[] = [];
  const push = (caseId: string, passed: boolean, surface: EliteAffixIdentityAuditSurface = 'other', affixId?: EliteAffixId): void => {
  ...
  const singleIds = new Set<EliteAffixId>();
  const doubleIds = new Set<EliteAffixId>();
  ...
  const overlapPolicyViolations = ELITE_AFFIX_IDENTITY_IDS.reduce((violations, _id, index) => {
    ...
    if (a === undefined || b === undefined) return violations + 1;
  ...
  const issues: string[] = [];
  if (samples.length !== 54) issues.push(`samples:${samples.length}`);
  if (atlas.coverage !== 1 || atlas.uniqueCellCount !== 6 || atlas.outOfBounds.length > 0) issues.push('atlas');
  if (singleAffixCoverage !== 1) issues.push('single-affix-coverage');
  if (doubleAffixCoverage !== 1) issues.push('double-affix-coverage');
  ...
  if (modifierMutationDetected) issues.push('modifier-mutation');
  if (enemyGeometryMutation) issues.push('enemy-geometry-mutation');
