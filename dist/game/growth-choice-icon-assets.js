...
export const GROWTH_CHOICE_GENERIC_IDS = ['maxHp', 'moveSpeed', 'spellPower', 'cooldown', 'pickupRadius', 'relic', 'fusion'];
...
function genericId(choiceId, kind) {
    if (kind === 'relic' || choiceId.startsWith('relic:'))
        return 'relic';
    if (kind === 'fusion' || choiceId.startsWith('fusion:'))
        return 'fusion';
    return Object.prototype.hasOwnProperty.call(CELL_BY_GENERIC, choiceId) ? choiceId : null;
}
function percent(column, row, columns, rows) {
    const x = columns <= 1 ? 0 : (column / (columns - 1)) * 100;
    const y = rows <= 1 ? 0 : (row / (rows - 1)) * 100;
    return `${x}% ${y}%`;
}
export function growthChoiceIcon(choiceId, kind, heroId) {
    const buildId = parseBuildIdentity(choiceId, kind);
    if (buildId) {
        const icon = buildIdentityIcon(buildId);
        return { assetId: buildId, atlasSrc: icon.atlasSrc, backgroundSize: icon.backgroundSize, backgroundPosition: icon.backgroundPosition, animated: false, motionAmplitude: 0, size: 52, compactSize: 40 };
    }
    const action = SPELL_ACTION[choiceId];
    if (action) {
        if (heroId) {
            const icon = heroAbilitySpellIdentityIcon(heroId, choiceId);
    ...
    const id = genericId(choiceId, kind);
    if (!id)
        return null;
...
export function growthChoiceIconStyle(choiceId, kind, heroId) {
    const icon = growthChoiceIcon(choiceId, kind, heroId);
    if (!icon)
        return '';
...
export function auditGrowthChoiceIconAtlas(ids) {
    const missing = [];
    const outOfBounds = [];
    const cells = new Set();
    for (const raw of ids) {
        const id = genericId(raw);
        if (!id) {
            missing.push(raw);
            continue;
        }
        ...
        if (column < 0 || row < 0 || column >= GROWTH_CHOICE_ICON_ATLAS.columns || row >= GROWTH_CHOICE_ICON_ATLAS.rows)
            outOfBounds.push(raw);
    }
