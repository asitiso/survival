...
old_floor="extend_floor('src/game/enemies.ts','ctx.globalAlpha = Math.max(projectileEffectiveFloor.bodyAlphaFloor'"
new_floor="extend_floor('src/game/enemies.ts','ctx.globalAlpha =Math.max(projectileEffectiveFloor.bodyAlphaFloor'"
count=runner.count(old_floor)
if count!=1:
    raise SystemExit(f'expected one projectile Train3 floor seam, got {count}')
runner=runner.replace(old_floor,new_floor,1)
'''
