...
export function auditEnemySpriteAssets() {
    const samples = [];
    ...
    if (atlas.coverage !== 1)
        issues.push('enemy-sprite-coverage');
    if (atlas.uniqueCellCount !== 12)
        issues.push('enemy-sprite-cell-collision');
    if (atlas.outOfBounds.length > 0)
        issues.push('enemy-sprite-out-of-bounds');
    if (motionAmplitude !== 0)
        issues.push('enemy-sprite-motion');
    if (!fallbackPreserved)
        issues.push('enemy-sprite-fallback');
    if (!bossExcluded)
        issues.push('enemy-sprite-boss-overreach');
