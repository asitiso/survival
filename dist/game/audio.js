...
export function soundDescriptor(kind) { return DESCRIPTORS[kind]; }
export function soundDescriptorWithVariation(kind, variation = {}) {
...
export function defaultAudioSettings() { return { enabled: true, volume: 0.65 }; }
...
export function loadAudioSettings(storage) {
    try {
        ...
        return raw === null ? defaultAudioSettings() : sanitizeAudioSettings(JSON.parse(raw));
    }
    catch {
...
export function saveAudioSettings(storage, settings) {
    try {
        storage.setItem(AUDIO_KEY, JSON.stringify(sanitizeAudioSettings(settings)));
    }
    catch { /* optional */ }
}
export class SoundScheduler {
    ...
    trySchedule(kind, now) {
        ...
        if (time - last < descriptor.cooldown)
            return false;
        if (this.voices.length >= this.maxVoices) {
            ...
            if (descriptor.priority <= weakestPriority)
                return false;
            if (weakestIndex >= 0)
                this.voices.splice(weakestIndex, 1);
        }
...
export class ArcaneAudio {
    scheduler = new SoundScheduler();
    settings;
    context = null;
    constructor(settings = defaultAudioSettings()) { this.settings = sanitizeAudioSettings(settings); }
    play(kind, nowSeconds, variation = {}) {
        const now = nowSeconds ?? (typeof performance !== 'undefined' ? performance.now() / 1000 : Date.now() / 1000);
        if (!this.settings.enabled || this.settings.volume <= 0 || !this.scheduler.trySchedule(kind, now))
            return false;
        ...
        try {
            ...
            if (kind === 'levelUp')
                gain.gain.setValueAtTime(desc.gain * this.settings.volume * .7, ctx.currentTime + .29);
            gain.gain.exponentialRampToValueAtTime(0.0001, ctx.currentTime + desc.duration);
            oscillator.connect(gain);
            gain.connect(ctx.destination);
            oscillator.start();
            oscillator.stop(ctx.currentTime + desc.duration);
        }
