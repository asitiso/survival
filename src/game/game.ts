...
export class Game {
  ...
  private drawBossArenaHazards(ctx: CanvasRenderingContext2D, motion: SecondaryCombatMotionPolicy): void {
    ...
    for (const hazard of this.bossArena.hazards) {
      ...
      ctx.fillStyle = color; ctx.strokeStyle = color; ctx.lineWidth = (hazard.telegraph > 0 ? 4 : 2)*hazardReadabilityContrast.strokeWidthScale*(hazard.telegraph > 0 ? 1 : hazardFinalSettle.strokeWidthScale);
