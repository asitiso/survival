...
function autoPriority(enemy, heroPos, corePos) {
    ...
    if (enemy.type === 'boss')
        ...
        score += 110;
    else if (enemy.type === 'bomber' || enemy.type === 'nullifier' || enemy.type === 'assassin')
        score += 70;
    if (enemy.target === 'core') {
        score += 250;
        if (corePos)
            score += Math.max(0, 170 - distance(corePos, enemy.pos) * .55);
    }
...
export function chooseSpellTarget(enemies, heroPos, corePos, autoAim, preferredAutoTargetId = null) {
    if (!autoAim) {
        const coreAttacker = nearest(enemies, heroPos, (enemy) => enemy.target === 'core');
        if (coreAttacker && distance(heroPos, coreAttacker.pos) < 620)
            return coreAttacker;
        const elite = nearest(enemies, heroPos, (enemy) => enemy.type === 'elite' || enemy.type === 'boss');
        if (elite && distance(heroPos, elite.pos) < 650)
            return elite;
        return nearest(enemies, heroPos);
    }
    let best = null, bestScore = Number.NEGATIVE_INFINITY;
    for (const enemy of enemies) {
        if (!enemy.alive || distance(heroPos, enemy.pos) > 720)
            continue;
        const score = autoPriority(enemy, heroPos, corePos);
        if (score > bestScore + .001 || (Math.abs(score - bestScore) <= .001 && best && enemy.id < best.id)) {
            best = enemy;
            bestScore = score;
        }
    }
    if (preferredAutoTargetId !== null && best) {
        ...
        if (preferred) {
            ...
            if (best.id === preferred.id || bestScore - preferredScore < AUTO_SWITCH_MARGIN)
                return preferred;
        }
    }
    return best;
