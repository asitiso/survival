...
const unit = (x, y) => { const len = Math.hypot(x, y) || 1; return { x: x / len, y: y / len }; };
export function advanceBossHeavyHitStaggerState(previous, event, dt, _reducedMotion = false) {
    const prev = previous ?? { stagger: 0, tier: null, directionX: -1, directionY: 0 };
    if (event) {
        const d = unit(event.directionX, event.directionY);
        return { stagger: event.tier === 'critical' ? 1 : .72, tier: event.tier, directionX: d.x, directionY: d.y };
    }
...
const archetypeProfile = (archetype) => {
    if (archetype === 'inferno')
        return { stance: 'inferno-rebound', x: 1, y: .22, rot: .55, sx: .025, sy: -.02 };
    if (archetype === 'summoner')
        return { stance: 'summoner-drift', x: .45, y: -.42, rot: .92, sx: -.015, sy: .03 };
    if (archetype === 'juggernaut')
        return { stance: 'juggernaut-brace', x: .24, y: 1.18, rot: .18, sx: .045, sy: -.055 };
    if (archetype === 'abyssWitch')
        return { stance: 'abyss-curl', x: .56, y: -.22, rot: .82, sx: -.025, sy: .04 };
    if (archetype === 'twinMaw')
        return { stance: 'twin-maw-yaw', x: .62, y: .12, rot: 1.05, sx: .02, sy: -.025 };
    return { stance: 'time-compress', x: .22, y: .48, rot: .38, sx: -.045, sy: .055 };
};
export function bossHeavyHitStaggerPresentation(archetype, phase, state, specialTimer, reducedMotion = false) {
    ...
    const magnitude = Math.hypot(offsetX, offsetY), cap = reducedMotion ? 3 : 7.8;
    if (magnitude > cap) {
        const r = cap / magnitude;
        offsetX *= r;
        offsetY *= r;
    }
    if (strength <= 0) {
