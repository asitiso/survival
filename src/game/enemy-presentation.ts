...
export function enemyStatusCue(kind: EnemyStatusKind): EnemyStatusCue {
  if (kind === 'burn') return { color: '#ff7b45', style: 'embers', pulse: 0.72 };
  if (kind === 'freeze') return { color: '#91ebff', style: 'ring', pulse: 0.52 };
  return { color: '#bca4ff', style: 'arcs', pulse: 0.62 };
}

export function enemyImpactVfxDescriptor(type:EnemyType,tier:'normal'|'heavy'|'critical'):EnemyImpactVfxDescriptor {
  const id=IDENTITY[type]; const tierScale=tier==='critical'?1.45:tier==='heavy'?1.15:.82;
...
export function enemyDeathCue(type: EnemyType): EnemyDeathCue {
  const id=IDENTITY[type];
  if (type === 'boss') return { radius: 150, particles: 24, duration: 0.9, color: id.color, motif:id.motif,rayCount:12,glowAlpha:.34 };
  if (type === 'elite') return { radius: 82, particles: 14, duration: 0.55, color: id.color, motif:id.motif,rayCount:8,glowAlpha:.26 };
