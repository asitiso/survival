...
import { advanceDamageReason, damageReasonCue, recordDamageReason } from './damage-reason-feedback.js';
...
export function auditDamageSourceIdentityAssets() {
    const atlas = auditDamageSourceIdentityAtlas();
    const samples = [];
    const push = (caseId, passed, source) => { samples.push({ caseId, passed, ...(source ? { source } : {}) }); };
    const cueSources = new Set();
    const mergeSources = new Set();
    const densitySources = new Set();
    ...
    const belowHeavy = damageReasonCue('contact', 11.99, 100);
    severities.add(belowHeavy.severity);
    const atHeavy = damageReasonCue('projectile', 12, 100);
    severities.add(atHeavy.severity);
    const belowCritical = damageReasonCue('explosion', 31.99, 100);
    severities.add(belowCritical.severity);
    const atCritical = damageReasonCue('arena', 32, 100);
    severities.add(atCritical.severity);
    ...
    for (const [id, ok] of thresholdChecks) {
        thresholdMutation = thresholdMutation || !ok;
        push(id, ok);
    }
    ...
    for (const [id, ok] of dwellChecks) {
        dwellMutation = dwellMutation || !ok;
        push(id, ok);
    }
    ...
    push('density:0.21-blocked', stillBlocked);
    push('density:0.23-released', released);
    ...
    for (const source of DAMAGE_SOURCE_IDENTITY_SOURCES) {
        const critical = damageReasonCue(source, 40, 100);
        severities.add(critical.severity);
    ...
    if (atlas.coverage !== 1 || atlas.uniqueCellCount !== 5 || atlas.outOfBounds.length)
        issues.push('atlas');
    if (cueCoverage !== 1)
        issues.push('cue-coverage');
    if (severityCoverage !== 1)
        issues.push('severity-coverage');
    if (repeatedSourceMergeCoverage !== 1)
        issues.push('merge-coverage');
    if (sourceSwitchDensityGuardCoverage !== 1)
        issues.push('density-coverage');
    ...
    if (thresholdMutation)
        issues.push('thresholds');
    if (dwellMutation)
        issues.push('dwell');
    if (densityGuardMutation)
        issues.push('density-guard');
    if (damageAmountMutation)
        issues.push('damage-amount');
    if (actionCount !== 9)
