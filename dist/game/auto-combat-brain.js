...
const AUTO_ACTION_ORDER = ['spell1', 'spell2', 'spell3', 'spell4'];
export class AutoCombatBrain {
    targetId = null;
    pendingTargetId = null;
    pendingTargetSince = 0;
    nextTargetReviewAt = Number.NEGATIVE_INFINITY;
    weakpointBossId = null;
    weakpointId = null;
    pendingWeakpointId = null;
    pendingWeakpointSince = 0;
    ...
    selectTarget(enemies, heroPos, corePos, nowSeconds) {
        ...
        if (now + 1e-9 < this.nextTargetReviewAt && !urgentCoreReview)
            return current;
        this.nextTargetReviewAt = now + AUTO_TARGET_REVIEW_SECONDS;
        ...
        if (this.pendingTargetId !== candidate.id) {
            ...
            this.pendingTargetSince = now;
            return current;
        }
        const reaction = candidate.target === 'core' ? AUTO_CORE_SWITCH_SECONDS : AUTO_NORMAL_SWITCH_SECONDS;
        if (now - this.pendingTargetSince + 1e-9 < reaction)
            return current;
        this.targetId = candidate.id;
        this.pendingTargetId = null;
        return candidate;
    }
    selectWeakpoint(activeBossId, nodes, heroPos, nowSeconds) {
        const now = Number.isFinite(nowSeconds) ? nowSeconds : 0;
        const live = nodes.filter((node) => node.alive && node.hp > 0 && distance(heroPos, node.pos) <= 760);
        if (activeBossId === null || live.length === 0) {
            this.weakpointBossId = null;
            this.weakpointId = null;
            this.pendingWeakpointId = null;
            return null;
        }
        if (this.weakpointBossId !== activeBossId) {
            this.weakpointBossId = activeBossId;
            this.weakpointId = null;
            this.pendingWeakpointId = null;
        }
        ...
        if (!current) {
            this.weakpointId = best?.id ?? null;
            this.pendingWeakpointId = null;
            return this.weakpointId;
        }
        if (!best || best.id === current.id) {
            this.pendingWeakpointId = null;
            return current.id;
        }
        if (this.pendingWeakpointId !== best.id) {
            this.pendingWeakpointId = best.id;
            this.pendingWeakpointSince = now;
            return current.id;
        }
        if (now - this.pendingWeakpointSince + 1e-9 < AUTO_WEAKPOINT_SWITCH_SECONDS)
            return current.id;
        this.weakpointId = best.id;
        this.pendingWeakpointId = null;
        return best.id;
    }
    chooseAutoCastAction(readyActions, nowSeconds) {
        const now = Number.isFinite(nowSeconds) ? nowSeconds : 0;
        if (now - this.lastCastAt + 1e-9 < AUTO_CAST_GAP_SECONDS || readyActions.length === 0)
            return null;
    ...
    recordAutoCast(action, nowSeconds) {
        this.lastCastAt = Number.isFinite(nowSeconds) ? nowSeconds : 0;
        const index = AUTO_ACTION_ORDER.indexOf(action);
        if (index >= 0)
            this.castCursor = (index + 1) % AUTO_ACTION_ORDER.length;
    }
}
