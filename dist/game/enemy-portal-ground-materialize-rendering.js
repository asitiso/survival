...
export function advanceEnemyPortalGroundMaterializeState(previous, event, dt, reducedMotion = false) {
    if (event)
        return { kind: event.kind, strength: 1 };
    if (!previous)
        return undefined;
...
export function enemyPortalGroundMaterializePresentation(state, reducedMotion = false) {
    const strength = clamp(state?.strength ?? 0, 0, 1), kind = state?.kind ?? 'regular';
    if (strength <= 0)
        return { locomotionScale: 1, shadowAlphaScale: 1, shadowWidthScale: 1, groundPulseScale: 1, groundOffsetY: 0 };
    ...
    if (reducedMotion) {
        locomotionScale = 1 - (1 - locomotionScale) * .72;
        shadowAlphaScale = 1 - (1 - shadowAlphaScale) * .74;
        shadowWidthScale = 1 + (shadowWidthScale - 1) * .62;
        groundOffsetY *= .58;
    }
