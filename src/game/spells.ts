  ...
  render(ctx: CanvasRenderingContext2D, motion?: ResidualCombatMotionPolicy, propVfxAtlasImage?: HTMLImageElement | null, propVfxAtlasReady = false, heroProjectileAtlasImage?: HTMLImageElement | null, heroProjectileAtlasReady = false, heroSpellSignatureAtlasImage?: HTMLImageElement | null, heroSpellSignatureAtlasReady = false, ultimateSignatureAtlasImage?: HTMLImageElement | null, ultimateSignatureAtlasReady = false, persistentSpellZoneVfxAtlasImage?: HTMLImageElement | null, persistentSpellZoneVfxAtlasReady = false, crowdControlPropagationVfxAtlasImage?: HTMLImageElement | null, crowdControlPropagationVfxAtlasReady = false, reducedFlash = false, ultimatePostImpactResidueVfxAtlasImage?: HTMLImageElement | null, ultimatePostImpactResidueVfxAtlasReady = false, reducedMotion = false, presentationQuality: PresentationQuality = 'high', primaryProjectileLabelBlockers:readonly Vec2[]=[], heroPos:Vec2|null=null, bossTelegraphZones:readonly {pos:Vec2;radius:number}[]=[]): void {
    ...
    if (heroProjectileAtlasReady && heroProjectileAtlasImage) {
      ...
      for (const impact of this.projectileImpactVisuals) {
        ...
        ctx.restore();
        ctx.save();ctx.globalAlpha=.38*arrivalContinuity.edgeAlphaScale*reactionCarry.aftermathAlphaScale*arrivalSettleRecovery.footprintAlphaScale*impactRetirement.footprintAlphaScale*impactResolutionBudget.effectStrength*impactSpatial.edgeAlphaScale*impactTemporal.edgeAlphaScale*impactPriority.impactAlphaScale*impactThreatBudget.impactDecorationScale*impactSpatial.fillAlphaScale*impactTemporal.decorationAlphaScale*impactTemporalBudget.secondaryAlphaScale*(impact.alphaScale??1)*(budget?.alphaScale??1)*impactDepth.edgeAlphaScale*telegraphDepth.impactEdgeAlphaScale*impactDepthHandoff.edgeAlphaScale*telegraphDepthRelease.impactEdgeAlphaScale*impactStackRetirement.edgeAlphaScale*impactCorridorSeparation.edgeAlphaScale*impactCorridorRelease.edgeAlphaScale*impactDenseArbitration.edgeAlphaScale*impactDepthPlane.edgeAlphaScale*impactDepthReentry.edgeAlphaScale*bossImpactFocus.edgeAlphaScale*impactCanonicalReacquisition.edgeAlphaScale*impactCanonicalReacquisitionBudget.criticalEdgeScale;ctx.strokeStyle=impact.secondaryKind==='splash'?'#ffd6a3':'#bcecff';ctx.lineWidth=1.4;ctx.beginPath();ctx.arc(impactVisualPos.x,impactVisualPos.y,Math.max(7,size*.34)*impactSpatial.radiusScale,0,Math.PI*2);ctx.stroke();ctx.restore();
      }
