...
export function runBossSafeResponseSlotHysteresisAudit() {
    const samples = [];
    let passed = true;
    for (let i = 0; i < 64; i++) {
        ...
        const ok = r.presentationOnly && r.placement.visible && r.placement.slot === 'above' && r.memory?.presentationOnly === true;
        passed &&= ok;
        samples.push(`${i}:${r.placement.slot}:${r.memory?.holdUntil.toFixed(3) ?? 'none'}`);
    }
