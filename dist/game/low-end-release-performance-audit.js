...
export function simulateLowEndStress(fps, pressure, frames) { let state = createDefaultMobileFrameGovernorState(), framesToMinimal = 0; for (let i = 1; i <= frames; i++) {
    state = advanceMobileFrameGovernor(state, { fps, adaptivePressure: pressure });
    if (state.tier === 'minimal' && framesToMinimal === 0)
        framesToMinimal = i;
} return { fps, pressure, frames, framesToMinimal, finalTier: state.tier, enemyLogicMultiplier: 1 }; }
export function auditLowEndReleasePerformance() {
    const profiles = [[24, .95], [30, .95], [38, .82], [42, .8], [28, .88], [35, .9]];
    const stress = profiles.map(([fps, p]) => simulateLowEndStress(fps, p, 180));
    let spike = createDefaultMobileFrameGovernorState();
    for (let i = 0; i < 45; i++)
        spike = advanceMobileFrameGovernor(spike, { fps: 34, adaptivePressure: .92 });
    let recovered = stress[0].finalTier === 'minimal' ? (() => { let s = createDefaultMobileFrameGovernorState(); for (let i = 0; i < 180; i++)
        s = advanceMobileFrameGovernor(s, { fps: 30, adaptivePressure: .95 }); let frames = 0; while (s.tier === 'minimal' && frames < 600) {
        s = advanceMobileFrameGovernor(s, { fps: 60, adaptivePressure: .15 });
        frames++;
    } return frames; })() : 0;
