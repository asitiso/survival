export interface BossAnticipationOriginCoherenceInput{bodyOffsetX:number;bodyOffsetY:number;rebaseOffsetX:number;rebaseOffsetY:number;displacementStrength:number;charge:number;recovery:number;stagger:number;radius:number}
export interface BossAnticipationOriginCoherencePresentation{owner:'body'|'ground-rebase';ringOffsetX:number;ringOffsetY:number;groundLock:number;bodyLock:number;presentationOnly:true}
...
export function bossAnticipationOriginCoherencePresentation(input:BossAnticipationOriginCoherenceInput,reducedMotion=false):BossAnticipationOriginCoherencePresentation{
...
function ringOwner(owner:'body'|'ground-rebase',stagger:number):'body'|'ground-rebase'{return stagger>.45?'body':owner;}
