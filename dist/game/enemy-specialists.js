...
export function specialistTarget(type) {
    return type === 'siegeGolem' ? 'core' : 'hero';
}
export function selectSpecialistEnemyType(seconds, roll) {
    const r = Math.max(0, Math.min(0.999999, roll));
    if (seconds < 300)
        return null;
    if (r < 0.03)
        return 'shieldbearer';
    if (seconds >= 360 && r < 0.065)
        return 'assassin';
    if (seconds >= 420 && r < 0.095)
        return 'siegeGolem';
    if (seconds >= 480 && r < 0.125)
        return 'nullifier';
    return null;
}
export function assassinBlinkPosition(from, hero) {
...
export function nullifierCooldownMultiplier(enemies, heroPos) {
    let nearby = 0;
    for (const enemy of enemies) {
        if (!enemy.alive || enemy.type !== 'nullifier')
            continue;
        if (Math.hypot(enemy.pos.x - heroPos.x, enemy.pos.y - heroPos.y) <= SPECIALIST_COMBAT_CONTRACT.nullifierEffectRadius + enemy.radius)
            nearby += 1;
    }
