...
import { MYTHIC_LAST_LAW_IDENTITY_IDS,auditMythicLastLawIdentityAtlas,mythicLastLawIdentityIcon,type MythicLastLawAssetId } from './mythic-last-law-identity-assets.js';

export interface MythicLastLawIdentityAssetSample{caseId:string;id?:MythicLastLawAssetId;passed:boolean;}
export interface MythicLastLawIdentityAssetAudit{
  samples:MythicLastLawIdentityAssetSample[];
  lawCount:number;coverage:number;uniqueCellCount:number;outOfBounds:MythicLastLawAssetId[];
  toastCoverage:number;safeLaneCoverage:number;fallbackCoverage:number;
  textFallbackPreserved:boolean;imageLoadFailureNonBlocking:boolean;iconMotionAmplitude:number;
  activationThresholdMutation:boolean;modifierMutation:boolean;safeZoneLifecycleMutation:boolean;
  actionCount:number;snapshotSchemaMutation:false;issues:string[];passed:boolean;
}

const ARCHETYPE_BY_ID:Readonly<Record<MythicLastLawAssetId,BossArchetype>>={
...
const EXPECTED:Readonly<Record<MythicLastLawAssetId,{label:string;boss:number;cadence:number;summon:number;dash:number;projectile:number;reward:number}>>={
...
export function auditMythicLastLawIdentityAssets():MythicLastLawIdentityAssetAudit{
  const atlas=auditMythicLastLawIdentityAtlas();const samples:MythicLastLawIdentityAssetSample[]=[];
  const push=(caseId:string,passed:boolean,id?:MythicLastLawAssetId):void=>{samples.push({caseId,passed,...(id?{id}:{})});};
  const toastSet=new Set<MythicLastLawAssetId>(),safeLaneSet=new Set<MythicLastLawAssetId>(),fallbackSet=new Set<MythicLastLawAssetId>();
  ...
  for(const id of MYTHIC_LAST_LAW_IDENTITY_IDS){
    const icon=mythicLastLawIdentityIcon(id);const archetype=ARCHETYPE_BY_ID[id];const expected=EXPECTED[id];
    const bodyOk=icon.sx>=0&&icon.sy>=0&&icon.sx+icon.sw<=288&&icon.sy+icon.sh<=192;push(`${id}:body`,bodyOk,id);
    ...
    const toastOk=icon.toastIdentitySupported;if(toastOk)toastSet.add(id);push(`${id}:toast`,toastOk,id);
    const safeLaneOk=icon.safeLaneIdentitySupported;if(safeLaneOk)safeLaneSet.add(id);push(`${id}:safe-lane`,safeLaneOk,id);
    const before=mythicLastLawIdentityProfile(mythic,archetype,.151,1);const threshold=mythicLastLawIdentityProfile(mythic,archetype,.15,1);
    const activationOk=!before.active&&before.lawId==='none'&&threshold.active&&threshold.lawId===id;activationThresholdMutation=activationThresholdMutation||!activationOk;push(`${id}:activation`,activationOk,id);
    const intact=mythicLastLawIdentityProfile(mythic,archetype,.1,1);const cleared=mythicLastLawIdentityProfile(mythic,archetype,.1,0);
    ...
    modifierMutation=modifierMutation||!modifierOk;push(`${id}:modifiers`,modifierOk,id);
  ...
  const inactiveLifecycle=lastLawSafeZoneLifecycle(false,.4);const activeLifecycle=lastLawSafeZoneLifecycle(true,0);const clearedLifecycle=lastLawSafeZoneLifecycle(true,1);
  ...
  if(toastCoverage!==1)issues.push('toast-coverage');if(safeLaneCoverage!==1)issues.push('safe-lane-coverage');if(fallbackCoverage!==1)issues.push('fallback-coverage');
  ...
  if(activationThresholdMutation)issues.push('activation-threshold-mutation');if(modifierMutation)issues.push('modifier-mutation');if(safeZoneLifecycleMutation)issues.push('safe-zone-lifecycle-mutation');
