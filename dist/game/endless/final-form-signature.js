...
export const FINAL_FORM_SIGNATURE_IDS = new Set(Object.keys(PROFILES));
export function createDefaultFinalFormSignatureState() {
    return { charge: 0, activeUntilMs: 0, cooldownUntilMs: 0, activations: 0, formId: null };
}
export function finalFormSignatureProfile(form) {
    return PROFILES[form.id];
}
function eventCharge(event, family) {
    if (event.type === 'spell_cast')
        return event.fusion ? (family === 'domain' ? 8 : 7) : (family === 'cycle' ? 5 : 4);
    if (event.type === 'enemy_killed')
        return event.elite ? 7 : 2;
    if (event.type === 'boss_defeated')
        return 28;
    if (event.type === 'hero_damaged')
        return family === 'fortress' ? 5 : 1;
    if (event.type === 'core_damaged')
        return family === 'fortress' ? 7 : 1;
    return 0;
}
export function advanceFinalFormSignature(state, form, events, elapsedMs) {
    const now = Math.max(0, elapsedMs);
    if (!form)
        return { state: createDefaultFinalFormSignatureState(), activated: false };
    ...
    for (const event of events)
        charge = clamp(charge + eventCharge(event, profile.family), 0, 100);
...
export function finalFormSignatureModifiers(state, form, elapsedMs) {
