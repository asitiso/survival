export const FUSION_MODIFIER_IDENTITY_IDS = ['damage', 'area', 'cooldown', 'chain', 'pierce', 'slow-duration', 'tick-power'];
const CELL = {
...
export const FUSION_MODIFIER_IDENTITY_ATLAS = { src: './assets/ui/fusion-modifier-icons.png', columns: 4, rows: 2, cellSize: 96, width: 384, height: 192 };
const pct = (index, count) => count <= 1 ? 0 : (index / (count - 1)) * 100;
export function fusionModifierIdentityIcon(id) { const [column, row] = CELL[id], meta = META[id]; return { id, label: meta.label, accent: meta.accent, sx: column * 96, sy: row * 96, sw: 96, sh: 96, animated: false, motionAmplitude: 0, textFallbackPreserved: true, loadFailureBlocksGameplay: false }; }
export function fusionModifierIdentityStyle(id) { const [column, row] = CELL[id]; return `--secondary-icon-image:url('${FUSION_MODIFIER_IDENTITY_ATLAS.src}');--secondary-icon-bg-size:400% 200%;--secondary-icon-bg-position:${pct(column, 4)}% ${pct(row, 2)}%`; }
