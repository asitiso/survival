...
export function coreProjectileGuardImpactHandoffPresentation(input, reducedFlash = false) {
    const prevented = clamp01(input.preventedRatio), life = input.impactMaxTtl > 0 ? clamp01(input.impactTtl / input.impactMaxTtl) : 0;
    if (life <= 0)
        return { owner: 'retired', threatAlphaScale: 0, ordinaryImpactAlphaScale: 0, coreGuardImpactAlpha: 0, arcRadius: 16, deflectDistance: 0, presentationOnly: true };
    if (prevented < .16)
        return { owner: 'impact', threatAlphaScale: 0, ordinaryImpactAlphaScale: 1, coreGuardImpactAlpha: 0, arcRadius: 16, deflectDistance: 0, presentationOnly: true };
