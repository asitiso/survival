export const ENEMY_TARGET_PRESSURE_VFX_CLASSES = ['regular', 'specialist', 'elite', 'boss'];
export const ENEMY_TARGET_PRESSURE_VFX_TARGETS = ['hero', 'core'];
...
export function enemyTargetPressureClassForEnemyType(type) {
    if (type === 'boss')
        return 'boss';
    if (type === 'elite')
        return 'elite';
...
export function enemyTargetPressureVisible(type, target) {
    if (target === 'core')
        return true;
...
export function enemyTargetPressureVfxSprite(enemyClass, target) {
    ...
    if (column < 0 || row < 0)
        throw new Error(`Unknown enemy target pressure VFX: ${enemyClass}:${target}`);
...
export function auditEnemyTargetPressureVfxAtlas() {
    ...
    for (const enemyClass of ENEMY_TARGET_PRESSURE_VFX_CLASSES)
        for (const target of ENEMY_TARGET_PRESSURE_VFX_TARGETS) {
            const r = enemyTargetPressureVfxSprite(enemyClass, target);
            cells.add(`${r.sx}:${r.sy}`);
            if (r.sx < 0 || r.sy < 0 || r.sx + r.sw > ENEMY_TARGET_PRESSURE_VFX_ATLAS.width || r.sy + r.sh > ENEMY_TARGET_PRESSURE_VFX_ATLAS.height)
                outOfBounds.push(`${enemyClass}:${target}`);
        }
