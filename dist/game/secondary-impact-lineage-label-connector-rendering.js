...
export function secondaryImpactLineageLabelConnectorPresentation(input, reducedFlash = false) {
    ...
    if (!owned)
        return { visible: false, from: { ...input.labelPos }, to: { ...input.labelPos }, alpha: 0, lineWidth: 1.1, presentationOnly: true };
    ...
    return { visible: true, from, to, alpha: baseAlpha * (reducedFlash ? .62 : 1), lineWidth: input.biasApplied ? 1.25 : 1.05, presentationOnly: true };
}
