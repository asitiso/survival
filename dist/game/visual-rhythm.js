...
export function edgeThreatVfxProfile(level, target) {
...
export function edgeThreatIndicator(pos, width, height) {
...
export function deathAfterglowProfile(type, quality) {
...
export function ultimateAftermathProfile(spellId, level, quality) {
    const tier = level >= 10 ? 2 : level >= 5 ? 1 : 0, q = quality === 'high' ? 1 : quality === 'medium' ? .74 : .48;
    if (spellId === 'meteorStorm')
        return { motion: 'embers', color: '#ff8b52', secondary: '#ffd36d', ringCount: [2, 3, 4][tier], particleCount: Math.max(3, Math.round([7, 9, 12][tier] * q)), alpha: Math.min(.24, [.16, .20, .24][tier] * q), ttl: [.42, .52, .62][tier], radius: [86, 112, 142][tier] };
...
export function bossSettleProfile(archetype, quality) {
...
export function createVfxQualityTransition(initial) { return { current: initial, candidate: initial, dwellSeconds: 0 }; }
function rank(q) { return q === 'high' ? 2 : q === 'medium' ? 1 : 0; }
function stepUp(q) { return q === 'low' ? 'medium' : 'high'; }
export function advanceVfxQualityTransition(state, target, dt) {
    const step = Math.max(0, Math.min(2, Number.isFinite(dt) ? dt : 0));
    if (rank(target) < rank(state.current))
        return { current: target, candidate: target, dwellSeconds: 0 };
    if (target === state.current)
        return { current: state.current, candidate: target, dwellSeconds: 0 };
    const dwell = state.candidate === target ? state.dwellSeconds + step : step;
    if (dwell >= 1.2)
        return { current: stepUp(state.current), candidate: target, dwellSeconds: 0 };
