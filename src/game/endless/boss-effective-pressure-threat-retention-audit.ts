...
export interface BossEffectivePressureThreatRetentionSample { id:string; passed:boolean; }
export interface BossEffectivePressureThreatRetentionAudit {
  samples:BossEffectivePressureThreatRetentionSample[];
  mixedCases:number;
  threatOnlyCases:number;
  opportunityOnlyCases:number;
  invariantCases:number;
  mixedThreatRetentionPassed:boolean;
  maxTwoPassed:boolean;
  noThreatCompatibilityPassed:boolean;
  stableTieOrderPassed:boolean;
...
export function auditBossEffectivePressureThreatRetention():BossEffectivePressureThreatRetentionAudit{
  const samples:BossEffectivePressureThreatRetentionSample[]=[];const push=(id:string,passed:boolean)=>samples.push({id,passed});
  ...
  for(const threatId of EFFECTS)for(const opportunityId of EFFECTS.filter(id=>id!==threatId))for(const opportunityMagnitude of [.2,.35] as const){
    const input=neutral();setMultiplier(input,threatId,threatValue(threatId,.02));setMultiplier(input,opportunityId,opportunityValue(opportunityId,opportunityMagnitude));
    ...
    mixedCases++;mixedThreatRetentionPassed&&=ok;maxTwoPassed&&=p.primaryEffects.length<=2;push(`mixed:${threatId}:${opportunityId}:${opportunityMagnitude}`,ok);
  }
  for(const id of EFFECTS)for(const magnitude of [.05,.12,.2] as const){
    const input=neutral();setMultiplier(input,id,threatValue(id,magnitude));const p=projectBossEffectivePressure(input),ok=p.primaryEffects.length===1&&p.primaryEffects[0]?.effectId===id&&p.primaryEffects[0]?.impact==='threat';
    threatOnlyCases++;maxTwoPassed&&=p.primaryEffects.length<=2;push(`threat:${id}:${magnitude}`,ok);
  }
  for(const id of EFFECTS)for(const magnitude of [.05,.12,.2] as const){
    const input=neutral();setMultiplier(input,id,opportunityValue(id,magnitude));const p=projectBossEffectivePressure(input),ok=p.primaryEffects.length===1&&p.primaryEffects[0]?.effectId===id&&p.primaryEffects[0]?.impact==='opportunity';
    opportunityOnlyCases++;noThreatCompatibilityPassed&&=ok;maxTwoPassed&&=p.primaryEffects.length<=2;push(`opportunity:${id}:${magnitude}`,ok);
  }
  ...
  const issues:string[]=[];
  if(samples.length!==64)issues.push(`samples:${samples.length}`);if(mixedCases!==24)issues.push(`mixed:${mixedCases}`);if(threatOnlyCases!==12)issues.push(`threat-only:${threatOnlyCases}`);if(opportunityOnlyCases!==12)issues.push(`opportunity-only:${opportunityOnlyCases}`);if(invariantCases!==16)issues.push(`invariants:${invariantCases}`);
  if(!mixedThreatRetentionPassed)issues.push('mixed-threat-retention');if(!maxTwoPassed)issues.push('max-two');if(!noThreatCompatibilityPassed)issues.push('no-threat-compatibility');if(!stableTieOrderPassed)issues.push('stable-tie-order');if(!existingAtlasReusePassed)issues.push('atlas-reuse');if(ACTION_BUTTONS.length!==9)issues.push(`actions:${ACTION_BUTTONS.length}`);if(samples.some(v=>!v.passed))issues.push('sample-failure');
