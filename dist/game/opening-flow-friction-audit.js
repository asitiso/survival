...
export function auditOpeningFlowFriction() {
    ...
    const issues = [];
    if (auto.initialAutoEnabled)
        issues.push('opening-auto-must-be-off');
    if (upgradeRecommendationCoverage < 1)
        issues.push('opening-upgrade-guidance');
    if (shopPointerTravelReduction < .45)
        issues.push('opening-shop-travel');
    if (bossPrepCoverage < 1)
        issues.push('opening-boss-prep');
    if (estimatedFrictionReduction < .3)
        issues.push('opening-friction-reduction');
