...
export class Game {
  ...
  private drawBossArenaHazards(ctx: CanvasRenderingContext2D, motion: SecondaryCombatMotionPolicy): void {
    ...
    if (safeLane) {
      ...
      ctx.save(); ctx.globalAlpha = safeLaneBaseAlpha*safeLaneHazardRecovery.pathAlphaScale; ctx.strokeStyle = '#8fffd3'; ctx.fillStyle = '#8fffd3'; ctx.lineWidth = 2; ctx.setLineDash([7,7]);
      ctx.beginPath();for(const segment of safeLanePathGap.segments){ctx.moveTo(segment.from.x,segment.from.y);ctx.lineTo(segment.to.x,segment.to.y);}ctx.stroke(); ctx.setLineDash([]);
      if(safeLanePathGap.locatorVisible){ctx.globalAlpha=safeLaneBaseAlpha*safeLaneHazardOcclusion.locatorAlphaScale*safeLaneHazardRecovery.locatorAlphaScale;ctx.beginPath(); ctx.arc(safeLaneVisualTarget.x, safeLaneVisualTarget.y, 14, 0, Math.PI*2); ctx.stroke();}
