...
import { RUN_TRAITS, runTraitBonuses } from './run-traits.js';
const THREATS = [0, 3, 5];
...
function archetypeProfile(archetype) {
    ...
    if (archetype === 'burst')
        return { offense: Math.pow(mods.spellPowerMultiplier, .72) * Math.pow(mods.bossDamageMultiplier, .28), survival: .97, core: .97, economy: 1 };
    if (archetype === 'cycle')
        return { offense: Math.pow(1 / mods.cooldownMultiplier, .72) * Math.pow(mods.fusionPowerMultiplier, .28), survival: 1, core: 1, economy: 1 };
    if (archetype === 'domain')
        return { offense: Math.pow(mods.spellPowerMultiplier, .55) * Math.pow(mods.areaMultiplier, .45), survival: 1.03, core: 1.05, economy: 1 };
...
function legalTraits(heroId) { return [...RUN_TRAITS.map((trait) => trait.id), masteryTraitId(heroId)]; }
export function heroBuildCombinationCheckpoints() {
    const points = [];
...
export function auditHeroBuildCombinations() {
    const checkpoints = heroBuildCombinationCheckpoints();
    let maxViabilitySpread = 1;
    for (const threat of THREATS)
        maxViabilitySpread = Math.max(maxViabilitySpread, spread(checkpoints.filter((point) => point.threat === threat).map((point) => point.viabilityIndex)));
    ...
    const archetypeAxisMeans = ARCHETYPES.map((archetype) => {
        ...
        const mean = (field) => values.reduce((sum, value) => sum + value[field], 0) / Math.max(1, values.length);
        return { offense: mean('offenseIndex'), survival: mean('survivalIndex'), core: mean('coreGuardIndex') };
    });
    const archetypeDistinctness = round(Math.max(spread(archetypeAxisMeans.map((value) => value.offense)), spread(archetypeAxisMeans.map((value) => value.survival)), spread(archetypeAxisMeans.map((value) => value.core))));
    ...
    for (const hero of HERO_PROFILES)
        for (const traitId of legalTraits(hero.id))
            for (const archetype of ARCHETYPES) {
                ...
                if (!(group[0].releaseMargin > group[1].releaseMargin && group[1].releaseMargin > group[2].releaseMargin))
