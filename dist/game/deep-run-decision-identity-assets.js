...
export const DEEP_RUN_ASCENSION_IDS = [
...
export const DEEP_RUN_CONTRACT_IDS = ['slayer', 'warden', 'arcane', 'hunter', 'survivor'];
export const DEEP_RUN_OATH_IDS = ['slayer', 'elite_hunt', 'boss_hunt', 'arcane_flow', 'core_guard', 'endure'];
const KEY_ORDER = [
    ...DEEP_RUN_ASCENSION_IDS.map(id => `ascension:${id}`),
    ...DEEP_RUN_CONTRACT_IDS.map(id => `contract:${id}`),
    ...DEEP_RUN_OATH_IDS.map(id => `oath:${id}`),
];
const CELL = new Map(KEY_ORDER.map((key, index) => [key, [index % DEEP_RUN_DECISION_ATLAS.columns, Math.floor(index / DEEP_RUN_DECISION_ATLAS.columns)]]));
const pct = (value, total) => total <= 1 ? 0 : (value / (total - 1)) * 100;
const keyOf = (identity) => `${identity.kind}:${identity.id}`;
export function deepRunDecisionIdentityIcon(identity) {
    const key = keyOf(identity);
    const cell = CELL.get(key);
    if (!cell)
        throw new Error(`Unknown deep-run decision identity: ${key}`);
    const [column, row] = cell;
    const size = DEEP_RUN_DECISION_ATLAS.cellSize;
...
export function deepRunDecisionIdentityStyle(identity) {
...
export function isDeepRunAscensionId(value) { return typeof value === 'string' && DEEP_RUN_ASCENSION_IDS.includes(value); }
export function isDeepRunContractId(value) { return typeof value === 'string' && DEEP_RUN_CONTRACT_IDS.includes(value); }
export function isDeepRunOathId(value) { return typeof value === 'string' && DEEP_RUN_OATH_IDS.includes(value); }
export function auditDeepRunDecisionIdentityAtlas() {
    const cells = new Set();
    const outOfBounds = [];
    for (const key of KEY_ORDER) {
        const [column, row] = CELL.get(key);
        const sx = column * 96, sy = row * 96;
        cells.add(`${sx}:${sy}`);
        if (column < 0 || row < 0 || column >= DEEP_RUN_DECISION_ATLAS.columns || row >= DEEP_RUN_DECISION_ATLAS.rows || sx + 96 > DEEP_RUN_DECISION_ATLAS.width || sy + 96 > DEEP_RUN_DECISION_ATLAS.height)
