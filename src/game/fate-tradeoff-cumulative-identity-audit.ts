...
import { FATE_CHECKPOINTS, composeFateModifiers, fateCheckpointIndex, type FatePathId } from './fate-paths.js';
...
export interface FateTradeoffCumulativeIdentitySample{caseId:string;path:FatePathId;passed:boolean;}
export interface FateTradeoffCumulativeIdentityAudit{samples:FateTradeoffCumulativeIdentitySample[];benefitIdentityCount:number;costIdentityCount:number;benefitCoverage:number;costCoverage:number;benefitUniqueCellCount:number;costUniqueCellCount:number;checkpoints:readonly[360,720,1080];gameplayMutation:boolean;actionCount:number;snapshotSchemaMutation:false;issues:string[];passed:boolean;}

const PATHS:readonly FatePathId[]=['frenzy','golden','guardian'];
const EXPECTED_FIRST={frenzy:{benefit:'xp-growth',cost:'boss-variant'},golden:{benefit:'gold-shop',cost:'boss-variant'},guardian:{benefit:'core-guard',cost:'growth-tax'}} as const;
const EXPECTED_REPEAT={frenzy:{benefit:'objective-reward',cost:'boss-variant'},golden:{benefit:'objective-reward',cost:'boss-variant'},guardian:{benefit:'objective-reward',cost:'growth-tax'}} as const;

export function auditFateTradeoffCumulativeIdentityAssets():FateTradeoffCumulativeIdentityAudit{
  const benefit=auditFateBenefitVectorAtlas(),cost=auditFateCostVectorAtlas(),samples:FateTradeoffCumulativeIdentitySample[]=[];let gameplayMutation=false;
  const push=(caseId:string,path:FatePathId,passed:boolean)=>{samples.push({caseId,path,passed});if(!passed)gameplayMutation=true;};
  ...
  const issues:string[]=[];if(samples.length!==60)issues.push(`samples:${samples.length}`);if(!benefit.passed)issues.push('benefit-atlas');if(!cost.passed)issues.push('cost-atlas');if(gameplayMutation)issues.push('gameplay');if(ACTION_BUTTONS.length!==9)issues.push('actions');if(JSON.stringify(FATE_CHECKPOINTS)!==JSON.stringify([360,720,1080]))issues.push('checkpoints');
