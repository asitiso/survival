...
export function auditThirtySixtyFlowHealth() {
    let samples = 0, goal = 0, shop = 0, hud = 0, reward = 0;
    for (const heroId of HEROES)
        for (const elapsedSeconds of [1900, 2200, 2500, 2800, 3400]) {
            samples += 4;
            if (secondBossBuildGoal({ elapsedSeconds, bossesKilled: 3, heroId, spellLevels: levels, activeRelic: 'abyss-eye', activeFusions: ['solar-detonation', 'storm-crucible'], equipment }))
                goal++;
            if (lateShopFastPath(elapsedSeconds, offer, equipment).promoteQuickBuy)
                shop++;
            const focus = completedBuildHudFocus({ elapsedSeconds, equipment, activeRelic: 'abyss-eye', activeFusionCount: 2 });
            if (focus.maxBuildLabels <= 2 && focus.keepCriticalBars)
                hud++;
            const guided = reduceRepeatBossRewardDecision(rewardChoices, { elapsedSeconds, activeRelic: 'abyss-eye', activeFusionCount: 2 });
