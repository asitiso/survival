...
export function runSecondaryImpactClusterReadabilityBudgetAudit(){
  ...
  for(const quality of ['high','medium','low'] as const)for(const reduced of [false,true])for(let sample=0;sample<12;sample++){
  ...
  return{samples,actionCount:ACTION_BUTTONS.length,presentationOnly:true as const,gameplayFormulaMutation:false as const,snapshotSchemaMutation:false as const,newAtlasCount:0,passed:samples.length===72&&samples.every(sample=>sample.passed)&&ACTION_BUTTONS.length===9};
}
