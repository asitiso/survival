...
export function dangerProjectileCues(projectiles, heroPos, corePos, limit = 6) {
    const cues = [];
    for (let index = 0; index < projectiles.length; index++) {
        const p = projectiles[index];
        const targetPos = p.target === 'core' ? corePos : heroPos;
        const threatPos = p.visualPos ?? p.pos;
        ...
        const hitWidth = Math.max(20, p.radius + targetRadius + 8);
        if (perpendicular > hitWidth)
            continue;
        ...
        const level = time <= .55 || p.damage >= 24 ? 'critical' : time <= 1.0 || p.damage >= 16 ? 'danger' : 'watch';
