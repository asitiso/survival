...
const safeNow = (value) => Number.isFinite(value) ? value : 0;
const clonedAnchor = (cue) => ({ x: cue.start.x, y: cue.start.y });
export function spawnLaneKindReentryMemoryStale(entry, siblings, nowValue) {
    const now = safeNow(nowValue);
    if (now - entry.lastSeen < SPAWN_LANE_KIND_REENTRY_FRESHNESS_SECONDS)
        return false;
...
export function spawnLaneSameKindResurrectionMemoryStale(entry, nowValue) {
    const now = safeNow(nowValue);
    return now - entry.lastSeen > SPAWN_LANE_SAME_KIND_RESURRECTION_GUARD_SECONDS;
}
export function spawnLaneSameKindSpatialReentryDistance(entry, cue) {
...
export function spawnLaneSameKindSpatialReentryMemoryStale(entry, cue) {
...
export function spawnLaneCumulativeAnchorOriginDriftDistance(entry, cue) {
...
export function spawnLaneCumulativeAnchorOriginDriftMemoryStale(entry, cue) {
...
export function spawnLaneEdgeCountDownwardDebounce(previous, cues, nowValue) {
    const now = safeNow(nowValue);
    ...
    for (const cue of cues) {
        ...
        if (entry && (spawnLaneKindReentryMemoryStale(entry, siblings, now) || spawnLaneSameKindResurrectionMemoryStale(entry, now) || spawnLaneSameKindSpatialReentryMemoryStale(entry, cue) || spawnLaneCumulativeAnchorOriginDriftMemoryStale(entry, cue))) {
            const staleIndex = memory.indexOf(entry);
            if (staleIndex >= 0)
                memory.splice(staleIndex, 1);
            entry = undefined;
        }
        if (!entry) {
        ...
        if (escalationFromNewerDifferent) {
            entry.displayCount = observed;
            entry.pendingCount = observed;
            entry.pendingSince = now;
            entry.kind = cue.kind;
            entry.originAnchorPos = clonedAnchor(cue);
        }
        else if (observed >= entry.displayCount) {
            entry.displayCount = observed;
            entry.pendingCount = observed;
            entry.pendingSince = now;
        }
        else if (entry.pendingCount !== observed) {
            entry.pendingCount = observed;
            entry.pendingSince = now;
        }
        else if (now - entry.pendingSince >= SPAWN_LANE_EDGE_COUNT_DOWNWARD_DEBOUNCE_SECONDS) {
            entry.displayCount = observed;
            entry.pendingCount = observed;
            entry.pendingSince = now;
        }
