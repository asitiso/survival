...
function copyWithPriceVariance(offer: ShopDisplayOffer, rng: () => number): ShopDisplayOffer {
  const variance = 0.92 + rng() * 0.16;
  return { ...offer, price: Math.max(40, Math.round(offer.price * variance / 10) * 10) };
}

function pickTwo(pool: readonly ShopDisplayOffer[], rng: () => number): ShopDisplayOffer[] {
...
export function generateShopOffers(rng: () => number = Math.random): ShopDisplayOffer[] {
...
export function equipmentBonuses(state: EquipmentState): EquipmentBonuses {
