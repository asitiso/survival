...
export function auditFourEightHourFlowHealth():FourEightHourFlowHealthAudit{
  const checkpoints=[14401,18000,21600,25200,28800];let samples=0,shop=0,reward=0,toast=0,priority=0;
  for(const elapsedSeconds of checkpoints)for(const mythicActive of [false,true])for(const finalFormActive of [false,true]){
    samples+=4;
    if(fourEightHourShopSilence(elapsedSeconds,equipment).suppressRoutinePressure)shop++;
    const r=focusFourEightHourBossRewards(rewards,{elapsedSeconds,completeBuild:true,finalFormActive:false});if(r.compact&&r.choices.filter(x=>x.best).length===1&&!r.autoSelect)reward++;
    if(!fourEightHourToastFocus(elapsedSeconds,'보급 획득 · 체력 물약 +1').show&&fourEightHourToastFocus(elapsedSeconds,mythicActive?'MYTHIC · 패턴 결합':'보스 전장전 · 약점을 파괴하세요').show)toast++;
    const p=fourEightHourPriorityFocus({elapsedSeconds,completeBuild:true,bossActive:mythicActive,mythicActive,finalFormActive,heroCritical:false,coreCritical:false});const labels=priorityBuildLabels(['시너지 · X','최종형 · Sovereign'],p);if((!mythicActive||p.preserveBossDanger)&&(!finalFormActive||labels.some(x=>x.startsWith('최종형'))))priority++;
  }
