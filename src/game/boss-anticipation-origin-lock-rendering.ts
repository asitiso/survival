export interface BossAnticipationOriginLockInput{desiredOwner:'body'|'ground-rebase';desiredOffsetX:number;desiredOffsetY:number;rebaseOffsetX:number;rebaseOffsetY:number;handoffStrength:number;charge:number;recovery:number;stagger:number}
export interface BossAnticipationOriginLockPresentation{owner:'body'|'ground-rebase';offsetX:number;offsetY:number;lockStrength:number;presentationOnly:true}
const c=(v:number)=>Math.max(0,Math.min(1,Number.isFinite(v)?v:0));
export function bossAnticipationOriginLockPresentation(input:BossAnticipationOriginLockInput,reducedMotion=false):BossAnticipationOriginLockPresentation{
 ...
 const blend=(reducedMotion?.66:1)*lockStrength;return{owner,offsetX:dx+(rx-dx)*blend,offsetY:dy+(ry-dy)*blend,lockStrength,presentationOnly:true};
}
