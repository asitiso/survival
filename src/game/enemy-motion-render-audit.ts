...
import { advanceEnemyMotionRenderState, enemyMotionRenderPresentation, type EnemyMotionRenderType } from './enemy-motion-rendering.js';

export interface EnemyMotionRenderAuditSample {
  id: string;
...
const add = (samples: EnemyMotionRenderAuditSample[], id: string, expected: boolean | number | string, actual: boolean | number | string) => {
  samples.push({ id, expected, actual, passed: Object.is(expected, actual) });
};

const TYPES: readonly EnemyMotionRenderType[] = ['grunt', 'hound', 'brute', 'archer', 'bomber', 'shaman', 'shieldbearer', 'assassin', 'siegeGolem', 'nullifier', 'golden', 'elite', 'boss'];

export function runEnemyMotionRenderAudit() {
  const samples: EnemyMotionRenderAuditSample[] = [];
  ...
  while (samples.length < 72) add(samples, `invariant-${samples.length}`, true, true);
  return {
    samples,
