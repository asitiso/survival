...
const MAGNITUDES = [.05, .12, .2, .32];
const neutral = () => ({ bossDamageTakenMultiplier: 1, specialCadenceMultiplier: 1, summonCountMultiplier: 1, dashDistanceMultiplier: 1 });
function withMultiplier(id, multiplier) {
    const out = neutral();
    if (id === 'special-cadence')
        out.specialCadenceMultiplier = multiplier;
    else if (id === 'summon-pressure')
        out.summonCountMultiplier = multiplier;
    else if (id === 'dash-distance')
        out.dashDistanceMultiplier = multiplier;
    else
        out.bossDamageTakenMultiplier = multiplier;
    return out;
}
function threatMultiplier(id, magnitude) {
    return id === 'special-cadence' || id === 'boss-vulnerability' ? 1 - magnitude : 1 + magnitude;
}
function opportunityMultiplier(id, magnitude) {
    return id === 'special-cadence' || id === 'boss-vulnerability' ? 1 + magnitude : 1 - magnitude;
}
export function auditBossEffectivePressureSemantics() {
    const samples = [];
    const threatSeen = new Set(), opportunitySeen = new Set();
    ...
    for (const id of EFFECTS) {
        for (const magnitude of MAGNITUDES) {
            ...
            if (threatOk)
                threatSeen.add(id);
            threatCases++;
            push(`threat:${id}:${magnitude}`, Boolean(threatOk));
            ...
            if (opportunityOk)
                opportunitySeen.add(id);
            opportunityCases++;
            push(`opportunity:${id}:${magnitude}`, Boolean(opportunityOk));
        }
    }
    const neutralValues = [1, Number.NaN, Number.POSITIVE_INFINITY, Number.NEGATIVE_INFINITY];
    for (const id of EFFECTS) {
        for (const value of neutralValues) {
            ...
            neutralCases++;
            push(`neutral:${id}:${String(value)}`, Boolean(ok));
        }
    }
    ...
    if (threatCases !== 16)
        issues.push(`threat-cases:${threatCases}`);
    if (opportunityCases !== 16)
        issues.push(`opportunity-cases:${opportunityCases}`);
    if (neutralCases !== 16)
        issues.push(`neutral-cases:${neutralCases}`);
    if (!semanticCoverageComplete)
        issues.push('semantic-coverage');
    if (!signInversionCoverageComplete)
        issues.push('sign-inversion');
    if (!textSemanticLabelPassed)
        issues.push('semantic-text');
