...
export function auditAutoTransitionLatency():AutoTransitionLatencyAudit{
  const hero={x:0,y:0},core={x:700,y:0};const brain=new AutoCombatBrain();let last:number|null=null,switches=0,unnecessarySwitches=0,materialFrame=-1,switchFrame=-1;
  for(let frame=0;frame<120;frame++){
    ...
    const chosen=brain.selectTarget([one,two],hero,core,frame/60);const id=chosen?.id??null;
  ...
  const coreBrain=new AutoCombatBrain();let coreSwitch=-1,coreMaterial=30;
  for(let frame=0;frame<60;frame++){
    const one=enemy(1,250),two=enemy(2,430,frame>=coreMaterial?'core':'hero');const chosen=coreBrain.selectTarget([one,two],hero,core,frame/60);if(frame>=coreMaterial&&chosen?.id===2&&coreSwitch<0)coreSwitch=frame;
  }
  const coreThreatSwitchLatencyFrames=coreSwitch>=0?coreSwitch-coreMaterial:99;
  const weakBrain=new AutoCombatBrain();let weakLast:number|null=null,weakSwitch=-1;
  for(let frame=0;frame<60;frame++){
    const nodes=[{id:1,pos:{x:280,y:20},hp:60,maxHp:100,alive:true,radius:20},{id:2,pos:{x:330,y:-20},hp:frame<30?80:40,maxHp:100,alive:true,radius:20}];const id=weakBrain.selectWeakpoint(10,nodes,hero,frame/60);
    if(weakLast!==null&&id!==weakLast&&weakSwitch<0)weakSwitch=frame;weakLast=id;
  }
  ...
  if(unnecessarySwitches)issues.push('target-flicker');
  if(materialSwitchLatencyFrames<normalMin||materialSwitchLatencyFrames>normalMax)issues.push('material-switch-humanization');
  if(coreThreatSwitchLatencyFrames<coreMin||coreThreatSwitchLatencyFrames>coreMax)issues.push('core-switch-humanization');
  if(weakpointSwitchLatencyFrames<weakMin||weakpointSwitchLatencyFrames>weakMax)issues.push('weakpoint-switch-humanization');
  if(maxSwitchesPerSecond>2)issues.push('switch-frequency');
