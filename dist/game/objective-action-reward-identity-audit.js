...
const IDS = ['riftSeal', 'beaconDefense', 'cursedAltar'];
const EXPECTED = {
...
const MULTIPLIERS = Object.freeze([1, 1.12]);
function sameRewards(a, b) { return a.length === b.length && a.every((value, index) => value.kind === b[index]?.kind && value.amount === b[index]?.amount); }
export function auditObjectiveActionRewardIdentityAssets() {
    const actionAtlas = auditObjectiveActionIdentityAtlas(), rewardAtlas = auditObjectiveRewardIdentityAtlas();
    const samples = [];
    const push = (caseId, objectiveId, passed) => samples.push({ caseId, objectiveId, passed });
    let gameplayContractMutation = ACTION_BUTTONS.length !== 9;
    for (const objectiveId of IDS) {
        const action = objectiveActionIdentityForObjective(objectiveId), actionIcon = objectiveActionIdentityIcon(action);
        if (!OBJECTIVE_ACTION_IDENTITY_IDS.includes(action) || actionIcon.maxVisibleIcons !== 1)
        ...
        for (let streak = 1; streak <= 10; streak++) {
            for (const multiplier of MULTIPLIERS) {
                ...
                push(`${objectiveId}:streak-${streak}:mult-${multiplier}`, objectiveId, rewardContract);
                if (!rewardContract)
                    gameplayContractMutation = true;
            }
        }
    }
    const durations = [objectiveDefinition('riftSeal').duration, objectiveDefinition('beaconDefense').duration, objectiveDefinition('cursedAltar').duration];
    if (durations[0] !== 34 || durations[1] !== 28 || durations[2] !== 22)
        gameplayContractMutation = true;
    const runtime = new ObjectiveRuntime();
    runtime.begin('riftSeal', { x: 0, y: 0 });
    runtime.stats.currentStreak = 3;
    runtime.failActive();
    if (runtime.stats.currentStreak !== 0)
        gameplayContractMutation = true;
    const issues = [];
    if (samples.length !== 60)
        issues.push(`samples:${samples.length}`);
    if (!actionAtlas.passed)
        issues.push('action-atlas');
    if (!rewardAtlas.passed)
        issues.push('reward-atlas');
