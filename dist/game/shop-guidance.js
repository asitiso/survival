...
import { equipmentDefinition } from './shop-data.js';
...
import { equipmentReadiness } from './equipment-survival-readiness.js';
...
function reasonFor(offer, context, currentSame) {
    if (currentSame)
        return '강화 재료 구매 · 보관함 저장';
    if (offer.kind === 'accessory')
        return `${equipmentSetForItem(offer.id)?.name ?? '장신구'} · ${offer.description}`;
...
function baseShopGuidanceForOffers(offers, context) {
    if (context.elapsedSeconds !== undefined && context.heroMaxHp !== undefined)
        return survivalGuidance(offers, context);
    const scored = offers.map((offer, index) => {
        ...
        if (offer.kind === 'accessory' && !current)
            score += 45;
        if (matching && matching.count > 0 && current?.id !== offer.id)
            score += 25;
...
function protectedReplacement(offer, state) {
    ...
    const activeSet = current ? equipmentSetStates(state).find(set => set.items.includes(current.id) && set.twoActive) : null;
    if (current && current.id !== offer.id && activeSet)
        return true;
...
export function safeQuickPurchase(offer, offers, state) {
    if (equipmentRecipes().some(recipe => recipe.result.id === offer.id))
        return false;
    ...
    if (current && offer.kind !== 'potion' && !canStoreInventoryItem(state, { id: offer.id, rank: 1 }))
        return false;
    return true;
}
export function quickShopRecommendation(offers, guidance, state, topRecommendations) {
    if (topRecommendations?.some(entry => entry.action && entry.action !== 'purchase'))
        return null;
    if (guidance.some(entry => entry.best && entry.action && entry.action !== 'purchase'))
        return null;
...
function recommendationGain(context, candidate) {
    if (context.elapsedSeconds === undefined || context.heroMaxHp === undefined)
        return 0;
    ...
    return Math.max((after.heroSurvivalHits - before.heroSurvivalHits) / Math.max(1, before.recommended.heroSurvivalHits), (after.firepowerIndex - before.firepowerIndex) / Math.max(1, before.recommended.firepowerIndex), (before.coreDamageMultiplier - after.coreDamageMultiplier) / Math.max(1, before.recommended.coreDamageMultiplier), 0);
}
function materiallyImprovesReadiness(gain) {
...
export function shopTopRecommendations(offers, context, guidance = baseShopGuidanceForOffers(offers, context)) {
    const candidates = guidance.filter(entry => entry.best).map(entry => ({ ...entry }));
    const add = (entry) => {
        if (!Number.isFinite(entry.score) || candidates.some(candidate => candidate.offerId === entry.offerId && candidate.action === entry.action))
            return;
    ...
    for (const kind of ['weapon', 'armor', 'accessory']) {
        ...
        if (!equipped || context.elapsedSeconds === undefined)
            continue;
...
export function shopGuidanceForOffers(offers, context) {
    ...
    if (!recommendation || !recommendation.action || recommendation.action === 'purchase' || entries.length === 0)
        return entries;
    const matchingOffer = entries.findIndex(entry => entry.offerId === recommendation.offerId);
    if (matchingOffer >= 0 || offers.some(offer => offer.kind !== 'potion'))
        return entries;
...
function survivalGuidance(offers, context) {
    const { state } = context;
    const read = (candidate) => equipmentReadiness({ state: candidate, elapsedSeconds: context.elapsedSeconds, heroMaxHp: context.heroMaxHp });
    const before = read(state), metric = before.weakestMetric;
    const gain = (after) => metric === 'hero'
    ...
    const reason = (after, action) => metric === 'hero'
    ...
    const entries = offers.map(offer => {
        if (offer.kind === 'potion')
            return { offerId: offer.id, label: '', reason: '비상 회복 보충', score: 0, best: false, action: 'purchase' };
        const current = state[offer.kind];
        ...
        if (blocked && improvement > 0)
            entry = { ...entry, label: '보관함 정리 필요', reason: `장비 판매 또는 조합 후 ${offer.name} 구매 가능`, action: 'cleanup', score: 1000 + improvement };
        if (offer.price > state.coins)
            entry.score = -100;
        if (current?.id === offer.id) {
            ...
            if (strengthened.ok && materiallyImprovesReadiness(gain(read(strengthened.state)))) {
                ...
                if (materiallyImprovesReadiness(gain(upgraded)))
                    entry = { ...entry, reason: `강화 재료 구매 · ${reason(upgraded, '대장간 강화')}`, score: offer.price <= state.coins ? 50 + gain(upgraded) : -100 };
            }
        }
        return entry;
    });
    const addInventoryAction = (offerId, candidate, action, name) => {
        const after = read(candidate);
        if (!materiallyImprovesReadiness(gain(after)))
            return;
    ...
    for (const stack of state.inventory ?? []) {
        ...
        if (equipped.ok)
            addInventoryAction(stack.id, equipped.state, 'equip', `${stack.name} 장착`);
    }
    for (const recipe of equipmentRecipes()) {
        ...
        if (equipped.ok)
            addInventoryAction(recipe.ingredientIds[0], equipped.state, 'forge', `${recipe.hidden && !state.discoveredRecipes.includes(recipe.id) ? '???' : recipe.result.name} 조합·장착`);
    }
    ...
    if (winner >= 0 && entries[winner].score >= 0 && (entries[winner].action !== 'purchase' || offers[winner].price <= state.coins)) {
        ...
        if (!entries[winner].label)
            entries[winner].label = '추천';
    }
    return entries;
}
