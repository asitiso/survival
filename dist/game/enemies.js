    ...
    renderEnemies(ctx, spriteAtlasImage = null, spriteAtlasReady = false, bossSpriteAtlasImage = null, bossSpriteAtlasReady = false, residualMotion, eliteAffixAtlasImage = null, eliteAffixAtlasReady = false, specialistIntentAtlasImage = null, specialistIntentAtlasReady = false, heroPos = null, specialistCombatVfxAtlasImage = null, specialistCombatVfxAtlasReady = false, bossPhaseOverlayVfxAtlasImage = null, bossPhaseOverlayVfxAtlasReady = false, interactionAtlasImage = null, interactionAtlasReady = false, spawnPressureVfxAtlasImage = null, spawnPressureVfxAtlasReady = false, regularEnemyActionVfxAtlasImage = null, regularEnemyActionVfxAtlasReady = false, eliteAffixLifecycleVfxAtlasImage = null, eliteAffixLifecycleVfxAtlasReady = false, enemyTargetPressureVfxAtlasImage = null, enemyTargetPressureVfxAtlasReady = false, corePos = null, specialistReactionLifecycleVfxAtlasImage = null, specialistReactionLifecycleVfxAtlasReady = false, reducedFlash = false, reducedMotion = false) {
        ...
        for (const enemy of this.enemies) {
            ...
            if (enemy.type === 'boss' && (enemy.specialTimer ?? 99) <= 1.2) {
                ...
                ctx.beginPath();
                ctx.arc(0, 0, enemy.radius + 22 + charge * 22 + (bossSpecialBodyLanguage?.auraRadiusBoost ?? 0), 0, Math.PI * 2);
                ctx.stroke();
                if (charge > 0.55) {
                    ctx.globalAlpha = 0.35 + charge * 0.35;
                    ctx.beginPath();
                    ctx.arc(0, 0, enemy.radius + 42 + charge * 34, 0, Math.PI * 2);
                    ctx.stroke();
                }
                ctx.globalAlpha = 1;
            }
            ...
            if (bossPresentation?.visible && bossSpriteAtlasImage && bossArchetype) {
                ...
                if (phase >= 2 && bossPhaseOverlayVfxAtlasReady && bossPhaseOverlayVfxAtlasImage) {
                    ...
                    ctx.save();
                    ctx.globalAlpha = phase === 3 ? 0.76 : 0.56;
