...
import { magicCriticalDamage } from './magic-critical.js';
...
import { heroLaunchOriginHandoffPresentation, type HeroLaunchOriginMemory } from './hero-launch-origin-handoff-rendering.js';
...
import { impactCorridorSeparationPresentation, spatialThreatSeparationBudgetPresentation } from './threat-impact-spatial-separation-rendering.js';
import { impactCorridorReleasePresentation, spatialRecoveryBudgetPresentation } from './threat-impact-spatial-recovery-rendering.js';
import { denseBattlefieldArbitrationPresentation, denseImpactArbitrationPresentation } from './threat-impact-dense-arbitration-rendering.js';
import { depthPlaneBudgetPresentation, impactDepthPlanePresentation } from './threat-impact-depth-plane-rendering.js';
import { depthReentryBudgetPresentation, impactDepthReentryPresentation } from './threat-impact-depth-plane-reentry-rendering.js';
...
import { impactThreatRhythmPresentation, threatRhythmBudgetPresentation } from './threat-impact-rhythm-stagger-rendering.js';
import { impactThreatRhythmRecoveryPresentation, threatRhythmRecoveryBudgetPresentation } from './threat-impact-rhythm-recovery-rendering.js';
import { denseImpactThreatRhythmPresentation, denseThreatRhythmBudgetPresentation } from './threat-impact-dense-rhythm-rendering.js';
...
interface PlayerProjectile {
  ...
  visualLaunchWorldOrigin?: Vec2;
  visualLaunchTravelTtl?: number;
  visualLaunchTravelMaxTtl?: number;
  visualImpactHandoffTtl?: number;
  visualImpactHandoffMaxTtl?: number;
  visualImpactLineageId?: string;
}
...
interface ProjectileImpactVisual {
  ...
  impactLineageKey?: string;
  impactDirection?: Vec2;
  impactResponseOwner?: 'canonical'|'guard'|'weakpoint';
  impactResponseStrength?: number;
  enemyReactionOwner?: 'hit'|'death'|'none';
}
...
export interface SpellWorld {
  ...
  visualActionFacing?: Vec2 | undefined;
  visualActionPoseStrength?: number | undefined;
  visualActionOwner?: 'movement'|'cast'|'ultimate'|'recovery' | undefined;
  reducedMotion?: boolean | undefined;
  reducedFlash?: boolean | undefined;
}

function projectileImpactResponseAt(world:SpellWorld,pos:Vec2,enemy?:Enemy):{owner:'canonical'|'guard'|'weakpoint';strength:number}{
  const weakpoint=world.weakpointAim?.nodes.filter((node)=>node.alive).reduce<BossEncounterNode|null>((best,node)=>{const d=distance(pos,node.pos);if(d>node.radius+82)return best;if(!best)return node;return d<distance(pos,best.pos)?node:best;},null);
  if(weakpoint){const reach=Math.max(1,weakpoint.radius+82),d=distance(pos,weakpoint.pos);return{owner:'weakpoint',strength:Math.max(.55,Math.min(1,1-d/reach*.45))};}
  const guard=Math.max(0,enemy?.guardHp??0);if(guard>0)return{owner:'guard',strength:Math.max(.4,Math.min(1,guard/Math.max(1,(enemy?.maxHp??1)*.24)))};
...
export class SpellSystem {
  ...
  private heroVisualLaunchMemory: HeroLaunchOriginMemory | null = null;
  private nextVisualImpactLineageId = 1;
  ...
  update(dt: number, world: SpellWorld): void {
    ...
    if(this.heroVisualLaunchMemory)this.heroVisualLaunchMemory.ttl=Math.max(0,this.heroVisualLaunchMemory.ttl-dt);
  ...
  render(ctx: CanvasRenderingContext2D, motion?: ResidualCombatMotionPolicy, propVfxAtlasImage?: HTMLImageElement | null, propVfxAtlasReady = false, heroProjectileAtlasImage?: HTMLImageElement | null, heroProjectileAtlasReady = false, heroSpellSignatureAtlasImage?: HTMLImageElement | null, heroSpellSignatureAtlasReady = false, ultimateSignatureAtlasImage?: HTMLImageElement | null, ultimateSignatureAtlasReady = false, persistentSpellZoneVfxAtlasImage?: HTMLImageElement | null, persistentSpellZoneVfxAtlasReady = false, crowdControlPropagationVfxAtlasImage?: HTMLImageElement | null, crowdControlPropagationVfxAtlasReady = false, reducedFlash = false, ultimatePostImpactResidueVfxAtlasImage?: HTMLImageElement | null, ultimatePostImpactResidueVfxAtlasReady = false, reducedMotion = false, presentationQuality: PresentationQuality = 'high', primaryProjectileLabelBlockers:readonly Vec2[]=[], heroPos:Vec2|null=null, bossTelegraphZones:readonly {pos:Vec2;radius:number}[]=[], includeGroundLayer = true): void {
    ...
    for (const projectile of this.projectiles) {
      ...
      if (heroProjectileAtlasReady && heroProjectileAtlasImage) {
        ...
        ctx.save(); ctx.translate(visualPos.x, visualPos.y); ctx.rotate(angle); ctx.globalAlpha = 0.96*postImpactSpriteScale;
    ...
    if (heroProjectileAtlasReady && heroProjectileAtlasImage) {
      ...
      for (const impact of this.projectileImpactVisuals) {
        ...
        const impactCorridorSeparation=impactCorridorSeparationPresentation({heroProximity:heroImpactProximity,safeLaneProximity:0,neighborCount:impactNeighborCount,critical:impactCritical},reducedFlash),impactSpatialSeparationBudget=spatialThreatSeparationBudgetPresentation({criticalCount:impactCritical?1:0,safeLaneVisible:false,projectileCount:this.projectiles.length,impactCount:this.projectileImpactVisuals.length,hazardCount:this.fields.length+this.holes.length,silhouetteCount:0},reducedMotion,reducedFlash);
        const impactCorridorRelease=impactCorridorReleasePresentation({life:impactLife,heroProximity:heroImpactProximity,neighborCount:impactNeighborCount,release:Math.max(0,1-Math.min(1,impactNeighborCount/8)),critical:impactCritical},reducedFlash),impactSpatialRecoveryBudget=spatialRecoveryBudgetPresentation({recoveringCount:this.projectileImpactVisuals.length,pressure:impactCorridorSeparation.pressure,criticalCount:impactCritical?1:0},reducedMotion,reducedFlash);
        const impactDenseArbitration=denseImpactArbitrationPresentation({impactCount:this.projectileImpactVisuals.length,indexFromNewest:impactResolutionRank.get(impact)??this.projectileImpactVisuals.length,heroProximity:heroImpactProximity,critical:impactCritical},reducedFlash),impactDenseBattlefield=denseBattlefieldArbitrationPresentation({criticalCount:impactCritical?1:0,hazardCount:this.fields.length+this.holes.length,projectileCount:this.projectiles.length,impactCount:this.projectileImpactVisuals.length,silhouetteCount:0,safeLaneVisible:false},reducedMotion,reducedFlash);
        const impactDepthPlane=impactDepthPlanePresentation({heroProximity:heroImpactProximity,crowd:Math.min(1,impactNeighborCount/8),critical:impactCritical},reducedFlash),impactDepthPlaneBudget=depthPlaneBudgetPresentation({criticalCount:impactCritical?1:0,projectileCount:this.projectiles.length,impactCount:this.projectileImpactVisuals.length,hazardCount:this.fields.length+this.holes.length,silhouetteCount:0,safeLaneVisible:false},reducedMotion,reducedFlash);
        const impactDepthReentry=impactDepthReentryPresentation({release:Math.max(0,1-Math.max(heroImpactProximity,Math.min(1,impactNeighborCount/8))),pressure:impactDepthPlane.pressure,critical:impactCritical},reducedFlash),impactDepthReentryBudget=depthReentryBudgetPresentation({reenteringCount:this.projectileImpactVisuals.length,pressure:impactDepthPlane.pressure,criticalCount:impactCritical?1:0},reducedMotion,reducedFlash);
        const bossImpactFocus=bossImpactFocusCorridorPresentation({bossProximity:bossTelegraphOverlap?1:0,crowd:Math.min(1,impactNeighborCount/8),critical:impactCritical},reducedFlash),bossImpactFocusBudget=bossFocusCorridorBudgetPresentation({bossActive:bossTelegraphOverlap,criticalCount:impactCritical?1:0,projectileCount:this.projectiles.length,impactCount:this.projectileImpactVisuals.length,hazardCount:this.fields.length+this.holes.length,silhouetteCount:0,safeLaneVisible:false},reducedMotion,reducedFlash);
        ...
        const impactDamageSourceReactionDensity=(impact.enemyReactionOwner??'none')!=='none'?projectileDamageSourceEnemyReactionDensityBudgetPresentation({activeCount:activeEnemyReactionImpacts.length,indexFromNewest:enemyReactionRank.get(impact)??activeEnemyReactionImpacts.length,reactionOwner:impact.enemyReactionOwner as 'hit'|'death',handoffOwner:impactDamageSourceReactionHandoff.owner},reducedMotion):null;
        if(impactDirection?.visible&&impactDirectionOwner.owner!=='retired'&&impactDirectionBudget.visible&&impactResponseDensityBudget.visible){const cueLength=impactDirection.cueLength*impactDirectionBudget.lengthScale*impactResponsePriority.directionLengthScale*impactResponseDensityBudget.lengthScale;ctx.save();ctx.globalAlpha=impactDirection.alphaScale*impactDirectionOwner.alphaScale*impactDirectionBudget.alphaScale*impactResponsePriority.directionAlphaScale*impactResponseRelease.directionAlphaScale*impactResponseDensityBudget.alphaScale*arrivalSettleRecovery.directionAlphaScale*impactRetirement.responseAlphaScale*impactResolutionBudget.effectStrength*impactPriority.impactAlphaScale*impactThreatBudget.impactDecorationScale*impactSpatial.fillAlphaScale*impactTemporal.decorationAlphaScale*impactTemporalBudget.secondaryAlphaScale*impactDepthBudget.secondaryAlphaScale*impactRecoveryBudget.secondaryRecoveryScale*impactUnifiedStack.secondaryAlphaScale*impactCorridorSeparation.secondaryAlphaScale*impactSpatialSeparationBudget.secondaryAlphaScale*impactCorridorRelease.secondaryAlphaScale*impactSpatialRecoveryBudget.secondaryRecoveryScale*impactDenseArbitration.edgeAlphaScale*impactDenseBattlefield.secondaryAlphaScale*(impact.alphaScale??1)*(budget?.alphaScale??1)*(reducedFlash?.62:1)*impactDirectionReacquisition.directionAlphaScale*impactDirectionReacquisitionBudget.primaryDirectionScale;ctx.strokeStyle=impact.secondaryKind==='splash'?'#ffe3b5':'#bcecff';ctx.lineWidth=1.7;ctx.beginPath();ctx.moveTo(impactVisualPos.x-impactDirection.facingX*cueLength,impactVisualPos.y-impactDirection.facingY*cueLength);ctx.lineTo(impactVisualPos.x,impactVisualPos.y);ctx.stroke();ctx.restore();}
        if(impactDamageSourceAftermath.owner!=='retired'&&impactDamageSourceAftermath.aftermathAlphaScale>0&&impactDamageSourceAftermathDensityBudget.effectVisible){ctx.save();ctx.globalAlpha=impactDamageSourceAftermath.aftermathAlphaScale*impactDamageSourceAftermathHandoff.aftermathAlphaScale*impactDamageSourceAftermathDensityBudget.aftermathAlphaScale*impactDamageSourceReaction.aftermathAlphaScale*impactDamageSourceReactionHandoff.reactionAlphaScale*impactRetirement.aftermathAlphaScale*impactResolutionBudget.effectStrength*(impactDamageSourceReactionDensity?.effectStrength??1)*(impact.alphaScale??1)*(budget?.alphaScale??1);ctx.strokeStyle=impactDamageSourceAftermath.sourceClass==='explosion'?'#ffcf8a':'#9edcff';ctx.lineWidth=1.5;if(impactDamageSourceAftermath.sourceClass==='explosion'){ctx.beginPath();ctx.arc(impactVisualPos.x,impactVisualPos.y,Math.max(7,size*.28)*impactDamageSourceAftermath.ringRadiusScale*impactDamageSourceReaction.reactionCarryScale,0,Math.PI*2);ctx.stroke();}else if(impactDirection){const len=Math.max(8,size*.28)*impactDamageSourceAftermath.lineLengthScale*impactDamageSourceReaction.reactionCarryScale;ctx.beginPath();ctx.moveTo(impactVisualPos.x-impactDirection.facingX*len,impactVisualPos.y-impactDirection.facingY*len);ctx.lineTo(impactVisualPos.x,impactVisualPos.y);ctx.stroke();}ctx.restore();}
  ...
  private castChainLightning(world: SpellWorld, tuning: SpellTuning, fusion: FusionSpellModifiers): void {
    ...
    for (let i = 0; i < chainJumpBudget(tuning.jumps + identity.chainJumpBonus + evolution.jumpBonus + fusion.jumpBonus, world.hero.temporaryChainJumpBonus) && current; i++) {
      ...
      const chainKilled=world.enemies.damage(current, magicCriticalDamage(tuning.damage * world.hero.spellPower * world.hero.equipmentSpellPower * identity.damageMultiplier * evolution.damageMultiplier * fusion.damageMultiplier * Math.pow(0.86, i), world.hero), hitSource);
  ...
  private castFrostNova(world: SpellWorld, tuning: SpellTuning, fusion: FusionSpellModifiers): void {
    ...
    for (const enemy of world.enemies.enemies) {
      ...
      world.enemies.damage(enemy, magicCriticalDamage(tuning.damage * world.hero.spellPower * world.hero.equipmentSpellPower * identity.novaDamageMultiplier * evolution.damageMultiplier * fusion.damageMultiplier * (seria ? 1.06 : 1), world.hero), world.hero.pos, 'freeze');
  ...
  private updateProjectiles(dt: number, world: SpellWorld): void {
    for (const p of this.projectiles) {
      ...
      for (const enemy of world.enemies.enemies) {
        ...
        const killedByImpact=world.enemies.damage(enemy, magicCriticalDamage(p.damage, world.hero), p.pos);
        ...
        if (p.splashRadius > 0 && p.splashDamage > 0) {
          for (const nearby of world.enemies.enemies) {
            ...
            if (distance(enemy.pos, nearby.pos) <= p.splashRadius + nearby.radius) { const splashResponse=projectileImpactResponseAt(world,nearby.pos,nearby); const splashKilled=world.enemies.damage(nearby, magicCriticalDamage(p.splashDamage, world.hero), enemy.pos, 'explosion'); const secondary=secondaryImpactCanonicalPresentation('splash',nearby.pos,world.reducedFlash??false),secondaryLineage=projectileImpactLineageTransferPresentation({sourceLineageKey:p.visualImpactLineageId??lineage.lineageKey,impactIndex:priorImpactCount,secondaryKind:'splash',continues},world.reducedMotion??false); this.projectileImpactVisuals.push({pos:secondary.pos,entryOffset:secondary.entryOffset,alphaScale:secondary.alphaScale,secondaryKind:'splash',impactLineageKey:secondaryLineage.lineageKey,impactDirection:{x:nearby.pos.x-enemy.pos.x,y:nearby.pos.y-enemy.pos.y},impactResponseOwner:splashResponse.owner,impactResponseStrength:splashResponse.strength,enemyReactionOwner:splashKilled?'death':'hit',heroId:p.heroId,ttl:.14,maxTtl:.14,size:Math.max(38,p.radius*4.2)*secondary.sizeScale}); if(this.projectileImpactVisuals.length>32)this.projectileImpactVisuals.splice(0,this.projectileImpactVisuals.length-32); }
          }
        }
    ...
    for (const field of this.fields) {
      ...
      for (const enemy of world.enemies.enemies) {
        if (enemy.alive && distance(field.pos, enemy.pos) <= field.radius + enemy.radius) {
          world.enemies.damage(enemy, magicCriticalDamage(field.damage, world.hero), field.pos);
        ...
        if (meteor.delay <= 0) {
          ...
          for (const enemy of world.enemies.enemies) {
            if (enemy.alive && distance(meteor.pos, enemy.pos) <= meteor.radius + enemy.radius) {
              world.enemies.damage(enemy, magicCriticalDamage(meteor.damage, world.hero), meteor.pos, 'ultimate');
  ...
  private updateBlackHoles(dt: number, world: SpellWorld): void {
    for (const hole of this.holes) {
      ...
      for (const enemy of world.enemies.enemies) {
        ...
        if (hole.tickTimer <= 0) {
          world.enemies.damage(enemy, magicCriticalDamage(hole.damage, world.hero), hole.pos, 'ultimate');
