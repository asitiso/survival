...
export function runResourceFlowSpawnPressureVfxAudit() {
    const samples = [];
    for (const kind of PICKUP_FLOW_VFX_KINDS)
        for (const state of PICKUP_FLOW_VFX_STATES) {
            const r = pickupFlowVfxSprite(kind, state);
            add(samples, `pickup-${kind}-${state}`, true, r.sx >= 0 && r.sy >= 0 && r.sx + r.sw <= PICKUP_FLOW_VFX_ATLAS.width && r.sy + r.sh <= PICKUP_FLOW_VFX_ATLAS.height);
        }
    for (const kind of SPAWN_PRESSURE_VFX_KINDS)
        for (const state of SPAWN_PRESSURE_VFX_STATES) {
            const r = spawnPressureVfxSprite(kind, state);
            add(samples, `spawn-${kind}-${state}`, true, r.sx >= 0 && r.sy >= 0 && r.sx + r.sw <= SPAWN_PRESSURE_VFX_ATLAS.width && r.sy + r.sh <= SPAWN_PRESSURE_VFX_ATLAS.height);
        }
    ...
    return { samples, actionCount: ACTION_BUTTONS.length, presentationOnly: true, loadFailureBlocksGameplay: false, gameplayFormulaMutation: false, snapshotSchemaMutation: false, newAtlasCount: 2, passed: samples.length === 64 && samples.every(x => x.passed) && ACTION_BUTTONS.length === 9 && pickup.passed && spawn.passed };
}
