export type HeroUltimateBodyKind='meteorStorm'|'blackHole';

export interface HeroUltimateBodyState{
  kind:HeroUltimateBodyKind|null;
  elapsed:number;
}

export interface HeroUltimateBodyPresentation{
  windup:number;
  release:number;
  recovery:number;
  stance:'neutral'|'meteor-windup'|'meteor-release'|'meteor-recovery'|'void-windup'|'void-release'|'void-recovery';
  castRecoverySuppression:number;
  offsetX:number;
  offsetY:number;
  rotation:number;
  scaleX:number;
...
export function advanceHeroUltimateBodyState(
  previous:HeroUltimateBodyState|undefined,
  trigger:HeroUltimateBodyKind|null,
  dt:number,
  _reducedMotion=false,
):HeroUltimateBodyState{
  if(trigger)return{kind:trigger,elapsed:0};
  const prev=previous??{kind:null,elapsed:0};
  if(!prev.kind)return prev;
  const elapsed=prev.elapsed+clamp(Number.isFinite(dt)?dt:0,0,.12);
  if(elapsed>=.62)return{kind:null,elapsed:0};
  return{kind:prev.kind,elapsed};
}

export function heroUltimateBodyPresentation(
  state:HeroUltimateBodyState|undefined,
  facingX:number,
  facingY:number,
  reducedMotion=false,
):HeroUltimateBodyPresentation{
  const current=state??{kind:null,elapsed:0};
  if(!current.kind)return{windup:0,release:0,recovery:0,stance:'neutral',castRecoverySuppression:0,offsetX:0,offsetY:0,rotation:0,scaleX:1,scaleY:1};
  ...
  let stance:HeroUltimateBodyPresentation['stance'];
