...
export interface TwoFourHourFlowHealthAudit{samples:number;childAuditCount:4;shopQuietCoverage:number;rewardCompactCoverage:number;hudMinimalCoverage:number;criticalPriorityCoverage:number;maxCombatStatInflation:0;estimatedDecisionPauseReduction:number;actionCount:9;snapshotMutation:false;economyMutation:false;criticalInfoPreserved:true;autoRewardSelection:false;passed:boolean;}
...
export function auditTwoFourHourFlowHealth():TwoFourHourFlowHealthAudit{
  let samples=0,shop=0,reward=0,hud=0,critical=0;const checkpoints=[7201,9000,10800,12600,14400];
  for(const elapsedSeconds of checkpoints)for(const bossActive of [false,true])for(const criticalState of [false,true]){
    samples+=4;
    if(ultraLongShopFocus(elapsedSeconds,equipment).deemphasizeShop)shop++;
    const compact=compactUltraLongBossRewards(rewards,{elapsedSeconds,activeRelic:'abyss-eye',activeFusionCount:2});if(compact.compact&&compact.choices.filter(x=>x.best).length===1&&!compact.autoSelect)reward++;
    const focus=fourHourHudFocus({elapsedSeconds,equipment,activeRelic:'abyss-eye',activeFusionCount:2,bossActive,mythicActive:false});if(focus.maxBuildLabels<=1&&focus.keepCriticalBars&&focus.keepDangerTelegraphs)hud++;
    const cue=ultraLongCriticalFocus({elapsedSeconds,bossActive,mythicActive:false,heroCritical:criticalState,coreCritical:false});if(cue.preserveBossCue&&cue.preserveDangerCue&&cue.keepAutoRing)critical++;
  }
