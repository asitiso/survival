...
export interface MobileInputRegressionAudit{profiles:number;actionCount:number;reachableActionCount:number;hingeClear:boolean;dragSamples:number;reachBurdenReduction:number;maxSoftReach:number;actionLayoutMutation:boolean;issues:string[];passed:boolean;}
export function mobileInputRegressionAudit():MobileInputRegressionAudit{
  ...
  if(actionCount!==9)issues.push('action-count');
  if(reachableActionCount!==9)issues.push('action-reachability');
  if(!hingeClear)issues.push('hinge-crossing');
