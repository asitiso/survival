...
export type EconomyBand='conservative'|'neutral'|'gold';
export interface ProgressionPurchasingPowerSample { threat:ThreatLevel;minute:30|60|120;economyBand:EconomyBand;estimatedGold:number;estimatedLevel:number;xpToNextLevel:number;shopTokens:number;affordableCorePurchases:number;goldPerAvailableShop:number; }
export interface ProgressionPurchasingPowerAudit { samples:ProgressionPurchasingPowerSample[];minAffordableCorePurchases:number;minGoldPerAvailableShop:number;maxEconomyBandSpread:number;goldMonotonic:boolean;levelMonotonic:boolean;shopPowerMonotonic:boolean;threatLevelParity:boolean;passed:boolean; }
const MINUTES=[30,60,120] as const;
const THREATS=[0,3,5] as const satisfies readonly ThreatLevel[];
const BANDS:readonly EconomyBand[]=['conservative','neutral','gold'];
const BAND_MULTIPLIER:Record<EconomyBand,number>={conservative:.85,neutral:1,gold:1.12};
const CORE_PURCHASE_PRICE=220;
function round(value:number):number{return Math.round(value*100)/100;}
function scheduledShopTokens(seconds:number):number{return seconds<SHOP_FIRST_TOKEN_AT?0:1+Math.floor((seconds-SHOP_FIRST_TOKEN_AT)/SHOP_TOKEN_INTERVAL);}
function cumulativeGold(minute:number,threat:ThreatLevel):number{
  let gold=0;
...
export function progressionPurchasingPowerSamples():ProgressionPurchasingPowerSample[]{
  const samples:ProgressionPurchasingPowerSample[]=[];
...
export function auditProgressionPurchasingPower():ProgressionPurchasingPowerAudit{
  ...
  for(const threat of THREATS)for(const minute of MINUTES){
    ...
    const gold=group.map((sample)=>sample.estimatedGold);maxEconomyBandSpread=Math.max(maxEconomyBandSpread,Math.max(...gold)/Math.max(1,Math.min(...gold)));
  }
  for(const threat of THREATS)for(const band of BANDS){
    const group=samples.filter((sample)=>sample.threat===threat&&sample.economyBand===band).sort((a,b)=>a.minute-b.minute);
    for(let i=1;i<group.length;i+=1){
      if(group[i]!.estimatedGold<=group[i-1]!.estimatedGold)goldMonotonic=false;
      if(group[i]!.estimatedLevel<=group[i-1]!.estimatedLevel)levelMonotonic=false;
      if(group[i]!.affordableCorePurchases<=group[i-1]!.affordableCorePurchases)shopPowerMonotonic=false;
    }
  }
  for(const minute of MINUTES){
    ...
    if(new Set(levels).size!==1)threatLevelParity=false;
  }
