import type { FreezeControlVfxClass } from './freeze-control-vfx-assets.js';
import type { SlowSource } from './enemies.js';

export type FreezeStatusBand='strong'|'medium'|'fading';
export interface FreezeStatusPresentationInput {
  slowFactor:number;
  slowTimer:number;
  enemyClass:FreezeControlVfxClass;
  reducedFlash?:boolean;
}
export interface FreezeStatusPresentation {
  visible:boolean;
  band:FreezeStatusBand;
  alpha:number;
  sizeScale:number;
  baseSize:number;
...
const BASE_SIZE:Readonly<Record<FreezeControlVfxClass,number>>={regular:78,specialist:92,elite:112,boss:142};
const clamp01=(value:number)=>Math.max(0,Math.min(1,Number.isFinite(value)?value:0));

export function freezeStatusPresentation(input:FreezeStatusPresentationInput):FreezeStatusPresentation {
  ...
  const band:FreezeStatusBand=fading?'fading':severity>=.5?'strong':'medium';
...
export type SlowSourceRenderMode='freezeAtlas'|'gravityOrbit'|'terrainGround'|'impactResistance'|'genericResistance';
export interface SlowSourceCuePresentationInput extends FreezeStatusPresentationInput { source:SlowSource; }
export interface SlowSourceCuePresentation {
  source:SlowSource;
  renderMode:SlowSourceRenderMode;
  useFreezeAtlas:boolean;
  visible:boolean;
  alpha:number;
  sizeScale:number;
  baseSize:number;
...
const SOURCE_STYLE:Readonly<Record<SlowSource,{renderMode:SlowSourceRenderMode;alphaScale:number;sizeScale:number;lineWidth:number;segmentCount:number;groundOffsetRatio:number;color:string}>>={
...
export function slowSourceCuePresentation(input:SlowSourceCuePresentationInput):SlowSourceCuePresentation {
...
export interface FreezeCrowdEntryInput {
  id:number;
  enemyClass:FreezeControlVfxClass;
  distanceToHero:number;
  currentTarget?:boolean;
  resultCueNearby?:boolean;
}
export interface FreezeCrowdBudgetContext {
  battlefieldStress?:number;
  protectedWarning?:boolean;
  safeLaneVisible?:boolean;
}
export interface FreezeCrowdBudgetEntry {
  id:number;
...
export interface FreezeCrowdBudgetResult {
  fullCueCapacity:number;
  entries:FreezeCrowdBudgetEntry[];
}

const CLASS_PRIORITY:Readonly<Record<FreezeControlVfxClass,number>>={regular:0,specialist:3,elite:4,boss:5};

export function freezeCrowdBudget(entries:readonly FreezeCrowdEntryInput[],context:FreezeCrowdBudgetContext={}):FreezeCrowdBudgetResult {
  ...
  const ranked=[...entries].sort((a,b)=>{
    ...
    if(a.distanceToHero!==b.distanceToHero)return a.distanceToHero-b.distanceToHero;
    return a.id-b.id;
  });
...
export interface FreezeStatusLifecycleState {
  visible:boolean;
  alpha:number;
  sizeScale:number;
  pulseScale:number;
...
export interface FreezeStatusLifecycleInput {
  active:boolean;
  targetAlpha:number;
...
export function createFreezeStatusLifecycleState():FreezeStatusLifecycleState {
...
export function advanceFreezeStatusLifecycle(previous:FreezeStatusLifecycleState,input:FreezeStatusLifecycleInput,dt:number,reducedMotion=false):FreezeStatusLifecycleState {
...
export interface FreezeStatusEdgeInput { x:number;y:number;size:number;viewportWidth:number;viewportHeight:number;margin?:number; }
export interface FreezeStatusEdgePresentation { sizeScale:number;offsetX:0;offsetY:0; }
export function freezeStatusEdgePresentation(input:FreezeStatusEdgeInput):FreezeStatusEdgePresentation {
...
export function slowDeathUsesFreezeShatter(source:SlowSource|undefined):boolean { return source===undefined||source==='frost'; }
