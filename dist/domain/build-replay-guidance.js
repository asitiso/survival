...
import { replayProgressPercent } from './build-replay.js';
...
const FATE_LABEL = { frenzy: '광란', golden: '황금', guardian: '수호' };
const FORM_LABEL = {
...
const ARCHETYPE_LABEL = { burst: '폭발', cycle: '순환', domain: '영역', fortress: '수호' };
function short(value, labels, fallback) {
    return value === null ? fallback : labels[value] ?? value.slice(0, 16);
}
function addListCandidates(out, target, current, totalScore, category, labels, prefix, order) {
    if (target.length === 0)
        return;
...
function spellCandidates(target, current) {
    const out = [];
...
export function replayGuidance(plan, currentRaw) {
    const current = sanitizeBuildCapsulePayload(currentRaw);
    const target = plan.target;
    const progress = replayProgressPercent(plan, current);
    if (progress >= 100)
        return { progress: 100, category: 'complete', label: '빌드 재현 완료' };
    const candidates = [];
    if (target.relic !== current.relic)
        candidates.push({ category: 'relic', score: 10, order: 0, label: `유물 · ${short(target.relic, RELIC_LABEL, '미장착')}` });
    ...
    if (target.finalForm !== current.finalForm)
        candidates.push({ category: 'final-form', score: 5, order: 60, label: `최종형 · ${short(target.finalForm, FORM_LABEL, '미해금')}` });
    if (target.archetype !== current.archetype)
        candidates.push({ category: 'archetype', score: 5, order: 70, label: `빌드 · ${ARCHETYPE_LABEL[target.archetype] ?? target.archetype}` });
...
export function replayGuidanceFinalFormIconStyle(plan) { return plan.target.finalForm ? finalFormIdentityIconStyle(plan.target.finalForm) : null; }
export function replayGuidanceMapIconStyle(plan) { return battlefieldEnvironmentIconStyle(plan.target.mapId, 0); }
