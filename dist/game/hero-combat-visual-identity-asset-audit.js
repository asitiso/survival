...
function heroMeterContractOk() {
    const durations = { arkan: 7, seria: 6.5, kain: 5.5, edric: 6 };
    for (const heroId of HERO_METER_IDENTITY_IDS) {
        const tr = updateHeroMeter({ ...createHeroMeterState(heroId), charge: .999 }, 0, { casts: 1, chilledHits: 1, moving: true, preventedDamageRatio: 1 });
        if (!tr.activated || !close(tr.state.activeTimer, durations[heroId]) || tr.state.charge !== 0)
            return false;
    }
...
function comboContractOk() {
    ...
    for (const [heroId, spell, legendary, relic, synergy, family] of samples) {
        const c = analyzeArcaneCombo({ heroId, evolvedSpells: [spell], legendaryIds: [legendary], relicId: relic, traitId: null, synergyIds: [synergy], meterActive: true, coreHpRatio: heroId === 'edric' ? .5 : 1, objectiveStreak: 2 });
        if (c.family !== family || c.tier !== 3 || c.label !== 'ASCENDANCY' || !close(c.powerMultiplier, 1.12) || !close(c.cooldownMultiplier, .94) || !close(c.areaMultiplier, 1.12))
            return false;
    }
    return arcaneComboTierBadge(1) === 'I' && arcaneComboTierBadge(2) === 'II' && arcaneComboTierBadge(3) === 'III';
}
function tacticalContractOk() {
    ...
    const directive = threatDirectiveAt(480);
    const dm = threatDirectiveModifiers(directive);
...
function fusionContractOk() { return FUSION_IDS.length === 6 && FUSION_IDS.every(id => fusionDefinition(id).id === id && buildIdentityIcon(id).id === id); }
export function auditHeroCombatVisualIdentityAssets() {
    const meterAtlas = auditHeroMeterIdentityAtlas(), comboAtlas = auditArcaneComboIdentityAtlas();
    const samples = [];
    ...
    for (const id of HERO_METER_IDENTITY_IDS) {
        const i = heroMeterIdentityIcon(id);
        push(`${id}:body`, i.sx >= 0 && i.sy >= 0 && i.sx + i.sw <= 192 && i.sy + i.sh <= 192);
        push(`${id}:hud`, i.hudIdentitySupported && i.activeGlowSupported);
        push(`${id}:toast`, i.activationToastIdentitySupported);
        push(`${id}:safety`, !i.animated && i.motionAmplitude === 0 && i.textFallbackPreserved && !i.loadFailureBlocksGameplay);
        textFallbackPreserved &&= i.textFallbackPreserved;
        imageLoadFailureNonBlocking &&= !i.loadFailureBlocksGameplay;
        iconMotionAmplitude = Math.max(iconMotionAmplitude, i.motionAmplitude);
    }
    for (const id of ARCANE_COMBO_IDENTITY_IDS) {
        const i = arcaneComboIdentityIcon(id);
        push(`${id}:body`, i.sx >= 0 && i.sy >= 0 && i.sx + i.sw <= 192 && i.sy + i.sh <= 192);
        push(`${id}:hud`, i.hudIdentitySupported && i.tierBadgeSupported);
        push(`${id}:toast`, i.tierToastIdentitySupported);
        push(`${id}:safety`, !i.animated && i.motionAmplitude === 0 && i.textFallbackPreserved && !i.loadFailureBlocksGameplay);
        textFallbackPreserved &&= i.textFallbackPreserved;
        imageLoadFailureNonBlocking &&= !i.loadFailureBlocksGameplay;
        iconMotionAmplitude = Math.max(iconMotionAmplitude, i.motionAmplitude);
    }
    let tacticalToastCount = 0;
    for (const id of TACTICAL_STATUS_ICON_IDS) {
        const p = tacticalStatusIconPresentation(id), ok = p.visible && !p.animated && p.motionAmplitude === 0;
        push(`tactical:${id}:toast`, ok);
        if (ok)
            tacticalToastCount++;
    }
    let fusionToastCount = 0;
    for (const id of FUSION_IDS) {
        const icon = buildIdentityIcon(id), ok = icon.id === id && icon.textFallbackPreserved && !icon.animated && icon.motionAmplitude === 0;
        push(`fusion:${id}:toast`, ok);
        if (ok)
            fusionToastCount++;
    }
    const meterOk = heroMeterContractOk(), comboOk = comboContractOk(), tacticalOk = tacticalContractOk(), fusionOk = fusionContractOk();
    push('contract:hero-meter', meterOk);
    push('contract:arcane-combo', comboOk);
    push('contract:tactical', tacticalOk);
    push('contract:fusion', fusionOk);
    push('contract:text-fallback', textFallbackPreserved);
    push('contract:image-nonblocking', imageLoadFailureNonBlocking);
    ...
    if (!meterAtlas.passed)
        issues.push('hero-meter-atlas');
    if (!comboAtlas.passed)
        issues.push('arcane-combo-atlas');
    if (tacticalToastCoverage !== 1)
        issues.push('tactical-toast-coverage');
    if (fusionToastCoverage !== 1)
        issues.push('fusion-toast-coverage');
