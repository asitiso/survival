...
export class ResultsOverlay {
    ...
    open(result, handlers) {
      ...
      <section class="modal-panel results-panel">
        ...
        this.root.querySelector('.result-retry')?.addEventListener('click', handlers.onRetrySameHero, { once: true });
        this.root.querySelector('.result-lobby')?.addEventListener('click', handlers.onLobby, { once: true });
    }
    hide() { this.isOpen = false; this.root.hidden = true; this.root.replaceChildren(); }
}
