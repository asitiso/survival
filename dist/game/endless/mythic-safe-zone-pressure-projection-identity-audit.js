...
import { mythicSafeZoneState } from './mythic-safe-zone.js';
...
const PHASES = [{ phase: 'stable', ms: 1000 }, { phase: 'collapse', ms: 5200 }, { phase: 'collapsed', ms: 7000 }, { phase: 'reform', ms: 8400 }];
const RATIOS = [0, .5, 1];
const close = (a, b) => Math.abs(a - b) < .0006;
const rounded = (v) => Math.round((v + Number.EPSILON) * 1000) / 1000;
export function auditMythicSafeZonePressureProjectionIdentityAssets() {
    const samples = [], issues = [], atlas = auditMythicSafeZonePressureEffectIdentityAtlas();
    const archetypeCoverage = new Set(), phaseCoverage = new Set(), ratioCoverage = new Set(), identityCoverage = new Set();
    for (const archetype of ARCHETYPES)
        for (const entry of PHASES)
            for (const ratio of RATIOS) {
                ...
                archetypeCoverage.add(archetype);
                phaseCoverage.add(zone.phase);
                ratioCoverage.add(ratio);
                for (const effect of projection.effects)
                    identityCoverage.add(effect.effectId);
    ...
    for (const id of MYTHIC_SAFE_ZONE_PRESSURE_EFFECT_IDENTITY_IDS) {
        const icon = mythicSafeZonePressureEffectIdentityIcon(id);
        samples.push({ id: `identity:${id}:static`, passed: icon.animated === false && icon.motionAmplitude === 0 });
        samples.push({ id: `identity:${id}:safe`, passed: icon.safeZoneHelperSupported && icon.maxVisibleHelperIcons === 2 && icon.textFallbackPreserved && !icon.loadFailureBlocksGameplay });
    }
    ...
    if (!archetypeCoverageComplete)
        issues.push('archetype-coverage');
    if (!phaseCoverageComplete)
        issues.push('phase-coverage');
    if (!destroyedRatioCoverageComplete)
        issues.push('ratio-coverage');
