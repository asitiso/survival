...
import { BOSS_WEAKPOINT_IDENTITY_KINDS, auditBossWeakpointIdentityAtlas, bossWeakpointIdentityIcon, } from './boss-weakpoint-identity-assets.js';
const ARCHETYPE_BY_KIND = {
...
export function auditBossWeakpointIdentityAssets() {
    ...
    for (const kind of BOSS_WEAKPOINT_IDENTITY_KINDS) {
        ...
        const boss = { id: 10, type: 'boss', pos: { x: 800, y: 450 } };
    ...
    if (motionAmplitude !== 0)
        issues.push('motion');
    if (radiusMutation)
        issues.push('node-radius');
    if (hpMutation)
        issues.push('node-hp');
    if (modifierMutation)
        issues.push('modifiers');
    if (autoMutation)
        issues.push('auto-weakpoint');
    if (actionCount !== 9)
