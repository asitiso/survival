...
export function lateShopFastPath(elapsedSeconds:number,quickOffer:ShopDisplayOffer|null,state:EquipmentState):LateShopFastPathProfile{
 ...
 if(quickOffer.kind==='potion')return{promoteQuickBuy:true,reason:'safe-potion',newControlCount:0,estimatedPointerTravelReduction:.46};
 const current=currentItem(quickOffer,state);const safe=Boolean(current&&current.id===quickOffer.id&&current.rank<5);
