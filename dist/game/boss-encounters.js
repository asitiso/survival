...
export class BossEncounterSystem {
    ...
    begin(bossId, archetype, bossPos, variantTier) {
    ...
    update(dt) { this.vulnerabilityTimer = Math.max(0, this.vulnerabilityTimer - Math.max(0, dt)); }
    hitMagic(pos, strength) {
        let target = null;
    ...
    get modifiers() {
        ...
        if (this.archetype === 'inferno') {
            if (this.vulnerabilityTimer > 0)
                return { ...NEUTRAL, bossDamageTakenMultiplier: 1.28 };
        ...
        if (this.archetype === 'juggernaut')
            return alive > 0 ? { ...NEUTRAL, bossDamageTakenMultiplier: 0.84, dashDistanceMultiplier: 1.18 } : { ...NEUTRAL, bossDamageTakenMultiplier: 1.18, dashDistanceMultiplier: 0.72 };
        if (this.archetype === 'abyssWitch')
            return alive > 0 ? { ...NEUTRAL, bossDamageTakenMultiplier: 0.86, specialCadenceMultiplier: 0.88 } : { ...NEUTRAL, bossDamageTakenMultiplier: 1.16, specialCadenceMultiplier: 1.12 };
        if (this.archetype === 'twinMaw')
            return alive > 0 ? { ...NEUTRAL, bossDamageTakenMultiplier: 0.88, specialCadenceMultiplier: 0.92 } : { ...NEUTRAL, bossDamageTakenMultiplier: 1.15 };
