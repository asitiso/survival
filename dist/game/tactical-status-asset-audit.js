...
export function auditTacticalStatusAssets() {
    const samples = [];
    ...
    const suppressedPrimaries = ['hero-critical', 'core-critical', 'damage-critical', 'boss-response', 'damage-heavy', 'boss-countdown'];
    ...
    if (atlas.uniqueCellCount !== 15)
        issues.push('icon-cell-collision');
    if (atlas.outOfBounds.length !== 0)
        issues.push('icon-out-of-bounds');
    if (maxMotionAmplitude > 0.05)
        issues.push('objective-motion-budget');
    if (reducedFlashMotionAmplitude !== 0)
        issues.push('reduced-flash-motion');
    if (suppressedCombatMotionAmplitude !== 0)
        issues.push('combat-attention-motion');
    if (iconMotionAmplitude !== 0)
        issues.push('icon-motion');
