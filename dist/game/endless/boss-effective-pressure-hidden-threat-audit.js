...
export function auditBossEffectivePressureHiddenThreatCount() {
    const samples = [];
    ...
    const profiles = [[.04, .08, .12], [.18, .06, .10], [.12, .12, .05], [.24, .09, .15]];
    for (let opportunityIndex = 0; opportunityIndex < EFFECTS.length; opportunityIndex++)
        for (const profile of profiles) {
            ...
            threeThreatCases++;
            threeThreatCountPassed &&= ok;
            hiddenLabelPassed &&= p.hiddenThreatLabel === '+1 위험';
            maxTwoPassed &&= p.primaryEffects.length <= 2;
            push(`three:${opportunityIndex}:${profile.join('-')}`, ok);
        }
    for (let index = 0; index < 16; index++) {
        const input = neutral();
        EFFECTS.forEach((id, effectIndex) => setMultiplier(input, id, threatValue(id, .04 + ((index + effectIndex) % 4) * .04)));
        ...
        fourThreatCases++;
        fourThreatCountPassed &&= ok;
        hiddenLabelPassed &&= p.hiddenThreatLabel === '+2 위험';
        maxTwoPassed &&= p.primaryEffects.length <= 2;
        push(`four:${index}`, ok);
    }
    for (const id of EFFECTS)
        for (const magnitude of [.04, .12]) {
            const input = neutral();
            setMultiplier(input, id, threatValue(id, magnitude));
            const p = projectBossEffectivePressure(input), ok = p.hiddenThreatCount === 0 && p.hiddenThreatLabel === '' && p.visibleThreatCount === 1;
            compatibilityCases++;
            zeroHiddenCompatibilityPassed &&= ok;
            push(`compat-threat:${id}:${magnitude}`, ok);
        }
    for (const id of EFFECTS)
        for (const magnitude of [.04, .12]) {
            const input = neutral();
            setMultiplier(input, id, opportunityValue(id, magnitude));
            const p = projectBossEffectivePressure(input), ok = p.hiddenThreatCount === 0 && p.hiddenThreatLabel === '' && p.visibleThreatCount === 0;
            compatibilityCases++;
            zeroHiddenCompatibilityPassed &&= ok;
            push(`compat-opportunity:${id}:${magnitude}`, ok);
        }
    ...
    if (threeThreatCases !== 16)
        issues.push(`three:${threeThreatCases}`);
    if (fourThreatCases !== 16)
        issues.push(`four:${fourThreatCases}`);
    if (compatibilityCases !== 16)
        issues.push(`compat:${compatibilityCases}`);
    if (invariantCases !== 16)
        issues.push(`invariants:${invariantCases}`);
    if (!threeThreatCountPassed)
        issues.push('three-threat-count');
    if (!fourThreatCountPassed)
        issues.push('four-threat-count');
    if (!zeroHiddenCompatibilityPassed)
        issues.push('zero-hidden-compatibility');
    if (!hiddenLabelPassed)
