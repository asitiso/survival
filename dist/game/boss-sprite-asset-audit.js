...
export function auditBossSpriteAssets() {
    const samples = [];
    ...
    if (atlas.coverage !== 1)
        issues.push('boss-sprite-coverage');
    if (atlas.uniqueCellCount !== 6)
        issues.push('boss-sprite-cell-collision');
    if (atlas.outOfBounds.length > 0)
        issues.push('boss-sprite-out-of-bounds');
    if (motionAmplitude !== 0)
        issues.push('boss-sprite-motion');
    if (!fallbackPreserved)
        issues.push('boss-sprite-fallback');
    if (!presentationOnly)
        issues.push('boss-sprite-gameplay-overreach');
