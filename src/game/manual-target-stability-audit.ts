...
export interface ManualTargetStabilityAudit {
  samples: number;
  sameTierSamples: number;
  priorityOverrideSamples: number;
  releaseSamples: number;
  bufferedCastSamples: number;
  autoIsolationSamples: number;
  actionCount: number;
  sameTierPassed: boolean;
  priorityOverridePassed: boolean;
  releasePassed: boolean;
  bufferedCastPassed: boolean;
  autoIsolationPassed: boolean;
...
const enemy = (id: number, type: SpellTargetCandidate['type'], x: number, target: SpellTargetCandidate['target'] = 'hero', alive = true): SpellTargetCandidate => ({
  id,
  type,
  pos: { x, y: 0 },
  target,
...
function sameTierChecks(): boolean[] {
  const cases: Array<[SpellTargetCandidate, SpellTargetCandidate]> = [
...
function priorityOverrideChecks(): boolean[] {
  const normalToElite = (() => {
    const memory = new ManualTargetMemory();
...
function releaseChecks(): boolean[] {
  const expiry = (() => {
    const memory = new ManualTargetMemory();
...
function bufferedCastChecks(): boolean[] {
...
function autoIsolationChecks(): boolean[] {
  const memory = new ManualTargetMemory();
...
export function auditManualTargetStability(): ManualTargetStabilityAudit {
  ...
  const issues: string[] = [];
  if (!sameTierPassed) issues.push('same-tier-stickiness');
  if (!priorityOverridePassed) issues.push('priority-override');
  if (!releasePassed) issues.push('release-safety');
  if (!bufferedCastPassed) issues.push('buffered-cast');
  if (!autoIsolationPassed) issues.push('auto-isolation');
