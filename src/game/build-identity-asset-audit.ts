...
import { BUILD_IDENTITY_IDS, BUILD_IDENTITY_ATLAS, auditBuildIdentityAtlas, buildIdentityIcon, type BuildIdentityId } from './build-identity-assets.js';
import { growthChoiceIcon } from './growth-choice-icon-assets.js';
import { FUSION_IDS } from './spell-fusions.js';

export interface BuildIdentityAssetSample { id:BuildIdentityId; surface:'reward-choice'|'persistent-build'; atlasMatch:boolean; visible:boolean; motionAmplitude:number; textFallbackPreserved:boolean; passed:boolean; }
export interface BuildIdentityAssetAudit { samples:BuildIdentityAssetSample[]; coverage:number; uniqueCellCount:number; assetSrc:string; rewardCoverage:number; persistentCoverage:number; motionAmplitude:number; textFallbackPreserved:boolean; actionCount:number; snapshotSchemaMutation:false; issues:string[]; passed:boolean; }

export function auditBuildIdentityAssets():BuildIdentityAssetAudit{
  const atlas=auditBuildIdentityAtlas();
  const samples:BuildIdentityAssetSample[]=[];
  for(const id of BUILD_IDENTITY_IDS){
    const kind=FUSION_IDS.includes(id as typeof FUSION_IDS[number])?'fusion':'relic';
  ...
  const reward=samples.filter(s=>s.surface==='reward-choice'); const persistent=samples.filter(s=>s.surface==='persistent-build');
  const issues:string[]=[];
  if(samples.length!==40)issues.push(`samples:${samples.length}`);
  if(atlas.coverage!==1||atlas.uniqueCellCount!==20||atlas.outOfBounds.length)issues.push('atlas');
  if(samples.some(s=>!s.passed))issues.push('surface');
  ...
  if(rewardCoverage!==1)issues.push('reward-coverage');
  if(persistentCoverage!==1)issues.push('persistent-coverage');
