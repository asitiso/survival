...
export function projectileFocusHoldPresentation(input, reducedFlash = false) {
    ...
    return { hold, bodyAlphaScale: 1, criticalAlphaScale: 1, directionAlphaScale: input.critical ? Math.max(.9, .92 + .08 * hold) : Math.max(.5, normalDirection), presentationOnly: true };
}
export function impactBurstSettlePresentation(input, reducedFlash = false) {
    ...
    return { density, decorationAlphaScale, edgeAlphaScale, presentationOnly: true };
}
export function hazardCorridorStabilityPresentation(input, reducedFlash = false) {
    if (!input.active)
        return { carve: 0, fillAlphaScale: 1, edgeAlphaScale: 1, presentationOnly: true };
    ...
    return { carve, fillAlphaScale, edgeAlphaScale, presentationOnly: true };
}
export function safeLaneAttentionHoldPresentation(input, reducedFlash = false) {
    ...
    return { hold, pathAlphaFloor: Math.max(.9, .9 + confidence * .06), safeLaneAlphaScale: 1 + focus * .16 * flash, presentationOnly: true };
}
export function silhouetteContrastRecoveryPresentation(input, reducedMotion = false) {
    ...
    return { bodyAlphaScale: 1, overlayAlphaScale: 1 - suppression * .34, trailAlphaScale: (1 - suppression * .46) * (reducedMotion ? .82 : 1), presentationOnly: true };
}
export function temporalThreatBudgetPresentation(input, reducedMotion = false, reducedFlash = false) {
    ...
    return { stress, criticalAlphaScale: 1, safeLaneAlphaScale: 1 + stress * .11 * flash, secondaryAlphaScale: (1 - stress * .38) * motion * flash, presentationOnly: true };
}
