...
import { composeCombatBuild, composeObjectiveCombatModifiers } from './build-modifiers.js';
import { activeSynergies, synergyHudNames } from './synergies.js';
import { SYNERGY_IDENTITY_ATLAS, synergyIdentityIcon } from './synergy-identity-assets.js';
import { SHOP_ITEM_ATLAS } from './shop-item-assets.js';
import { activeLegendaryAwakeningRecall, legendaryProcIdentity } from './legendary-awakening-recall.js';
import { RunMissionDirector, missionProgress } from './run-missions.js';
import { threatDirectiveAt, threatDirectiveModifiers } from './threat-directives.js';
import { dangerUiState, priorityThreatIds, criticalHapticEvents } from './danger-ui.js';
...
import { HERO_RESPONSE_VFX_ATLAS, heroResponseVfxSprite } from './hero-response-vfx-assets.js';
...
import { BOSS_WEAKPOINT_WORLD_VFX_ATLAS, bossWeakpointWorldVfxSprite } from './boss-weakpoint-world-vfx-assets.js';
...
import { PERFECT_EVADE_IDENTITY_ATLAS, normalizedPerfectEvadeStreak, perfectEvadeIdentityIcon } from './perfect-evade-identity-assets.js';
...
import { WORLD_EVOLUTION_IDENTITY_ATLAS, worldEvolutionIdentityIcon } from './endless/world-evolution-identity-assets.js';
...
import { catastropheTransitionForecastLabel, catastropheTransitionHint, projectCatastropheTransition, projectCatastropheTransitionForecast } from './catastrophe-transition-projection.js';
...
import { MYTHIC_PHASE_IDENTITY_ATLAS, mythicPhaseIdentityIcon, mythicPhasePressureSegments } from './endless/mythic-phase-identity-assets.js';
...
import { projectMythicSafeZonePressureEffects } from './endless/mythic-safe-zone-pressure-projection.js';
import { projectBossEffectivePressure } from './endless/boss-effective-pressure-projection.js';
...
import { RUN_MISSION_PACE_IDENTITY_ATLAS, runMissionPaceIdentityForRatios, runMissionPaceIdentityIcon } from './run-mission-pace-identity-assets.js';
...
import { OATH_BOON_OUTCOME_IDENTITY_ATLAS, oathBoonOutcomeIdentityIcon } from './oath-boon-outcome-identity-assets.js';
...
import { HERO_METER_IDENTITY_ATLAS, heroMeterIdentityIcon } from './hero-meter-identity-assets.js';
import { ARCANE_COMBO_IDENTITY_ATLAS, arcaneComboIdentityIcon, arcaneComboTierBadge } from './arcane-combo-identity-assets.js';
import { relicResonanceRecallIcon, relicResonanceRecallPresentation, relicResonanceTierBadge } from './relic-resonance-recall-assets.js';
import { RELIC_RESONANCE_IMPACT_IDENTITY_ATLAS, relicResonanceImpactIdentityIcon, relicResonanceImpactIdentityStyle } from './relic-resonance-impact-identity-assets.js';
import { RELIC_RESONANCE_TIER_IDENTITY_ATLAS, relicResonanceTierIdentityForTier, relicResonanceTierIdentityIcon, relicResonanceTierIdentityStyle } from './relic-resonance-tier-identity-assets.js';
...
import { FINAL_FORM_WORLD_VFX_ATLAS, finalFormWorldVfxSprite } from './final-form-world-vfx-assets.js';
...
import { bossHazardRhythmRecoveryPresentation, rhythmRecoveryBudgetPresentation, safeLaneRhythmRecoveryPresentation } from './threat-rhythm-recovery-rendering.js';
import { criticalTemporalPriorityPresentation, safeLaneTemporalPriorityPresentation, unifiedTemporalArbitrationPresentation } from './threat-rhythm-temporal-arbitration-rendering.js';
...
import { BOSS_ARENA_TRANSITION_WORLD_VFX_ATLAS, bossArenaTransitionWorldVfxSprite } from './boss-arena-transition-world-vfx-assets.js';
...
import { FIELD_EVENT_LIFECYCLE_WORLD_VFX_ATLAS, fieldEventLifecycleWorldVfxSprite } from './field-event-lifecycle-world-vfx-assets.js';
...
import { battlefieldEvolutionImpactHint, projectBattlefieldEvolutionImpact, projectBattlefieldMechanics } from './battlefield-mechanic-projection.js';
import { ASCENSION_TIER_PRESSURE_ATLAS, ascensionTierPressureIdentityIcon } from './ascension-tier-pressure-identity-assets.js';
import { ascensionTierForecastLabel, ascensionTierPressureHint, projectAscensionTierForecast, projectAscensionTierOutcome } from './ascension-tier-pressure-projection.js';
import { DEEP_RUN_DECISION_ATLAS, deepRunDecisionIdentityIcon, deepRunDecisionIdentityStyle } from './deep-run-decision-identity-assets.js';
...
import { createHeroMeterState, heroMeterLabel, heroMeterModifiers, updateHeroMeter } from './hero-meters.js';
import { composeHeroMeterCombat, heroMeterCastSignals, heroMeterKillSignals } from './hero-meter-integration.js';
import { mapEvolutionLabel, mapEnvironmentVfxDescriptor, environmentDestructionVfxDescriptor } from './map-evolution.js';
import { defaultThreatProfile, loadThreatProfile, saveThreatProfile, selectThreatLevel } from '../domain/threat-profile.js';
import { threatLevelModifiers, threatLevelName } from '../domain/threat-level.js';
import { defaultRunRecords, loadRunRecords, saveRunRecords } from '../domain/run-records.js';
import { completeRunProgression } from '../domain/run-completion.js';
import { composeThreatPressure } from './phase14-runtime.js';
import { ArcaneAudio, defaultAudioSettings, loadAudioSettings, saveAudioSettings } from './audio.js';
...
import { calculateTacticalScoreBonus, tacticalRecapLines } from '../domain/tactical-recap.js';
...
import { defaultMasteryProfile, grantMasteryXp, loadMasteryProfile, saveMasteryProfile } from '../domain/mastery-profile.js';
import { masteryXpForRun } from '../domain/mastery-rewards.js';
import { PauseState } from './pause-state.js';
import { clearRunSnapshot, saveRunSnapshot } from '../domain/run-snapshot.js';
...
import { restoreExtension, serializeExtension } from './endless/snapshot.js';
import { acceptContract, contractHudLine, getContractModifiers } from './endless/contracts.js';
import { buildLegacyRunView, composeEndlessHostModifiers, contractChoiceCards } from './endless/host.js';
import { consumeFieldNode } from './endless/world-evolution.js';
import { getBossAdaptations } from './endless/nemesis.js';
...
import { bossArenaMutationModifiers, createBossArenaMutation } from './endless/boss-arena-mutations.js';
...
import { ASCENSION_MUTATOR_IDENTITY_ATLAS, ascensionMutatorIdentityIcon } from './endless/ascension-mutator-identity-assets.js';
...
import { advanceLifecycleCheckpoint, createLifecycleCheckpointState } from './lifecycle-idempotency-audit.js';
import { loadRetryBlueprint, saveRetryBlueprint } from '../domain/retry-blueprint.js';
import { compareRunResult } from '../domain/run-comparison.js';
import { encodeBuildCapsule } from '../domain/build-capsule.js';
import { createBuildReplayPlan } from '../domain/build-replay.js';
import { replayGuidance } from '../domain/build-replay-guidance.js';
import { deriveHeroFinalForm, finalFormModifiers } from './endless/final-form.js';
import { overdriveModifiers, resolveBuildArchetype } from './endless/build-overdrive.js';
...
import { advanceFinalFormFlow, createDefaultFinalFormFlowState, finalFormFlowModifiers, recordFinalFormFlowCast } from './endless/final-form-flow.js';
...
import { combatAttentionPolicy, targetGuidanceMotionPolicy, secondaryCombatMotionPolicy, residualCombatMotionPolicy } from './combat-cue-priority.js';
import { worldVfxPriorityPolicy } from './world-vfx-priority-arbitration.js';
import { worldVfxOcclusionScale } from './world-vfx-occlusion-guard.js';
import { resolveWorldVfxOccupancy } from './world-vfx-occupancy-budget.js';
...
import { bossSafeResponseSlotHysteresis } from './boss-safe-response-slot-hysteresis.js';
...
import { longRunOathModifiers, oathHudLine } from './endless/long-run-oaths.js';
...
import { advanceThermalRecovery, createThermalRecoveryState, thermalPolicyForEffectiveTier } from './endless/thermal-recovery-hysteresis.js';
...
import { openingWaveCeremony } from './opening-wave-ceremony.js';
import { openingBossEntrance } from './opening-boss-entrance.js';
...
import { advanceArenaDodgeTracker, createArenaDodgeTracker } from './endless/arena-dodge-reward.js';
import { arenaDodgeChainReward, breakArenaDodgeChain, createArenaDodgeChain, recordArenaDodgeChain } from './endless/arena-dodge-chain.js';
...
import { createMythicTacticAttackLink } from './endless/mythic-tactic-attack-link.js';
...
import { advanceSafeLaneLink, consumeSafeLanePerfectEvade, createSafeLaneLink } from './endless/safe-lane-link.js';
export class Game {
    canvas;
    input;
    storage = createBrowserSessionStorage();
    lifecycleCheckpointState = createLifecycleCheckpointState();
    hero = createHero('arkan');
    core = createGuardianCore();
    elapsed = 0;
    paused = true;
    ...
    bossActionAssistArchetype = null;
    bossResponseAckAction = null;
    bossResponseAckSince = 0;
    bossResponseAckBossId = null;
    bossResponseAckArchetype = null;
    bossResponseAckCycle = null;
    bossSafeResponseWindowUntil = 0;
    bossSafeResponseBossId = null;
    bossSafeResponseCycle = null;
    bossSafeResponseShownCycle = null;
    bossSafeResponseSlotMemory = null;
    spawnLaneEdgeCountDebounceMemory = [];
    enemies = new EnemyManager();
    spells = new SpellSystem();
    castIntentBuffer = new CastIntentBuffer();
    manualTargetMemory = new ManualTargetMemory();
    pickups = new PickupManager();
    terrain = new TerrainSystem();
    feedback = new CombatFeedbackSystem();
    presentation = new PresentationRuntime('high');
    bossPresentation = new BossPresentationTracker();
    killChainVfx = new KillChainVfxTracker();
    fieldEvents = new FieldEventDirector();
    legendaryEffects = new LegendaryEffectController();
    runMissions = new RunMissionDirector();
    battlefieldObjectives = new BattlefieldObjectiveDirector();
    objectiveRuntime = new ObjectiveRuntime();
    bossEncounter = new BossEncounterSystem();
    bossArena = new BossArenaSystem();
    comboRuntime = new ComboRuntime();
    fusionRuntime = new FusionRuntime();
    fateRuntime = new FateRuntime();
    decisionPickGuard = new DecisionPickGuard();
    decisionSessionActive = false;
    decisionReplay = null;
    onboarding = new OnboardingController(defaultOnboardingState());
    levelUpOverlay;
    shopOverlay;
    heroSelectOverlay;
    resultsOverlay;
    ...
    catastrophe = null;
    lastCatastropheId = null;
    catastropheBannerTimer = 0;
    catastropheBannerTransitionProjection = null;
    kainOverload = 0;
    ...
    eventToastMythicTacticArchetype = null;
    eventToastMythicPhase = null;
    eventToastBossArchetype = null;
    eventToastBossVariantTier = null;
    eventToastApexSecondaryArchetype = null;
    eventToastBossWeakpointBreakArchetype = null;
    eventToastPerfectEvadeStreak = null;
    eventToastAscensionMutator = null;
    eventToastFatePath = null;
    eventToastFateImpact = null;
    eventToastOathKind = null;
    eventToastOathHelper = null;
    eventToastContractFamily = null;
    eventToastContractBoonFamily = null;
    eventToastRelicResonance = null;
    eventToastRelicProjection = null;
    eventToastNemesisAdaptations = [];
    eventToastWorldEvolution = null;
    eventToastBossArenaMutation = null;
    eventToastHeroMeterId = null;
    eventToastArcaneComboFamily = null;
    eventToastTacticalStatusIconId = null;
    eventToastBuildIdentityId = null;
    eventToastSynergyId = null;
    eventToastLegendaryItemId = null;
    eventToastSpellEvolution = null;
    eventToastSpellEvolutionProjection = null;
    eventToastRunTraitId = null;
    eventToastAscensionSelectionId = null;
    eventToastHeroAscensionProjection = null;
    eventToastFusionProjection = null;
    eventToastBuildOverdriveProjection = null;
    eventToastBattlefieldEvolutionProjection = null;
    eventToastAscensionTierProjection = null;
    eventToastObjectiveRewards = [];
    eventToastObjectiveRewardMultiplier = 1;
    eventToastMissionReward = null;
    eventToastFieldEventId = null;
    runFoundationIdentityInitialized = false;
    lastRunFoundationTrait = null;
    synergyIdentityInitialized = false;
    lastSynergyIdentityIds = [];
    lastRelicResonanceRelic = null;
    lastRelicResonanceTier = 0;
    metaProfile = defaultMetaProfile();
    masteryProfile = defaultMasteryProfile();
    ...
    resumeSnapshot = null;
    nextSnapshotAt = 15;
    nextRecoveryJournalAt = 60;
    currentRunBlueprint = null;
    currentReplayPlan = null;
    audio = new ArcaneAudio(this.audioSettings);
    seenBossIds = new Set();
    bossPressureRatioById = new Map();
    selectedTrait = null;
    runGoldMultiplier = 1;
    runHeroDamageTakenMultiplier = 1;
    runCoreDamageTakenMultiplier = 1;
    activeRelic = null;
    pendingBossArchetype = null;
    eliteKills = 0;
    objectivePowerTimer = 0;
    timeWarpTimer = 0;
    timeWarpCooldownMultiplier = 1;
    bossEncounterNodesDestroyed = 0;
    lastBossEncounterDestroyedNodes = 0;
    threatDirective = null;
    dangerState = dangerUiState(1, 1);
    bossWarningHapticActive = false;
    hapticArbiter = new CombatHapticArbiter();
    presentationSettings = loadPresentationSettings(this.storage);
    bossPhaseCue = null;
    bossPhaseCueTimer = 0;
    killChainCue = null;
    killChainCueTimer = 0;
    ...
    actionReadyState = {};
    ultimatePulseUntil = {};
    actionIconAtlasImage = null;
    actionIconAtlasReady = false;
    heroAbilityIconAtlasImage = null;
    heroAbilityIconAtlasReady = false;
    spellEvolutionCrestAtlasImage = null;
    spellEvolutionCrestAtlasReady = false;
    spellEvolutionModifierIdentityAtlasImage = null;
    spellEvolutionModifierIdentityAtlasReady = false;
    heroBattleSpriteAtlasImage = null;
    heroBattleSpriteAtlasReady = false;
    heroMotionRenderAtlasImage = null;
    heroMotionRenderAtlasReady = false;
    heroCastRenderAtlasImage = null;
    heroCastRenderAtlasReady = false;
    heroRenderMotionBlend = 0;
    heroRenderStride = 0;
    heroRenderTurnTilt = 0;
    heroRenderRecoveryBlend = 0;
    heroRenderLastMoving = false;
    heroRenderPreviousFacing = { x: 1, y: 0 };
    ...
    heroResponseVfxAtlasReady = false;
    heroResponseVfx = [];
    heroUltimateSignatureVfxAtlasImage = null;
    heroUltimateSignatureVfxAtlasReady = false;
    heroSpellSignatureVfxAtlasImage = null;
    heroSpellSignatureVfxAtlasReady = false;
    enemyCombatVfxAtlasImage = null;
    enemyCombatVfxAtlasReady = false;
    enemyDeathImageBursts = [];
    enemyDefeatBodyTransitions = [];
    enemySpriteAtlasImage = null;
    enemySpriteAtlasReady = false;
    eliteAffixIdentityAtlasImage = null;
    eliteAffixIdentityAtlasReady = false;
    specialistIntentAtlasImage = null;
    specialistIntentAtlasReady = false;
    specialistCombatVfxAtlasImage = null;
    specialistCombatVfxAtlasReady = false;
    bossWeakpointIdentityAtlasImage = null;
    bossWeakpointWorldVfxAtlasImage = null;
    bossWeakpointWorldVfxAtlasReady = false;
    bossWeakpointBreakWorldVfx = [];
    bossWeakpointIdentityAtlasReady = false;
    bossWeakpointBreakIdentityAtlasImage = null;
    bossWeakpointBreakIdentityAtlasReady = false;
    bossCounterplayBenefitIdentityAtlasImage = null;
    bossCounterplayBenefitIdentityAtlasReady = false;
    bossPhase2EscalationAtlasImage = null;
    bossPhase2EscalationAtlasReady = false;
    bossPhase3EnrageAtlasImage = null;
    bossPhase3EnrageAtlasReady = false;
    bossVariantPressureAtlasImage = null;
    bossVariantPressureAtlasReady = false;
    apexSecondaryPatternAtlasImage = null;
    apexSecondaryPatternAtlasReady = false;
    bossArenaHazardIdentityAtlasImage = null;
    bossArenaLifecycleVfxAtlasImage = null;
    bossArenaLifecycleVfxAtlasReady = false;
    bossArenaHazardIdentityAtlasReady = false;
    bossArchetypeIdentityAtlasImage = null;
    bossArchetypeIdentityAtlasReady = false;
    bossSpecialIntentAtlasImage = null;
    bossSpecialIntentAtlasReady = false;
    bossResponseAckIdentityAtlasImage = null;
    bossResponseAckIdentityAtlasReady = false;
    perfectEvadeIdentityAtlasImage = null;
    perfectEvadeIdentityAtlasReady = false;
    damageSourceIdentityAtlasImage = null;
    damageSourceIdentityAtlasReady = false;
    fieldNodeIdentityAtlasImage = null;
    fieldNodeIdentityAtlasReady = false;
    worldEvolutionIdentityAtlasImage = null;
    worldEvolutionIdentityAtlasReady = false;
    catastropheIdentityAtlasImage = null;
    catastropheIdentityAtlasReady = false;
    catastropheTransitionIdentityAtlasImage = null;
    catastropheTransitionIdentityAtlasReady = false;
    mythicLastLawIdentityAtlasImage = null;
    mythicLastLawIdentityAtlasReady = false;
    mythicTacticIdentityAtlasImage = null;
    mythicTacticIdentityAtlasReady = false;
    mythicPhaseIdentityAtlasImage = null;
    mythicPhaseIdentityAtlasReady = false;
    mythicArenaGeometryIdentityAtlasImage = null;
    mythicArenaGeometryIdentityAtlasReady = false;
    mythicSafeZoneLifecycleIdentityAtlasImage = null;
    mythicSafeZoneLifecycleIdentityAtlasReady = false;
    mythicSafeZonePressureEffectIdentityAtlasImage = null;
    mythicSafeZonePressureEffectIdentityAtlasReady = false;
    safeZoneTransitionDirectionAtlasImage = null;
    safeZoneTransitionDirectionAtlasReady = false;
    ascensionMutatorIdentityAtlasImage = null;
    ascensionMutatorIdentityAtlasReady = false;
    nemesisAdaptationIdentityAtlasImage = null;
    nemesisAdaptationIdentityAtlasReady = false;
    nemesisAdaptationEffectIdentityAtlasImage = null;
    nemesisAdaptationEffectIdentityAtlasReady = false;
    bossArenaMutationIdentityAtlasImage = null;
    bossArenaMutationIdentityAtlasReady = false;
    decisionPathIconAtlasImage = null;
    decisionPathIconAtlasReady = false;
    bossSpriteAtlasImage = null;
    bossSpriteAtlasReady = false;
    bossSignatureVfxAtlasImage = null;
    bossSignatureVfxAtlasReady = false;
    bossSpecialCombatVfxAtlasImage = null;
    bossSpecialCombatVfxAtlasReady = false;
    bossPhaseOverlayVfxAtlasImage = null;
    bossPhaseOverlayVfxAtlasReady = false;
    bossSignatureEntranceBossId = null;
    bossSignatureEntranceUntil = 0;
    tacticalStatusIconAtlasImage = null;
    tacticalStatusIconAtlasReady = false;
    objectiveActionIdentityAtlasImage = null;
    objectiveActionIdentityAtlasReady = false;
    objectiveRewardIdentityAtlasImage = null;
    objectiveRewardIdentityAtlasReady = false;
    runMissionPaceIdentityAtlasImage = null;
    runMissionPaceIdentityAtlasReady = false;
    runContractBoonEffectIdentityAtlasImage = null;
    runContractBoonEffectIdentityAtlasReady = false;
    oathRequirementIdentityAtlasImage = null;
    oathRequirementIdentityAtlasReady = false;
    oathBoonOutcomeIdentityAtlasImage = null;
    oathBoonOutcomeIdentityAtlasReady = false;
    relicResonanceImpactIdentityAtlasImage = null;
    relicResonanceImpactIdentityAtlasReady = false;
    relicResonanceTierIdentityAtlasImage = null;
    relicResonanceTierIdentityAtlasReady = false;
    heroAscensionModifierIdentityAtlasImage = null;
    heroAscensionModifierIdentityAtlasReady = false;
    heroAscensionBuildDirectionAtlasImage = null;
    heroAscensionBuildDirectionAtlasReady = false;
    fusionModifierIdentityAtlasImage = null;
    fusionModifierIdentityAtlasReady = false;
    fusionComponentRelationAtlasImage = null;
    fusionComponentRelationAtlasReady = false;
    fateBenefitVectorAtlasImage = null;
    fateBenefitVectorAtlasReady = false;
    fateCostVectorAtlasImage = null;
    fateCostVectorAtlasReady = false;
    fieldEventResponseIdentityAtlasImage = null;
    fieldEventResponseIdentityAtlasReady = false;
    fieldEventEffectProfileIdentityAtlasImage = null;
    fieldEventEffectProfileIdentityAtlasReady = false;
    buildIdentityAtlasImage = null;
    buildIdentityAtlasReady = false;
    buildOverdriveEffectAtlasImage = null;
    buildOverdriveEffectAtlasReady = false;
    heroMeterIdentityAtlasImage = null;
    heroMeterIdentityAtlasReady = false;
    arcaneComboIdentityAtlasImage = null;
    arcaneComboIdentityAtlasReady = false;
    synergyIdentityAtlasImage = null;
    synergyIdentityAtlasReady = false;
    legendaryAwakeningAtlasImage = null;
    legendaryAwakeningAtlasReady = false;
    finalFormIdentityAtlasImage = null;
    finalFormIdentityAtlasReady = false;
    battlefieldPropVfxAtlasImage = null;
    battlefieldPropVfxAtlasReady = false;
    battlefieldObstacleStateVfxAtlasImage = null;
    battlefieldObstacleStateVfxAtlasReady = false;
    battlefieldInteractionVfxAtlasImage = null;
    battlefieldInteractionVfxAtlasReady = false;
    pickupFlowVfxAtlasImage = null;
    pickupFlowVfxAtlasReady = false;
    spawnPressureVfxAtlasImage = null;
    spawnPressureVfxAtlasReady = false;
    survivalResponseVfxAtlasImage = null;
    ...
    eliteAffixLifecycleVfxAtlasReady = false;
    enemyTargetPressureVfxAtlasImage = null;
    enemyTargetPressureVfxAtlasReady = false;
    finalFormWorldVfxAtlasImage = null;
    finalFormWorldVfxAtlasReady = false;
    finalFormWorldVfx = [];
    fusionWorldVfxAtlasImage = null;
    fusionWorldVfxAtlasReady = false;
    fusionWorldVfx = [];
    heroMeterWorldVfxAtlasImage = null;
    heroMeterWorldVfxAtlasReady = false;
    heroMeterWorldVfx = [];
    bossProjectileLifecycleVfxAtlasImage = null;
    bossProjectileLifecycleVfxAtlasReady = false;
    persistentSpellZoneVfxAtlasImage = null;
    persistentSpellZoneVfxAtlasReady = false;
    crystalInteractionLifecycleVfxAtlasImage = null;
    crystalInteractionLifecycleVfxAtlasReady = false;
    crystalInteractionLifecycleVfx = [];
    bossPhaseAftermathVfxAtlasImage = null;
    bossPhaseAftermathVfxAtlasReady = false;
    bossPhaseTransitionVfx = [];
    specialistReactionLifecycleVfxAtlasImage = null;
    specialistReactionLifecycleVfxAtlasReady = false;
    mapEvolutionAftermathVfxAtlasImage = null;
    mapEvolutionAftermathVfxAtlasReady = false;
    mapEvolutionAftermathVfx = [];
    bossHazardAftermathVfxAtlasImage = null;
    bossHazardAftermathVfxAtlasReady = false;
    ...
    enemyFinisherVfx = [];
    heroCrisisVfxAtlasImage = null;
    heroCrisisVfxAtlasReady = false;
    heroCrisisVfx = [];
    lastHeroCrisisHpRatio = 1;
    perfectEvadeTrailVfxAtlasImage = null;
    perfectEvadeTrailVfxAtlasReady = false;
    perfectEvadeTrailVfx = [];
    crowdControlPropagationVfxAtlasImage = null;
    crowdControlPropagationVfxAtlasReady = false;
    bossCounterplayRewardVfxAtlasImage = null;
    bossCounterplayRewardVfxAtlasReady = false;
    bossCounterplayRewardBurstVfx = [];
    objectiveCompletionCeremonyVfxAtlasImage = null;
    objectiveCompletionCeremonyVfxAtlasReady = false;
    objectiveCompletionCeremonyVfx = [];
    ultimatePostImpactResidueVfxAtlasImage = null;
    ultimatePostImpactResidueVfxAtlasReady = false;
    mapSafeLaneTransitionVfxAtlasImage = null;
    mapSafeLaneTransitionVfxAtlasReady = false;
    objectiveActivationMaterializationVfxAtlasImage = null;
    objectiveActivationMaterializationVfxAtlasReady = false;
    objectiveActivationMaterializationVfx = [];
    bossArenaTransitionWorldVfxAtlasImage = null;
    bossArenaTransitionWorldVfxAtlasReady = false;
    bossArenaTransitionWorldVfx = [];
    mapCombatBoundaryWarningVfxAtlasImage = null;
    mapCombatBoundaryWarningVfxAtlasReady = false;
    objectiveFailureDissolveVfxAtlasImage = null;
    objectiveFailureDissolveVfxAtlasReady = false;
    objectiveFailureDissolveVfx = [];
    fieldEventLifecycleWorldVfxAtlasImage = null;
    fieldEventLifecycleWorldVfxAtlasReady = false;
    fieldEventLifecycleWorldVfx = [];
    fieldEventWorldAnchor = null;
    elitePackApproachFormationVfxAtlasImage = null;
    elitePackApproachFormationVfxAtlasReady = false;
    elitePackApproachFormationVfx = [];
    activeWorldVfxPriorityPolicy = worldVfxPriorityPolicy('normal', 'high');
    ...
    battlefieldDepthOverlayAtlasReady = false;
    battlefieldEnvironmentReactionVfxAtlasImage = null;
    battlefieldEnvironmentReactionVfxAtlasReady = false;
    battlefieldMechanicAtlasImage = null;
    battlefieldMechanicAtlasReady = false;
    ascensionTierPressureAtlasImage = null;
    ascensionTierPressureAtlasReady = false;
    deepRunDecisionIdentityAtlasImage = null;
    deepRunDecisionIdentityAtlasReady = false;
    finalFormIdentityCueId = null;
    finalFormIdentityCueUntil = 0;
    endlessState = createDefaultEndlessState();
    frameEndlessEvents = [];
    endlessBossStartedAt = 0;
    endlessBossCoreHpAtStart = 0;
    endlessBossKey = null;
    mythicLastLawBossId = null;
    mythicPhaseBossId = null;
    lastMythicPhase = 0;
    finalFormMotion = { x: 0, y: 0 };
    finalFormFlow = createDefaultFinalFormFlowState();
    lastOpeningWaveBeatId = null;
    lastOpeningBossEntranceStage = null;
    arenaDodgeTracker = createArenaDodgeTracker();
    arenaDodgeChain = createArenaDodgeChain();
    safeLaneLink = createSafeLaneLink();
    arenaEvadeBoostUntilMs = 0;
    arenaEvadeMoveMultiplier = 1;
    flowImpactTimer = 0;
    mythicTacticBoostUntilMs = 0;
    mythicTacticBossDamageMultiplier = 1;
    mythicTacticAttackLink = null;
    ...
    initializeHeroAbilityIconAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeSpellEvolutionCrestAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeSpellEvolutionModifierIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeHeroBattleSpriteAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeHeroMotionRenderAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeHeroResponseVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = HERO_RESPONSE_VFX_ATLAS.src;
        this.heroResponseVfxAtlasImage = image;
    }
    initializeHeroProjectileVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeHeroUltimateSignatureVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeHeroSpellSignatureVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeEliteAffixIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeSpecialistIntentAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeSpecialistCombatVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeBossWeakpointIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeBossWeakpointWorldVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = BOSS_WEAKPOINT_WORLD_VFX_ATLAS.src;
        this.bossWeakpointWorldVfxAtlasImage = image;
    }
    initializeBossWeakpointBreakIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = BOSS_WEAKPOINT_BREAK_IDENTITY_ATLAS.src;
        this.bossWeakpointBreakIdentityAtlasImage = image;
    }
    initializeBossCounterplayBenefitIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = BOSS_COUNTERPLAY_BENEFIT_IDENTITY_ATLAS.src;
        this.bossCounterplayBenefitIdentityAtlasImage = image;
    }
    initializeBossPhase2EscalationAtlas() {
        ...
        image.src = BOSS_PHASE2_ESCALATION_ATLAS.src;
        this.bossPhase2EscalationAtlasImage = image;
    }
    initializeBossPhase3EnrageAtlas() {
        ...
        image.src = BOSS_PHASE3_ENRAGE_ATLAS.src;
        this.bossPhase3EnrageAtlasImage = image;
    }
    initializeBossVariantPressureAtlas() {
        ...
        image.src = BOSS_VARIANT_PRESSURE_ATLAS.src;
        this.bossVariantPressureAtlasImage = image;
    }
    initializeApexSecondaryPatternAtlas() {
        ...
        image.src = APEX_SECONDARY_PATTERN_ATLAS.src;
        this.apexSecondaryPatternAtlasImage = image;
    }
    initializeBossArenaHazardIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = BOSS_ARENA_HAZARD_IDENTITY_ATLAS.src;
        this.bossArenaHazardIdentityAtlasImage = image;
    }
    initializeBossArenaLifecycleVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = BOSS_ARENA_LIFECYCLE_VFX_ATLAS.src;
        this.bossArenaLifecycleVfxAtlasImage = image;
    }
    initializeBossArchetypeIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = BOSS_ARCHETYPE_IDENTITY_ATLAS.src;
        this.bossArchetypeIdentityAtlasImage = image;
    }
    initializeBossSpecialIntentAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = BOSS_SPECIAL_INTENT_ATLAS.src;
        this.bossSpecialIntentAtlasImage = image;
    }
    initializeBossResponseAckIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = BOSS_RESPONSE_ACK_IDENTITY_ATLAS.src;
        this.bossResponseAckIdentityAtlasImage = image;
    }
    initializePerfectEvadeIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = PERFECT_EVADE_IDENTITY_ATLAS.src;
        this.perfectEvadeIdentityAtlasImage = image;
    }
    initializeDamageSourceIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeFieldNodeIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeWorldEvolutionIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeCatastropheIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeCatastropheTransitionIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeMythicLastLawIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeMythicTacticIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeMythicPhaseIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeMythicArenaGeometryIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = MYTHIC_ARENA_GEOMETRY_IDENTITY_ATLAS.src;
        this.mythicArenaGeometryIdentityAtlasImage = image;
    }
    initializeMythicSafeZoneLifecycleIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = MYTHIC_SAFE_ZONE_LIFECYCLE_IDENTITY_ATLAS.src;
        this.mythicSafeZoneLifecycleIdentityAtlasImage = image;
    }
    initializeMythicSafeZonePressureEffectIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeSafeZoneTransitionDirectionAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = SAFE_ZONE_TRANSITION_DIRECTION_ATLAS.src;
        this.safeZoneTransitionDirectionAtlasImage = image;
    }
    initializeAscensionMutatorIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeNemesisAdaptationIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeNemesisAdaptationEffectIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeBossArenaMutationIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeDecisionPathIconAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeBossSignatureVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeBossSpecialCombatVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeBossPhaseOverlayVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeTacticalStatusIconAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeObjectiveActionIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = OBJECTIVE_ACTION_IDENTITY_ATLAS.src;
        this.objectiveActionIdentityAtlasImage = image;
    }
    initializeObjectiveRewardIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = OBJECTIVE_REWARD_IDENTITY_ATLAS.src;
        this.objectiveRewardIdentityAtlasImage = image;
    }
    initializeRunMissionPaceIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = RUN_MISSION_PACE_IDENTITY_ATLAS.src;
        this.runMissionPaceIdentityAtlasImage = image;
    }
    initializeRunContractBoonEffectIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = RUN_CONTRACT_BOON_EFFECT_IDENTITY_ATLAS.src;
        this.runContractBoonEffectIdentityAtlasImage = image;
    }
    initializeOathRequirementIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = OATH_REQUIREMENT_IDENTITY_ATLAS.src;
        this.oathRequirementIdentityAtlasImage = image;
    }
    initializeOathBoonOutcomeIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = OATH_BOON_OUTCOME_IDENTITY_ATLAS.src;
        this.oathBoonOutcomeIdentityAtlasImage = image;
    }
    initializeRelicResonanceImpactIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = RELIC_RESONANCE_IMPACT_IDENTITY_ATLAS.src;
        this.relicResonanceImpactIdentityAtlasImage = image;
    }
    initializeRelicResonanceTierIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = RELIC_RESONANCE_TIER_IDENTITY_ATLAS.src;
        this.relicResonanceTierIdentityAtlasImage = image;
    }
    initializeHeroAscensionModifierIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = HERO_ASCENSION_MODIFIER_IDENTITY_ATLAS.src;
        this.heroAscensionModifierIdentityAtlasImage = image;
    }
    initializeHeroAscensionBuildDirectionAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = HERO_ASCENSION_BUILD_DIRECTION_ATLAS.src;
        this.heroAscensionBuildDirectionAtlasImage = image;
    }
    initializeFusionModifierIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = FUSION_MODIFIER_IDENTITY_ATLAS.src;
        this.fusionModifierIdentityAtlasImage = image;
    }
    initializeFusionComponentRelationAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = FUSION_COMPONENT_RELATION_ATLAS.src;
        this.fusionComponentRelationAtlasImage = image;
    }
    initializeFateBenefitVectorAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = FATE_BENEFIT_VECTOR_ATLAS.src;
        this.fateBenefitVectorAtlasImage = image;
    }
    initializeFateCostVectorAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = FATE_COST_VECTOR_ATLAS.src;
        this.fateCostVectorAtlasImage = image;
    }
    initializeFieldEventResponseIdentityAtlas() {
        ...
        image.src = FIELD_EVENT_RESPONSE_IDENTITY_ATLAS.src;
        this.fieldEventResponseIdentityAtlasImage = image;
    }
    initializeFieldEventEffectProfileIdentityAtlas() {
        ...
        image.src = FIELD_EVENT_EFFECT_PROFILE_IDENTITY_ATLAS.src;
        this.fieldEventEffectProfileIdentityAtlasImage = image;
    }
    initializeBuildIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeBuildOverdriveEffectAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeHeroMeterIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeArcaneComboIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeSynergyIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = SYNERGY_IDENTITY_ATLAS.src;
        this.synergyIdentityAtlasImage = image;
    }
    initializeLegendaryAwakeningAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = SHOP_ITEM_ATLAS.src;
        this.legendaryAwakeningAtlasImage = image;
    }
    initializeFinalFormIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeBattlefieldPropVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeBattlefieldObstacleStateVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeBattlefieldInteractionVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeSpawnPressureVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeSurvivalResponseVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeFreezeControlVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeRegularEnemyActionVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeEliteAffixLifecycleVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeEnemyTargetPressureVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeFinalFormWorldVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeHeroMeterWorldVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeBossProjectileLifecycleVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializePersistentSpellZoneVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeCrystalInteractionLifecycleVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    initializeBossPhaseAftermathVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = BOSS_PHASE_AFTERMATH_VFX_ATLAS.src;
        this.bossPhaseAftermathVfxAtlasImage = image;
    }
    initializeSpecialistReactionLifecycleVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = SPECIALIST_REACTION_LIFECYCLE_VFX_ATLAS.src;
        this.specialistReactionLifecycleVfxAtlasImage = image;
    }
    initializeMapEvolutionAftermathVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = MAP_EVOLUTION_AFTERMATH_VFX_ATLAS.src;
        this.mapEvolutionAftermathVfxAtlasImage = image;
    }
    initializeBossHazardAftermathVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = BOSS_HAZARD_AFTERMATH_VFX_ATLAS.src;
        this.bossHazardAftermathVfxAtlasImage = image;
    }
    initializeEnemyFinisherVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = ENEMY_FINISHER_VFX_ATLAS.src;
        this.enemyFinisherVfxAtlasImage = image;
    }
    initializeHeroCrisisVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = HERO_CRISIS_VFX_ATLAS.src;
        this.heroCrisisVfxAtlasImage = image;
    }
    initializePerfectEvadeTrailVfxAtlas() { if (typeof Image === 'undefined')
        return; const image = new Image(); image.decoding = 'async'; image.onload = () => { this.perfectEvadeTrailVfxAtlasReady = true; }; image.onerror = () => { this.perfectEvadeTrailVfxAtlasReady = false; }; image.src = PERFECT_EVADE_TRAIL_VFX_ATLAS.src; this.perfectEvadeTrailVfxAtlasImage = image; }
    initializeCrowdControlPropagationVfxAtlas() { if (typeof Image === 'undefined')
        return; const image = new Image(); image.decoding = 'async'; image.onload = () => { this.crowdControlPropagationVfxAtlasReady = true; }; image.onerror = () => { this.crowdControlPropagationVfxAtlasReady = false; }; image.src = CROWD_CONTROL_PROPAGATION_VFX_ATLAS.src; this.crowdControlPropagationVfxAtlasImage = image; }
    initializeBossCounterplayRewardVfxAtlas() { if (typeof Image === 'undefined')
        return; const image = new Image(); image.decoding = 'async'; image.onload = () => { this.bossCounterplayRewardVfxAtlasReady = true; }; image.onerror = () => { this.bossCounterplayRewardVfxAtlasReady = false; }; image.src = BOSS_COUNTERPLAY_REWARD_VFX_ATLAS.src; this.bossCounterplayRewardVfxAtlasImage = image; }
    initializeObjectiveCompletionCeremonyVfxAtlas() { if (typeof Image === 'undefined')
        return; const image = new Image(); image.decoding = 'async'; image.onload = () => { this.objectiveCompletionCeremonyVfxAtlasReady = true; }; image.onerror = () => { this.objectiveCompletionCeremonyVfxAtlasReady = false; }; image.src = OBJECTIVE_COMPLETION_CEREMONY_VFX_ATLAS.src; this.objectiveCompletionCeremonyVfxAtlasImage = image; }
    initializeUltimatePostImpactResidueVfxAtlas() { if (typeof Image === 'undefined')
        return; const image = new Image(); image.decoding = 'async'; image.onload = () => { this.ultimatePostImpactResidueVfxAtlasReady = true; }; image.onerror = () => { this.ultimatePostImpactResidueVfxAtlasReady = false; }; image.src = ULTIMATE_POST_IMPACT_RESIDUE_VFX_ATLAS.src; this.ultimatePostImpactResidueVfxAtlasImage = image; }
    initializeMapSafeLaneTransitionVfxAtlas() { if (typeof Image === 'undefined')
        return; const image = new Image(); image.decoding = 'async'; image.onload = () => { this.mapSafeLaneTransitionVfxAtlasReady = true; }; image.onerror = () => { this.mapSafeLaneTransitionVfxAtlasReady = false; }; image.src = MAP_SAFE_LANE_TRANSITION_VFX_ATLAS.src; this.mapSafeLaneTransitionVfxAtlasImage = image; }
    initializeObjectiveActivationMaterializationVfxAtlas() { if (typeof Image === 'undefined')
    ...
    initializeBossArenaTransitionWorldVfxAtlas() { if (typeof Image === 'undefined')
        return; const image = new Image(); image.decoding = 'async'; image.onload = () => { this.bossArenaTransitionWorldVfxAtlasReady = true; }; image.onerror = () => { this.bossArenaTransitionWorldVfxAtlasReady = false; }; image.src = BOSS_ARENA_TRANSITION_WORLD_VFX_ATLAS.src; this.bossArenaTransitionWorldVfxAtlasImage = image; }
    initializeMapCombatBoundaryWarningVfxAtlas() { if (typeof Image === 'undefined')
        return; const image = new Image(); image.decoding = 'async'; image.onload = () => { this.mapCombatBoundaryWarningVfxAtlasReady = true; }; image.onerror = () => { this.mapCombatBoundaryWarningVfxAtlasReady = false; }; image.src = MAP_COMBAT_BOUNDARY_WARNING_VFX_ATLAS.src; this.mapCombatBoundaryWarningVfxAtlasImage = image; }
    initializeObjectiveFailureDissolveVfxAtlas() { if (typeof Image === 'undefined')
        return; const image = new Image(); image.decoding = 'async'; image.onload = () => { this.objectiveFailureDissolveVfxAtlasReady = true; }; image.onerror = () => { this.objectiveFailureDissolveVfxAtlasReady = false; }; image.src = OBJECTIVE_FAILURE_DISSOLVE_VFX_ATLAS.src; this.objectiveFailureDissolveVfxAtlasImage = image; }
    initializeFieldEventLifecycleWorldVfxAtlas() { if (typeof Image === 'undefined')
    ...
    initializeBattlefieldAtmosphereVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = BATTLEFIELD_ATMOSPHERE_VFX_ATLAS.src;
        this.battlefieldAtmosphereVfxAtlasImage = image;
    }
    initializeBattlefieldDepthOverlayAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = BATTLEFIELD_DEPTH_OVERLAY_ATLAS.src;
        this.battlefieldDepthOverlayAtlasImage = image;
    }
    initializeBattlefieldEnvironmentReactionVfxAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = BATTLEFIELD_ENVIRONMENT_REACTION_VFX_ATLAS.src;
        this.battlefieldEnvironmentReactionVfxAtlasImage = image;
    }
    initializeBattlefieldMechanicAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = BATTLEFIELD_MECHANIC_ATLAS.src;
        this.battlefieldMechanicAtlasImage = image;
    }
    initializeAscensionTierPressureAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
        ...
        image.src = ASCENSION_TIER_PRESSURE_ATLAS.src;
        this.ascensionTierPressureAtlasImage = image;
    }
    initializeDeepRunDecisionIdentityAtlas() {
        if (typeof Image === 'undefined')
            return;
        const image = new Image();
        image.decoding = 'async';
    ...
    worldVfxLayerAlpha(priority) { return this.activeWorldVfxPriorityPolicy.alpha[priority]; }
    currentWorldVfxProtectedAnchors() {
        const anchors = [];
        if (this.dangerState.heroCritical)
            anchors.push({ x: this.hero.pos.x, y: this.hero.pos.y, radius: 112 });
        if (this.dangerState.coreCritical)
            anchors.push({ x: this.core.pos.x, y: this.core.pos.y, radius: 126 });
        return anchors;
    }
    worldVfxCueAlpha(priority, x, y, radius) {
    ...
    currentWorldVfxOccupancyPolicy(combatPrimary) {
        const candidates = [];
        const add = (id, priority, count, diameter) => { if (count > 0)
            candidates.push({ id, priority, area: count * Math.PI * (diameter * .5) ** 2 }); };
    ...
    worldVfxLayerAllowed(id) {
        return this.activeWorldVfxOccupancyPolicy.allowedIds.includes(id);
    }
    currentSecondaryCombatMotionPolicy(attention = this.currentCombatAttentionPolicy()) {
    ...
    currentResidualCombatMotionPolicy(attention, secondary) {
    ...
    clearBossResponseAcknowledgement() {
    ...
    get manuallyPaused() { return this.pauseState.has('manual'); }
    restart() {
    ...
    resetRun(heroId, traitId, retryBlueprint = null, replayPlan = null) {
        this.hapticArbiter.clear();
        this.input.clearStrategicActionArms();
        this.clearBufferedCastIntents();
        ...
        }, metaBonuses(this.metaProfile), runTraitBonuses(traitId));
        ...
        this.terrain.reset();
        if (retryBlueprint)
            this.terrain.restore(retryBlueprint.mapId, 0);
        this.feedback.reset();
        ...
        for (const key of Object.keys(this.actionReadyState))
            delete this.actionReadyState[key];
        for (const key of Object.keys(this.ultimatePulseUntil))
            delete this.ultimatePulseUntil[key];
    ...
    update(dt) {
        ...
        if (this.elapsed >= this.nextRecoveryJournalAt) {
            this.saveCurrentRunSnapshot();
            try {
                if (typeof window !== 'undefined' && this.resumeSnapshot)
                    appendRecoveryCheckpoint(this.storage, this.resumeSnapshot);
            }
        ...
        if (this.heroCastRenderCast > 0) {
            ...
            if (this.heroCastRenderCast <= 0)
                this.heroCastRenderRecover = Math.max(this.heroCastRenderRecover, 1);
        }
        else {
        ...
        if (nextThreat?.id !== this.threatDirective?.id) {
            ...
            if (nextThreat)
                this.showTacticalStatusEventToast(`전투 지시 · ${nextThreat.name}`, nextThreat.id);
        }
        else
            this.threatDirective = nextThreat;
        const eventTransition = this.fieldEvents.update(dt, this.elapsed, this.enemies.bossCountdown);
        if (eventTransition.started)
            this.handleFieldEventStart(eventTransition.started);
        if (eventTransition.ended)
            this.handleFieldEventEnd(eventTransition.ended);
        ...
        if (objectiveTransition.ended && this.objectiveRuntime.active) {
            ...
            if (failed.failed) {
                this.queueObjectiveFailureDissolveVfx(objectiveTransition.ended.id, failedAnchor.x, failedAnchor.y);
                this.showTacticalStatusEventToast('전장 목표 실패', objectiveTransition.ended.id);
            }
        }
        ...
        if (bossEntrance.stage && bossEntrance.stage !== this.lastOpeningBossEntranceStage) {
            ...
            if (bossEntrance.soundKind)
                this.audio.play(bossEntrance.soundKind);
        ...
        if (this.elapsed >= this.nextShopTokenAt) {
            ...
            this.shopTokens += 1;
            if (firstScheduledShopToken)
                this.showEventToast('상점권 획득 · 추천 구매 1탭');
        ...
        if ((desiredMove.x !== 0 || desiredMove.y !== 0) && this.onboarding.signal('move'))
        ...
        if (!isMoving && this.heroRenderLastMoving)
            this.heroRenderRecoveryBlend = Math.max(this.heroRenderRecoveryBlend, this.heroRenderMotionBlend || 0.85);
        ...
        this.enemies.update(dt, {
            ...
            onHeroDamage: (amount, source = 'contact') => {
                ...
                if (applied > 0) {
                    ...
                    const crisisKind = damageRatio >= .32 ? 'critical' : damageRatio >= .12 ? 'heavy' : 'hit';
                    if (beforeHpRatio > .22 && afterHpRatio <= .22)
                        this.queueHeroCrisisVfx('nearDeath');
                    else
                ...
                if (prevented >= this.hero.maxHp * .002)
                    this.queueSurvivalResponseVfx('heroGuard');
        ...
        if (evolved) {
            this.showBattlefieldEvolutionToast(evolved);
            this.emitMapEvolutionVfx(evolved);
        }
        ...
        this.continueDecisionSession();
        if (this.decisionSessionActive || this.paused || this.pauseState.paused || this.shopOverlay.isOpen)
            this.hapticArbiter.clear();
        else
            this.flushCombatHaptics();
        if (this.hero.hp <= 0 || this.core.hp <= 0)
            this.endRun();
        this.input.endFrame();
    }
    updateLongRunRewardRate() {
        if (this.elapsed - this.rewardRateWindowStartedAt < 60)
            return;
    ...
    processEnemyDeaths() {
        for (let wave = 0; wave < 6; wave++) {
            ...
            for (const death of deaths) {
                ...
                if (killChainCue) {
                    this.killChainCue = killChainCue;
                    this.killChainCueTimer = 0.72;
                ...
                if (this.enemyDefeatBodyTransitions.length > 20)
                    this.enemyDefeatBodyTransitions.splice(0, this.enemyDefeatBodyTransitions.length - 20);
                this.emitDeathPresentation(death);
                if (death.type === 'boss') {
                    const archetype = death.bossArchetype ?? 'inferno';
                    this.queueBossArenaTransitionWorldVfx(archetype, 'exit', death.x, death.y, 72);
                    const cinematic = bossLifecycleCinematicProfile(archetype, 'death');
                    for (let i = 0; i < cinematic.shockwaveCount; i++)
                        this.presentation.emitScreenEffect({ kind: 'shockwave', x: death.x, y: death.y, radius: 118 + i * 42, color: cinematic.color, ttl: .26 + i * .055, alpha: Math.min(.30, cinematic.flashAlpha), width: 4 + i * .6 });
                    ...
                    for (let i = 0; i < cinematic.rayCount; i++) {
                        const a = Math.PI * 2 * i / cinematic.rayCount;
                        this.presentation.emitTrail({ x1: death.x + Math.cos(a) * 38, y1: death.y + Math.sin(a) * 38, x2: death.x + Math.cos(a) * (118 + (i % 3) * 24), y2: death.y + Math.sin(a) * (118 + (i % 3) * 24), color: cinematic.color, ttl: .24, alpha: .56, width: 2.4 });
                    }
                    this.feedback.addCameraPressure('bossDeath');
                    const settle = bossSettleProfile(archetype, this.presentation.quality);
                    for (let i = 0; i < settle.rayCount; i++) {
                        const a = Math.PI * 2 * i / Math.max(1, settle.rayCount);
                        this.presentation.emitTrail({ x1: death.x + Math.cos(a) * settle.radius * .20, y1: death.y + Math.sin(a) * settle.radius * .20, x2: death.x + Math.cos(a) * settle.radius, y2: death.y + Math.sin(a) * settle.radius, color: settle.color, ttl: settle.ttl, alpha: settle.alpha, width: 1.6 + (i % 2) * .4 });
                    }
                ...
                if (death.type === 'golden') {
                    this.goldenGoblinEnemyId = null;
                    if (this.fieldEvents.active?.id === 'goldenGoblin') {
                        const ended = this.fieldEvents.completeActive(this.elapsed);
                        if (ended)
                            this.finishFieldEventLifecycleWorldVfx(ended);
                    }
                ...
                if (this.hero.profileId === 'arkan' && death.type !== 'boss') {
                    if (Math.random() < 0.18 + combatBuild.arkanExplosionChanceBonus)
                        this.triggerArkanExplosion(death, combatBuild);
                }
            }
        }
    }
    triggerArkanExplosion(death, build = this.currentCombatBuild()) {
        ...
        for (const enemy of this.enemies.enemies) {
            if (enemy.alive && Math.hypot(enemy.pos.x - death.x, enemy.pos.y - death.y) <= radius + enemy.radius)
                this.enemies.damage(enemy, damage, undefined, 'explosion');
        }
    }
    updateEndlessRuntime(dt) {
        ...
        for (const effect of result.effects)
            this.applyEndlessEffect(effect);
        ...
        if (!previousFinalForm && finalForm) {
            this.showEventToast(`최종 변신 · ${finalForm.name}`);
            this.showFinalFormTransformationCue(finalForm);
        }
    }
    applyEndlessEffect(effect) {
        if (effect.type === 'show_contract_offer')
            this.showEventToast('런 계약 도착 · 3가지 중 하나를 선택하세요');
            ...
            if (boon)
                this.showRunContractSuccessToast(boon.family);
            else
                this.showEventToast('런 계약 성공 · 90초 강화 획득');
        }
        else if (effect.type === 'contract_failed')
            this.showEventToast('런 계약 실패 · 다음 계약을 노리세요', null, null, null, null, null, effect.family);
        else if (effect.type === 'world_evolved')
            this.showEventToast(`전장 변이 · ${this.endlessWorldName(effect.world)}`, null, null, null, null, null, null, null, [], effect.world);
        else if (effect.type === 'ascension_tier')
            this.showAscensionTierEventToast(effect.tier);
        else if (effect.type === 'ascension_mutator')
            this.showAscensionMutatorEventToast(effect);
        ...
        else if (effect.type === 'show_hero_ascension_offer')
            this.showEventToast(`영웅 승천 · ${effect.milestone}분 선택 도착`);
            ...
            if (kind)
                this.eventToastOathHelper = oathRequirementBoonIdentity(kind);
        }
            ...
            if (boonId)
                this.eventToastOathHelper = { boonId };
        }
        else if (effect.type === 'oath_failed') {
            this.showEventToast(`서약 실패 · ${effect.title}`, null, null, null, null, longRunOathKindFromTitle(effect.title));
            this.eventToastOathHelper = null;
        }
        else if (effect.type === 'oath_expired') {
            this.showEventToast(`서약 종료 · ${effect.title}`, null, null, null, null, longRunOathKindFromTitle(effect.title));
            this.eventToastOathHelper = null;
        }
        ...
        else if (effect.type === 'run_milestone_recap')
            this.showEventToast(`${effect.title} · ${effect.headline} · K+${effect.killsDelta} B+${effect.bossesDelta}`);
    ...
    triggerFinalFormPattern(formId, color) {
        ...
        if (pattern.kind === 'chain') {
            ...
            for (const enemy of living) {
                if (distance(enemy.pos, this.hero.pos) > radius + enemy.radius)
                    continue;
                if (this.enemies.damage(enemy, baseDamage))
                    this.enemies.markLastDeathVisualSource('finalForm');
                if (pushDistance > 0)
        ...
        if (pattern.coreHealPercent > 0)
            this.core.hp = Math.min(this.core.maxHp, this.core.hp + this.core.maxHp * pattern.coreHealPercent);
    ...
    emitMythicTacticLinkFeedback(archetype) {
    ...
    continueDecisionSession(nowMs = this.decisionNowMs()) {
        ...
        if (kind === 'fate') {
            if (this.levelUpOverlay.isOpen)
                this.levelUpOverlay.close();
            const checkpoint = this.fateRuntime.choices.length;
            const renderDecision = (activeGeneration) => this.fateSelectOverlay.open(checkpoint, this.fateRuntime.choices, (id) => {
                this.finishDecisionPick(activeGeneration, () => {
                    const before = [...this.fateRuntime.choices];
                    const impact = fateChoiceImpact(before, id);
                    if (this.fateRuntime.choose(id)) {
                        this.showEventToast(`운명 선택 · ${fateHudSummary([id])}`, null, null, null, id);
                        this.eventToastFateImpact = impact;
                    }
                });
            });
            ...
            if (this.fateSelectOverlay.isOpen)
                this.fateSelectOverlay.hide();
            if (kind === 'heroAscension')
                this.openPendingHeroAscension(generation);
            else if (kind === 'runContract')
                this.openPendingEndlessContract(generation);
            else if (kind === 'bossReward')
                this.openNextBossReward(generation);
            else
                this.openNextLevelUp(generation);
        }
        return true;
    }
    finishDecisionPick(generation, apply) {
        const nowMs = this.decisionNowMs();
        if (!this.decisionPickGuard.accept(generation, nowMs))
            return;
        apply();
        this.continueDecisionSession(nowMs);
    }
    openPendingHeroAscension(generation) {
        ...
        const renderDecision = (activeGeneration) => this.levelUpOverlay.open(cards, (card) => {
            this.finishDecisionPick(activeGeneration, () => {
    ...
    openPendingEndlessContract(generation) {
    ...
    updateEndlessFieldNodes() {
        const nowMs = this.elapsed * 1000;
        const nodes = this.endlessState.world.nodes.filter((node) => node.expiresAtMs > nowMs);
        if (nodes.length !== this.endlessState.world.nodes.length)
            this.endlessState = { ...this.endlessState, world: { ...this.endlessState.world, nodes } };
        for (const node of nodes) {
            const pos = this.endlessFieldNodePosition(node);
            const radius = 24 + node.radius * 190;
            if (distance(this.hero.pos, pos) > this.hero.radius + radius)
                continue;
    ...
    drawEndlessFieldNodes(ctx, motion) {
        const nowMs = this.elapsed * 1000;
        ...
        const primaryFieldNodeId = motion.owner === 'field-node'
            ? activeNodes.reduce((best, node) => !best || distance(this.hero.pos, this.endlessFieldNodePosition(node)) < distance(this.hero.pos, this.endlessFieldNodePosition(best)) ? node : best, null)?.nodeId ?? null
            : null;
        for (const node of activeNodes) {
            ...
            ctx.save();
            ctx.globalAlpha = 0.30 + Math.sin(this.elapsed * 3 + node.x * 9) * amplitude;
            ctx.fillStyle = color;
            ctx.beginPath();
            ...
            if (this.battlefieldInteractionVfxAtlasReady && this.battlefieldInteractionVfxAtlasImage) {
                const icon = battlefieldInteractionSprite('field-node', node.kind);
                const size = Math.max(40, Math.min(58, radius * .82));
                ctx.globalAlpha = .96;
                ...
                const icon = fieldNodeIdentityIcon(node.kind);
                const size = Math.max(30, Math.min(42, radius * .62));
                ctx.globalAlpha = .96;
    ...
    currentMythicSafeZone(boss, destroyedRatio) {
    ...
    endlessBossEncounterModifiers(base) {
        ...
        for (const adaptation of getBossAdaptations(this.endlessState.nemesis, key)) {
            if (adaptation.kind === 'spell_guard')
                out.bossDamageTakenMultiplier = Math.max(0.72, out.bossDamageTakenMultiplier * (1 - adaptation.rank * 0.035));
            else if (adaptation.kind === 'mirror_affinity')
                out.bossDamageTakenMultiplier = Math.max(0.74, out.bossDamageTakenMultiplier * 0.94);
            else if (adaptation.kind === 'blink_hunt')
                out.dashDistanceMultiplier = Math.min(1.45, out.dashDistanceMultiplier * (1 + adaptation.rank * 0.05));
            else if (adaptation.kind === 'core_siege')
                out.summonCountMultiplier = Math.min(1.5, out.summonCountMultiplier * (1 + adaptation.rank * 0.05));
            else if (adaptation.kind === 'enrage_clock')
                out.specialCadenceMultiplier = Math.max(0.7, out.specialCadenceMultiplier * (1 - adaptation.rank * 0.04));
        }
    ...
    syncRelicResonanceRecallTracker() {
    ...
    updateRelicResonanceRecall() {
        ...
        if (current.tier > this.lastRelicResonanceTier) {
            ...
            if (badge)
                this.showEventToast(`유물 공명 · ${relicDefinition(this.activeRelic).name} · 공명 ${badge.label}`, null, null, null, null, null, null, { relicId: this.activeRelic, tier: badge.tier });
        }
        this.lastRelicResonanceTier = current.tier;
    }
    currentHeroFinalForm() {
    ...
    updateRunMission(dt) {
        ...
        if (transition.started) {
            this.showTacticalStatusEventToast(`미션 시작 · ${transition.started.name}`, transition.started.id);
            this.eventToastMissionReward = { ...transition.started.reward };
        }
        ...
        if (transition.failed)
            this.showRunMissionFailureToast(transition.failed);
    }
    updateCriticalFeedback() {
        const next = dangerUiState(this.hero.hp / Math.max(1, this.hero.maxHp), this.core.hp / Math.max(1, this.core.maxHp), this.dangerState);
        for (const event of criticalHapticEvents(this.dangerState, next)) {
            if (event === 'hero')
                this.hapticArbiter.queue('heroCritical');
            else
                this.hapticArbiter.queue('coreCritical');
        }
        this.dangerState = next;
        const bossWarning = this.enemies.bossCountdown > 0 && this.enemies.bossCountdown <= 8;
        if (bossWarning && !this.bossWarningHapticActive)
            this.hapticArbiter.queue('bossCountdown');
    ...
    flushCombatHaptics() {
        ...
        if (decision.pattern !== null)
            this.vibrate(decision.pattern);
    }
    vibrate(pattern) {
        if (!this.presentationSettings.haptics)
            return;
        try {
    ...
    render() {
        ...
        if (residualMotion.owner !== 'none' && secondaryMotion.owner === 'core-ambient')
            secondaryMotion = { ...secondaryMotion, owner: 'none', coreAmbientMotionAmplitude: 0 };
        ...
        if (this.enemies.bossCountdown > 0 && this.enemies.bossCountdown <= 8)
            this.drawBossWarning(ctx);
        if (this.bossPhaseCueTimer > 0 && this.bossPhaseCue)
            this.drawBossPhaseCue(ctx);
        if (this.killChainCueTimer > 0 && this.killChainCue)
            this.drawKillChainCue(ctx);
        this.drawTacticalStack(ctx);
        if (this.catastropheBannerTimer > 0 && this.catastrophe)
            this.drawCatastropheBanner(ctx);
        if (this.eventToastTimer > 0 && this.eventToast)
            this.drawEventToast(ctx);
    }
    updatePresentationQuality() {
        ...
        const thermal = thermalPolicyForEffectiveTier(desiredThermal, this.thermalRecoveryState.tier);
        if (adaptive.visualDensity < 0.56)
            next = 'low';
        else if (adaptive.visualDensity < 0.78 && next === 'high')
            next = 'medium';
        if (governor.maxQuality === 'low')
            next = 'low';
        else if (governor.maxQuality === 'medium' && next === 'high')
            next = 'medium';
        if (this.presentationSettings.quality === 'low')
            next = 'low';
        else if (this.presentationSettings.quality === 'medium' && next === 'high')
            next = 'medium';
    ...
    handleManualCastPress(action, spellWorld) {
        const request = this.castIntentBuffer.request(action, this.spells.cooldownRemaining(action));
        if (request !== 'ready')
            return;
        this.prepareManualTarget(spellWorld);
        if (this.spells.tryCast(action, { ...spellWorld, autoAim: false }))
            this.handleSuccessfulCast(action, 'manual');
    }
    flushBufferedManualCasts(spellWorld) {
        for (const action of COMBAT_CAST_ACTIONS) {
            if (!this.castIntentBuffer.consumeIfReady(action, this.spells.cooldownRemaining(action)))
                continue;
            this.prepareManualTarget(spellWorld);
            if (this.spells.tryCast(action, { ...spellWorld, autoAim: false }))
                this.handleSuccessfulCast(action, 'manual');
    ...
    recordBossResponseAcknowledgement(action, source) {
        if (source !== 'manual')
            return;
        const boss = this.enemies.enemies.find((enemy) => enemy.alive && enemy.type === 'boss') ?? null;
        if (!boss)
        ...
        if (!Number.isFinite(specialTimer) || specialTimer < 0 || specialTimer > 1.05)
            return;
        const archetype = boss.bossArchetype ?? 'inferno';
        if (!bossResponseActions(archetype).includes(action))
            return;
    ...
    handleSuccessfulCast(action, source = 'manual') {
        this.recordBossResponseAcknowledgement(action, source);
        if ((action === 'spell1' || action === 'spell2' || action === 'spell3' || action === 'spell4') && this.onboarding.signal('spell'))
            this.saveStoredOnboardingState();
        if ((action === 'ultimate1' || action === 'ultimate2') && this.onboarding.signal('ultimate'))
            this.saveStoredOnboardingState();
        ...
        const sound = action === 'ultimate1' || action === 'ultimate2' ? 'ultimate' : this.hero.profileId === 'arkan' ? 'fire' : this.hero.profileId === 'seria' ? 'ice' : this.hero.profileId === 'kain' ? 'lightning' : 'holy';
        ...
        if (flowForm && shouldEmitFlowCue(previousFlowStreak, this.finalFormFlow.streak)) {
            ...
            if (impact) {
                ...
                for (let i = 0; i < impact.particleCount; i++) {
                    const a = Math.PI * 2 * i / impact.particleCount;
        ...
        const spellByAction = { spell1: 'fireBolt', spell2: 'chainLightning', spell3: 'frostNova', spell4: 'flameField', ultimate1: 'meteorStorm', ultimate2: 'blackHole' };
        ...
        if (spellId) {
            ...
            for (const fusionId of fusionProcs)
                if (this.fusionRuntime.tryTrigger(fusionId))
                    this.triggerFusionProc(fusionId);
        }
    }
    triggerFusionProc(fusionId) {
        ...
        for (const enemy of this.enemies.enemies) {
            if (!enemy.alive || distance(enemy.pos, origin) > 105 + enemy.radius)
                continue;
    ...
    advanceHeroMeter(dt, signals) {
        ...
        if (transition.releaseShockwave)
            this.triggerEdricJudgment();
    }
    triggerEdricJudgment() {
        const radius = 260;
        const damage = 78 * this.hero.spellPower * this.hero.equipmentSpellPower;
        for (const enemy of this.enemies.enemies) {
            if (!enemy.alive || distance(enemy.pos, this.core.pos) > radius + enemy.radius)
                continue;
    ...
    triggerSeriaShatter(death) {
        const meter = heroMeterModifiers(this.heroMeter);
        if (meter.shatterRadius <= 0)
            return;
    ...
    emitSpellCastVfx(action) {
        ...
        for (let i = 0; i < residue.count; i++) {
            const t = residue.count <= 1 ? 0 : i / (residue.count - 1);
            const spread = (t - .5) * residue.spread;
            let a = facingAngle + spread;
            let vx = Math.cos(a) * residue.speed, vy = Math.sin(a) * residue.speed;
            if (residue.motion === 'frost-halo' || residue.motion === 'void-orbit') {
                a = Math.PI * 2 * i / Math.max(1, residue.count);
                vx = Math.cos(a) * residue.speed * (residue.motion === 'void-orbit' ? .45 : .7);
                vy = Math.sin(a) * residue.speed * (residue.motion === 'void-orbit' ? .45 : .7);
            }
            else if (residue.motion === 'heat-haze') {
                vx = (i - (residue.count - 1) / 2) * 3;
                vy = -residue.speed;
            }
            else if (residue.motion === 'meteor-smoke') {
                vx = -this.hero.facing.x * residue.speed * .45 + (i % 3 - 1) * 8;
                vy = -residue.speed * .68;
            }
            else if (residue.motion === 'ember-tail') {
                vx = -this.hero.facing.x * residue.speed + Math.sin(i * 1.7) * 8;
                vy = -this.hero.facing.y * residue.speed - 10;
            }
        ...
        for (let i = 0; i < echo.echoCount; i++) {
            const offset = (i - (echo.echoCount - 1) / 2) * echo.spacing;
            let a = echoAngle, x1 = launchOrigin.x, y1 = launchOrigin.y, x2 = launchOrigin.x, y2 = launchOrigin.y;
            if (echo.shape === 'halo-echo' || echo.shape === 'orbit-echo') {
                a = Math.PI * 2 * i / Math.max(1, echo.echoCount);
                x1 += Math.cos(a) * 18;
                y1 += Math.sin(a) * 18;
                x2 = x1 + Math.cos(a) * echo.length * .42;
                y2 = y1 + Math.sin(a) * echo.length * .42;
            }
            else if (echo.shape === 'drop-echo') {
                x1 += offset;
                y1 -= echo.length * .58;
                x2 = x1 + this.hero.facing.x * 10;
                y2 = launchOrigin.y + 12;
            }
            else {
                const perpX = -this.hero.facing.y, perpY = this.hero.facing.x;
                const fork = echo.shape === 'fork-echo' ? (i % 2 === 0 ? -.18 : .18) : 0;
                a = echoAngle + fork;
                x1 += perpX * offset;
                y1 += perpY * offset;
                x2 = x1 + Math.cos(a) * echo.length;
                y2 = y1 + Math.sin(a) * echo.length;
            }
        ...
        for (let i = 0; i < waveCount; i++)
            this.presentation.emitScreenEffect({ kind: 'shockwave', x: this.hero.pos.x, y: this.hero.pos.y, radius: descriptor.burstRadius * (0.8 + i * 0.22), color: descriptor.primary, ttl: 0.18 + i * 0.035, alpha: Math.min(descriptor.flashAlpha, 0.30), width: 2.5 + descriptor.tier });
        if (descriptor.screenPulse >= 0.5) {
            ...
            if (spellId === 'meteorStorm')
                this.feedback.addCameraPressure('meteor');
            if (spellId === 'blackHole')
                this.feedback.addCameraPressure('vortex');
        }
        ...
        if (descriptor.ultimate) {
            const aftermath = ultimateAftermathProfile(spellId, this.spells.levels[spellId], this.presentation.quality);
            for (let i = 0; i < aftermath.ringCount; i++)
                this.presentation.emitScreenEffect({ kind: 'pulse', x: this.hero.pos.x, y: this.hero.pos.y, radius: aftermath.radius * (.72 + i * .14), color: i % 2 ? aftermath.secondary : aftermath.color, ttl: aftermath.ttl * .55 + i * .025, alpha: aftermath.alpha * .72, width: 2 + i * .35 });
            for (let i = 0; i < aftermath.particleCount; i++) {
                const a = Math.PI * 2 * i / Math.max(1, aftermath.particleCount);
                const inward = aftermath.motion === 'collapse' ? -1 : 1;
                this.presentation.emitParticle({ x: this.hero.pos.x + Math.cos(a) * aftermath.radius * .35, y: this.hero.pos.y + Math.sin(a) * aftermath.radius * .35, vx: cosmeticMotionVelocity(Math.cos(a) * 26 * inward, this.presentationSettings.reducedMotion), vy: cosmeticMotionVelocity(Math.sin(a) * 26 * inward + (aftermath.motion === 'embers' ? -34 : 0), this.presentationSettings.reducedMotion), color: i % 2 ? aftermath.secondary : aftermath.color, ttl: aftermath.ttl, size: 2.5 + (i % 3) * .7, alpha: aftermath.alpha });
            }
        }
    }
    emitDeathPresentation(death) {
        if (isEnemyCombatVfxType(death.type))
            this.enemyDeathImageBursts.push({ x: death.x, y: death.y, type: death.type, startedAt: this.elapsed, until: this.elapsed + 0.34 });
        ...
        for (let i = 0; i < rays; i++) {
            const a = Math.PI * 2 * i / Math.max(1, rays);
            const inner = cue.radius * .25, outer = cue.radius * (.78 + (i % 3) * .12);
            this.presentation.emitTrail({ x1: death.x + Math.cos(a) * inner, y1: death.y + Math.sin(a) * inner, x2: death.x + Math.cos(a) * outer, y2: death.y + Math.sin(a) * outer, color: cue.color, ttl: .16, alpha: .46, width: cue.motif === 'shield' || cue.motif === 'fracture' ? 3 : 2 });
        }
        const afterglow = deathAfterglowProfile(death.type, this.presentation.quality);
        if (afterglow.alpha > 0)
            this.presentation.emitScreenEffect({ kind: 'glow', x: death.x, y: death.y, radius: afterglow.radius, color: afterglow.color, ttl: afterglow.ttl, alpha: afterglow.alpha });
        for (let i = 0; i < afterglow.particleCount; i++) {
            const a = Math.PI * 2 * i / Math.max(1, afterglow.particleCount);
            this.presentation.emitParticle({ x: death.x, y: death.y, vx: cosmeticMotionVelocity(Math.cos(a) * afterglow.drift, this.presentationSettings.reducedMotion), vy: cosmeticMotionVelocity(Math.sin(a) * afterglow.drift * .55 - 8, this.presentationSettings.reducedMotion), color: afterglow.color, ttl: afterglow.ttl, size: 1.8 + (i % 2), alpha: afterglow.alpha * .92 });
        }
    }
    drawEnemyDefeatBodyTransitions(ctx) {
        ...
        for (const cue of this.enemyDefeatBodyTransitions) {
            ...
            if (retirement && retirement.shadowAlpha > 0) {
                ctx.save();
                ctx.translate(cue.death.x + retirement.shadowOffsetX, cue.death.y + retirement.shadowOffsetY);
                ctx.globalAlpha = retirement.shadowAlpha;
                ctx.fillStyle = 'rgba(8,12,18,.92)';
                ctx.beginPath();
                ctx.ellipse(0, pose.radius + 8, pose.radius * 1.05 * retirement.widthScale, pose.radius * retirement.heightScale, 0, 0, Math.PI * 2);
                ctx.fill();
                ctx.restore();
            }
            if (retirement && retirement.groundPulseScale > 0.01) {
                ctx.save();
                ctx.globalAlpha = .14 * retirement.groundPulseScale;
                ctx.strokeStyle = 'rgba(190,214,240,.5)';
                ctx.beginPath();
                ctx.ellipse(cue.death.x, cue.death.y + pose.radius + 8, pose.radius * 1.25, pose.radius * .28, 0, 0, Math.PI * 2);
                ctx.stroke();
                ctx.restore();
            }
            ctx.save();
            ...
            if (spritePresentation.visible && this.enemySpriteAtlasImage) {
                ...
                ctx.fillStyle = 'rgba(145,158,176,.52)';
                ctx.beginPath();
    ...
    drawEnemyCombatImageVfx(ctx) {
        ...
        for (const enemy of this.enemies.enemies) {
            ...
            ctx.save();
            ctx.globalAlpha = Math.min(alphaCap, 0.22 + strength * 0.56);
        ...
        for (const burst of this.enemyDeathImageBursts) {
            ...
            ctx.save();
            ctx.globalAlpha = Math.min(alphaCap, (1 - progress) * 0.82);
    ...
    updateBossPresentation() {
        for (const enemy of this.enemies.enemies) {
            ...
            if (!this.seenBossIds.has(enemy.id)) {
                ...
                for (let i = 0; i < cinematic.shockwaveCount; i++)
                    this.presentation.emitScreenEffect({ kind: 'shockwave', x: enemy.pos.x, y: enemy.pos.y, radius: 94 + i * 38, color: cinematic.color, ttl: .22 + i * .045, alpha: Math.min(.24, cinematic.flashAlpha), width: 3.5 + i * .6 });
            ...
            {
                const maxTtl = 1.16;
                this.bossPhaseTransitionVfx.push({ archetype, phase: cue.phase, x: enemy.pos.x, y: enemy.pos.y, ttl: maxTtl, maxTtl });
                if (this.bossPhaseTransitionVfx.length > 12)
                    this.bossPhaseTransitionVfx.splice(0, this.bossPhaseTransitionVfx.length - 12);
            }
            ...
            for (let i = 0; i < cue.cinematic.shockwaveCount; i++)
                this.presentation.emitScreenEffect({ kind: 'shockwave', x: enemy.pos.x, y: enemy.pos.y, radius: cue.ringRadius * (0.82 + i * 0.22), color: cue.color, ttl: 0.22 + i * 0.05, alpha: Math.min(0.30, cue.cinematic.edgePulseAlpha), width: 4 + i });
    ...
    updateMapEnvironmentVfx(dt) {
        ...
        if (previous <= .35 && current > .35)
            this.queueHeroCrisisVfx('recovery');
        this.lastHeroCrisisHpRatio = current;
        for (const cue of this.bossWeakpointBreakWorldVfx)
            cue.ttl -= safeDt;
        this.bossWeakpointBreakWorldVfx = this.bossWeakpointBreakWorldVfx.filter((cue) => cue.ttl > 0);
        for (const cue of this.battlefieldEnvironmentReactionVfx)
            cue.ttl -= safeDt;
        ...
        while (this.mapVfxAccumulator >= 1 && emitted < 3) {
            this.mapVfxAccumulator -= 1;
            emitted += 1;
            this.mapVfxSequence += 1;
            ...
            const layer = this.mapVfxSequence % Math.max(1, ambient.layers);
            const layerScale = 1 - layer * .16;
            const drift = Math.sin(phase * 0.73) * 12 * layerScale;
    ...
    emitMapEvolutionVfx(stage) {
        if (stage > 0) {
            const maxTtl = 1.55;
            this.mapEvolutionAftermathVfx.push({ mapId: this.terrain.currentLayout.id, stage: stage, x: LOGICAL_WIDTH / 2, y: LOGICAL_HEIGHT / 2, size: Math.min(LOGICAL_WIDTH, LOGICAL_HEIGHT) * 0.72, ttl: maxTtl, maxTtl });
            if (this.mapEvolutionAftermathVfx.length > 8)
                this.mapEvolutionAftermathVfx.splice(0, this.mapEvolutionAftermathVfx.length - 8);
        }
        ...
        for (let i = 0; i < collapse.debrisCount; i++) {
            const a = Math.PI * 2 * i / collapse.debrisCount;
            this.presentation.emitParticle({ x: LOGICAL_WIDTH / 2, y: LOGICAL_HEIGHT / 2, vx: cosmeticMotionVelocity(Math.cos(a) * collapse.debrisSpeed, this.presentationSettings.reducedMotion), vy: cosmeticMotionVelocity(Math.sin(a) * collapse.debrisSpeed, this.presentationSettings.reducedMotion), color: collapse.color, ttl: .38, size: 2.5 + (i % 3), alpha: .44 });
        }
    }
    emitTerrainDestructionVfx() {
        for (const event of this.terrain.drainPresentationEvents()) {
            if (event.kind === 'crystalBlast') {
                const maxTtl = .72;
                this.crystalInteractionLifecycleVfx.push({ mapId: this.terrain.currentLayout.id, x: event.x, y: event.y, size: Math.max(132, event.radius * 2.45), ttl: maxTtl, maxTtl });
                if (this.crystalInteractionLifecycleVfx.length > 12)
                    this.crystalInteractionLifecycleVfx.splice(0, this.crystalInteractionLifecycleVfx.length - 12);
            }
            ...
            for (let i = 0; i < d.waveCount; i++)
                this.presentation.emitScreenEffect({ kind: 'shockwave', x: event.x, y: event.y, radius: event.radius * (.72 + i * .22), color: d.color, ttl: .18 + i * .05, alpha: Math.min(.28, d.glowAlpha), width: 3 + i });
            ...
            for (let i = 0; i < d.debrisCount; i++) {
                const a = Math.PI * 2 * i / d.debrisCount;
                this.presentation.emitParticle({ x: event.x, y: event.y, vx: cosmeticMotionVelocity(Math.cos(a) * d.debrisSpeed, this.presentationSettings.reducedMotion), vy: cosmeticMotionVelocity(Math.sin(a) * d.debrisSpeed, this.presentationSettings.reducedMotion), color: d.color, ttl: .34, size: 2.5 + (i % 3), alpha: .58 });
            }
        }
    }
    queueBattlefieldEnvironmentReactionVfx(kind, x, y, size) {
        ...
        if (this.battlefieldEnvironmentReactionVfx.length > 12)
            this.battlefieldEnvironmentReactionVfx.splice(0, this.battlefieldEnvironmentReactionVfx.length - 12);
    }
    queueBossHazardAftermathVfx(kind, x, y, radius) {
        const maxTtl = .78;
        this.bossHazardAftermathVfx.push({ kind, x, y, radius: Math.max(42, radius), ttl: maxTtl, maxTtl });
        if (this.bossHazardAftermathVfx.length > 16)
            this.bossHazardAftermathVfx.splice(0, this.bossHazardAftermathVfx.length - 16);
        const memoryMaxTtl = 1.25;
        this.bossHazardClearedGroundMemory.push({ kind, x, y, radius: Math.max(42, radius), ttl: memoryMaxTtl, maxTtl: memoryMaxTtl });
        if (this.bossHazardClearedGroundMemory.length > 16)
            this.bossHazardClearedGroundMemory.splice(0, this.bossHazardClearedGroundMemory.length - 16);
    }
    attachBossHazardClearedGeometry(geometryShape, angle, length) {
        const apply = (cue) => { if (!cue)
            return; if (geometryShape)
            cue.geometryShape = geometryShape; if (angle !== undefined)
            cue.angle = angle; if (length !== undefined)
            cue.length = length; };
    ...
    queueHeroCrisisVfx(state) {
        const maxTtl = state === 'nearDeath' ? .92 : state === 'recovery' ? .72 : state === 'critical' ? .54 : state === 'heavy' ? .42 : .30;
        this.heroCrisisVfx.push({ heroId: this.hero.profileId, state, x: this.hero.pos.x, y: this.hero.pos.y, ttl: maxTtl, maxTtl });
        if (this.heroCrisisVfx.length > 12)
            this.heroCrisisVfx.splice(0, this.heroCrisisVfx.length - 12);
    }
    drawBossHazardAftermathVfx(ctx) {
        if (!this.worldVfxLayerAllowed('boss-hazard-aftermath'))
            return;
    ...
    drawBossHazardClearedGroundMemory(ctx) {
        ...
        for (const cue of this.bossHazardClearedGroundMemory) {
            ...
            const forecastLane = bossClearedSafeLaneForecastTarget({ currentTarget: this.currentMythicSafeLanePresentation?.target, currentConfidence: this.currentMythicSafeLanePresentation?.confidence, nextTarget: this.currentMythicSafeLanePresentation?.forecastTarget, forecastUrgency: this.currentMythicSafeLanePresentation?.forecastUrgency, transitionMs: this.currentMythicSafeLanePresentation?.forecastTransitionMs }, this.presentationSettings.reducedFlash);
            const agreement = bossClearedSafeLaneArbitrationPresentation({ shape: geometry.shape, center: { x: cue.x, y: cue.y }, angle: geometry.angle, halfLength: geometry.halfLength, halfWidth: geometry.halfWidth, safeLaneTarget: forecastLane.target, safeLaneConfidence: forecastLane.confidence }, this.presentationSettings.reducedFlash), clearedAlphaScale = agreement.clearedAlphaScale;
            if (clearedAlphaScale <= 0)
                continue;
            ctx.save();
            ...
            if (geometry.shape === 'circle')
                ...
                ctx.rect(-geometry.halfLength, -geometry.halfWidth, geometry.halfLength * 2, geometry.halfWidth * 2);
            else {
                ctx.moveTo(-geometry.halfLength, 0);
                ctx.lineTo(geometry.halfLength, 0);
                ctx.moveTo(0, -geometry.halfLength);
                ctx.lineTo(0, geometry.halfLength);
            }
            ctx.stroke();
            ctx.setLineDash([]);
            ctx.restore();
    ...
    drawHeroCrisisVfx(ctx) {
        if (!this.heroCrisisVfxAtlasReady || !this.heroCrisisVfxAtlasImage)
            return;
        for (const cue of this.heroCrisisVfx) {
            const progress = 1 - Math.max(0, cue.ttl / cue.maxTtl);
            const sprite = heroCrisisVfxSprite(cue.heroId, cue.state);
            const anchorActive = cue.state === 'nearDeath' || cue.state === 'recovery';
            const x = anchorActive ? this.hero.pos.x : cue.x, y = anchorActive ? this.hero.pos.y : cue.y;
            const size = (cue.state === 'nearDeath' ? 126 : cue.state === 'critical' ? 112 : cue.state === 'heavy' ? 96 : cue.state === 'recovery' ? 108 : 84) * (1 + progress * .16);
            ctx.save();
            ctx.globalAlpha = Math.min(this.presentationSettings.reducedFlash ? 0.38 : 0.7, (1 - progress) * .82 + .05);
            ctx.drawImage(this.heroCrisisVfxAtlasImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, x - size / 2, y - size / 2, size, size);
            ctx.restore();
        }
    }
    queuePerfectEvadeTrailVfx(dirX, dirY) { const mag = Math.hypot(dirX, dirY) || 1, maxTtl = .78; this.perfectEvadeTrailVfx.push({ heroId: this.hero.profileId, x: this.hero.pos.x, y: this.hero.pos.y, dirX: dirX / mag, dirY: dirY / mag, ttl: maxTtl, maxTtl }); if (this.perfectEvadeTrailVfx.length > 12)
        this.perfectEvadeTrailVfx.splice(0, this.perfectEvadeTrailVfx.length - 12); }
    drawPerfectEvadeTrailVfx(ctx) { if (!this.perfectEvadeTrailVfxAtlasReady || !this.perfectEvadeTrailVfxAtlasImage)
        return; for (const cue of this.perfectEvadeTrailVfx) {
        const progress = 1 - Math.max(0, cue.ttl / cue.maxTtl), state = progress < 0.28 ? 'escape' : progress < 0.72 ? 'slipstream' : 'success', sprite = perfectEvadeTrailVfxSprite(cue.heroId, state), angle = Math.atan2(cue.dirY, cue.dirX), travel = state === 'success' ? 18 : progress * 64, x = cue.x + cue.dirX * travel, y = cue.y + cue.dirY * travel, size = state === 'slipstream' ? 126 : 108;
        ctx.save();
        ctx.translate(x, y);
        ctx.rotate(angle);
        ctx.globalAlpha = Math.min(this.presentationSettings.reducedFlash ? 0.34 : 0.68, (1 - progress) * .78 + .08);
        ctx.drawImage(this.perfectEvadeTrailVfxAtlasImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, -size / 2, -size / 2, size, size);
        ctx.restore();
    } }
    queueBossCounterplayRewardBurstVfx(archetype, x, y) { const maxTtl = .68; this.bossCounterplayRewardBurstVfx.push({ archetype, x, y, ttl: maxTtl, maxTtl }); if (this.bossCounterplayRewardBurstVfx.length > 8)
        this.bossCounterplayRewardBurstVfx.splice(0, this.bossCounterplayRewardBurstVfx.length - 8); }
    drawBossCounterplayRewardVfx(ctx) { if (!this.bossCounterplayRewardVfxAtlasReady || !this.bossCounterplayRewardVfxAtlasImage)
        return; for (const cue of this.bossCounterplayRewardBurstVfx) {
        const progress = 1 - Math.max(0, cue.ttl / cue.maxTtl), sprite = bossCounterplayRewardVfxSprite(cue.archetype, 'burst'), size = 138 * (1 + progress * .38);
        ctx.save();
        ctx.globalAlpha = Math.min(this.presentationSettings.reducedFlash ? 0.36 : 0.72, (1 - progress) * .84 + .06);
        ctx.drawImage(this.bossCounterplayRewardVfxAtlasImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, cue.x - size / 2, cue.y - size / 2, size, size);
        ctx.restore();
    } const archetype = this.bossEncounter.archetype; if (!archetype || !bossCounterplayBenefitActive(archetype, this.bossEncounter.modifiers))
        return; const boss = this.enemies.enemies.find(enemy => enemy.alive && enemy.id === this.bossEncounter.activeBossId); if (!boss)
        return; const sprite = bossCounterplayRewardVfxSprite(archetype, 'aura'), size = Math.max(170, boss.radius * 4.4); ctx.save(); ctx.globalAlpha = this.presentationSettings.reducedFlash ? 0.30 : 0.48; ctx.drawImage(this.bossCounterplayRewardVfxAtlasImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, boss.pos.x - size / 2, boss.pos.y - size / 2, size, size); ctx.restore(); }
    drawBossPhaseTransitionVfx(ctx) {
        if (!this.bossPhaseAftermathVfxAtlasReady || !this.bossPhaseAftermathVfxAtlasImage)
            return;
        for (const cue of this.bossPhaseTransitionVfx) {
            const progress = 1 - Math.max(0, cue.ttl / cue.maxTtl);
            const state = progress < 0.34 ? 'burst' : 'aftermath';
            const sprite = bossPhaseAftermathVfxSprite(cue.archetype, cue.phase, state);
            const scale = state === 'burst' ? (1 + progress * .38) : (1.12 + progress * .18);
            const size = (cue.phase === 3 ? 238 : 204) * scale;
            ctx.save();
            ctx.globalAlpha = Math.min(this.presentationSettings.reducedFlash ? 0.42 : 0.72, (1 - progress) * 0.86 + 0.08);
            ctx.drawImage(this.bossPhaseAftermathVfxAtlasImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, cue.x - size / 2, cue.y - size / 2, size, size);
            ctx.restore();
        }
    }
    drawMapEvolutionAftermathVfx(ctx) {
        if (!this.worldVfxLayerAllowed('map-evolution-aftermath'))
            return;
        const layerAlpha = this.worldVfxLayerAlpha('decorative');
        if (layerAlpha <= 0)
            return;
        if (!this.mapEvolutionAftermathVfxAtlasReady || !this.mapEvolutionAftermathVfxAtlasImage)
            return;
        for (const cue of this.mapEvolutionAftermathVfx) {
            const progress = 1 - Math.max(0, cue.ttl / cue.maxTtl);
            const state = progress < 0.24 ? 'collapse' : progress < 0.62 ? 'debris' : 'settle';
            const sprite = mapEvolutionAftermathVfxSprite(cue.mapId, cue.stage, state);
            const size = cue.size * (state === 'collapse' ? 0.82 + progress * .55 : state === 'debris' ? 1.02 : 1.08);
            ctx.save();
            ctx.globalAlpha = Math.min(this.presentationSettings.reducedFlash ? 0.34 : 0.56, (1 - progress) * 0.62 + 0.10) * layerAlpha;
            ctx.drawImage(this.mapEvolutionAftermathVfxAtlasImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, cue.x - size / 2, cue.y - size / 2, size, size);
            ctx.restore();
        }
    }
    drawCrystalInteractionLifecycleVfx(ctx) {
        if (!this.crystalInteractionLifecycleVfxAtlasReady || !this.crystalInteractionLifecycleVfxAtlasImage)
            return;
        for (const cue of this.crystalInteractionLifecycleVfx) {
            const progress = 1 - Math.max(0, cue.ttl / cue.maxTtl);
            const state = progress < 0.45 ? 'blast' : 'afterglow';
            const sprite = crystalInteractionLifecycleVfxSprite(cue.mapId, state);
            const size = cue.size * (1 + progress * .22);
            ctx.save();
            ctx.globalAlpha = Math.min(this.presentationSettings.reducedFlash ? .46 : .78, (1 - progress) * .86);
            ctx.drawImage(this.crystalInteractionLifecycleVfxAtlasImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, cue.x - size / 2, cue.y - size / 2, size, size);
            ctx.restore();
        }
    }
    drawBattlefieldEnvironmentReactionVfx(ctx) {
        if (!this.battlefieldEnvironmentReactionVfxAtlasReady || !this.battlefieldEnvironmentReactionVfxAtlasImage)
            return;
        for (const cue of this.battlefieldEnvironmentReactionVfx) {
            const sprite = battlefieldEnvironmentReactionVfxSprite(cue.mapId, cue.kind);
            const t = Math.max(0, Math.min(1, cue.ttl / cue.maxTtl));
            ctx.save();
            ctx.globalAlpha = Math.min(this.presentationSettings.reducedFlash ? .52 : .78, .18 + t * .62);
            ctx.drawImage(this.battlefieldEnvironmentReactionVfxAtlasImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, cue.x - cue.size / 2, cue.y - cue.size / 2, cue.size, cue.size);
            ctx.restore();
        }
    }
    drawBattlefieldAtmosphereVfx(ctx) {
        ...
        if (layerAlpha <= 0)
            return;
        if (!this.battlefieldAtmosphereVfxAtlasReady || !this.battlefieldAtmosphereVfxAtlasImage)
            return;
        ...
        ctx.save();
        ctx.globalAlpha = (this.presentationSettings.quality === 'low' ? .16 : this.presentationSettings.quality === 'medium' ? .22 : .28) * layerAlpha;
        ctx.drawImage(this.battlefieldAtmosphereVfxAtlasImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, -8 + drift, -5, LOGICAL_WIDTH + 16, LOGICAL_HEIGHT + 10);
        ctx.restore();
    }
    drawBattlefieldDepthOverlays(ctx) {
        const layerAlpha = this.worldVfxLayerAlpha('decorative');
        if (layerAlpha <= 0)
            return;
        if (!this.battlefieldDepthOverlayAtlasReady || !this.battlefieldDepthOverlayAtlasImage)
            return;
    ...
    drawBossHealthPressure(ctx) {
        ...
        if (profile.edgeAlpha <= 0)
            return;
        ctx.save();
        ctx.strokeStyle = profile.color;
        ctx.lineWidth = (profile.tier === 'desperate' ? 5 : 3) * envelope.lineWidthScale;
        ctx.globalAlpha = profile.edgeAlpha * envelope.edgeScale * focus.bossPressureScale;
        ctx.strokeRect(profile.inset, profile.inset, LOGICAL_WIDTH - profile.inset * 2, LOGICAL_HEIGHT - profile.inset * 2);
        ctx.globalAlpha = profile.glowAlpha * envelope.glowScale * focus.bossPressureScale;
        ctx.lineWidth = 12 * envelope.lineWidthScale;
        ctx.strokeRect(profile.inset + 8, profile.inset + 8, LOGICAL_WIDTH - (profile.inset + 8) * 2, LOGICAL_HEIGHT - (profile.inset + 8) * 2);
        ctx.restore();
    }
    drawKillChainCue(ctx) {
        if (!this.killChainCue)
            return;
        const alpha = Math.min(1, this.killChainCueTimer / 0.18);
        ctx.save();
        ctx.globalAlpha = this.presentationSettings.reducedFlash ? Math.min(0.72, alpha) : alpha;
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.font = `900 ${18 + this.killChainCue.tier * 3}px system-ui`;
        ctx.fillStyle = this.killChainCue.color;
    ...
    drawBossPhaseCue(ctx) {
        if (!this.bossPhaseCue)
            return;
        const alpha = Math.min(1, this.bossPhaseCueTimer / 0.28);
        ctx.save();
        ctx.globalAlpha = this.presentationSettings.reducedFlash ? Math.min(0.58, alpha) : alpha;
        ...
        ctx.save();
        ctx.globalAlpha = vignetteAlpha * alpha;
        ctx.strokeStyle = this.bossPhaseCue.color;
        ctx.lineWidth = 20;
        ctx.strokeRect(10, 10, LOGICAL_WIDTH - 20, LOGICAL_HEIGHT - 20);
        ctx.restore();
        ctx.fillStyle = 'rgba(5,8,15,.88)';
        ctx.fillRect(570, 178, 460, 56);
        this.drawBossPhaseEscalationCueIcon(ctx);
        ctx.strokeStyle = this.bossPhaseCue.color;
        ctx.lineWidth = 3;
        ctx.strokeRect(570, 178, 460, 56);
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillStyle = '#fff';
        ctx.font = '900 22px system-ui';
        ctx.fillText(this.bossPhaseCue.title, 800, 206);
        ctx.restore();
    }
    createPresentationControls(parent) {
        ...
        const sync = () => {
            ...
            for (const def of defs) {
                ...
                if (def.pressed !== null)
                    button.setAttribute('aria-pressed', String(def.pressed));
    ...
    drawArena(ctx) {
        ...
        for (let x = 80; x < LOGICAL_WIDTH; x += 80) {
            ctx.beginPath();
            ctx.moveTo(x, 80);
            ctx.lineTo(x, LOGICAL_HEIGHT - 40);
            ctx.stroke();
        }
        for (let y = 100; y < LOGICAL_HEIGHT; y += 80) {
            ctx.beginPath();
            ctx.moveTo(40, y);
            ctx.lineTo(LOGICAL_WIDTH - 40, y);
            ctx.stroke();
        }
    ...
    drawCore(ctx, motion) {
        ...
        if (this.hero.profileId === 'edric' && distance(this.hero.pos, this.core.pos) <= auraRadius) {
            ctx.strokeStyle = 'rgba(244,205,105,.25)';
            ctx.lineWidth = 5;
            ctx.beginPath();
            ctx.arc(0, 0, auraRadius, 0, Math.PI * 2);
            ctx.stroke();
        }
        ctx.strokeStyle = 'rgba(79,209,255,.18)';
        ctx.lineWidth = 3;
        ctx.beginPath();
        ...
        grad.addColorStop(.3, '#68d7ff');
        grad.addColorStop(1, '#154a78');
        ctx.fillStyle = grad;
        ctx.beginPath();
    ...
    drawBossSignatureVfx(ctx) {
        if (!this.bossSignatureVfxAtlasReady || !this.bossSignatureVfxAtlasImage)
            return;
        const boss = this.enemies.enemies.find((enemy) => enemy.alive && enemy.type === 'boss');
        if (!boss)
            return;
        ...
        if (!entrance && specialCharge <= 0 && !phaseActive)
            return;
    ...
    drawTerrainSpriteOverlays(ctx, motion) {
        ...
        for (const crystal of this.terrain.crystals) {
            ...
            if (active && crystal.charge > 0 && this.crystalInteractionLifecycleVfxAtlasReady && this.crystalInteractionLifecycleVfxAtlasImage) {
                ...
                ctx.save();
                ctx.globalAlpha = chargeRatio >= 0.7 ? 0.78 : 0.52;
                ctx.drawImage(this.crystalInteractionLifecycleVfxAtlasImage, interactionSprite.sx, interactionSprite.sy, interactionSprite.sw, interactionSprite.sh, crystal.x - interactionSize / 2, crystal.y - interactionSize * .56, interactionSize, interactionSize);
                ctx.restore();
            }
            if (active) {
                ctx.globalAlpha = 1;
    ...
    queueFinalFormWorldVfx(formId, state, x, y, radius) {
        const maxTtl = state === 'flow' ? .72 : .6;
        this.finalFormWorldVfx.push({ formId: formId, state, x, y, size: Math.max(220, Math.min(520, radius * 1.55)), ttl: maxTtl, maxTtl });
        if (this.finalFormWorldVfx.length > 24)
            this.finalFormWorldVfx.splice(0, this.finalFormWorldVfx.length - 24);
    }
    drawFinalFormWorldVfx(ctx) {
        if (!this.finalFormWorldVfxAtlasReady || !this.finalFormWorldVfxAtlasImage)
            return;
        for (const vfx of this.finalFormWorldVfx) {
            ...
            ctx.save();
            ctx.globalAlpha = Math.min(this.presentationSettings.reducedFlash ? .46 : baseAlpha, t * baseAlpha);
            ctx.drawImage(this.finalFormWorldVfxAtlasImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, vfx.x - size / 2, vfx.y - size / 2, size, size);
            ctx.restore();
        }
    }
    queueFusionWorldVfx(fusionId, origin) {
        const maxTtl = .68;
        this.fusionWorldVfx.push({ fusionId, x: origin.x, y: origin.y, ttl: maxTtl, maxTtl });
        if (this.fusionWorldVfx.length > 32)
            this.fusionWorldVfx.splice(0, this.fusionWorldVfx.length - 32);
    }
    drawFusionWorldVfx(ctx) {
        if (!this.fusionWorldVfxAtlasReady || !this.fusionWorldVfxAtlasImage)
            return;
        for (const vfx of this.fusionWorldVfx) {
            ...
            ctx.save();
            ctx.globalAlpha = Math.min(this.presentationSettings.reducedFlash ? .48 : .9, Math.max(.12, alpha));
            ctx.drawImage(this.fusionWorldVfxAtlasImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, vfx.x - size / 2, vfx.y - size / 2, size, size);
            ctx.restore();
        }
    }
    queueHeroMeterWorldVfx(heroId) {
        const maxTtl = .66;
        this.heroMeterWorldVfx.push({ heroId, x: this.hero.pos.x, y: this.hero.pos.y, ttl: maxTtl, maxTtl });
        if (this.heroMeterWorldVfx.length > 12)
            this.heroMeterWorldVfx.splice(0, this.heroMeterWorldVfx.length - 12);
    }
    drawHeroMeterWorldVfx(ctx) {
        if (!this.heroMeterWorldVfxAtlasReady || !this.heroMeterWorldVfxAtlasImage)
            return;
        if (this.heroMeter.activeTimer > 0) {
            ...
            ctx.save();
            ctx.globalAlpha = this.presentationSettings.reducedFlash ? .24 : .42;
            ctx.drawImage(this.heroMeterWorldVfxAtlasImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, this.hero.pos.x - size / 2, this.hero.pos.y - size / 2, size, size);
            ctx.restore();
        }
        for (const vfx of this.heroMeterWorldVfx) {
            ...
            ctx.save();
            ctx.globalAlpha = Math.min(this.presentationSettings.reducedFlash ? .42 : .86, t * .9);
            ctx.drawImage(this.heroMeterWorldVfxAtlasImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, vfx.x - size / 2, vfx.y - size / 2, size, size);
            ctx.restore();
        }
    }
    drawSurvivalResponseVfx(ctx) {
        ...
        for (const [cueIndex, cue] of this.survivalResponseVfx.entries()) {
            ...
            if (cue.kind === 'coreGuard') {
                ...
                arbitrationAlphaScale = arbitration.survivalAlphaScale;
    ...
    queueFreezeShatterVfx(type, x, y) {
        ...
        if (this.freezeShatterVfx.length > 16)
            this.freezeShatterVfx.splice(0, this.freezeShatterVfx.length - 16);
    }
    drawFreezeShatterVfx(ctx) {
        ...
        for (const cue of this.freezeShatterVfx) {
            ...
            ctx.drawImage(this.freezeControlVfxAtlasImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, cue.x - size / 2, cue.y - size / 2, size, size);
            ctx.restore();
        }
    }
    queueHeroResponseVfx(kind, _intensity = 1) { const maxTtl = kind === 'hit' ? .32 : kind === 'perfectEvade' ? .48 : .42; if (kind === 'hit') {
        this.heroRenderHitRecoil = Math.max(this.heroRenderHitRecoil, Math.max(0, Math.min(1.25, _intensity)));
        this.heroCrisisGroundSettleState = advanceHeroCrisisGroundSettleState(this.heroCrisisGroundSettleState, _intensity, 0, this.presentationSettings.reducedMotion);
        this.heroActionTransitionState = advanceHeroActionTransitionState(this.heroActionTransitionState, 'hit', 0, this.presentationSettings.reducedMotion);
    }
    else if (kind === 'perfectEvade')
        this.heroActionTransitionState = advanceHeroActionTransitionState(this.heroActionTransitionState, 'evade', 0, this.presentationSettings.reducedMotion); this.heroResponseVfx.push({ kind, x: this.hero.pos.x, y: this.hero.pos.y, ttl: maxTtl, maxTtl }); if (this.heroResponseVfx.length > 10)
        this.heroResponseVfx.splice(0, this.heroResponseVfx.length - 10); }
    drawHeroResponseVfx(ctx) { if (!this.heroResponseVfxAtlasReady || !this.heroResponseVfxAtlasImage)
        return; for (const cue of this.heroResponseVfx) {
        const sprite = heroResponseVfxSprite(this.hero.profileId, cue.kind);
        const t = Math.max(0, Math.min(1, cue.ttl / cue.maxTtl)), progress = 1 - t, size = (cue.kind === 'perfectEvade' ? 118 : 92) * (1 + progress * .22);
        ctx.save();
        ctx.globalAlpha = Math.min(this.presentationSettings.reducedFlash ? .5 : .86, t * (cue.kind === 'hit' ? .72 : .92));
        ctx.drawImage(this.heroResponseVfxAtlasImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, cue.x - size / 2, cue.y - size / 2, size, size);
        ctx.restore();
    } }
    queueBossWeakpointBreakWorldVfx(kind, x, y) { const maxTtl = .62; this.bossWeakpointBreakWorldVfx.push({ kind, x, y, ttl: maxTtl, maxTtl }); if (this.bossWeakpointBreakWorldVfx.length > 8)
        this.bossWeakpointBreakWorldVfx.splice(0, this.bossWeakpointBreakWorldVfx.length - 8); }
    drawBossWeakpointBreakWorldVfx(ctx) { if (!this.bossWeakpointWorldVfxAtlasReady || !this.bossWeakpointWorldVfxAtlasImage)
        return; for (const cue of this.bossWeakpointBreakWorldVfx) {
        const sprite = bossWeakpointWorldVfxSprite(cue.kind, 'break'), t = Math.max(0, Math.min(1, cue.ttl / cue.maxTtl)), size = 108 + (1 - t) * 48;
        ctx.save();
        ctx.globalAlpha = Math.min(this.presentationSettings.reducedFlash ? .48 : .82, t * .9);
        ctx.drawImage(this.bossWeakpointWorldVfxAtlasImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, cue.x - size / 2, cue.y - size / 2, size, size);
        ctx.restore();
    } }
    drawHero(ctx, motion) {
        ...
        void accelerationLean;
        void turnAnticipation;
        void decelerationSettle;
        void castFocus;
        ctx.save();
        ctx.translate(x, y);
    ...
    drawHud(ctx) {
        ...
        ctx.save();
        ctx.fillStyle = 'rgba(4,8,15,.82)';
        ctx.fillRect(18, 16, 440, 94);
        ctx.strokeStyle = 'rgba(177,220,255,.24)';
        ctx.lineWidth = 2;
        ctx.strokeRect(18, 16, 440, 94);
        ...
        ctx.fillText(profile.name, 34, 28);
        ctx.fillStyle = '#fff';
        ctx.font = '700 22px system-ui';
        ctx.fillText(`LV ${this.hero.level}`, 34, 55);
        ctx.font = '600 16px system-ui';
        ctx.fillStyle = '#d8e7f3';
        ctx.fillText(`HP ${Math.ceil(this.hero.hp)} / ${this.hero.maxHp}`, 98, 49);
        if (density.showXpNumbers && focus.showXpNumbers && ultraHudFocus.showXpNumbers) {
            ctx.fillStyle = '#9bd7ff';
            ctx.fillText(`EXP ${Math.floor(this.hero.xp)} / ${this.hero.xpNext}`, 98, 91);
        }
        ...
        if (this.hero.profileId === 'kain') {
            ...
            if (density.showMeterText && focus.showMeterText && ultraHudFocus.showMeterText) {
                ctx.fillStyle = '#cbbdff';
                ctx.font = '700 10px system-ui';
                ctx.fillText(`과부하 ${Math.round(this.kainOverload * 100)}%`, 34, 153);
            }
        }
        ctx.fillStyle = 'rgba(4,8,15,.78)';
        ctx.fillRect(safeArea.statusPanel.x, safeArea.statusPanel.y, safeArea.statusPanel.width, safeArea.statusPanel.height);
        ctx.textAlign = 'center';
        ctx.fillStyle = '#fff';
        ctx.font = `${safeArea.aspectClass === 'foldable' ? 22 : 29}px system-ui`;
        ctx.fillText(this.formatTime(this.elapsed), safeArea.headerCenterX, 47);
        ctx.font = `600 ${safeArea.aspectClass === 'foldable' ? 14 : 17}px system-ui`;
        ctx.fillStyle = '#f7cd76';
        ...
        if (safeArea.aspectClass !== 'foldable')
            this.drawCatastropheTransitionForecast(ctx, this.elapsed, safeArea.statusPanel.x + safeArea.statusPanel.width - 218, safeArea.statusPanel.y + 9);
        ...
        for (const label of buildLabels) {
            ...
            ctx.fillStyle = 'rgba(10,10,20,.76)';
            ctx.fillRect(18, buildY, 440, 23);
            ctx.strokeStyle = accent;
            ctx.globalAlpha = 0.58;
            ctx.lineWidth = 1;
            ctx.strokeRect(18, buildY, 440, 23);
            ctx.globalAlpha = 1;
            ctx.fillStyle = accent;
            ctx.font = '800 11px system-ui';
            ctx.textAlign = 'left';
            ctx.fillText(label, 34, buildY + 16);
            if (label.startsWith('CONTRACT') && this.endlessState.contracts.active) {
                this.drawDeepRunDecisionIdentityHud(ctx, { kind: 'contract', id: this.endlessState.contracts.active.family }, 425, buildY + 3, 17);
                this.drawRunContractPaceBoonRecall(ctx, this.endlessState.contracts.active, buildY + 3);
            }
            if (label.startsWith('서약') && this.endlessState.oaths.active) {
                this.drawDeepRunDecisionIdentityHud(ctx, { kind: 'oath', id: this.endlessState.oaths.active.kind }, 425, buildY + 3, 17);
                this.drawLongRunOathRequirementBoonRecall(ctx, this.endlessState.oaths.active.kind, buildY + 3);
            }
            if (finalForm && (label.startsWith('최종형') || label.startsWith('SIGNATURE') || label.startsWith('FLOW')))
                this.drawFinalFormIdentityHud(ctx, finalForm, 425, buildY + 3, 17);
            if (label.startsWith('REPLAY') && this.currentReplayPlan)
                this.drawBattlefieldIdentityHud(ctx, this.currentReplayPlan.target.mapId, 0, 399, buildY + 5, 23, 13);
            if (replayGuide?.category === 'final-form' && this.currentReplayPlan?.target.finalForm && label.startsWith('REPLAY'))
                this.drawFinalFormIdentityHud(ctx, { id: this.currentReplayPlan.target.finalForm }, 425, buildY + 3, 17);
            buildY += 26;
        }
        ...
        if (deepRunIdentityFocus.showAscensionRecall)
            this.drawDeepRunAscensionRecall(ctx, this.endlessState.heroAscension.selected, deepRunIdentityFocus.maxAscensionIcons, 322, buildIdentityY + 2);
        ...
        ctx.fillStyle = 'rgba(4,8,15,.78)';
        ctx.fillRect(1125, 18, 452, 62);
        ctx.textAlign = 'left';
        ctx.fillStyle = '#bfefff';
        ctx.font = '700 17px system-ui';
        ctx.fillText('수호핵', 1145, 43);
        this.drawBar(ctx, 1220, 29, 330, 18, this.core.hp / this.core.maxHp, '#55c9f0', '#0b2b38');
        ctx.fillStyle = '#d9f8ff';
        ctx.font = '600 14px system-ui';
        ctx.fillText(`${Math.ceil(this.core.hp)} / ${this.core.maxHp}`, 1220, 68);
        ctx.restore();
    }
    showFinalFormIdentityCue(id, durationSeconds) {
        this.finalFormIdentityCueId = id;
        this.finalFormIdentityCueUntil = this.elapsed + Math.max(0, durationSeconds);
    }
    showFinalFormTransformationCue(form) {
        this.showFinalFormIdentityCue(form.id, 2.4);
    }
    drawBattlefieldIdentityHud(ctx, mapId, stage, x, y, width = 28, height = 16) {
        if (!this.battlefieldEnvironmentAtlasReady || !this.battlefieldEnvironmentAtlasImage)
            return;
    ...
    drawBattlefieldMechanicRecall(ctx, mapId, stage, x, y, width = 34, height = 19) {
        if (!this.battlefieldMechanicAtlasReady || !this.battlefieldMechanicAtlasImage)
            return;
        ...
        ctx.drawImage(this.battlefieldMechanicAtlasImage, mechanic.sx, mechanic.sy, mechanic.sw, mechanic.sh, x + 2, y + 2, 10, 10);
        ctx.fillStyle = 'rgba(4,8,14,.86)';
        ctx.fillRect(x + width - 11, y + height - 10, 10, 9);
        ctx.drawImage(this.battlefieldMechanicAtlasImage, stageIcon.sx, stageIcon.sy, stageIcon.sw, stageIcon.sh, x + width - 10, y + height - 9, 8, 8);
        ctx.restore();
    }
    drawFinalFormIdentityHud(ctx, form, x, y, size = 18) {
        if (!this.finalFormIdentityAtlasReady || !this.finalFormIdentityAtlasImage)
            return;
    ...
    drawFinalFormTransformationCue(ctx) {
        if (!this.finalFormIdentityCueId || this.elapsed >= this.finalFormIdentityCueUntil)
            return;
        if (!this.finalFormIdentityAtlasReady || !this.finalFormIdentityAtlasImage)
            return;
        ...
        const size = 76;
        const x = LOGICAL_WIDTH / 2 - size / 2;
        const y = 104;
        ctx.save();
    ...
    drawDeepRunDecisionIdentityHud(ctx, identity, x, y, size = 18) {
        if (!this.deepRunDecisionIdentityAtlasReady || !this.deepRunDecisionIdentityAtlasImage)
            return;
        const icon = deepRunDecisionIdentityIcon(identity);
        ctx.save();
        ctx.globalAlpha = .96;
        ctx.drawImage(this.deepRunDecisionIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size);
        ctx.restore();
    }
    drawAscensionMutatorRecall(ctx, ids, maxIcons, x, y) {
        if (!this.ascensionMutatorIdentityAtlasReady || !this.ascensionMutatorIdentityAtlasImage || maxIcons <= 0)
            return;
        const visible = ids.slice(-Math.min(3, maxIcons));
        const size = 18, gap = 3;
        ctx.save();
        for (let i = 0; i < visible.length; i++) {
            const icon = ascensionMutatorIdentityIcon(visible[i]);
            const dx = x + i * (size + gap);
            ctx.fillStyle = 'rgba(6,12,22,.88)';
            ctx.fillRect(dx - 1, y - 1, size + 2, size + 2);
            ctx.drawImage(this.ascensionMutatorIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, dx, y, size, size);
        }
        ctx.restore();
    }
    drawDeepRunAscensionRecall(ctx, ids, maxIcons, x, y) {
        if (!this.deepRunDecisionIdentityAtlasReady || !this.deepRunDecisionIdentityAtlasImage || maxIcons === 0)
            return;
        const visible = ids.slice(-maxIcons);
        const size = 18, gap = 3;
        ctx.save();
        for (let i = 0; i < visible.length; i++) {
            const icon = deepRunDecisionIdentityIcon({ kind: 'ascension', id: visible[i] });
            const dx = x + i * (size + gap);
            ctx.fillStyle = 'rgba(6,12,22,.88)';
            ctx.fillRect(dx - 1, y - 1, size + 2, size + 2);
    ...
    hideAscensionTierPressureIdentity() {
        const boss = this.enemies.enemies.find(enemy => enemy.alive && enemy.type === 'boss') ?? null;
    ...
    drawAscensionTierForecast(ctx, elapsedMs, x, y) {
        const projection = projectAscensionTierForecast(elapsedMs);
        if (!projection.visible || this.hideAscensionTierPressureIdentity() || !this.ascensionTierPressureAtlasReady || !this.ascensionTierPressureAtlasImage)
            return;
        const w = 82, h = 22, size = 9;
        ctx.save();
        ctx.globalAlpha = .96;
        ctx.fillStyle = 'rgba(7,10,18,.9)';
        ctx.fillRect(x, y, w, h);
        ctx.strokeStyle = projection.mutatorThreshold ? '#ff6fab' : '#ffbf78';
        ctx.lineWidth = 1;
        ...
        ctx.textAlign = 'left';
        ctx.textBaseline = 'middle';
        ctx.fillText(ascensionTierForecastLabel(projection), x + 4, y + h / 2 + .5);
        ...
        if (projection.mutatorThreshold) {
            const icon = ascensionTierPressureIdentityIcon('mutator-threshold');
            ctx.drawImage(this.ascensionTierPressureAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x + w - 11, y + 2, 8, 8);
        }
        ctx.restore();
    }
    drawBuildOverdriveRecall(ctx, state, elapsedMs, x, y, compact) {
        const recall = buildOverdriveRecallPresentation(state, elapsedMs, compact);
        const height = 18;
        ctx.save();
        if (recall.mode === 'active' && !recall.compact) {
            ctx.fillStyle = 'rgba(6,12,22,.88)';
            ctx.fillRect(x - 1, y - 1, 57, height + 2);
            ctx.strokeStyle = '#ffb85c';
            ctx.globalAlpha = .7;
            ctx.strokeRect(x - .5, y - .5, 56, height + 1);
            ctx.globalAlpha = 1;
            ctx.fillStyle = '#ffcf8a';
            ctx.font = '900 10px system-ui';
            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle';
            ctx.fillText(recall.numericLabel, x + 16, y + height / 2 + 1);
            ctx.restore();
            return;
        }
        const segmentW = 7, gap = 2;
        const gaugeW = recall.totalSegments * segmentW + (recall.totalSegments - 1) * gap;
        const boxW = recall.compact ? gaugeW + 4 : gaugeW + 27;
        ctx.fillStyle = 'rgba(6,12,22,.88)';
        ctx.fillRect(x - 1, y - 1, boxW, height + 2);
        for (let i = 0; i < recall.totalSegments; i++) {
            const sx = x + 2 + i * (segmentW + gap);
            ctx.fillStyle = i < recall.filledSegments ? '#ffb85c' : 'rgba(255,184,92,.18)';
            ctx.fillRect(sx, y + 5, segmentW, 8);
            ctx.strokeStyle = 'rgba(255,207,138,.52)';
            ctx.lineWidth = 1;
            ctx.strokeRect(sx + .5, y + 5.5, segmentW - 1, 7);
        }
        if (!recall.compact && recall.numericLabel) {
            ctx.fillStyle = recall.mode === 'ready' ? '#fff0a6' : '#ffcf8a';
            ctx.font = '900 9px system-ui';
            ctx.textAlign = 'left';
            ctx.textBaseline = 'middle';
            ctx.fillText(recall.numericLabel, x + gaugeW + 7, y + height / 2 + 1);
        }
        ctx.restore();
    }
    hideBuildOverdriveEffectIdentity() {
        const boss = this.enemies.enemies.find(enemy => enemy.alive && enemy.type === 'boss') ?? null;
        return this.dangerState.heroCritical || this.dangerState.coreCritical || (boss?.specialTimer ?? 99) <= 1.2;
    }
    drawBuildOverdriveEffectRecall(ctx, state, archetype, elapsedMs, x, y, compact) {
        const projection = projectBuildOverdriveEffects(state, archetype, elapsedMs);
        if (!projection.active || compact || this.hideBuildOverdriveEffectIdentity() || !this.buildOverdriveEffectAtlasReady || !this.buildOverdriveEffectAtlasImage)
            return;
        const size = 10, gap = 2;
        ctx.save();
        ctx.globalAlpha = .96;
        projection.effects.slice(0, 2).forEach((effect, index) => { const icon = buildOverdriveEffectIdentityIcon(effect.id), dx = x + index * (size + gap); ctx.fillStyle = 'rgba(6,12,22,.9)'; ctx.fillRect(dx - 1, y - 1, size + 2, size + 2); ctx.drawImage(this.buildOverdriveEffectAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, dx, y, size, size); });
        ctx.restore();
    }
    currentSynergyIdentityIds() {
    ...
    syncSynergyIdentityTracker(showToast = true) {
        const ids = this.currentSynergyIdentityIds();
        if (!this.synergyIdentityInitialized) {
            this.lastSynergyIdentityIds = [...ids];
            this.synergyIdentityInitialized = true;
            return;
        }
        const previous = new Set(this.lastSynergyIdentityIds);
        const added = ids.find(id => !previous.has(id));
        this.lastSynergyIdentityIds = [...ids];
        if (showToast && added) {
            const def = activeSynergies({ heroId: this.hero.profileId, traitId: this.selectedTrait, relicId: this.activeRelic, equipment: this.equipmentState }).find(v => v.id === added);
            if (def)
                this.showSynergyEventToast(`시너지 활성 · ${def.name}`, added);
        }
    }
    drawSynergyIdentityHud(ctx, x, y, size = 16, maxIcons = 2) {
        const ids = this.currentSynergyIdentityIds().slice(0, Math.max(0, maxIcons));
        if (ids.length === 0 || !this.synergyIdentityAtlasReady || !this.synergyIdentityAtlasImage)
            return;
        const gap = 3;
        ctx.save();
        ctx.globalAlpha = .95;
        ids.forEach((id, index) => { const icon = synergyIdentityIcon(id), dx = x + index * (size + gap); ctx.fillStyle = 'rgba(6,12,22,.9)'; ctx.fillRect(dx - 1, y - 1, size + 2, size + 2); ctx.drawImage(this.synergyIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, dx, y, size, size); });
        ctx.restore();
    }
    drawLegendaryAwakeningRecall(ctx, x, y, size = 16) {
        const recalls = activeLegendaryAwakeningRecall(this.equipmentState, this.legendaryEffects.modifiers);
        if (recalls.length === 0 || !this.legendaryAwakeningAtlasReady || !this.legendaryAwakeningAtlasImage)
            return;
        const gap = 3;
        ctx.save();
        ctx.globalAlpha = .96;
        recalls.forEach((recall, index) => { const dx = x + index * (size + gap); ctx.fillStyle = 'rgba(6,12,22,.9)'; ctx.fillRect(dx - 1, y - 1, size + 2, size + 2); ctx.drawImage(this.legendaryAwakeningAtlasImage, recall.icon.sx, recall.icon.sy, recall.icon.sw, recall.icon.sh, dx, y, size, size); ctx.strokeStyle = '#ffd879'; ctx.lineWidth = 1; ctx.strokeRect(dx + .5, y + .5, size - 1, size - 1); });
        ctx.restore();
    }
    drawBuildIdentityStrip(ctx, x, y) {
        if (!this.buildIdentityAtlasReady || !this.buildIdentityAtlasImage)
            return;
        ...
        for (let i = 0; i < ids.length; i++) {
            ...
            if (i === 0 && this.activeRelic && ids[i] === this.activeRelic) {
                this.drawRelicResonanceProgressFrame(ctx, dx, y, size, relicResonanceNextTierProgress(resonance));
                if (resonancePresentation)
                    this.drawRelicResonanceTierBadge(ctx, dx, y, size, resonancePresentation.badge.label);
            }
        }
        ctx.restore();
    }
    drawPriorityThreats(ctx, motion) {
        ...
        ctx.save();
        for (const enemy of this.enemies.enemies) {
            if (!enemy.alive || !ids.has(enemy.id))
                continue;
    ...
    drawDangerVignette(ctx) {
        ...
        ctx.fillStyle = top;
        ctx.fillRect(0, 0, LOGICAL_WIDTH, 150);
        ...
        ctx.fillRect(0, LOGICAL_HEIGHT - 150, LOGICAL_WIDTH, 150);
        ...
        ctx.fillStyle = left;
        ctx.fillRect(0, 0, 145, LOGICAL_HEIGHT);
        ...
        ctx.fillStyle = right;
        ctx.fillRect(LOGICAL_WIDTH - 145, 0, 145, LOGICAL_HEIGHT);
        ctx.restore();
    }
    drawCriticalWarnings(ctx) {
        const boss = this.enemies.enemies.find((enemy) => enemy.alive && enemy.type === 'boss') ?? null;
        ...
        if (attention.showHeroWarning) {
            ...
            ctx.fillStyle = 'rgba(91,6,18,.88)';
            ctx.fillRect(18, 260, 160, 34);
            ctx.strokeStyle = '#ff5268';
            ctx.lineWidth = 2;
            ctx.strokeRect(18, 260, 160, 34);
            ctx.fillStyle = '#fff';
        ...
        if (attention.showCoreWarning) {
            ...
            ctx.fillStyle = 'rgba(71,12,17,.9)';
            ctx.fillRect(1330, 86, 247, 34);
            ctx.strokeStyle = '#ff6b79';
            ctx.lineWidth = 2;
            ctx.strokeRect(1330, 86, 247, 34);
            ctx.fillStyle = '#fff';
    ...
    drawTacticalStack(ctx) {
        const rows = [];
        const onboardingStep = this.onboarding.current;
        if (onboardingStep)
            rows.push({ accent: '#fff3a0', title: `첫 전투 · ${onboardingStep.title}`, detail: onboardingStep.hint });
        if (this.fateRuntime.choices.length > 0)
            rows.push({ accent: '#e7a1ff', title: '운명', detail: fateHudSummary(this.fateRuntime.choices), fateIds: this.fateRuntime.choices });
        const event = this.fieldEvents.active;
        if (event)
            rows.push({ accent: event.accent, title: event.name, detail: `${Math.max(0, Math.ceil(event.remaining))}초 · ${event.description}`, iconId: event.id, fieldEventId: event.id });
        ...
        if (this.threatDirective)
            rows.push({ accent: this.threatDirective.accent, title: `전투 지시 · ${this.threatDirective.name}`, detail: this.threatDirective.description, iconId: this.threatDirective.id });
        if (rows.length === 0)
            return;
        ...
        for (const row of rows.slice(0, openingFocus.maxTacticalRows)) {
            ...
            if (row.fateIds) {
                this.drawFatePathRecall(ctx, row.fateIds, y);
                if (!this.hideFateImpactHelperIdentity())
                    this.drawFateCumulativeImpactRecall(ctx, row.fateIds, y);
            }
            if (row.fieldEventId && !this.hideFieldEventHelperIdentity())
                this.drawFieldEventResponseEffectRecall(ctx, row.fieldEventId, y);
            if (row.objectiveRewards && !this.hideObjectiveHelperIdentity())
                this.drawObjectiveRewardPreview(ctx, row.objectiveRewards.slice(0, 2), row.objectiveRewardMultiplier ?? 1, y);
            if (row.missionPaceId && row.missionReward && !this.hideRunMissionHelperIdentity())
                this.drawRunMissionPaceRewardRecall(ctx, row.missionPaceId, row.missionReward, y);
            y += 48;
        }
    }
    drawFatePathRecall(ctx, ids, y) {
        if (!this.decisionPathIconAtlasReady || !this.decisionPathIconAtlasImage)
            return;
        const visible = ids.slice(0, 3);
        const size = 18, gap = 3, x = 626;
        ctx.save();
        for (let i = 0; i < visible.length; i++) {
            const icon = fatePathRecallIcon(visible[i]);
            const sprite = icon.sprite;
            const dx = x + i * (size + gap), dy = y + 20;
            ctx.fillStyle = 'rgba(6,12,22,.88)';
    ...
    drawFateImpactPair(ctx, impact, x, y, size) {
        const benefitImage = this.fateBenefitVectorAtlasImage, costImage = this.fateCostVectorAtlasImage;
        if (!this.fateBenefitVectorAtlasReady || !benefitImage || !this.fateCostVectorAtlasReady || !costImage)
            return;
        const benefit = fateBenefitVectorIcon(impact.benefitId), cost = fateCostVectorIcon(impact.costId);
        ctx.save();
        ctx.globalAlpha = .95;
        ctx.fillStyle = 'rgba(5,9,17,.9)';
        ctx.fillRect(x - 1, y - 1, size * 2 + 6, size + 2);
        ctx.drawImage(benefitImage, benefit.sx, benefit.sy, benefit.sw, benefit.sh, x, y, size, size);
        ctx.drawImage(costImage, cost.sx, cost.sy, cost.sw, cost.sh, x + size + 4, y, size, size);
        ctx.restore();
    }
    drawFateCumulativeImpactRecall(ctx, ids, y) {
        if (ids.length === 0)
            return;
        const impact = fateCumulativeImpact(ids);
        this.drawFateImpactPair(ctx, impact, 695, y + 20, 18);
    }
    drawStatusRow(ctx, y, accent, title, detail, iconId) {
        ctx.save();
        ctx.fillStyle = 'rgba(5,10,19,.86)';
        ctx.fillRect(570, y, 480, 42);
        ctx.strokeStyle = accent;
        ctx.lineWidth = 2;
        ctx.strokeRect(570, y, 480, 42);
        ...
        ctx.font = '900 14px system-ui';
        ctx.textAlign = 'left';
        ctx.fillText(title, titleX, y + 17);
        ctx.fillStyle = '#d8e3ec';
        ctx.font = '700 12px system-ui';
        ctx.textAlign = 'right';
        ctx.fillText(detail, 1034, y + 17);
        ctx.restore();
    }
    drawBossResponseAckIdentity(ctx, boss, x, y, radius) {
        if (!this.bossResponseAckIdentityAtlasReady || !this.bossResponseAckIdentityAtlasImage)
            return;
    ...
    drawControls(ctx) {
        ...
        for (const button of ACTION_BUTTONS) {
            if (button.id === 'potion') {
                if (this.hero.healingPotions > 0)
                    readyActions.add('potion');
                continue;
            }
            if (button.id.startsWith('spell') || button.id.startsWith('ultimate')) {
                if (this.spells.cooldownRemaining(button.id) <= 0)
                    readyActions.add(button.id);
            }
        }
        let actionAssist = null;
        if (boss && (boss.specialTimer ?? 99) <= 1.05) {
            ...
            if (this.bossResponseAckAction && !sameAckCycle)
                this.clearBossResponseAcknowledgement();
            const queuedActions = new Set(COMBAT_CAST_ACTIONS.filter((id) => this.castIntentBuffer.isQueued(id)));
            ...
            if (actionAssist) {
                if (!previousCue || actionAssist.actionId !== previousCue.actionId)
                    this.bossActionAssistCueSince = this.elapsed;
                this.bossActionAssistCue = actionAssist;
                this.bossActionAssistBossId = boss.id;
                this.bossActionAssistArchetype = archetype;
            }
            else
                this.clearBossActionAssistCue();
        }
        ...
        if (this.input.joystickActive) {
            ...
            ctx.fillStyle = 'rgba(191,220,255,.10)';
            ctx.beginPath();
            ctx.arc(b.x, b.y, 100, 0, Math.PI * 2);
            ctx.fill();
            ctx.strokeStyle = 'rgba(210,234,255,.32)';
            ctx.lineWidth = 4;
            ctx.stroke();
            ctx.fillStyle = 'rgba(220,236,255,.28)';
            ctx.beginPath();
            ctx.arc(t.x, t.y, 42, 0, Math.PI * 2);
            ctx.fill();
        }
        else {
            ctx.strokeStyle = 'rgba(210,234,255,.14)';
            ctx.lineWidth = 3;
            ctx.beginPath();
            ctx.arc(180, 720, 88, 0, Math.PI * 2);
            ctx.stroke();
            ctx.fillStyle = 'rgba(210,234,255,.12)';
            ctx.beginPath();
            ctx.arc(180, 720, 34, 0, Math.PI * 2);
            ctx.fill();
        }
        for (const button of ACTION_BUTTONS) {
            ...
            ctx.fill();
            ctx.strokeStyle = button.id.startsWith('ultimate') ? '#d8a9ff' : '#9fd9ff';
            ctx.lineWidth = held ? 5 : 3;
            ctx.stroke();
            const cooldown = this.spells.cooldownRemaining(button.id);
            const ratio = this.spells.cooldownRatio(button.id, this.hero);
            const queuedCast = COMBAT_CAST_ACTIONS.includes(button.id) && this.castIntentBuffer.isQueued(button.id);
            ...
            if (cuePresentation.clearReadyPulse)
                this.ultimatePulseUntil[button.id] = 0;
            else if (buttonState.pulse)
                this.ultimatePulseUntil[button.id] = this.elapsed + 0.48;
            ...
            if (cuePresentation.outerCue === 'assist' && controlAssist?.actionId === button.id) {
                ...
                ctx.save();
                ctx.globalAlpha = .72;
                ctx.strokeStyle = controlAssist.accent;
                ctx.lineWidth = 5;
                ctx.beginPath();
                ctx.arc(button.x, button.y, button.radius * radiusScale, 0, Math.PI * 2);
                ctx.stroke();
                const showAssistLabel = cuePresentation.showAssistLabel && (!compactBossAssist || combatAttention.showBossAssistLabel);
                if (showAssistLabel) {
                    ctx.fillStyle = controlAssist.accent;
                    ctx.font = '900 10px system-ui';
                    ctx.textAlign = 'center';
            ...
            if (responseAckActive && boss)
                this.drawBossResponseAckIdentity(ctx, boss, button.x, button.y, button.radius);
            ...
            if (iconPresentation.visible) {
                ...
                if (heroAbilityIconImage && isHeroAbilityActionId(button.id)) {
                    ...
                    ctx.drawImage(heroAbilityIconImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, button.x - iconSize / 2, button.y + iconPresentation.iconOffsetY - iconSize / 2, iconSize, iconSize);
                }
                else if (legacyActionIconImage) {
                    const sprite = actionIconSprite(button.id);
                    ctx.drawImage(legacyActionIconImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, button.x - iconSize / 2, button.y + iconPresentation.iconOffsetY - iconSize / 2, iconSize, iconSize);
                }
                ctx.restore();
            }
            if (ratio > 0) {
                ...
                ctx.arc(button.x, button.y, button.radius - 2, -Math.PI / 2, -Math.PI / 2 + Math.PI * 2 * ratio);
                ctx.closePath();
                ctx.fill();
            }
    ...
    handleFieldEventStart(event) {
        ...
        if (event.id === 'goldenGoblin') {
            ...
            const eliteRushEnemyIds = [];
            let heroTargets = 0, coreTargets = 0;
            for (let i = 0; i < count; i++) {
                const target = Math.random() < 0.3 ? 'core' : 'hero';
                if (target === 'hero')
                    heroTargets += 1;
                else
                    coreTargets += 1;
                eliteRushEnemyIds.push(this.enemies.spawnEventEnemy('elite', danger, target));
            }
            const spawned = eliteRushEnemyIds.map(id => this.enemies.enemies.find(enemy => enemy.id === id)).filter((enemy) => Boolean(enemy));
            if (spawned.length > 0)
                anchor = { id: event.id, x: spawned.reduce((sum, enemy) => sum + enemy.pos.x, 0) / spawned.length, y: spawned.reduce((sum, enemy) => sum + enemy.pos.y, 0) / spawned.length };
    ...
    updateSupplyCrate() {
        if (!this.supplyCrate || this.fieldEvents.active?.id !== 'supplyDrop')
            return;
    ...
    drawHeroMeterIdentityHud(ctx, x, y, size) {
        if (!this.heroMeterIdentityAtlasReady || !this.heroMeterIdentityAtlasImage)
            return;
    ...
    updateArcaneCombo() {
        const evolvedSpells = Object.keys(this.spells.levels).filter((id) => this.spells.levels[id] >= 5);
        const legendaryIds = [this.equipmentState.weapon, this.equipmentState.armor].filter((item) => item?.legendary === true).map((item) => item.id);
        ...
        if (this.comboRuntime.current.tier > previousTier && this.comboRuntime.current.tier > 0) {
            this.showArcaneComboEventToast(`ARCANE ${this.comboRuntime.current.label} · ${this.comboRuntime.current.name}`, this.comboRuntime.current.family);
        }
    }
    drawArcaneComboHud(ctx) {
        const combo = this.comboRuntime.current;
        if (combo.tier <= 0 || combo.family === 'none')
            return;
        const accent = combo.tier === 3 ? '#ffe07a' : combo.tier === 2 ? '#9fdcff' : '#cab5ff';
        ctx.save();
        ctx.fillStyle = 'rgba(6,11,20,.82)';
        ctx.fillRect(36, 306, 276, 34);
        ctx.strokeStyle = accent;
        ctx.lineWidth = 2;
        ctx.strokeRect(36, 306, 276, 34);
        const hasIdentity = this.drawArcaneComboIdentityHud(ctx, combo.family, combo.tier, 47, 313, 20);
        ctx.fillStyle = accent;
        ctx.font = '900 12px system-ui';
        ctx.textAlign = 'left';
        ctx.fillText(`ARCANE ${combo.label}`, hasIdentity ? 76 : 48, 328);
        ctx.fillStyle = '#eef6ff';
        ctx.textAlign = 'right';
        ctx.fillText(combo.name, 300, 328);
        ctx.restore();
    }
    drawArcaneComboIdentityHud(ctx, family, tier, x, y, size) {
        if (!this.arcaneComboIdentityAtlasReady || !this.arcaneComboIdentityAtlasImage)
            return false;
        const icon = arcaneComboIdentityIcon(family), badge = arcaneComboTierBadge(tier);
        ctx.save();
        ctx.globalAlpha = .96;
        ctx.drawImage(this.arcaneComboIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size);
        if (badge) {
            const bw = badge === 'III' ? 13 : 9, bh = 9, bx = x + size - bw + 2, by = y + size - bh + 2;
            ctx.fillStyle = 'rgba(5,9,18,.96)';
    ...
    syncBossEncounter() {
        ...
        if (this.bossEncounter.activeBossId === boss.id)
            return;
        if (this.bossEncounter.activeBossId !== null)
            this.bossEncounterNodesDestroyed += this.bossEncounter.destroyedNodes;
        ...
        if (mythic.active) {
            this.mythicPhaseBossId = boss.id;
            this.lastMythicPhase = 1;
            this.showEventToast(`${mythic.label} · ${mythic.channels.length}중 패턴 결합`, null, null, null, null, null, null, null, [], null, mutation?.kind ?? null, { mythicPhase: 1, bossArchetype: boss.bossArchetype ?? 'inferno' });
        }
        else {
            const variantTier = boss.bossVariantTier ?? 0;
            const secondary = boss.isApex ? boss.apexSecondaryArchetype : null;
            this.showEventToast(mutation ? `보스 전장 변이 · ${mutation.label}` : '보스 전장전 · 약점을 파괴하세요', null, null, null, null, null, null, null, [], null, mutation?.kind ?? null, { bossArchetype: boss.bossArchetype ?? 'inferno', ...(variantTier > 0 ? { bossVariantTier: variantTier } : {}), ...(secondary ? { apexSecondaryArchetype: secondary } : {}) });
        }
    }
    syncBossWeakpointBreakFeedback() {
        ...
        if (breakKind)
            this.queueBossWeakpointBreakWorldVfx(breakKind, breakCenter.x, breakCenter.y);
    ...
    updateBossArena(dt) {
        ...
        if (boss.isMythic) {
            ...
            if (this.mythicPhaseBossId !== boss.id) {
                this.mythicPhaseBossId = boss.id;
                this.lastMythicPhase = activePhase.phase;
            }
            else if (activePhase.phase > this.lastMythicPhase && activePhase.phase > 0) {
                this.lastMythicPhase = activePhase.phase;
                const message = activePhase.phase === 2 ? 'MYTHIC PHASE II · 압박 격화' : 'MYTHIC PHASE III · 최종 폭주';
                this.showMythicPhaseEventToast(message, activePhase.phase);
            }
        ...
        for (const hazard of expiringHazards) {
            this.queueBossHazardAftermathVfx(hazard.kind, hazard.pos.x, hazard.pos.y, hazard.radius);
            this.attachBossHazardClearedGeometry(hazard.geometryShape, hazard.angle, hazard.length);
        }
        ...
        if (dodgeStep.reward) {
            ...
            if (shouldTriggerArenaDodgeFinisher(previousEvadeCount, this.arenaDodgeChain.count)) {
                ...
                if (finalFormId)
                    this.showFinalFormIdentityCue(finalFormId, 1.2);
                ...
                if (finisher.coreHealPercent > 0)
                    this.core.hp = Math.min(this.core.maxHp, this.core.hp + this.core.maxHp * finisher.coreHealPercent);
                ...
                for (let ring = 0; ring < finisherFeedback.ringCount; ring++)
                    this.presentation.emitTelegraph({ x: this.hero.pos.x, y: this.hero.pos.y, radius: (finisher.radius + ring * finisherFeedback.ringStep) * ringScale, color: ring % 2 === 0 ? primaryAccent : secondaryAccent, ttl: Math.min(.5, finisherFeedback.ttl + ring * .035), width: 5 - ring * .7, alpha: .86 - ring * .12 });
                for (let i = 0; i < finisherFeedback.particleCount; i++) {
                    const sides = finisherSignature?.particleSides ?? finisherFeedback.particleCount;
                    const spoke = i % Math.max(1, sides);
                    const a = Math.PI * 2 * spoke / Math.max(1, sides) + formAngle + (Math.floor(i / Math.max(1, sides)) * .09);
                    const speed = finisherFeedback.particleSpeed * (.82 + (i % 3) * .09);
                    this.presentation.emitParticle({ x: this.hero.pos.x, y: this.hero.pos.y, vx: Math.cos(a) * speed, vy: Math.sin(a) * speed, color: i % 2 === 0 ? primaryAccent : secondaryAccent, ttl: finisherFeedback.ttl, size: 2.8 + (i % 2) * 1.2, alpha: .82 });
                }
                for (let i = 0; i < finisherFeedback.trailCount; i++) {
                    const skew = finisherSignature?.trailSkew ?? 0;
                    const a = Math.PI * 2 * i / Math.max(1, finisherFeedback.trailCount) + formAngle + skew * (i % 2 === 0 ? 1 : -1);
                    const len = finisher.radius * (.55 + (i % 2) * .16);
                    this.presentation.emitTrail({ x1: this.hero.pos.x, y1: this.hero.pos.y, x2: this.hero.pos.x + Math.cos(a) * len, y2: this.hero.pos.y + Math.sin(a) * len, color: i % 2 === 0 ? primaryAccent : secondaryAccent, ttl: finisherFeedback.ttl * .8, width: 2.4, alpha: .72 });
                }
    ...
    drawEnemySpawnLaneReadability(ctx) {
        if (!this.worldVfxLayerAllowed('enemy-spawn-lane'))
            return;
        ...
        const countDebounce = spawnLaneEdgeCountDownwardDebounce(this.spawnLaneEdgeCountDebounceMemory, stackedCues, this.elapsed);
        this.spawnLaneEdgeCountDebounceMemory = countDebounce.memory;
        for (let cueIndex = 0; cueIndex < stackedCues.length; cueIndex++) {
            const cue = stackedCues[cueIndex], displayCount = countDebounce.counts[cueIndex] ?? cue.count;
            const dx = cue.end.x - cue.start.x, dy = cue.end.y - cue.start.y, len = Math.hypot(dx, dy);
            if (len < 1)
                continue;
            const nx = dx / len, ny = dy / len;
            const start = { x: cue.start.x + nx * 28, y: cue.start.y + ny * 28 }, end = { x: cue.end.x - nx * 64, y: cue.end.y - ny * 64 };
            const alpha = cue.alpha * this.worldVfxCueAlpha('tactical', cue.start.x, cue.start.y, 80);
            if (alpha <= 0)
                continue;
            ...
            ctx.strokeStyle = accent;
            ctx.fillStyle = accent;
            ctx.lineWidth = cue.kind === 'boss' ? 3 : 2;
            ...
            ctx.stroke();
            ctx.setLineDash([]);
            const angle = Math.atan2(dy, dx), arrow = cue.kind === 'boss' ? 12 : 9;
            ctx.beginPath();
            ctx.moveTo(end.x, end.y);
            ctx.lineTo(end.x - Math.cos(angle - .55) * arrow, end.y - Math.sin(angle - .55) * arrow);
            ctx.lineTo(end.x - Math.cos(angle + .55) * arrow, end.y - Math.sin(angle + .55) * arrow);
            ctx.closePath();
            ctx.fill();
            const labelFade = spawnLaneEdgeLabelFade({ ...cue, count: displayCount });
            const debouncedLabelVisible = displayCount > 1 && (cue.labelVisible || displayCount > cue.count);
            if (debouncedLabelVisible && labelFade.visible) {
                ctx.globalAlpha = labelFade.labelAlpha * this.worldVfxCueAlpha('tactical', cue.start.x, cue.start.y, 80);
                ctx.font = '700 11px system-ui';
                ctx.textAlign = 'center';
                ctx.fillText(`×${displayCount}`, cue.labelPos.x, cue.labelPos.y);
            }
            ctx.restore();
        }
    }
    updateBossSafeResponseWindowConfirmation() {
        const boss = this.enemies.enemies.find((enemy) => enemy.alive && enemy.type === 'boss') ?? null;
        if (!boss) {
            this.bossSafeResponseWindowUntil = 0;
            this.bossSafeResponseBossId = null;
        ...
        if (this.bossSafeResponseBossId !== null && this.bossSafeResponseBossId !== boss.id) {
            this.bossSafeResponseWindowUntil = 0;
            this.bossSafeResponseBossId = null;
            this.bossSafeResponseCycle = null;
            this.bossSafeResponseShownCycle = null;
        ...
        if (this.bossResponseAckBossId === boss.id && this.bossResponseAckCycle !== null && cycle === this.bossResponseAckCycle + 1) {
            this.bossSafeResponseBossId = boss.id;
            this.bossSafeResponseCycle = cycle;
        }
        if (this.bossSafeResponseBossId !== boss.id || this.bossSafeResponseCycle !== cycle || this.bossSafeResponseShownCycle === cycle)
            return;
        ...
        if (result.confirmed) {
            this.bossSafeResponseWindowUntil = this.elapsed + result.visualSeconds;
            this.bossSafeResponseShownCycle = cycle;
        }
    }
    // Canonical boss safe-response label retained for source-contract continuity: 대응 여유
    drawBossSafeResponseWindowConfirmation(ctx) {
        const active = this.elapsed < this.bossSafeResponseWindowUntil;
        const boss = this.enemies.enemies.find((enemy) => enemy.alive && enemy.type === 'boss' && enemy.id === this.bossSafeResponseBossId) ?? null;
        if (!boss)
            return;
        ...
        const slotHysteresis = bossSafeResponseSlotHysteresis({ previous: this.bossSafeResponseSlotMemory, current: strictLabelPlacement, placementInput, bossId: boss.id, cycle: boss.bossCycle ?? 0, now: this.elapsed });
        this.bossSafeResponseSlotMemory = slotHysteresis.memory;
        const labelPlacement = slotHysteresis.placement;
        const alpha = this.worldVfxLayerAlpha('tactical') * (this.presentationSettings.reducedFlash ? .58 : presentation.ringAlpha);
        if (alpha <= 0)
            return;
        ctx.save();
        ctx.globalAlpha = alpha;
        ctx.strokeStyle = '#79e6b4';
        ctx.fillStyle = '#b8f7dc';
        ctx.lineWidth = presentation.mode === 'compact' ? 2 : 2.5;
        ctx.setLineDash([8, 6]);
        ctx.beginPath();
        ctx.arc(boss.pos.x, boss.pos.y, boss.radius + 24, Math.PI * .12, Math.PI * .88);
        ctx.stroke();
        ctx.setLineDash([]);
        const showPlacedLabel = presentation.showLabel && labelPlacement.visible;
        if (presentation.showLabel) {
            if (showPlacedLabel) {
                ctx.font = '700 12px system-ui';
                ctx.textAlign = 'center';
                ctx.fillText(presentation.label, labelPlacement.pos.x, labelPlacement.pos.y);
            }
        }
        ctx.restore();
    }
    drawProjectileThreatVisibility(ctx) {
        ...
        for (const cue of cues) {
            const p = projectiles[cue.index];
            if (!p)
                continue;
            const speed = Math.hypot(p.vel.x, p.vel.y) || 1, nx = p.vel.x / speed, ny = p.vel.y / speed;
            ctx.save();
            ctx.globalAlpha = cue.level === 'critical' ? .9 : cue.level === 'danger' ? .72 : .48;
            ctx.strokeStyle = cue.accent;
            ctx.lineWidth = cue.level === 'critical' ? 3 : 2;
            const cuePos = p.visualPos ?? p.pos;
            ctx.beginPath();
            ctx.moveTo(cuePos.x, cuePos.y);
            ctx.lineTo(cuePos.x - nx * cue.trailLength, cuePos.y - ny * cue.trailLength);
            ctx.stroke();
            ctx.beginPath();
            ctx.arc(cuePos.x, cuePos.y, p.radius + cue.radiusBoost, 0, Math.PI * 2);
            ctx.stroke();
            ctx.restore();
        }
    }
    drawEdgeThreatVfx(ctx) {
        const projectiles = this.enemies.projectileThreatViews();
        ...
        ctx.save();
        for (const cue of cues) {
            const projectile = projectiles[cue.index];
            if (!projectile)
                continue;
            ...
            for (let i = 0; i < profile.segmentCount; i++) {
                ...
                if (indicator.edge === 'left' || indicator.edge === 'right') {
                    const y = indicator.position * LOGICAL_HEIGHT + offset;
                    const x = indicator.edge === 'left' ? indicator.inset : LOGICAL_WIDTH - indicator.inset - thickness;
                    ctx.fillRect(x, y - length / 2, thickness, length);
                }
                else {
                    const x = indicator.position * LOGICAL_WIDTH + offset;
                    const y = indicator.edge === 'top' ? indicator.inset : LOGICAL_HEIGHT - indicator.inset - thickness;
                    ctx.fillRect(x - length / 2, y, length, thickness);
                }
            }
        }
        ctx.restore();
    }
    drawAutoTargetVisibility(ctx) {
        if (!this.autoCastNormal)
            return;
        ...
        ctx.save();
        ctx.globalAlpha = .38 + .34 * cue.urgency;
        ctx.strokeStyle = cue.accent;
        ctx.lineWidth = 3;
        ctx.setLineDash([7, 6]);
        ctx.beginPath();
        ctx.arc(target.pos.x, target.pos.y, (target.radius + cue.radius * .35) * pulse, 0, Math.PI * 2);
        ctx.stroke();
        ctx.setLineDash([]);
        const ultraCritical = ultraLongCriticalFocus({ elapsedSeconds: this.elapsed, bossActive: Boolean(boss), mythicActive: Boolean(boss?.isMythic), heroCritical: this.dangerState.heroCritical, coreCritical: this.dangerState.coreCritical });
        if (policy.showAutoLabel && ultraCritical.showAutoLabel) {
            if (cue.specialistIntent && this.specialistIntentAtlasReady && this.specialistIntentAtlasImage) {
                ...
                ctx.globalAlpha = .92;
                ctx.drawImage(this.specialistIntentAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x - size / 2, y - size / 2, size, size);
            }
            ctx.globalAlpha = .9;
            ctx.fillStyle = cue.accent;
            ctx.font = '900 10px system-ui';
            ctx.textAlign = 'center';
            ctx.fillText(cue.label, target.pos.x, target.pos.y - target.radius - 22);
        }
        ctx.restore();
    }
    drawDamageReasonFeedback(ctx) {
        const state = this.damageReasonState;
        if (!state)
            return;
        ...
        ctx.globalAlpha = alpha;
        ctx.textAlign = 'center';
        ctx.font = `900 ${critical ? 15 : 13}px system-ui`;
        ...
        ctx.fillRect(x - width / 2, y - 20, width, 28);
        ctx.strokeStyle = accent;
        ctx.lineWidth = 2;
        ctx.strokeRect(x - width / 2, y - 20, width, 28);
        ...
        ctx.fillStyle = accent;
        ctx.fillText(text, x + (hasIcon ? 10 : 0), y);
        ctx.restore();
    }
    drawBossEncounterNodes(ctx) {
        const primaryWeakpointId = primaryWeakpointNode(this.bossEncounter.nodes, this.hero.pos)?.id ?? null;
        const boss = this.enemies.enemies.find((enemy) => enemy.alive && enemy.type === 'boss') ?? null;
        const policy = combatAttentionPolicy({ heroCritical: this.dangerState.heroCritical, coreCritical: this.dangerState.coreCritical, damageSeverity: this.damageReasonState?.severity ?? null, bossSpecialTimer: boss?.specialTimer ?? 99, reducedFlash: this.presentationSettings.reducedFlash,
            reducedMotion: this.presentationSettings.reducedMotion, bossCountdown: this.enemies.bossCountdown });
        ...
        for (const node of this.bossEncounter.nodes) {
            ...
            if (this.bossWeakpointWorldVfxAtlasReady && this.bossWeakpointWorldVfxAtlasImage) {
                const world = bossWeakpointWorldVfxSprite(node.kind, 'active');
                const worldSize = Math.max(70, node.radius * 2.7);
                ctx.save();
                ctx.globalAlpha = this.presentationSettings.reducedFlash ? .42 : .68;
                ctx.drawImage(this.bossWeakpointWorldVfxAtlasImage, world.sx, world.sy, world.sw, world.sh, -worldSize / 2, -worldSize / 2, worldSize, worldSize);
                ctx.restore();
            }
            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle';
            ...
            ctx.fillStyle = '#31151a';
            ctx.fillRect(-node.radius, node.radius + 8, node.radius * 2, 5);
            ctx.fillStyle = color;
            ctx.fillRect(-node.radius, node.radius + 8, node.radius * 2 * node.hp / Math.max(1, node.maxHp), 5);
            ...
            if (weakCue) {
                ...
                const pulse = 1 + Math.sin(this.elapsed * 7 + node.id) * amplitude;
                ctx.shadowBlur = 0;
                ctx.globalAlpha = .48 + .28 * weakCue.urgency;
                ctx.strokeStyle = weakCue.accent;
                ctx.lineWidth = 3;
                ctx.beginPath();
                ctx.arc(0, 0, weakCue.radius * pulse, 0, Math.PI * 2);
                ctx.stroke();
                if (weakCue.label && policy.showWeakpointLabel && ultraCritical.showWeakpointLabel) {
                    ctx.globalAlpha = .92;
                    ctx.fillStyle = weakCue.accent;
                    ctx.font = '900 10px system-ui';
                    ctx.fillText(weakCue.label, 0, -weakCue.radius - 9);
                }
            }
            ctx.restore();
        }
    }
    drawBossArenaHazards(ctx, motion) {
        this.currentMythicSafeLanePresentation = null;
        const boss = this.enemies.enemies.find((enemy) => enemy.alive && enemy.type === 'boss');
        let safeZone = null;
        let safeZoneLabelAnchor = null;
        ...
        if (boss?.isMythic) {
            ...
            ctx.lineWidth = safeZone.phase === 'collapse' ? 4 : 3;
            ctx.globalAlpha = safeZone.phase === 'collapsed' ? .22 : safeZone.phase === 'collapse' ? .42 : .28;
            if (safeZone.phase !== 'stable')
                ctx.setLineDash([8, 7]);
            ctx.beginPath();
            ctx.arc(zoneCenter.x, zoneCenter.y, safeZone.radius, 0, Math.PI * 2);
            ctx.stroke();
            if (safeZone.active) {
                ctx.globalAlpha *= .18;
                ctx.beginPath();
            ...
            ctx.textAlign = 'center';
            ctx.fillText(safeZone.phase === 'collapsed' ? 'SAFE ZONE · REFORM' : safeZone.phase === 'collapse' ? 'SAFE ZONE · COLLAPSE' : safeZone.label, zoneCenter.x, zoneCenter.y - safeZone.radius - 10);
            ctx.restore();
        }
        const safePreference = safeZone?.active ? { target: safeZone.center, radius: safeZone.radius, weight: safeZone.preferenceWeight } : null;
        ...
        const lawIdentity = boss?.isMythic ? mythicLastLawIdentityProfile(mythicBossProfile(this.elapsed, this.runThreatLevel, boss.bossOrdinal ?? 0), boss.bossArchetype ?? 'inferno', boss.hp / Math.max(1, boss.maxHp), 1 - destroyedRatio) : null;
        ...
        if (safeLane) {
            ...
            const safeLaneRhythmRecovery = safeLaneRhythmRecoveryPresentation({ suppression: 1 - Math.min(safeLaneBossPeak.bossInteriorScale, safeLaneCriticalCorridor.secondaryScale), release: safeLaneSecondaryRecoveryGate.release, stress: Math.max(safeLanePeakBudget.load, safeLaneSecondaryRecoveryGateBudget.hold), confidence: safeLane.confidence, critical: safeLaneFinalSettleBudget.stress > .72 }, this.presentationSettings.reducedMotion, this.presentationSettings.reducedFlash), safeLaneRhythmRecoveryBudget = rhythmRecoveryBudgetPresentation({ recoveringFamilies: 2, stress: Math.max(safeLanePeakBudget.load, safeLaneSecondaryRecoveryGateBudget.hold), criticalCount: safeLaneFinalSettleBudget.stress > .72 ? 2 : 0, safeLaneVisible: true, bossActive: safeLaneFocusTransfer.outgoingScale > .55 }, this.presentationSettings.reducedMotion, this.presentationSettings.reducedFlash);
            const safeLaneTemporalPriority = safeLaneTemporalPriorityPresentation({ confidence: safeLane.confidence, crossing: safeLaneFocusTransfer.transfer, bossPressure: safeLaneFocusTransfer.outgoingScale, secondaryPeak: safeLaneRhythmRecovery.decorationScale, critical: safeLaneFinalSettleBudget.stress > .72 }, this.presentationSettings.reducedMotion, this.presentationSettings.reducedFlash), safeLaneTemporalUnified = unifiedTemporalArbitrationPresentation({ criticalCount: safeLaneFinalSettleBudget.stress > .72 ? 2 : 0, safeLaneVisible: true, laneConfidence: safeLane.confidence, bossActive: safeLaneFocusTransfer.outgoingScale > .55, bossCharge: safeLaneFocusTransfer.outgoingScale, projectilePressure: safeLanePeakBudget.load, impactPressure: 0, hazardPressure: safeLaneFinalSettleBudget.stress, secondaryFamilies: 2, heroProximity: this.dangerState.heroCritical ? 1 : 0 }, this.presentationSettings.reducedMotion, this.presentationSettings.reducedFlash);
            ...
            for (const feather of safeLaneGapFeather.featherSegments) {
                ctx.globalAlpha = safeLaneBaseAlpha * safeLaneHazardRecovery.pathAlphaScale * feather.alphaScale * safeLaneFinalSettle.secondaryScale * safeLaneFinalSettleBudget.secondaryScale * safeLaneSecondaryRecoveryGate.secondaryScale * safeLaneSecondaryRecoveryGateBudget.secondaryScale * safeLaneFocusTransfer.secondaryScale * safeLaneFocusTransferBudget.secondaryScale * safeLaneBossPeak.bossInteriorScale * safeLaneCriticalCorridor.secondaryScale * safeLanePeakBudget.secondaryScale * safeLaneRhythmRecovery.decorationScale * safeLaneRhythmRecoveryBudget.secondaryScale * safeLaneTemporalPriority.secondaryScale * safeLaneTemporalUnified.secondaryScale;
                ctx.beginPath();
            ...
            if (safeLaneGapFeather.locatorVisible) {
                ctx.globalAlpha = safeLaneBaseAlpha * safeLaneHazardOcclusion.locatorAlphaScale * safeLaneHazardRecovery.locatorAlphaScale * safeLaneFinalSettle.secondaryScale * safeLaneFinalSettleBudget.secondaryScale * safeLaneSecondaryRecoveryGate.secondaryScale * safeLaneSecondaryRecoveryGateBudget.secondaryScale * safeLaneFocusTransfer.secondaryScale * safeLaneFocusTransferBudget.secondaryScale * safeLaneBossPeak.bossInteriorScale * safeLaneCriticalCorridor.secondaryScale * safeLanePeakBudget.secondaryScale * safeLaneRhythmRecovery.decorationScale * safeLaneRhythmRecoveryBudget.secondaryScale * safeLaneTemporalPriority.secondaryScale * safeLaneTemporalUnified.secondaryScale;
                ctx.beginPath();
            ...
            if (lawTimeline) {
                ...
                ctx.globalAlpha = .86;
                ctx.fillStyle = lawTimeline.accent;
                ctx.fillRect(barX, barY, barW * Math.max(.06, 1 - lawTimeline.urgency * .72), 4);
            }
            ctx.restore();
        }
        ...
        for (const hazard of this.bossArena.hazards) {
            ...
            if (hazard.launchOrigin && hazardHandoff.launchCueAlpha > 0) {
                ...
                ctx.setLineDash([6, 9]);
                ctx.beginPath();
                ctx.moveTo(hazard.launchOrigin.x, hazard.launchOrigin.y);
                ctx.lineTo(hazard.pos.x, hazard.pos.y);
                ctx.stroke();
                ctx.setLineDash([]);
                ctx.restore();
            }
            ...
            const hazardBossPeakSeparation = bossHazardPeakSeparationPresentation({ bossPeak: hazardFocusTransfer.outgoingScale, hazardPeak: hazardFinalSettle.settle, overlap: hazardFocusTransfer.transfer, critical: hazardFinalSettleBudget.stress > .72 }, this.presentationSettings.reducedMotion, this.presentationSettings.reducedFlash), hazardPeakBudget = crossFamilyPeakBudgetPresentation({ activePeakFamilies: 2, crowd: hazardFinalSettleBudget.stress, criticalCount: hazardFinalSettleBudget.stress > .72 ? 1 : 0, safeLaneVisible: false, bossActive: hazardFocusTransfer.outgoingScale > .55 }, this.presentationSettings.reducedMotion, this.presentationSettings.reducedFlash);
            const hazardRhythmRecovery = bossHazardRhythmRecoveryPresentation({ suppression: 1 - hazardBossPeakSeparation.hazardInteriorScale, release: hazardSecondaryRecoveryGate.release, stress: Math.max(hazardPeakBudget.load, hazardSecondaryRecoveryGateBudget.hold), critical: hazardFinalSettleBudget.stress > .72 }, this.presentationSettings.reducedMotion, this.presentationSettings.reducedFlash), hazardRhythmRecoveryBudget = rhythmRecoveryBudgetPresentation({ recoveringFamilies: 2, stress: Math.max(hazardPeakBudget.load, hazardSecondaryRecoveryGateBudget.hold), criticalCount: hazardFinalSettleBudget.stress > .72 ? 1 : 0, safeLaneVisible: false, bossActive: hazardFocusTransfer.outgoingScale > .55 }, this.presentationSettings.reducedMotion, this.presentationSettings.reducedFlash);
            const hazardTemporalCritical = criticalTemporalPriorityPresentation({ criticalCount: hazardFinalSettleBudget.stress > .72 ? 1 : 0, pressure: hazardRhythmRecoveryBudget.load, secondaryPeak: hazardRhythmRecovery.hazardInteriorScale, heroProximity: this.dangerState.heroCritical ? 1 : 0 }, this.presentationSettings.reducedMotion, this.presentationSettings.reducedFlash), hazardTemporalUnified = unifiedTemporalArbitrationPresentation({ criticalCount: hazardFinalSettleBudget.stress > .72 ? 1 : 0, safeLaneVisible: false, laneConfidence: 0, bossActive: hazardFocusTransfer.outgoingScale > .55, bossCharge: hazardFocusTransfer.outgoingScale, projectilePressure: 0, impactPressure: 0, hazardPressure: hazardRhythmRecoveryBudget.load, secondaryFamilies: 2, heroProximity: this.dangerState.heroCritical ? 1 : 0 }, this.presentationSettings.reducedMotion, this.presentationSettings.reducedFlash);
            const hazardFillAlpha = hazardBaseAlpha * hazardExpiry.fillAlphaScale * denseBattleSafeLane.hazardFillScale * battlefieldHazardReclaim.hazardAlphaScale * hazardSpatial.fillAlphaScale * hazardTemporal.fillAlphaScale * hazardDepthBudget.secondaryAlphaScale * hazardRecoveryBudget.secondaryRecoveryScale * hazardDepthRelease.impactFillAlphaScale * hazardStackOrder.decorationAlphaScale * hazardUnifiedStack.secondaryAlphaScale * hazardSafeLaneSeparation.secondaryAlphaScale * hazardSpatialSeparationBudget.secondaryAlphaScale * hazardSafeLaneRelease.secondaryAlphaScale * hazardSpatialRecoveryBudget.secondaryRecoveryScale * hazardDenseArbitration.fillAlphaScale * hazardDenseBattlefield.secondaryAlphaScale * hazardDepthPlane.fillAlphaScale * hazardDepthPlaneBudget.secondaryAlphaScale * hazardDepthReentry.fillAlphaScale * hazardDepthReentryBudget.secondaryReentryScale * hazardBossFocus.secondaryScale * hazardBossFocusBudget.secondaryScale * hazardCanonicalReacquisition.fillAlphaScale * hazardCanonicalReacquisitionBudget.staleDecorationScale * hazardCriticalReengagement.staleDecorationScale * hazardCriticalReengagementBudget.secondaryScale * hazardSecondaryCeiling.fillScale * hazardSecondaryCeilingBudget.secondaryScale * hazardBossSecondaryCeiling.staleScale * hazardReadabilityContrast.fillScale * hazardReadabilityContrastBudget.secondaryScale * hazardFinalSettle.secondaryScale * hazardFinalSettleBudget.secondaryScale * hazardSecondaryRecoveryGate.secondaryScale * hazardSecondaryRecoveryGateBudget.secondaryScale * hazardFocusTransfer.secondaryScale * hazardFocusTransferBudget.secondaryScale * hazardBossPeakSeparation.hazardInteriorScale * hazardPeakBudget.secondaryScale * hazardRhythmRecovery.hazardInteriorScale * hazardRhythmRecoveryBudget.secondaryScale * hazardTemporalCritical.secondaryScale * hazardTemporalUnified.secondaryScale, hazardEdgeAlpha = Math.max(hazardEffectiveFloor.edgeAlphaFloor * hazardEffectiveFloorBudget.canonicalFloorScale, hazardReadabilityContrast.edgeAlphaFloor * hazardReadabilityContrastBudget.primaryScale, hazardFinalSettle.primaryFloor, hazardBaseAlpha * hazardExpiry.edgeAlphaScale * denseBattleSafeLane.hazardEdgeScale * hazardResidueRelease.hazardEdgeScale * hazardGroundResolution.hazardEdgeAlphaScale * hazardSpatial.hazardEdgeAlphaScale * hazardTemporal.edgeAlphaScale * hazardTelegraphDepth.telegraphEdgeAlphaScale * hazardDepthRelease.telegraphEdgeAlphaScale * hazardSafeLaneSeparation.telegraphEdgeAlphaScale * hazardSafeLaneRelease.telegraphEdgeAlphaScale * hazardDenseArbitration.edgeAlphaScale * hazardDepthPlane.edgeAlphaScale * hazardDepthReentry.edgeAlphaScale * hazardBossFocus.edgeAlphaScale * hazardBossFocusBudget.telegraphEdgeScale * hazardCanonicalReacquisition.edgeAlphaScale * hazardCanonicalReacquisitionBudget.criticalEdgeScale * hazardDirectionReacquisition.edgeDirectionScale * hazardDirectionReacquisitionBudget.primaryDirectionScale * hazardCriticalReengagement.edgeAlphaScale * hazardCriticalReengagementBudget.telegraphEdgeScale);
            ctx.save();
            ...
            if (shape === 'corridor' || shape === 'cross') {
                ...
                ctx.globalAlpha = hazard.telegraph > 0 ? hazardBaseAlpha * hazardTelegraphDepth.telegraphEdgeAlphaScale * hazardDepthRelease.telegraphEdgeAlphaScale * hazardStackOrder.edgeAlphaScale * hazardUnifiedStack.bossTelegraphEdgeAlphaScale : hazardEdgeAlpha;
                ctx.stroke();
                if (shape === 'cross') {
            ...
            if (this.bossArenaLifecycleVfxAtlasReady && this.bossArenaLifecycleVfxAtlasImage) {
    ...
    drawBossArenaHazardIdentity(ctx, kind, x, y, size) {
        if (!this.bossArenaHazardIdentityAtlasReady || !this.bossArenaHazardIdentityAtlasImage)
            return;
        const icon = bossArenaHazardIdentityIcon(kind);
        ctx.save();
        ctx.globalAlpha = .94;
        ctx.fillStyle = 'rgba(4,8,14,.82)';
        ctx.fillRect(x - size / 2 - 2, y - 2, size + 4, size + 4);
        ctx.drawImage(this.bossArenaHazardIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x - size / 2, y, size, size);
        ctx.restore();
    }
    drawMythicArenaGeometrySafeLaneIcon(ctx, boss, destroyedRatio, x, y, size, lawActive) {
        if (!boss.isMythic || this.dangerState.heroCritical || this.dangerState.coreCritical || lawActive)
            return;
        if (!this.mythicArenaGeometryIdentityAtlasReady || !this.mythicArenaGeometryIdentityAtlasImage)
            return;
        const profile = mythicArenaGeometryProfile(boss.bossArchetype ?? 'inferno', destroyedRatio), icon = mythicArenaGeometryIdentityIcon(profile.id);
        ctx.save();
        ctx.globalAlpha = .92;
        ctx.fillStyle = 'rgba(4,8,14,.82)';
        ctx.fillRect(x - 2, y - 2, size + 4, size + 4);
        ctx.drawImage(this.mythicArenaGeometryIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size);
        ctx.restore();
    }
    bossEffectivePressureLastLawActive(boss) {
        if (!boss.isMythic)
            return false;
        const mythic = mythicBossProfile(this.elapsed, this.runThreatLevel, boss.bossOrdinal ?? this.bossesKilled);
    ...
    hideBossEffectivePressureRecall(boss) {
    ...
    preferBossEffectivePressureSummary(boss, lawActive = false) {
        return !lawActive && !this.hideBossEffectivePressureRecall(boss);
    }
    drawBossEffectivePressureRecall(ctx, boss) {
        if (this.hideBossEffectivePressureRecall(boss))
            return;
        const projection = projectBossEffectivePressure(this.enemies.getBossEncounterModifiers());
        ...
        effects.forEach((effect, index) => {
            ...
            ctx.strokeStyle = effect.impact === 'threat' ? 'rgba(255,151,122,.38)' : 'rgba(143,255,211,.34)';
            ctx.lineWidth = 1;
            ctx.strokeRect(x + .5, y + .5, chipW - 1, chipH - 1);
            ...
            ctx.fillText(effect.label, x + 17, y + chipH / 2 + .5);
            ctx.textAlign = 'right';
            ctx.fillStyle = effect.impact === 'threat' ? '#ff977a' : '#8fffd3';
            ctx.fillText(effect.impactLabel, x + chipW - 4, y + chipH / 2 + .5);
            ctx.textAlign = 'left';
        ...
        if (hiddenThreatLabel) {
            ...
            ctx.fillStyle = 'rgba(35,8,8,.92)';
            ctx.fillRect(x, y, overflowW, chipH);
            ctx.strokeStyle = 'rgba(255,151,122,.5)';
            ctx.strokeRect(x + .5, y + .5, overflowW - 1, chipH - 1);
            ctx.fillStyle = '#ffb09c';
            ctx.font = '900 8px system-ui';
            ctx.textAlign = 'center';
            ctx.fillText(hiddenThreatLabel, x + overflowW / 2, y + chipH / 2 + .5);
            ctx.textAlign = 'left';
        }
        ctx.restore();
    }
    hideMythicSafeZonePressureIdentity(boss, lawActive) {
        const { heroCritical, coreCritical } = this.dangerState;
        return heroCritical || coreCritical || lawActive || (boss.specialTimer ?? 99) <= 1.2;
    }
    drawMythicSafeZonePressureHelpers(ctx, boss, projection, anchorX, anchorY, lawActive) {
        if (this.hideMythicSafeZonePressureIdentity(boss, lawActive))
            return;
        if (this.preferBossEffectivePressureSummary(boss, lawActive))
            return;
        const effects = projection.primaryEffects.slice(0, 2);
        if (effects.length === 0)
            return;
        ...
        effects.forEach((effect, index) => {
            ...
            ctx.fillStyle = icon.accent;
            ctx.fillText(effect.label, x + 20, y + chipH / 2 + .5);
        });
        ctx.restore();
    }
    drawMythicSafeZoneLifecycleIcon(ctx, phase, x, y, size, lawActive) {
        if (this.dangerState.heroCritical || this.dangerState.coreCritical || lawActive)
            return;
        if (!this.mythicSafeZoneLifecycleIdentityAtlasReady || !this.mythicSafeZoneLifecycleIdentityAtlasImage)
            return;
        ...
        ctx.fillStyle = 'rgba(4,8,14,.82)';
        ctx.fillRect(x - 2, y - 2, size + 4, size + 4);
        ctx.drawImage(this.mythicSafeZoneLifecycleIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size);
        ctx.restore();
    }
    drawSafeZoneTransitionDirectionIcon(ctx, direction, x, y, size) {
        if (!this.safeZoneTransitionDirectionAtlasReady || !this.safeZoneTransitionDirectionAtlasImage)
            return;
        const icon = safeZoneTransitionDirectionIcon(direction);
        ctx.save();
        ctx.globalAlpha = .94;
        ctx.fillStyle = 'rgba(4,8,14,.82)';
        ctx.fillRect(x - 2, y - 2, size + 4, size + 4);
    ...
    */
    drawMapSafeLaneTransitionVfx(ctx, safeLane, safeLaneVisualTarget = safeLane.target, arrivalAlphaScale = 1) {
        if (this.mapSafeLaneTransitionVfxAtlasReady && this.mapSafeLaneTransitionVfxAtlasImage) {
            ...
            if (distanceToTarget > 42) {
                const pathSize = Math.min(132, Math.max(78, distanceToTarget * .34));
                ctx.save();
                ctx.translate(this.hero.pos.x + dx * .52, this.hero.pos.y + dy * .52);
                ctx.rotate(angle);
                ctx.globalAlpha = transitionAlpha;
                ctx.drawImage(this.mapSafeLaneTransitionVfxAtlasImage, pathSprite.sx, pathSprite.sy, pathSprite.sw, pathSprite.sh, -pathSize / 2, -pathSize / 2, pathSize, pathSize);
                ctx.restore();
            }
    ...
    queueObjectiveActivationMaterializationVfx(objectiveId, x, y) { const maxTtl = 1.08; this.objectiveActivationMaterializationVfx.push({ objectiveId, x, y, ttl: maxTtl, maxTtl }); if (this.objectiveActivationMaterializationVfx.length > 8)
        this.objectiveActivationMaterializationVfx.splice(0, this.objectiveActivationMaterializationVfx.length - 8); }
    drawObjectiveActivationMaterializationVfx(ctx) {
        if (!this.worldVfxLayerAllowed('objective-activation'))
            return;
        if (!this.objectiveActivationMaterializationVfxAtlasReady || !this.objectiveActivationMaterializationVfxAtlasImage)
            return;
        for (const cue of this.objectiveActivationMaterializationVfx) {
            const progress = 1 - Math.max(0, cue.ttl / cue.maxTtl), alphaCap = this.presentationSettings.reducedFlash ? 0.26 : 0.48;
            const materializeSprite = objectiveActivationMaterializationVfxSprite(cue.objectiveId, 'materialize'), locatorSprite = objectiveActivationMaterializationVfxSprite(cue.objectiveId, 'locator');
            const materializeSize = 146 * (.9 + progress * .22), cueAlpha = this.worldVfxCueAlpha('tactical', cue.x, cue.y, materializeSize * .5);
            if (cueAlpha <= 0)
                continue;
            ctx.save();
            ctx.globalAlpha = Math.min(alphaCap, (1 - progress) * .66 + .08) * cueAlpha;
            ctx.drawImage(this.objectiveActivationMaterializationVfxAtlasImage, materializeSprite.sx, materializeSprite.sy, materializeSprite.sw, materializeSprite.sh, cue.x - materializeSize / 2, cue.y - materializeSize / 2, materializeSize, materializeSize);
            ctx.restore();
            const dx = cue.x - this.hero.pos.x, dy = cue.y - this.hero.pos.y, distanceToCue = Math.hypot(dx, dy);
            if (distanceToCue > 54 && progress < .82) {
                const angle = Math.atan2(dy, dx), locatorSize = Math.min(122, Math.max(74, distanceToCue * .24));
                ctx.save();
                ctx.translate(this.hero.pos.x + dx * .48, this.hero.pos.y + dy * .48);
                ctx.rotate(angle);
                ctx.globalAlpha = Math.min(alphaCap * .92, (1 - progress) * .52 + .06) * cueAlpha;
                ctx.drawImage(this.objectiveActivationMaterializationVfxAtlasImage, locatorSprite.sx, locatorSprite.sy, locatorSprite.sw, locatorSprite.sh, -locatorSize / 2, -locatorSize / 2, locatorSize, locatorSize);
                ctx.restore();
            }
        }
    }
    queueBossArenaTransitionWorldVfx(archetype, state, x, y, radius) { const maxTtl = state === 'entrance' ? 1.18 : .96; this.bossArenaTransitionWorldVfx.push({ archetype, state, x, y, radius: Math.max(56, radius), ttl: maxTtl, maxTtl }); if (this.bossArenaTransitionWorldVfx.length > 12)
        this.bossArenaTransitionWorldVfx.splice(0, this.bossArenaTransitionWorldVfx.length - 12); }
    drawBossArenaTransitionWorldVfx(ctx) {
        if (!this.worldVfxLayerAllowed('boss-arena-transition'))
            return;
        if (!this.bossArenaTransitionWorldVfxAtlasReady || !this.bossArenaTransitionWorldVfxAtlasImage)
            return;
        for (const cue of this.bossArenaTransitionWorldVfx) {
            const progress = 1 - Math.max(0, cue.ttl / cue.maxTtl), sprite = bossArenaTransitionWorldVfxSprite(cue.archetype, cue.state), baseSize = Math.max(340, cue.radius * 6.4), size = cue.state === 'entrance' ? baseSize * (.74 + progress * .34) : baseSize * (.92 + progress * .42), alphaCap = this.presentationSettings.reducedFlash ? 0.30 : 0.58, cueAlpha = this.worldVfxCueAlpha('tactical', cue.x, cue.y, size * .5);
            if (cueAlpha <= 0)
                continue;
            ctx.save();
            ctx.globalAlpha = Math.min(alphaCap, (1 - progress) * .64 + .06) * cueAlpha;
            ctx.drawImage(this.bossArenaTransitionWorldVfxAtlasImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, cue.x - size / 2, cue.y - size / 2, size, size);
            ctx.restore();
        }
    }
    drawMapCombatBoundaryWarnings(ctx) { if (!this.mapCombatBoundaryWarningVfxAtlasReady || !this.mapCombatBoundaryWarningVfxAtlasImage)
        return; const mapId = this.terrain.currentLayout.id, alphaCap = this.presentationSettings.reducedFlash ? 0.22 : 0.40; const left = ARENA_MARGIN, right = LOGICAL_WIDTH - ARENA_MARGIN, top = ARENA_MARGIN + 38, bottom = LOGICAL_HEIGHT - ARENA_MARGIN; const boundaryDistances = [this.hero.pos.x - left, right - this.hero.pos.x, this.hero.pos.y - top, bottom - this.hero.pos.y]; let nearestBoundaryIndex = 0; for (let i = 1; i < boundaryDistances.length; i++)
        if (boundaryDistances[i] < boundaryDistances[nearestBoundaryIndex])
            nearestBoundaryIndex = i; const nearestBoundaryDistance = boundaryDistances[nearestBoundaryIndex]; if (nearestBoundaryDistance <= 118) {
        const sprite = mapCombatBoundaryWarningVfxSprite(mapId, 'boundary'), x = nearestBoundaryIndex === 0 ? left : nearestBoundaryIndex === 1 ? right : this.hero.pos.x, y = nearestBoundaryIndex === 2 ? top : nearestBoundaryIndex === 3 ? bottom : this.hero.pos.y, rotation = nearestBoundaryIndex < 2 ? Math.PI / 2 : 0, size = 96;
        ctx.save();
        ctx.translate(x, y);
        ctx.rotate(rotation);
        ctx.globalAlpha = alphaCap * Math.max(.22, 1 - nearestBoundaryDistance / 142);
        ctx.drawImage(this.mapCombatBoundaryWarningVfxAtlasImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, -size / 2, -size / 2, size, size);
        ctx.restore();
    } const obstacleSprite = mapCombatBoundaryWarningVfxSprite(mapId, 'obstacle'); const nearWalls = this.terrain.walls.map((wall) => { const px = clamp(this.hero.pos.x, wall.x, wall.x + wall.w), py = clamp(this.hero.pos.y, wall.y, wall.y + wall.h); return { px, py, distance: Math.hypot(this.hero.pos.x - px, this.hero.pos.y - py) }; }).filter((entry) => entry.distance <= 112).sort((a, b) => a.distance - b.distance).slice(0, 3); for (const entry of nearWalls) {
        const size = 82;
        ctx.save();
        ctx.globalAlpha = alphaCap * Math.max(.18, 1 - entry.distance / 128);
        ctx.drawImage(this.mapCombatBoundaryWarningVfxAtlasImage, obstacleSprite.sx, obstacleSprite.sy, obstacleSprite.sw, obstacleSprite.sh, entry.px - size / 2, entry.py - size / 2, size, size);
        ctx.restore();
    } }
    queueObjectiveFailureDissolveVfx(objectiveId, x, y) { const maxTtl = 1.02; this.objectiveFailureDissolveVfx.push({ objectiveId, x, y, ttl: maxTtl, maxTtl }); if (this.objectiveFailureDissolveVfx.length > 8)
        this.objectiveFailureDissolveVfx.splice(0, this.objectiveFailureDissolveVfx.length - 8); }
    drawObjectiveFailureDissolveVfx(ctx) {
        if (!this.worldVfxLayerAllowed('objective-failure'))
            return;
        if (!this.objectiveFailureDissolveVfxAtlasReady || !this.objectiveFailureDissolveVfxAtlasImage)
            return;
        for (const cue of this.objectiveFailureDissolveVfx) {
            const progress = 1 - Math.max(0, cue.ttl / cue.maxTtl), state = progress < 0.46 ? 'fracture' : 'dissolve', sprite = objectiveFailureDissolveVfxSprite(cue.objectiveId, state), size = (state === 'fracture' ? 148 : 166) * (1 + progress * .15), alphaCap = this.presentationSettings.reducedFlash ? 0.28 : 0.54, cueAlpha = this.worldVfxCueAlpha('tactical', cue.x, cue.y, size * .5);
            if (cueAlpha <= 0)
                continue;
            ctx.save();
            ctx.globalAlpha = Math.min(alphaCap, (1 - progress) * .68 + .05) * cueAlpha;
            ctx.drawImage(this.objectiveFailureDissolveVfxAtlasImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, cue.x - size / 2, cue.y - size / 2, size, size);
            ctx.restore();
        }
    }
    queueFieldEventLifecycleWorldVfx(eventId, state, x, y) { const maxTtl = state === 'entrance' ? 1.06 : .88; this.fieldEventLifecycleWorldVfx.push({ eventId, state, x, y, ttl: maxTtl, maxTtl }); if (this.fieldEventLifecycleWorldVfx.length > 10)
        this.fieldEventLifecycleWorldVfx.splice(0, this.fieldEventLifecycleWorldVfx.length - 10); }
    finishFieldEventLifecycleWorldVfx(event) { const anchor = this.fieldEventWorldAnchor?.id === event.id ? this.fieldEventWorldAnchor : { id: event.id, x: this.hero.pos.x, y: this.hero.pos.y }; this.queueFieldEventLifecycleWorldVfx(event.id, 'exit', anchor.x, anchor.y); this.fieldEventWorldAnchor = null; }
    drawFieldEventLifecycleWorldVfx(ctx) {
        if (!this.worldVfxLayerAllowed('field-event-lifecycle'))
            return;
        if (this.worldVfxLayerAlpha('informational') <= 0)
            return;
        if (!this.fieldEventLifecycleWorldVfxAtlasReady || !this.fieldEventLifecycleWorldVfxAtlasImage)
            return;
        for (const cue of this.fieldEventLifecycleWorldVfx) {
            const progress = 1 - Math.max(0, cue.ttl / cue.maxTtl), sprite = fieldEventLifecycleWorldVfxSprite(cue.eventId, cue.state), size = (cue.state === 'entrance' ? 172 : 154) * (1 + (cue.state === 'entrance' ? progress * .18 : progress * .28)), alphaCap = this.presentationSettings.reducedFlash ? 0.26 : 0.50, cueAlpha = this.worldVfxCueAlpha('informational', cue.x, cue.y, size * .5);
            if (cueAlpha <= 0)
                continue;
            ctx.save();
            ctx.globalAlpha = Math.min(alphaCap, (1 - progress) * .62 + .05) * cueAlpha;
            ctx.drawImage(this.fieldEventLifecycleWorldVfxAtlasImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, cue.x - size / 2, cue.y - size / 2, size, size);
            ctx.restore();
        }
    }
    queueElitePackApproachFormationVfx(enemyIds, target) { if (enemyIds.length < 2)
        return; const maxTtl = 4.4; this.elitePackApproachFormationVfx.push({ enemyIds: [...enemyIds], target, ttl: maxTtl, maxTtl }); if (this.elitePackApproachFormationVfx.length > 3)
        this.elitePackApproachFormationVfx.splice(0, this.elitePackApproachFormationVfx.length - 3); }
    drawElitePackApproachFormationVfx(ctx) {
        if (!this.worldVfxLayerAllowed('elite-pack-approach'))
            return;
        if (!this.elitePackApproachFormationVfxAtlasReady || !this.elitePackApproachFormationVfxAtlasImage)
            return;
        for (const cue of this.elitePackApproachFormationVfx) {
            const members = cue.enemyIds.map(id => this.enemies.enemies.find(enemy => enemy.alive && enemy.id === id)).filter((enemy) => Boolean(enemy));
            if (members.length < 2)
                continue;
            const centroid = { x: members.reduce((sum, enemy) => sum + enemy.pos.x, 0) / members.length, y: members.reduce((sum, enemy) => sum + enemy.pos.y, 0) / members.length };
            const spread = Math.max(...members.map(enemy => Math.hypot(enemy.pos.x - centroid.x, enemy.pos.y - centroid.y)));
            const targetPos = cue.target === 'hero' ? this.hero.pos : this.core.pos, dx = targetPos.x - centroid.x, dy = targetPos.y - centroid.y, distanceToTarget = Math.hypot(dx, dy), angle = Math.atan2(dy, dx), alphaCap = this.presentationSettings.reducedFlash ? 0.22 : 0.42;
            const formation = elitePackApproachFormationVfxSprite(cue.target, 'formation'), formationSize = Math.max(126, Math.min(250, spread * 2.25 + 92)), formationAlpha = this.worldVfxCueAlpha('tactical', centroid.x, centroid.y, formationSize * .5);
            if (formationAlpha > 0) {
                ctx.save();
                ctx.globalAlpha = alphaCap * formationAlpha;
                ctx.drawImage(this.elitePackApproachFormationVfxAtlasImage, formation.sx, formation.sy, formation.sw, formation.sh, centroid.x - formationSize / 2, centroid.y - formationSize / 2, formationSize, formationSize);
                ctx.restore();
            }
            if (distanceToTarget > 96) {
                const approach = elitePackApproachFormationVfxSprite(cue.target, 'approach'), approachSize = Math.min(146, Math.max(86, distanceToTarget * .28)), approachX = centroid.x + dx * .43, approachY = centroid.y + dy * .43, approachAlpha = this.worldVfxCueAlpha('tactical', approachX, approachY, approachSize * .5);
                if (approachAlpha > 0) {
                    ctx.save();
                    ctx.translate(approachX, approachY);
                    ctx.rotate(angle);
                    ctx.globalAlpha = alphaCap * .92 * approachAlpha;
                    ctx.drawImage(this.elitePackApproachFormationVfxAtlasImage, approach.sx, approach.sy, approach.sw, approach.sh, -approachSize / 2, -approachSize / 2, approachSize, approachSize);
                    ctx.restore();
                }
            }
            if (distanceToTarget < 360) {
                const focus = elitePackApproachFormationVfxSprite(cue.target, 'focus'), focusSize = 84, focusAlpha = this.worldVfxCueAlpha('tactical', targetPos.x, targetPos.y, focusSize * .5);
                if (focusAlpha > 0) {
                    ctx.save();
                    ctx.globalAlpha = alphaCap * .78 * focusAlpha;
                    ctx.drawImage(this.elitePackApproachFormationVfxAtlasImage, focus.sx, focus.sy, focus.sw, focus.sh, targetPos.x - focusSize / 2, targetPos.y - focusSize / 2, focusSize, focusSize);
                    ctx.restore();
                }
            }
        }
    }
    updateBattlefieldObjective(dt) {
        const active = this.objectiveRuntime.active;
        if (!active)
            return;
        let nearbyEnemies = 0;
        for (const enemy of this.enemies.enemies) {
            if (enemy.alive && distance(enemy.pos, active.pos) <= 155 + enemy.radius)
                nearbyEnemies += 1;
        }
    ...
    queueObjectiveCompletionCeremonyVfx(objectiveId, x, y, rewards) { const maxTtl = .86; this.objectiveCompletionCeremonyVfx.push({ objectiveId, x, y, rewardCount: rewards.length, ttl: maxTtl, maxTtl }); if (this.objectiveCompletionCeremonyVfx.length > 8)
        this.objectiveCompletionCeremonyVfx.splice(0, this.objectiveCompletionCeremonyVfx.length - 8); }
    drawObjectiveCompletionCeremonyVfx(ctx) {
        if (!this.worldVfxLayerAllowed('objective-completion'))
            return;
        if (!this.objectiveCompletionCeremonyVfxAtlasReady || !this.objectiveCompletionCeremonyVfxAtlasImage)
            return;
        for (const cue of this.objectiveCompletionCeremonyVfx) {
            const progress = 1 - Math.max(0, cue.ttl / cue.maxTtl), state = progress < 0.48 ? 'burst' : 'reward', sprite = objectiveCompletionCeremonyVfxSprite(cue.objectiveId, state), rewardScale = 1 + Math.min(2, cue.rewardCount) * .06, size = (state === 'burst' ? 150 : 128) * rewardScale * (1 + progress * .16), cueAlpha = this.worldVfxCueAlpha('informational', cue.x, cue.y, size * .5);
            if (cueAlpha <= 0)
                continue;
            ctx.save();
            ctx.globalAlpha = Math.min(this.presentationSettings.reducedFlash ? 0.34 : 0.66, (1 - progress) * .78 + .06) * cueAlpha;
            ctx.drawImage(this.objectiveCompletionCeremonyVfxAtlasImage, sprite.sx, sprite.sy, sprite.sw, sprite.sh, cue.x - size / 2, cue.y - size / 2, size, size);
            ctx.restore();
        }
    }
    applyObjectiveRewards(rewards) {
    ...
    drawBattlefieldObjective(ctx) {
        ...
        ctx.lineWidth = 5;
        ctx.beginPath();
        ctx.arc(0, 0, active.id === 'beaconDefense' ? 72 : 58, 0, Math.PI * 2);
        ctx.stroke();
        ctx.globalAlpha = 0.18;
        ctx.fillStyle = def.accent;
        ctx.beginPath();
        ctx.arc(0, 0, active.id === 'beaconDefense' ? 68 : 54, 0, Math.PI * 2);
        ctx.fill();
        ctx.globalAlpha = 1;
        ...
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.arc(0, 0, active.id === 'beaconDefense' ? 82 : 68, -Math.PI / 2, -Math.PI / 2 + Math.PI * 2 * clamp(ratio, 0, 1));
        ctx.stroke();
        ...
        if (!this.hideObjectiveHelperIdentity())
            this.drawObjectiveActionIdentity(ctx, active.id, active.id === 'beaconDefense' ? 34 : 30, -34, 22);
        ctx.fillStyle = '#fff';
    ...
    hideRunMissionHelperIdentity() {
        const boss = this.enemies.enemies.find(enemy => enemy.alive && enemy.type === 'boss') ?? null;
        const heroCritical = this.dangerState.heroCritical, coreCritical = this.dangerState.coreCritical, bossSpecialTimer = boss?.specialTimer ?? 99;
        return heroCritical || coreCritical || bossSpecialTimer <= 1.2;
    }
    drawRunMissionPaceRewardRecall(ctx, paceId, reward, y) {
        if (this.hideRunMissionHelperIdentity())
            return;
        const size = 18, dy = y + 21;
        if (this.runMissionPaceIdentityAtlasReady && this.runMissionPaceIdentityAtlasImage) {
            const pace = runMissionPaceIdentityIcon(paceId), image = this.runMissionPaceIdentityAtlasImage, x = 900;
            ctx.save();
            ctx.globalAlpha = .96;
            ctx.fillStyle = 'rgba(4,8,14,.9)';
            ctx.fillRect(x - 2, dy - 2, size + 4, size + 4);
            ctx.drawImage(image, pace.sx, pace.sy, pace.sw, pace.sh, x, dy, size, size);
            ctx.restore();
        }
        if (this.objectiveRewardIdentityAtlasReady && this.objectiveRewardIdentityAtlasImage) {
            const icon = objectiveRewardIdentityIcon(reward.kind), image = this.objectiveRewardIdentityAtlasImage, x = 944;
            ctx.save();
            ctx.globalAlpha = .96;
            ctx.fillStyle = 'rgba(4,8,14,.9)';
            ctx.fillRect(x - 2, dy - 2, size + 4, size + 4);
            ctx.drawImage(image, icon.sx, icon.sy, icon.sw, icon.sh, x, dy, size, size);
            ctx.fillStyle = '#eef8ff';
            ctx.font = '800 10px system-ui';
            ctx.textAlign = 'left';
            ctx.fillText(`×${reward.amount}`, x + 20, dy + 13);
            ctx.restore();
        }
    }
    showRunMissionCompletionToast(mission) {
    ...
    showRunMissionFailureToast(mission) {
        this.showTacticalStatusEventToast(`미션 종료 · ${mission.name}`, mission.id);
        this.eventToastMissionReward = null;
    }
    hideObjectiveHelperIdentity() {
        const boss = this.enemies.enemies.find(enemy => enemy.alive && enemy.type === 'boss') ?? null;
        const heroCritical = this.dangerState.heroCritical, coreCritical = this.dangerState.coreCritical, bossSpecialTimer = boss?.specialTimer ?? 99;
        return heroCritical || coreCritical || bossSpecialTimer <= 1.2;
    }
    hideFieldEventHelperIdentity() {
        const boss = this.enemies.enemies.find(enemy => enemy.alive && enemy.type === 'boss') ?? null;
        const heroCritical = this.dangerState.heroCritical, coreCritical = this.dangerState.coreCritical, bossSpecialTimer = boss?.specialTimer ?? 99;
        return heroCritical || coreCritical || bossSpecialTimer <= 1.2;
    }
    drawFieldEventResponseIdentity(ctx, eventId, x, y, size) {
        if (this.hideFieldEventHelperIdentity() || !this.fieldEventResponseIdentityAtlasReady || !this.fieldEventResponseIdentityAtlasImage)
            return;
        const icon = fieldEventResponseIdentityIcon(fieldEventResponseIdentityForEvent(eventId));
        const image = this.fieldEventResponseIdentityAtlasImage;
        ctx.save();
        ctx.globalAlpha = .96;
        ctx.fillStyle = 'rgba(4,8,14,.9)';
        ctx.fillRect(x - size / 2 - 2, y - size / 2 - 2, size + 4, size + 4);
        ctx.drawImage(image, icon.sx, icon.sy, icon.sw, icon.sh, x - size / 2, y - size / 2, size, size);
        ctx.restore();
    }
    drawFieldEventEffectProfileIdentity(ctx, eventId, x, y, size) {
        if (this.hideFieldEventHelperIdentity() || !this.fieldEventEffectProfileIdentityAtlasReady || !this.fieldEventEffectProfileIdentityAtlasImage)
            return;
        const icon = fieldEventEffectProfileIdentityIcon(fieldEventEffectProfileIdentityForEvent(eventId));
        const image = this.fieldEventEffectProfileIdentityAtlasImage;
        ctx.save();
        ctx.globalAlpha = .96;
        ctx.fillStyle = 'rgba(4,8,14,.9)';
        ctx.fillRect(x - size / 2 - 2, y - size / 2 - 2, size + 4, size + 4);
    ...
    drawFieldEventResponseEffectRecall(ctx, eventId, y) {
    ...
    drawGoldenGoblinEventResponseIdentity(ctx) {
        if (this.fieldEvents.active?.id !== 'goldenGoblin' || this.goldenGoblinEnemyId === null)
            return;
        const enemy = this.enemies.enemies.find(value => value.alive && value.id === this.goldenGoblinEnemyId);
        if (!enemy)
            return;
        this.drawFieldEventResponseIdentity(ctx, 'goldenGoblin', enemy.pos.x, enemy.pos.y - enemy.radius - 26, 20);
    }
    drawObjectiveActionIdentity(ctx, objectiveId, x, y, size) {
        if (!this.objectiveActionIdentityAtlasReady || !this.objectiveActionIdentityAtlasImage)
            return;
        ...
        ctx.fillRect(x - size / 2 - 2, y - size / 2 - 2, size + 4, size + 4);
        ctx.drawImage(this.objectiveActionIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x - size / 2, y - size / 2, size, size);
        ctx.restore();
    }
    drawObjectiveRewardPreview(ctx, rewards, rewardMultiplier, y) {
        if (!this.objectiveRewardIdentityAtlasReady || !this.objectiveRewardIdentityAtlasImage)
            return;
    ...
    showObjectiveCompletionToast(id, rewards) {
    ...
    showRunContractSuccessToast(family) {
        this.showEventToast('런 계약 성공 · 90초 강화 획득', null, null, null, null, null, family);
        this.eventToastContractBoonFamily = family;
    }
    showEventToast(message, lastLawId = null, mythicTacticArchetype = null, ascensionMutator = null, fatePath = null, oathKind = null, contractFamily = null, relicResonance = null, nemesisAdaptations = [], worldEvolution = null, bossArenaMutation = null, visualIdentity = null) {
        if (!eightTwelveHourToastFocus(this.elapsed, message).show)
            return;
        if (!fourEightHourToastFocus(this.elapsed, message).show)
            return;
    ...
    showBattlefieldEvolutionToast(stage) {
    ...
    showAscensionTierEventToast(tier) {
    ...
    showAscensionMutatorEventToast(effect) {
        const projection = this.eventToastAscensionTierProjection ?? projectAscensionTierOutcome(this.endlessState.ascension.tier);
        this.showEventToast(`승천 변이 · ${this.endlessMutatorName(effect.mutator)}`, null, null, effect.mutator);
        this.eventToastAscensionTierProjection = projection;
    }
    showBuildOverdriveActivationToast(archetype, state) {
    ...
    showHeroMeterEventToast(message, id) { this.showEventToast(message, null, null, null, null, null, null, null, [], null, null, { heroMeterId: id }); }
    showArcaneComboEventToast(message, family) { this.showEventToast(message, null, null, null, null, null, null, null, [], null, null, { arcaneComboFamily: family }); }
    showTacticalStatusEventToast(message, id) { this.showEventToast(message, null, null, null, null, null, null, null, [], null, null, { tacticalStatusIconId: id }); }
    showFieldEventStartToast(message, id) { this.showEventToast(message, null, null, null, null, null, null, null, [], null, null, { tacticalStatusIconId: id, fieldEventId: id }); }
    showBuildIdentityEventToast(message, id) { this.showEventToast(message, null, null, null, null, null, null, null, [], null, null, { buildIdentityId: id }); }
    showSynergyEventToast(message, id) { this.showEventToast(message, null, null, null, null, null, null, null, [], null, null, { synergyId: id }); }
    showLegendaryAwakeningEventToast(message, id) { this.showEventToast(message, null, null, null, null, null, null, null, [], null, null, { legendaryItemId: id }); }
    showSpellEvolutionEventToast(message, spellId, tier, projection = null) { this.showEventToast(message, null, null, null, null, null, null, null, [], null, null, { spellEvolution: { spellId, tier } }); this.eventToastSpellEvolutionProjection = projection; }
    showRunTraitEventToast(message, id) { this.showEventToast(message, null, null, null, null, null, null, null, [], null, null, { runTraitId: id }); }
    showDeepRunAscensionEventToast(message, id, projection) { this.showEventToast(message, null, null, null, null, null, null, null, [], null, null, { ascensionSelectionId: id }); this.eventToastHeroAscensionProjection = projection; }
    showFusionProjectionEventToast(message, id, projection) { this.showBuildIdentityEventToast(message, id); this.eventToastFusionProjection = projection; }
    showMythicPhaseEventToast(message, phase, bossArenaMutation = null) { this.showEventToast(message, null, null, null, null, null, null, null, [], null, bossArenaMutation, { mythicPhase: phase }); }
    showPerfectEvadeEventToast(message, streak) { this.showEventToast(message, null, null, null, null, null, null, null, [], null, null, { perfectEvadeStreak: normalizedPerfectEvadeStreak(streak) }); }
    syncRunFoundationIdentityTracker(showToast = true) {
        const trait = this.selectedTrait;
        if (!this.runFoundationIdentityInitialized) {
            this.lastRunFoundationTrait = trait;
            this.runFoundationIdentityInitialized = true;
            if (showToast && trait)
                this.showRunTraitEventToast(`전투 성향 · ${[...RUN_TRAITS, ...MASTERY_RUN_TRAITS].find(v => v.id === trait)?.name ?? trait}`, trait);
            return;
        }
        this.lastRunFoundationTrait = trait;
    }
    drawRunTraitRecall(ctx, x, y, size = 18) {
        const id = this.selectedTrait;
        if (!id || !this.decisionPathIconAtlasReady || !this.decisionPathIconAtlasImage)
            return;
        const icon = runTraitIdentity(id);
        ctx.save();
        ctx.globalAlpha = .95;
        ctx.fillStyle = 'rgba(6,12,22,.9)';
        ctx.fillRect(x - 1, y - 1, size + 2, size + 2);
        ctx.drawImage(this.decisionPathIconAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size);
        ctx.restore();
    }
    notifySpellEvolutionIfChanged(spellId, beforeTier) {
        const currentLevel = this.spells.levels[spellId], evolution = spellEvolution(this.hero.profileId, spellId, currentLevel);
        if (evolution.tier === 0 || evolution.tier <= beforeTier)
            return false;
        const tier = evolution.tier, projection = projectSpellEvolutionSelection(this.hero.profileId, spellId, currentLevel - 1);
    ...
    drawHeroMeterToastIcon(ctx) {
        const id = this.eventToastHeroMeterId;
        if (!id || !this.heroMeterIdentityAtlasReady || !this.heroMeterIdentityAtlasImage)
            return;
        const icon = heroMeterIdentityIcon(id);
        ctx.save();
        ctx.globalAlpha = .96;
        ctx.shadowColor = icon.accent;
        ctx.shadowBlur = 7;
        ctx.drawImage(this.heroMeterIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, 602, 827, 28, 28);
        ctx.restore();
    }
    drawArcaneComboToastIcon(ctx) {
        const family = this.eventToastArcaneComboFamily;
        if (!family || !this.arcaneComboIdentityAtlasReady || !this.arcaneComboIdentityAtlasImage)
            return;
        const icon = arcaneComboIdentityIcon(family);
        ctx.save();
        ctx.globalAlpha = .96;
        ctx.drawImage(this.arcaneComboIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, 602, 827, 28, 28);
        ctx.restore();
    }
    drawTacticalStatusToastIcon(ctx) {
        const id = this.eventToastTacticalStatusIconId;
        if (!id || !this.tacticalStatusIconAtlasReady || !this.tacticalStatusIconAtlasImage)
            return;
        const icon = tacticalStatusIconPresentation(id);
        if (!icon.visible)
            return;
        const sp = icon.sprite;
        ctx.save();
        ctx.globalAlpha = .96;
        ctx.drawImage(this.tacticalStatusIconAtlasImage, sp.sx, sp.sy, sp.sw, sp.sh, 602, 827, 28, 28);
        ctx.restore();
    }
    drawFieldEventToastHelperIcons(ctx) {
        const id = this.eventToastFieldEventId;
        if (!id || this.hideFieldEventHelperIdentity())
            return;
        this.drawFieldEventResponseIdentity(ctx, id, 648, 841, 24);
        this.drawFieldEventEffectProfileIdentity(ctx, id, 680, 841, 24);
    }
    drawObjectiveRewardToastIcons(ctx) {
        if (this.eventToastObjectiveRewards.length === 0 || !this.objectiveRewardIdentityAtlasReady || !this.objectiveRewardIdentityAtlasImage)
            return;
    ...
    drawRunMissionToastRewardIcon(ctx) {
        const reward = this.eventToastMissionReward;
        if (!reward || this.hideRunMissionHelperIdentity() || !this.objectiveRewardIdentityAtlasReady || !this.objectiveRewardIdentityAtlasImage)
            return;
        const icon = objectiveRewardIdentityIcon(reward.kind), image = this.objectiveRewardIdentityAtlasImage, size = 24, x = 634;
        ctx.save();
        ctx.globalAlpha = .96;
        ctx.fillStyle = 'rgba(4,8,14,.92)';
        ctx.fillRect(x - 1, 827, size + 2, size + 2);
        ctx.drawImage(image, icon.sx, icon.sy, icon.sw, icon.sh, x, 828, size, size);
        ctx.fillStyle = '#fff';
        ctx.font = '800 9px system-ui';
        ctx.textAlign = 'left';
        ctx.fillText(`×${reward.amount}`, x + 25, 844);
        ctx.restore();
    }
    drawBuildIdentityToastIcon(ctx) {
        const id = this.eventToastBuildIdentityId;
        if (!id || !this.buildIdentityAtlasReady || !this.buildIdentityAtlasImage)
            return;
    ...
    drawSynergyToastIcon(ctx) {
        const id = this.eventToastSynergyId;
        if (!id || !this.synergyIdentityAtlasReady || !this.synergyIdentityAtlasImage)
            return;
        const icon = synergyIdentityIcon(id);
        ctx.save();
        ctx.globalAlpha = .96;
        ctx.drawImage(this.synergyIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, 602, 827, 28, 28);
        ctx.restore();
    }
    drawLegendaryAwakeningToastIcon(ctx) {
        const id = this.eventToastLegendaryItemId;
        if (!id || !this.legendaryAwakeningAtlasReady || !this.legendaryAwakeningAtlasImage)
            return;
        const recall = legendaryProcIdentity({ type: id === 'blast-rod' ? 'nova' : id === 'golden-wand' ? 'bonusGold' : id === 'magnet-cloak' ? 'magnet' : 'coreHeal', ...(id === 'blast-rod' ? { radius: 0 } : id === 'golden-wand' ? { amount: 0 } : id === 'magnet-cloak' ? { duration: 0 } : { fraction: 0 }) }, this.equipmentState);
        const icon = recall?.icon;
        if (!icon)
            return;
        ctx.save();
        ctx.globalAlpha = .96;
        ctx.drawImage(this.legendaryAwakeningAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, 602, 827, 28, 28);
        ctx.restore();
    }
    drawSpellEvolutionToastIcon(ctx) {
        const state = this.eventToastSpellEvolution;
        if (!state || !this.spellEvolutionCrestAtlasReady || !this.spellEvolutionCrestAtlasImage || !this.heroAbilityIconAtlasReady || !this.heroAbilityIconAtlasImage)
            return;
        const base = heroAbilityIdentityIcon(this.hero.profileId, { fireBolt: 'spell1', chainLightning: 'spell2', frostNova: 'spell3', flameField: 'spell4', meteorStorm: 'ultimate1', blackHole: 'ultimate2' }[state.spellId]);
        const crest = spellEvolutionCrestFor(this.hero.profileId, state.tier);
        if (!crest)
            return;
        ctx.save();
        ctx.globalAlpha = .96;
        ctx.drawImage(this.heroAbilityIconAtlasImage, base.sx, base.sy, base.sw, base.sh, 596, 826, 30, 30);
        ctx.fillStyle = 'rgba(5,10,18,.92)';
        ctx.fillRect(618, 840, 18, 18);
        ctx.drawImage(this.spellEvolutionCrestAtlasImage, crest.sx, crest.sy, crest.sw, crest.sh, 619, 841, 16, 16);
        ctx.strokeStyle = state.tier === 2 ? '#ffe794' : 'rgba(210,235,255,.86)';
        ctx.lineWidth = 1.5;
        ctx.strokeRect(618.5, 840.5, 17, 17);
        ctx.restore();
    }
    hideSpellEvolutionProjectionIdentity() {
    ...
    drawSpellEvolutionProjectionToastIcons(ctx) {
        const projection = this.eventToastSpellEvolutionProjection;
        if (!projection || this.hideSpellEvolutionProjectionIdentity() || !this.spellEvolutionModifierIdentityAtlasReady || !this.spellEvolutionModifierIdentityAtlasImage)
            return;
        const size = 20, y = 830;
    ...
    drawSpellEvolutionActionCrest(ctx, actionId, x, y, radius) {
        const spellId = spellEvolutionSpellForAction(actionId);
        if (!spellId)
            return;
        const tier = spellEvolutionTier(this.spells.levels[spellId]);
        if (tier === 0 || !this.spellEvolutionCrestAtlasReady || !this.spellEvolutionCrestAtlasImage)
            return;
        const crest = spellEvolutionCrestFor(this.hero.profileId, tier);
        if (!crest)
            return;
        ...
        ctx.fillRect(dx - 1, dy - 1, size + 2, size + 2);
        ctx.drawImage(this.spellEvolutionCrestAtlasImage, crest.sx, crest.sy, crest.sw, crest.sh, dx, dy, size, size);
        ctx.strokeStyle = tier === 2 ? '#ffe794' : 'rgba(196,229,255,.78)';
        ctx.lineWidth = tier === 2 ? 2 : 1;
        ctx.strokeRect(dx - .5, dy - .5, size + 1, size + 1);
        ctx.restore();
    }
    drawMythicPhaseToastIcon(ctx) {
        const phase = this.eventToastMythicPhase;
        if (!phase || !this.mythicPhaseIdentityAtlasReady || !this.mythicPhaseIdentityAtlasImage)
            return;
        const icon = mythicPhaseIdentityIcon(phase), x = this.eventToastBossArenaMutation ? 634 : 602;
        ctx.save();
        ctx.globalAlpha = .96;
        ctx.fillStyle = 'rgba(5,9,16,.9)';
        ctx.fillRect(x - 1, 826, 30, 30);
        ctx.drawImage(this.mythicPhaseIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, 827, 28, 28);
        ctx.restore();
    }
    drawBossWeakpointBreakToastIcon(ctx) {
        const archetype = this.eventToastBossWeakpointBreakArchetype;
        if (!archetype || !this.bossWeakpointBreakIdentityAtlasReady || !this.bossWeakpointBreakIdentityAtlasImage)
            return;
        const icon = bossWeakpointBreakIdentityIcon(archetype);
        ctx.save();
        ctx.globalAlpha = .96;
        ctx.fillStyle = 'rgba(5,9,16,.9)';
        ctx.fillRect(600, 825, 32, 32);
        ctx.drawImage(this.bossWeakpointBreakIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, 602, 827, 28, 28);
        ctx.restore();
    }
    drawBossPhaseEscalationCueIcon(ctx) {
        if (!this.bossPhaseCue)
            return;
        ...
        ctx.globalAlpha = .97;
        ctx.fillStyle = 'rgba(4,8,14,.92)';
        ctx.fillRect(584, 188, 36, 36);
        ctx.drawImage(image, icon.sx, icon.sy, icon.sw, icon.sh, 586, 190, 32, 32);
        ctx.restore();
    }
    drawBossPhaseEscalationRecall(ctx, boss) {
        if (boss.isMythic)
            return;
        if (this.dangerState.heroCritical || this.dangerState.coreCritical)
            return;
        if ((boss.specialTimer ?? 99) <= 1.2)
            return;
        const phase = bossPhaseForRatio(boss.hp / Math.max(1, boss.maxHp));
        if (phase === 1)
            return;
        const archetype = boss.bossArchetype ?? 'inferno';
        ...
        const ready = phase === 2 ? this.bossPhase2EscalationAtlasReady : this.bossPhase3EnrageAtlasReady;
        if (!ready || !image)
            return;
        ...
        ctx.fillRect(x - 2, y - 2, size + 4, size + 4);
        ctx.drawImage(image, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size);
        ctx.restore();
    }
    drawBossCounterplayBenefitRecall(ctx, boss) {
        if (this.dangerState.heroCritical || this.dangerState.coreCritical)
            return;
        if ((boss.specialTimer ?? 99) <= 1.2)
            return;
        ...
        if (boss.isMythic) {
            ...
            const lastLaw = mythicLastLawIdentityProfile(mythic, archetype, boss.hp / Math.max(1, boss.maxHp), weakpointRatio);
            if (lastLaw.active)
                return;
        }
        if (!bossCounterplayBenefitActive(archetype, this.bossEncounter.modifiers))
            return;
        if (!this.bossCounterplayBenefitIdentityAtlasReady || !this.bossCounterplayBenefitIdentityAtlasImage)
            return;
        ...
        ctx.fillStyle = 'rgba(5,9,16,.84)';
        ctx.fillRect(x - 2, y - 2, size + 4, size + 4);
        ctx.drawImage(this.bossCounterplayBenefitIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size);
        ctx.restore();
    }
    drawBossVariantPressureRecall(ctx, boss) {
        if (boss.isMythic)
            return;
        if (this.dangerState.heroCritical || this.dangerState.coreCritical)
            return;
        if ((boss.specialTimer ?? 99) <= 1.2)
            return;
        const tier = boss.bossVariantTier ?? 0;
        if (tier <= 0 || !this.bossVariantPressureAtlasReady || !this.bossVariantPressureAtlasImage)
            return;
        ...
        ctx.fillStyle = 'rgba(5,9,16,.84)';
        ctx.fillRect(x - 2, y - 2, size + 4, size + 4);
        ctx.drawImage(this.bossVariantPressureAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size);
        if (badge) {
            ctx.fillStyle = 'rgba(4,8,14,.95)';
            ctx.fillRect(x + size - 7, y + size - 7, 9, 9);
            ctx.fillStyle = icon.accent;
            ctx.font = '900 6px system-ui';
            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle';
            ctx.fillText(badge, x + size - 2.5, y + size - 2.5);
        }
        ctx.restore();
    }
    drawApexSecondaryPatternRecall(ctx, boss) {
        if (boss.isMythic)
            return;
        if (this.dangerState.heroCritical || this.dangerState.coreCritical)
            return;
        if ((boss.specialTimer ?? 99) <= 1.2)
            return;
        const secondary = boss.apexSecondaryArchetype;
        if (!boss.isApex || !secondary || !this.apexSecondaryPatternAtlasReady || !this.apexSecondaryPatternAtlasImage)
            return;
        ...
        ctx.fillStyle = 'rgba(5,9,16,.84)';
        ctx.fillRect(x - 2, y - 2, size + 4, size + 4);
        ctx.drawImage(this.apexSecondaryPatternAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size);
        ctx.restore();
    }
    drawBossArchetypeRecall(ctx, boss) {
        if (!this.bossArchetypeIdentityAtlasReady || !this.bossArchetypeIdentityAtlasImage)
            return;
        ...
        ctx.fillStyle = 'rgba(5,9,16,.84)';
        ctx.fillRect(x - 2, y - 2, size + 4, size + 4);
        ctx.drawImage(this.bossArchetypeIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size);
        ctx.restore();
    }
    drawBossSpecialIntentCue(ctx) {
        const boss = this.enemies.enemies.find((enemy) => enemy.alive && enemy.type === 'boss');
        if (!boss || !this.bossSpecialIntentAtlasReady || !this.bossSpecialIntentAtlasImage)
            return;
        const timer = boss.specialTimer ?? 99, segments = bossSpecialIntentSegments(timer);
        if (segments === 0)
            return;
        ...
        ctx.beginPath();
        ctx.roundRect(x - 4, y - 4, size + 8, size + 15, 7);
        ctx.fill();
        ctx.drawImage(this.bossSpecialIntentAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size);
        ctx.fillStyle = icon.accent;
        for (let i = 0; i < segments; i++)
            ctx.fillRect(x + i * (size / 3) + 1, y + size + 3, Math.max(4, size / 3 - 2), 3);
        this.drawApexSecondarySpecialIntent(ctx, boss, timer, segments, x + size + 7, y, Math.max(18, size - 4));
        ctx.restore();
    }
    drawApexSecondarySpecialIntent(ctx, boss, timer, segments, x, y, size) {
        if (!boss.isApex || boss.isMythic || segments === 0 || bossSpecialIntentSegments(timer) === 0)
            return;
        const secondary = boss.apexSecondaryArchetype;
        if (!secondary || !this.apexSecondaryPatternAtlasReady || !this.apexSecondaryPatternAtlasImage)
            return;
        const icon = apexSecondaryPatternIcon(secondary);
        ctx.fillStyle = 'rgba(4,8,14,.78)';
        ctx.fillRect(x - 2, y - 2, size + 4, size + 9);
        ctx.drawImage(this.apexSecondaryPatternAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size);
        ctx.fillStyle = icon.accent;
        for (let i = 0; i < segments; i++)
            ctx.fillRect(x + i * (size / 3) + 1, y + size + 2, Math.max(3, size / 3 - 2), 2);
    }
    drawPerfectEvadeToastIcon(ctx) {
        const streak = this.eventToastPerfectEvadeStreak;
        if (!streak || !this.perfectEvadeIdentityAtlasReady || !this.perfectEvadeIdentityAtlasImage)
            return;
        ...
        ctx.fillStyle = 'rgba(5,9,16,.9)';
        ctx.fillRect(600, 825, 32, 32);
        ctx.drawImage(this.perfectEvadeIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, 602, 827, 28, 28);
        ctx.restore();
    }
    drawBossVariantPressureToastIcon(ctx) {
        const tier = this.eventToastBossVariantTier, archetype = this.eventToastBossArchetype;
        if (!tier || tier <= 0 || !archetype || !this.bossVariantPressureAtlasReady || !this.bossVariantPressureAtlasImage)
            return;
        const icon = bossVariantPressureIcon(archetype), badge = bossVariantTierBadge(tier), x = this.eventToastBossArenaMutation ? 666 : 634;
        ctx.save();
        ctx.globalAlpha = .96;
        ctx.drawImage(this.bossVariantPressureAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, 827, 28, 28);
        if (badge) {
            ctx.fillStyle = 'rgba(4,8,14,.96)';
            ctx.fillRect(x + 20, 847, 9, 9);
            ctx.fillStyle = icon.accent;
            ctx.font = '900 6px system-ui';
            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle';
            ctx.fillText(badge, x + 24.5, 851.5);
        }
        ctx.restore();
    }
    drawApexSecondaryPatternToastIcon(ctx) {
        const secondary = this.eventToastApexSecondaryArchetype;
        if (!secondary || !this.apexSecondaryPatternAtlasReady || !this.apexSecondaryPatternAtlasImage)
            return;
        const icon = apexSecondaryPatternIcon(secondary), x = this.eventToastBossArenaMutation ? 698 : 666;
        ctx.save();
        ctx.globalAlpha = .96;
        ctx.drawImage(this.apexSecondaryPatternAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, 827, 28, 28);
        ctx.restore();
    }
    drawBossArchetypeToastIcon(ctx) {
        const archetype = this.eventToastBossArchetype;
        if (!archetype || !this.bossArchetypeIdentityAtlasReady || !this.bossArchetypeIdentityAtlasImage)
            return;
        const icon = bossArchetypeIdentityIcon(archetype), x = this.eventToastMythicPhase ? 666 : this.eventToastBossArenaMutation ? 634 : 602;
        ctx.save();
        ctx.globalAlpha = .96;
        ctx.drawImage(this.bossArchetypeIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, 827, 28, 28);
        ctx.restore();
    }
    drawMythicPhaseRecall(ctx, boss) {
        if (!boss.isMythic)
            return;
        const mythic = mythicBossProfile(this.elapsed, this.runThreatLevel, boss.bossOrdinal ?? this.bossesKilled), total = this.bossEncounter.nodes.length, alive = this.bossEncounter.nodes.filter(node => node.alive).length, weakpointRatio = total > 0 ? alive / total : 0;
        const phase = mythicPhaseProfile(mythic, boss.hp / Math.max(1, boss.maxHp), weakpointRatio);
        if (phase.phase === 0 || !this.mythicPhaseIdentityAtlasReady || !this.mythicPhaseIdentityAtlasImage)
            return;
        const icon = mythicPhaseIdentityIcon(phase.phase), compact = this.elapsed >= 4 * 60 * 60, size = compact ? 16 : 20, x = 1062, y = compact ? 68 : 64, segments = mythicPhasePressureSegments(weakpointRatio);
        ctx.save();
        ctx.globalAlpha = .95;
        ctx.fillStyle = 'rgba(5,9,16,.86)';
        ctx.fillRect(x - 2, y - 2, size + 18, size + 4);
        ctx.drawImage(this.mythicPhaseIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size);
        ctx.fillStyle = icon.accent;
        for (let i = 0; i < segments; i++)
            ctx.fillRect(x + size + 3, y + 2 + i * 5, 6, 3);
        ctx.fillStyle = 'rgba(5,9,16,.76)';
        ...
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText(String(phase.phase), x + size / 2, y + size / 2 + .5);
        ctx.restore();
    }
    drawMythicLastLawToastIcon(ctx) {
        const id = this.eventToastLastLawId;
        if (!id || id === 'none' || !this.mythicLastLawIdentityAtlasReady || !this.mythicLastLawIdentityAtlasImage)
            return;
    ...
    drawMythicLastLawSafeLaneIcon(ctx, x, y, size, id) {
        if (id === 'none' || !this.mythicLastLawIdentityAtlasReady || !this.mythicLastLawIdentityAtlasImage)
            return;
        const icon = mythicLastLawIdentityIcon(id);
        ctx.save();
        ctx.globalAlpha = .92;
    ...
    drawMythicTacticToastIcon(ctx) {
        const archetype = this.eventToastMythicTacticArchetype;
        if (!archetype || !this.mythicTacticIdentityAtlasReady || !this.mythicTacticIdentityAtlasImage)
            return;
    ...
    drawMythicTacticPrimedIcon(ctx) {
        const link = this.mythicTacticAttackLink;
        if (!link)
            return;
        const boss = this.enemies.enemies.find((enemy) => enemy.alive && enemy.type === 'boss' && enemy.isMythic && enemy.bossArchetype === link.archetype);
        if (!boss)
            return;
        const projection = projectMythicTacticAttackLink(link, this.elapsed * 1000, link.archetype);
        if (!projection)
            return;
        ...
        if (this.mythicTacticIdentityAtlasReady && this.mythicTacticIdentityAtlasImage) {
            ...
            ctx.beginPath();
            ctx.arc(boss.pos.x, boss.pos.y - boss.radius - 24, 17, 0, Math.PI * 2);
            ctx.fill();
    ...
    drawAscensionMutatorToastIcon(ctx) {
        const id = this.eventToastAscensionMutator;
        if (!id || !this.ascensionMutatorIdentityAtlasReady || !this.ascensionMutatorIdentityAtlasImage)
            return;
    ...
    drawFateTradeoffToastIcons(ctx) {
        const impact = this.eventToastFateImpact;
        if (!impact)
            return;
        this.drawFateImpactPair(ctx, impact, 633, 829, 22);
    }
    drawLongRunOathToastIcon(ctx) {
        const id = this.eventToastOathKind;
        if (!id || !this.deepRunDecisionIdentityAtlasReady || !this.deepRunDecisionIdentityAtlasImage)
            return;
    ...
    drawRunContractToastIcon(ctx) {
        const family = this.eventToastContractFamily;
        if (!family || !this.deepRunDecisionIdentityAtlasReady || !this.deepRunDecisionIdentityAtlasImage)
            return;
    ...
    drawRelicResonanceToastIcon(ctx) {
        const identity = this.eventToastRelicResonance;
        if (!identity || !this.buildIdentityAtlasReady || !this.buildIdentityAtlasImage)
            return;
    ...
    drawRelicResonanceProgressFrame(ctx, x, y, size, progress) {
        ...
        ctx.save();
        ctx.lineWidth = 1.5;
        ctx.strokeStyle = 'rgba(120,160,190,.22)';
        ctx.strokeRect(x - .5, y - .5, size + 1, size + 1);
        ctx.strokeStyle = accent;
        ctx.globalAlpha = .94;
        if (filled >= 1) {
    ...
    hideRelicResonanceProjectionIdentity() {
        const boss = this.enemies.enemies.find(enemy => enemy.alive && enemy.type === 'boss') ?? null, heroCritical = this.dangerState.heroCritical, coreCritical = this.dangerState.coreCritical, bossSpecialTimer = boss?.specialTimer ?? 99;
        return heroCritical || coreCritical || bossSpecialTimer <= 1.2;
    }
    drawRelicResonanceProjectionToastIcons(ctx) {
        const projection = this.eventToastRelicProjection;
        if (!projection || this.hideRelicResonanceProjectionIdentity())
            return;
        const size = 24, y = 828;
        if (this.relicResonanceImpactIdentityAtlasReady && this.relicResonanceImpactIdentityAtlasImage) {
            const icon = relicResonanceImpactIdentityIcon(projection.impactId), x = 634;
            ctx.save();
            ctx.globalAlpha = .96;
            ctx.fillStyle = 'rgba(4,8,14,.92)';
            ctx.fillRect(x - 1, y - 1, size + 2, size + 2);
            ctx.drawImage(this.relicResonanceImpactIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size);
            ctx.restore();
        }
        if (this.relicResonanceTierIdentityAtlasReady && this.relicResonanceTierIdentityAtlasImage) {
            const icon = relicResonanceTierIdentityIcon(projection.tierId), x = 662;
            ctx.save();
            ctx.globalAlpha = .96;
            ctx.fillStyle = 'rgba(4,8,14,.92)';
            ctx.fillRect(x - 1, y - 1, size + 2, size + 2);
            ctx.drawImage(this.relicResonanceTierIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size);
            ctx.restore();
        }
    }
    drawRelicResonanceTierBadge(ctx, x, y, size, label) {
        const width = label === 'III' ? 14 : 10;
        const height = 10;
        const bx = x + size - width + 2;
        const by = y + size - height + 2;
        ctx.save();
        ...
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText(label, bx + width / 2, by + height / 2 + .5);
        ctx.restore();
    }
    hideLongRunOathHelperIdentity() {
        const boss = this.enemies.enemies.find(enemy => enemy.alive && enemy.type === 'boss') ?? null;
        const heroCritical = this.dangerState.heroCritical, coreCritical = this.dangerState.coreCritical, bossSpecialTimer = boss?.specialTimer ?? 99;
        return heroCritical || coreCritical || bossSpecialTimer <= 1.2;
    }
    drawLongRunOathRequirementBoonRecall(ctx, kind, y) {
        if (this.hideLongRunOathHelperIdentity())
            return;
        const profile = oathRequirementBoonIdentity(kind), size = 17;
        if (this.oathRequirementIdentityAtlasReady && this.oathRequirementIdentityAtlasImage) {
            const icon = oathRequirementIdentityIcon(profile.requirementId), x = 377;
            ctx.save();
            ctx.globalAlpha = .95;
            ctx.fillStyle = 'rgba(5,9,17,.9)';
            ctx.fillRect(x - 1, y - 1, size + 2, size + 2);
            ctx.drawImage(this.oathRequirementIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size);
            ctx.restore();
        }
        if (this.oathBoonOutcomeIdentityAtlasReady && this.oathBoonOutcomeIdentityAtlasImage) {
            const icon = oathBoonOutcomeIdentityIcon(profile.boonId), x = 401;
            ctx.save();
            ctx.globalAlpha = .95;
            ctx.fillStyle = 'rgba(5,9,17,.9)';
            ctx.fillRect(x - 1, y - 1, size + 2, size + 2);
            ctx.drawImage(this.oathBoonOutcomeIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size);
            ctx.restore();
        }
    }
    drawLongRunOathHelperToastIcons(ctx) {
        const helper = this.eventToastOathHelper;
        if (!helper || this.hideLongRunOathHelperIdentity())
            return;
        if ('requirementId' in helper && this.oathRequirementIdentityAtlasReady && this.oathRequirementIdentityAtlasImage) {
            const icon = oathRequirementIdentityIcon(helper.requirementId), x = 634, size = 24;
            ctx.save();
            ctx.globalAlpha = .96;
            ctx.fillStyle = 'rgba(4,8,14,.92)';
            ctx.fillRect(x - 1, 827, size + 2, size + 2);
            ctx.drawImage(this.oathRequirementIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, 828, size, size);
            ctx.restore();
        }
        if (this.oathBoonOutcomeIdentityAtlasReady && this.oathBoonOutcomeIdentityAtlasImage) {
            const icon = oathBoonOutcomeIdentityIcon(helper.boonId), x = 'requirementId' in helper ? 662 : 634, size = 24;
            ctx.save();
            ctx.globalAlpha = .96;
            ctx.fillStyle = 'rgba(4,8,14,.92)';
            ctx.fillRect(x - 1, 827, size + 2, size + 2);
            ctx.drawImage(this.oathBoonOutcomeIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, 828, size, size);
            ctx.restore();
        }
    }
    hideRunContractHelperIdentity() {
        const boss = this.enemies.enemies.find(enemy => enemy.alive && enemy.type === 'boss') ?? null;
        const heroCritical = this.dangerState.heroCritical, coreCritical = this.dangerState.coreCritical, bossSpecialTimer = boss?.specialTimer ?? 99;
        return heroCritical || coreCritical || bossSpecialTimer <= 1.2;
    }
    runContractPaceId(active) {
    ...
    drawRunContractPaceBoonRecall(ctx, active, y) {
        if (this.hideRunContractHelperIdentity())
            return;
        const size = 17, paceId = this.runContractPaceId(active), boonId = runContractBoonEffectIdentityForFamily(active.family);
        if (this.runMissionPaceIdentityAtlasReady && this.runMissionPaceIdentityAtlasImage) {
            const icon = runMissionPaceIdentityIcon(paceId), image = this.runMissionPaceIdentityAtlasImage, x = 377;
            ctx.save();
            ctx.globalAlpha = .95;
            ctx.fillStyle = 'rgba(5,9,17,.9)';
            ctx.fillRect(x - 1, y - 1, size + 2, size + 2);
        ...
        if (this.runContractBoonEffectIdentityAtlasReady && this.runContractBoonEffectIdentityAtlasImage) {
            const icon = runContractBoonEffectIdentityIcon(boonId), image = this.runContractBoonEffectIdentityAtlasImage, x = 401;
            ctx.save();
            ctx.globalAlpha = .95;
            ctx.fillStyle = 'rgba(5,9,17,.9)';
            ctx.fillRect(x - 1, y - 1, size + 2, size + 2);
    ...
    drawRunContractBoonEffectToastIcon(ctx) {
        const family = this.eventToastContractBoonFamily;
        if (!family || !this.runContractBoonEffectIdentityAtlasReady || !this.runContractBoonEffectIdentityAtlasImage)
            return;
        const icon = runContractBoonEffectIdentityIcon(runContractBoonEffectIdentityForFamily(family)), image = this.runContractBoonEffectIdentityAtlasImage;
        ctx.save();
        ctx.globalAlpha = .96;
        ctx.fillStyle = 'rgba(4,8,14,.92)';
        ctx.fillRect(633, 827, 26, 26);
        ctx.drawImage(image, icon.sx, icon.sy, icon.sw, icon.sh, 634, 828, 24, 24);
        ctx.restore();
    }
    drawRunContractBoonRecall(ctx, boons, elapsedMs, x, y) {
        const recall = activeRunContractBoonRecall(boons, elapsedMs);
        if (!recall || !this.deepRunDecisionIdentityAtlasReady || !this.deepRunDecisionIdentityAtlasImage)
            return;
        const size = 18;
        ctx.save();
        ctx.fillStyle = 'rgba(6,12,22,.88)';
        ctx.fillRect(x - 1, y - 1, 52, size + 2);
        ...
        ctx.textAlign = 'left';
        ctx.textBaseline = 'middle';
        ctx.fillText(`${recall.remainingSeconds}s`, x + 22, y + size / 2 + 1);
        ctx.restore();
    }
    drawWorldEvolutionToastIcon(ctx) {
        const id = this.eventToastWorldEvolution;
        if (!id || !this.worldEvolutionIdentityAtlasReady || !this.worldEvolutionIdentityAtlasImage)
            return;
        const icon = worldEvolutionIdentityIcon(id);
        ctx.save();
        ctx.globalAlpha = .96;
        ctx.drawImage(this.worldEvolutionIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, 602, 827, 28, 28);
        ctx.restore();
    }
    drawWorldEvolutionRecall(ctx, world, x, y, size = 22, showLabel = true) {
        if (world === 'calm' || !this.worldEvolutionIdentityAtlasReady || !this.worldEvolutionIdentityAtlasImage)
            return;
        const icon = worldEvolutionIdentityIcon(world);
        ctx.save();
        ctx.globalAlpha = .94;
        ctx.fillStyle = 'rgba(5,10,18,.82)';
        ctx.fillRect(x - 2, y - 2, size + 4, size + 4);
        ctx.drawImage(this.worldEvolutionIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size);
        if (showLabel) {
            ctx.fillStyle = icon.accent;
            ctx.font = '800 10px system-ui';
            ctx.textAlign = 'left';
            ctx.textBaseline = 'middle';
            ctx.fillText(icon.label, x + size + 5, y + size / 2 + 1);
        }
        ctx.restore();
    }
    drawNemesisAdaptationRankBadge(ctx, x, y, size, adaptation) {
        ...
        ctx.save();
        ctx.fillStyle = 'rgba(7,9,16,.94)';
        ctx.fillRect(bx, by, width, height);
        ctx.strokeStyle = '#ffbf78';
        ctx.lineWidth = 1;
        ctx.strokeRect(bx + .5, by + .5, width - 1, height - 1);
    ...
    drawNemesisAdaptationToastIcons(ctx) {
        const adaptations = this.eventToastNemesisAdaptations.slice(0, Math.min(3, this.eventToastNemesisAdaptations.length));
        if (adaptations.length === 0 || !this.nemesisAdaptationIdentityAtlasReady || !this.nemesisAdaptationIdentityAtlasImage)
            return;
        const size = 20, gap = 3, startX = 598, y = 831;
        ctx.save();
        ctx.globalAlpha = .96;
        adaptations.forEach((adaptation, index) => { const icon = nemesisAdaptationIdentityIcon(adaptation.kind); const x = startX + index * (size + gap); ctx.drawImage(this.nemesisAdaptationIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size); this.drawNemesisAdaptationRankBadge(ctx, x, y, size, adaptation); if (adaptation.kind === 'mirror_affinity' && adaptation.affinity) {
            ctx.fillStyle = '#d7fff6';
            ctx.font = '800 6px system-ui';
            ctx.textAlign = 'center';
            ctx.fillText(adaptation.affinity.slice(0, 3).toUpperCase(), x + size / 2, y + size - 2);
        } });
        ctx.restore();
    }
    drawNemesisAdaptationRecall(ctx, boss, maxIcons = 3) {
        const key = boss.bossArchetype ?? this.endlessBossKey;
        if (!key || maxIcons <= 0)
            return;
        const adaptations = getBossAdaptations(this.endlessState.nemesis, key).slice(0, Math.min(3, maxIcons));
        if (adaptations.length === 0)
            return;
        if (!this.nemesisAdaptationIdentityAtlasReady || !this.nemesisAdaptationIdentityAtlasImage)
            return;
        ...
        ctx.restore();
        if (!this.hideNemesisAdaptationEffectIdentity(boss))
            this.drawNemesisAdaptationEffectRecall(ctx, boss, adaptations, y, size);
    }
    hideNemesisAdaptationEffectIdentity(boss) {
        const heroCritical = this.dangerState.heroCritical, coreCritical = this.dangerState.coreCritical, bossSpecialTimer = boss.specialTimer ?? 99;
        return heroCritical || coreCritical || bossSpecialTimer <= 1.2 || this.bossEffectivePressureLastLawActive(boss);
    }
    drawNemesisAdaptationEffectRecall(ctx, boss, adaptations, y, size) {
        if (this.hideNemesisAdaptationEffectIdentity(boss) || !this.nemesisAdaptationEffectIdentityAtlasReady || !this.nemesisAdaptationEffectIdentityAtlasImage)
            return;
        if (this.preferBossEffectivePressureSummary(boss))
            return;
        ...
        ctx.save();
        ctx.globalAlpha = .9;
        projection.primaryEffects.slice(0, 2).forEach((effect, index) => { const icon = nemesisAdaptationEffectIdentityIcon(effect.effectId), x = startX + index * (helperSize + 28 + gap); ctx.fillStyle = 'rgba(4,8,14,.86)'; ctx.fillRect(x - 1, helperY - 1, helperSize + 30, helperSize + 2); ctx.drawImage(this.nemesisAdaptationEffectIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, helperY, helperSize, helperSize); ctx.fillStyle = icon.accent; ctx.font = '900 7px system-ui'; ctx.textAlign = 'left'; ctx.textBaseline = 'middle'; ctx.fillText(`${effect.pressurePercent}%`, x + helperSize + 3, helperY + helperSize / 2 + .5); });
        ctx.restore();
    }
    drawNemesisAdaptationEffectToastIcons(ctx) {
        if (this.eventToastNemesisAdaptations.length === 0 || !this.nemesisAdaptationEffectIdentityAtlasReady || !this.nemesisAdaptationEffectIdentityAtlasImage)
            return;
        const projection = projectNemesisAdaptationEffects(this.eventToastNemesisAdaptations), size = 10, y = 851, startX = 598;
        ctx.save();
        ctx.globalAlpha = .94;
        projection.primaryEffects.slice(0, 2).forEach((effect, index) => { const icon = nemesisAdaptationEffectIdentityIcon(effect.effectId), x = startX + index * 23; ctx.drawImage(this.nemesisAdaptationEffectIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size); });
        ctx.restore();
    }
    drawBossArenaMutationToastIcon(ctx) {
        const id = this.eventToastBossArenaMutation;
        if (!id || !this.bossArenaMutationIdentityAtlasReady || !this.bossArenaMutationIdentityAtlasImage)
            return;
        const icon = bossArenaMutationIdentityIcon(id);
        ctx.save();
        ctx.globalAlpha = .96;
        ctx.drawImage(this.bossArenaMutationIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, 602, 827, 28, 28);
        ctx.restore();
    }
    drawBossArenaMutationRecall(ctx, boss) {
        ...
        if (!mutation || !this.bossArenaMutationIdentityAtlasReady || !this.bossArenaMutationIdentityAtlasImage)
            return;
        ...
        ctx.fillStyle = 'rgba(5,9,16,.84)';
        ctx.fillRect(x - 2, y - 2, size + 18, size + 4);
        ctx.drawImage(this.bossArenaMutationIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size);
        ctx.fillStyle = icon.accent;
        for (let i = 0; i < 3; i++)
            ctx.fillRect(x + size + 3, y + 2 + i * 5, 6, 3);
        ctx.fillStyle = 'rgba(5,9,16,.78)';
        for (let i = segments; i < 3; i++)
            ctx.fillRect(x + size + 3, y + 2 + i * 5, 6, 3);
        ctx.restore();
    }
    drawRunTraitToastIcon(ctx) { const id = this.eventToastRunTraitId; if (!id || !this.decisionPathIconAtlasReady || !this.decisionPathIconAtlasImage)
        return; const icon = runTraitIdentity(id); ctx.save(); ctx.globalAlpha = .96; ctx.drawImage(this.decisionPathIconAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, 602, 827, 28, 28); ctx.restore(); }
    drawAscensionSelectionToastIcon(ctx) { const id = this.eventToastAscensionSelectionId; if (!id || !this.deepRunDecisionIdentityAtlasReady || !this.deepRunDecisionIdentityAtlasImage)
        return; const icon = ascensionSelectionIdentity(id); ctx.save(); ctx.globalAlpha = .96; ctx.drawImage(this.deepRunDecisionIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, 602, 827, 28, 28); ctx.restore(); }
    hideHeroAscensionProjectionIdentity() {
        const boss = this.enemies.enemies.find(enemy => enemy.alive && enemy.type === 'boss') ?? null, heroCritical = this.dangerState.heroCritical, coreCritical = this.dangerState.coreCritical, bossSpecialTimer = boss?.specialTimer ?? 99;
        return heroCritical || coreCritical || bossSpecialTimer <= 1.2;
    }
    drawHeroAscensionProjectionToastIcons(ctx) {
        const projection = this.eventToastHeroAscensionProjection;
        if (!projection || this.hideHeroAscensionProjectionIdentity())
            return;
        const size = 22, y = 829;
        if (this.heroAscensionBuildDirectionAtlasReady && this.heroAscensionBuildDirectionAtlasImage) {
            const icon = heroAscensionBuildDirectionIdentityIcon(projection.directionId), x = 634;
            ctx.save();
            ctx.globalAlpha = .96;
            ctx.fillStyle = 'rgba(4,8,14,.92)';
            ctx.fillRect(x - 1, y - 1, size + 2, size + 2);
            ctx.drawImage(this.heroAscensionBuildDirectionAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size);
            ctx.restore();
        }
    ...
    hideFusionProjectionIdentity() {
        const boss = this.enemies.enemies.find(enemy => enemy.alive && enemy.type === 'boss') ?? null, heroCritical = this.dangerState.heroCritical, coreCritical = this.dangerState.coreCritical, bossSpecialTimer = boss?.specialTimer ?? 99;
        return heroCritical || coreCritical || bossSpecialTimer <= 1.2;
    }
    drawFusionProjectionToastIcons(ctx) {
        const projection = this.eventToastFusionProjection;
        if (!projection || this.hideFusionProjectionIdentity())
            return;
        const size = 22, y = 829;
        if (this.fusionComponentRelationAtlasReady && this.fusionComponentRelationAtlasImage) {
            const icon = fusionComponentRelationIdentityIcon(projection.relationId), x = 634;
            ctx.save();
            ctx.globalAlpha = .96;
            ctx.fillStyle = 'rgba(4,8,14,.92)';
            ctx.fillRect(x - 1, y - 1, size + 2, size + 2);
            ctx.drawImage(this.fusionComponentRelationAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size);
            ctx.restore();
        }
        if (this.fusionModifierIdentityAtlasReady && this.fusionModifierIdentityAtlasImage) {
            projection.modifierIds.slice(0, 2).forEach((id, index) => { const icon = fusionModifierIdentityIcon(id), x = 660 + index * 26; ctx.save(); ctx.globalAlpha = .96; ctx.fillStyle = 'rgba(4,8,14,.92)'; ctx.fillRect(x - 1, y - 1, size + 2, size + 2); ctx.drawImage(this.fusionModifierIdentityAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size); ctx.restore(); });
        }
    }
    hideBattlefieldEvolutionProjectionIdentity() {
        const boss = this.enemies.enemies.find(enemy => enemy.alive && enemy.type === 'boss') ?? null;
        return this.dangerState.heroCritical || this.dangerState.coreCritical || (boss?.specialTimer ?? 99) <= 1.2;
    }
    drawAscensionTierPressureToastIcons(ctx) {
        const projection = this.eventToastAscensionTierProjection;
        if (!projection || this.hideAscensionTierPressureIdentity() || !this.ascensionTierPressureAtlasReady || !this.ascensionTierPressureAtlasImage)
            return;
        const size = 20, y = 830, startX = this.eventToastAscensionMutator ? 634 : 604;
        ...
        if (projection.mutatorThreshold) {
            const icon = ascensionTierPressureIdentityIcon('mutator-threshold'), x = startX + 48;
            ctx.save();
            ctx.globalAlpha = .96;
            ctx.drawImage(this.ascensionTierPressureAtlasImage, icon.sx, icon.sy, icon.sw, icon.sh, x, y, size, size);
            ctx.restore();
        }
    }
    drawBattlefieldEvolutionToastIcons(ctx) {
        const projection = this.eventToastBattlefieldEvolutionProjection;
        if (!projection || this.hideBattlefieldEvolutionProjectionIdentity() || !this.battlefieldMechanicAtlasReady || !this.battlefieldMechanicAtlasImage)
            return;
        const size = 22, y = 829;
    ...
    drawBuildOverdriveEffectToastIcons(ctx) {
        const projection = this.eventToastBuildOverdriveProjection;
        if (!projection || !projection.active || this.hideBuildOverdriveEffectIdentity() || !this.buildOverdriveEffectAtlasReady || !this.buildOverdriveEffectAtlasImage)
            return;
        const size = 22, y = 829;
    ...
    drawEventToast(ctx) {
        ...
        ctx.save();
        ctx.globalAlpha = alpha;
        ctx.fillStyle = 'rgba(4,8,14,.84)';
        ctx.fillRect(590, 820, 420, 42);
        ctx.strokeStyle = 'rgba(170,220,255,.35)';
        ctx.strokeRect(590, 820, 420, 42);
        ...
        ctx.fillText(this.eventToast, 800, 841);
        ctx.restore();
    }
    drawBossWarning(ctx) {
    ...
    hideCatastropheTransitionIdentity() {
        const boss = this.enemies.enemies.find(enemy => enemy.alive && enemy.type === 'boss') ?? null;
        const heroCritical = this.dangerState.heroCritical, coreCritical = this.dangerState.coreCritical, bossSpecialTimer = boss?.specialTimer ?? 99;
        return heroCritical || coreCritical || bossSpecialTimer <= 1.2;
    }
    drawCatastropheTransitionForecast(ctx, seconds, x, y) {
        const projection = projectCatastropheTransitionForecast(seconds);
        if (!projection.visible || this.hideCatastropheTransitionIdentity() || !this.catastropheTransitionIdentityAtlasReady || !this.catastropheTransitionIdentityAtlasImage)
            return;
        const w = 82, h = 22, size = 10;
        ctx.save();
        ctx.globalAlpha = .96;
        ctx.fillStyle = 'rgba(7,10,18,.9)';
        ctx.fillRect(x, y, w, h);
        const statusIcon = catastropheTransitionIdentityIcon(projection.status);
        ctx.strokeStyle = statusIcon.accent;
        ctx.lineWidth = 1;
        ctx.strokeRect(x + .5, y + .5, w - 1, h - 1);
        ...
        ctx.textAlign = 'left';
        ctx.textBaseline = 'middle';
        ctx.fillText(catastropheTransitionForecastLabel(projection), x + 4, y + h / 2 + .5);
        const transitionIcon = catastropheTransitionIdentityIcon('transition');
        ctx.drawImage(this.catastropheTransitionIdentityAtlasImage, transitionIcon.sx, transitionIcon.sy, transitionIcon.sw, transitionIcon.sh, x + 54, y + 6, size, size);
        ctx.drawImage(this.catastropheTransitionIdentityAtlasImage, statusIcon.sx, statusIcon.sy, statusIcon.sw, statusIcon.sh, x + 68, y + 6, size, size);
        ctx.restore();
    }
    drawCatastropheStatusIcon(ctx, x, y, size) {
        if (!this.catastrophe || !this.catastropheIdentityAtlasReady || !this.catastropheIdentityAtlasImage)
            return;
    ...
    drawCatastropheBanner(ctx) {
        if (!this.catastrophe)
            return;
        const alpha = Math.min(1, this.catastropheBannerTimer / 0.55);
        ctx.save();
        ctx.globalAlpha = alpha;
        ctx.fillStyle = 'rgba(6,8,14,.84)';
        ctx.fillRect(500, 350, 600, 96);
        ctx.strokeStyle = this.catastrophe.id === 'goldenNight' ? '#f3d36b' : '#ff6675';
        ctx.lineWidth = 3;
        ctx.strokeRect(500, 350, 600, 96);
        ...
        ctx.fillText(this.catastrophe.name, 800, 387);
        ctx.font = '700 16px system-ui';
        ctx.fillStyle = '#c9d6e0';
        ctx.fillText(this.catastrophe.description, 800, 420);
        const projection = this.catastropheBannerTransitionProjection;
        if (projection) {
            ctx.font = '800 11px system-ui';
            ctx.fillStyle = catastropheTransitionIdentityIcon(projection.status).accent;
            ctx.fillText(catastropheTransitionHint(projection, 2), 800, 439);
            if (this.catastropheTransitionIdentityAtlasReady && this.catastropheTransitionIdentityAtlasImage) {
                const transitionIcon = catastropheTransitionIdentityIcon('transition'), statusIcon = catastropheTransitionIdentityIcon(projection.status);
                ctx.drawImage(this.catastropheTransitionIdentityAtlasImage, transitionIcon.sx, transitionIcon.sy, transitionIcon.sw, transitionIcon.sh, 947, 428, 14, 14);
                ctx.drawImage(this.catastropheTransitionIdentityAtlasImage, statusIcon.sx, statusIcon.sy, statusIcon.sw, statusIcon.sh, 965, 428, 14, 14);
            }
        }
        ctx.restore();
    }
    useHealingPotion() {
        if (this.hero.healingPotions <= 0 || this.hero.hp >= this.hero.maxHp)
            return;
        ...
        if (this.hero.hp > beforeHp) {
            if (efficiency > 1.001)
                this.queueSurvivalResponseVfx('heroPotionBoost');
            else
                this.queueSurvivalResponseVfx('heroPotion');
        }
        this.syncEquipmentState();
    }
    openShop() {
        if (this.shopTokens <= 0 || this.shopOverlay.isOpen || this.gameOver)
            return;
        this.input.clearStrategicActionArms();
        if (this.onboarding.signal('shop'))
            this.saveStoredOnboardingState();
        this.shopTokens -= 1;
        this.clearBufferedCastIntents();
        this.paused = true;
    ...
    refreshShopOverlay() {
        ...
        const handlers = {
            ...
            onReroll: () => {
                ...
                if (this.equipmentState.coins < price)
                    return;
    ...
    syncEquipmentState(build = this.currentCombatBuild()) {
        this.hero.coins = this.equipmentState.coins;
    ...
    applyLegendaryProcs(procs, origin) {
        for (const proc of procs) {
            if (proc.type === 'bonusGold') {
                ...
                const identity = legendaryProcIdentity(proc, this.equipmentState);
                if (proc.amount >= 250 && identity)
                    this.showLegendaryAwakeningEventToast(`미다스의 손 · +${proc.amount}G`, identity.itemId);
            }
            else if (proc.type === 'magnet') {
                this.pickups.setGlobalMagnet(proc.duration);
                const identity = legendaryProcIdentity(proc, this.equipmentState);
                if (identity)
                    this.showLegendaryAwakeningEventToast('심연의 자석망토 · 전역 흡수', identity.itemId);
                else
                    this.showEventToast('심연의 자석망토 · 전역 흡수');
            }
            else if (proc.type === 'coreHeal') {
                this.core.hp = Math.min(this.core.maxHp, this.core.hp + this.core.maxHp * proc.fraction);
                const identity = legendaryProcIdentity(proc, this.equipmentState);
                if (identity)
                    this.showLegendaryAwakeningEventToast('영원의 성벽 · 수호핵 복구', identity.itemId);
                else
                    this.showEventToast('영원의 성벽 · 수호핵 복구');
            }
            else if (proc.type === 'nova') {
                const damage = 82 * this.hero.spellPower * this.hero.equipmentSpellPower;
                for (const enemy of this.enemies.enemies) {
                    if (enemy.alive && Math.hypot(enemy.pos.x - origin.x, enemy.pos.y - origin.y) <= proc.radius + enemy.radius)
                        this.enemies.damage(enemy, damage);
                }
                this.feedback.addImpact(origin, 'final');
                const identity = legendaryProcIdentity(proc, this.equipmentState);
                if (identity)
                    this.showLegendaryAwakeningEventToast('폭발 지팡이 · NOVA', identity.itemId);
            }
        }
    }
    gainXp(value) {
    ...
    openNextBossReward(generation) {
        ...
        const renderDecision = (activeGeneration) => this.levelUpOverlay.open(presentedChoices, (choice) => {
            this.finishDecisionPick(activeGeneration, () => {
                ...
                if (choice.kind === 'relic') {
                    ...
                    const projection = projectFusionSelection(this.fusionRuntime.equipped, choice.fusionId, this.hero.profileId);
                    if (this.fusionRuntime.equip(choice.fusionId)) {
                        rewardFeedback = '';
                        this.showFusionProjectionEventToast(`마법 융합 · ${fusionDefinition(choice.fusionId).name}`, choice.fusionId, projection);
                    }
                }
                else {
                ...
                if (rewardFeedback)
                    this.showEventToast(nextGoal?.label ?? rewardFeedback);
    ...
    openNextLevelUp(generation) {
        if (this.queuedLevelUps <= 0 || this.gameOver)
            return;
        if (this.onboarding.signal('levelup'))
            this.saveStoredOnboardingState();
    ...
    saveStoredOnboardingState() {
        try {
            if (typeof window !== 'undefined')
                saveOnboardingState(this.storage, this.onboarding.snapshot());
        }
        catch { /* optional persistence */ }
    }
    loadStoredMetaProfile() {
        try {
            if (typeof window === 'undefined')
    ...
    saveStoredMetaProfile() {
        try {
            if (typeof window !== 'undefined')
                saveMetaProfile(this.storage, this.metaProfile);
        }
        catch {
    ...
    loadStoredRetryBlueprint() {
        try {
            return typeof window === 'undefined' ? null : loadRetryBlueprint(this.storage);
        }
        catch {
            return null;
        }
    }
    saveStoredRetryBlueprint(blueprint) {
        try {
            if (typeof window !== 'undefined')
                saveRetryBlueprint(this.storage, blueprint);
        }
        catch { /* optional persistence */ }
    }
    ...
    saveCurrentRunSnapshot() {
        if (this.gameOver || this.elapsed < 5 || this.selectedTrait === null)
            return;
        const snapshot = {
        ...
        try {
            if (typeof window !== 'undefined')
                saveRunSnapshot(this.storage, snapshot);
        }
        catch { /* optional */ }
    }
    restoreRunSnapshot(snapshot) {
        ...
        this.hero.level = snapshot.hero.level;
        this.hero.xp = snapshot.hero.xp;
        this.hero.xpNext = snapshot.hero.xpNext;
        this.hero.hp = snapshot.hero.hp;
        ...
        this.bossesKilled = snapshot.progression.bossesKilled;
        this.goldEarned = snapshot.progression.goldEarned;
        this.shopTokens = snapshot.progression.shopTokens;
    ...
    loadStoredMasteryProfile() {
        try {
            return typeof window === 'undefined' ? defaultMasteryProfile() : loadMasteryProfile(this.storage);
        }
        catch {
            return defaultMasteryProfile();
        }
    }
    saveStoredMasteryProfile() {
        try {
            if (typeof window !== 'undefined')
                saveMasteryProfile(this.storage, this.masteryProfile);
        }
        catch { /* optional */ }
    }
    loadStoredThreatProfile() {
        try {
            return typeof window === 'undefined' ? defaultThreatProfile() : loadThreatProfile(this.storage);
        }
        catch {
            return defaultThreatProfile();
        }
    }
    saveStoredThreatProfile() {
        try {
            if (typeof window !== 'undefined')
                saveThreatProfile(this.storage, this.threatProfile);
        }
        catch { /* optional */ }
    }
    loadStoredRunRecords() {
        try {
            return typeof window === 'undefined' ? defaultRunRecords() : loadRunRecords(this.storage);
        }
        catch {
            return defaultRunRecords();
        }
    }
    saveStoredRunRecords() {
        try {
            if (typeof window !== 'undefined')
                saveRunRecords(this.storage, this.runRecords);
        }
        catch { /* optional */ }
    }
    loadStoredRunHistory() {
        try {
            return typeof window === 'undefined' ? [] : loadRunHistory(this.storage);
        }
        catch {
            return [];
        }
    ...
    saveRunHistory(entry) {
        try {
            if (typeof window !== 'undefined')
                appendRunHistory(this.storage, entry);
        }
        catch { /* optional */ }
    }
    loadStoredAudioSettings() {
        try {
            return typeof window === 'undefined' ? defaultAudioSettings() : loadAudioSettings(this.storage);
        }
        catch {
            return defaultAudioSettings();
        }
    }
    currentBuildCapsulePayload(seed = this.currentRunBlueprint?.seed ?? this.endlessState.rng.seed) {
        const finalForm = this.currentHeroFinalForm();
        return {
            version: 1,
            heroId: this.hero.profileId,
        ...
        this.resultsOverlay.open({
            ...
            onRetrySameHero: () => {
                ...
                if (nextReplayPlan) {
                    ...
                    this.openTraitSelect(this.hero.profileId);
            },
            onLobby: () => this.restart(),
        });
    }
    drawBar(ctx, x, y, w, h, ratio, fill, bg) {
        ctx.fillStyle = bg;
        ctx.fillRect(x, y, w, h);
        ctx.fillStyle = fill;
