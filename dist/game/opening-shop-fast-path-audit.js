...
export function auditOpeningShopFastPathSuccess() {
    const samples = [];
    for (const heroId of heroes)
        for (const archetype of archetypes)
            for (const coins of [250, 500]) {
                const state = { coins, weapon: null, armor: null, healingPotions: 1, inventory: [], discoveredRecipes: [] };
    ...
    const issues = [];
    if (unsafeExposureCount > 0)
        issues.push('unsafe-fast-path');
    if (unaffordableExposureCount > 0)
        issues.push('unaffordable-fast-path');
    if (actionableCoverage < .85)
        issues.push('actionable-coverage');
    if (estimatedSuccessfulOneTapRate < .85)
        issues.push('one-tap-success');
