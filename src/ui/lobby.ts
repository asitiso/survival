import type { MetaProfile, MetaUpgradeId } from '../domain/meta-profile.js';
import { metaUpgradeCost } from '../domain/meta-profile.js';
import type { ThreatProfile } from '../domain/threat-profile.js';
import type { MasteryProfile } from '../domain/mastery-profile.js';
import { masteryXpNeeded } from '../domain/mastery-profile.js';
import { heroProfile, type HeroId } from '../game/hero-profiles.js';
import type { RunSnapshot } from '../domain/run-snapshot.js';
import { threatLevelName, type ThreatLevel } from '../domain/threat-level.js';
import type { RunHistoryEntry } from '../domain/run-history.js';
...
export interface LobbyHandlers {
  onPurchase: (id: MetaUpgradeId) => MetaProfile;
  onContinue: () => void;
...
export function lobbyThreatChoices(profile: ThreatProfile): LobbyThreatChoice[] {
  return ([0,1,2,3,4,5] as ThreatLevel[]).map((level) => ({ level, name: `T${level} · ${threatLevelName(level)}`, selected: profile.selected === level, locked: level > profile.unlocked }));
}



export interface LobbyMasteryCard {
  heroId: HeroId;
  name: string;
  level: number;
  xp: number;
...
export function lobbyMasteryCards(profile: MasteryProfile): LobbyMasteryCard[] {
  return (['arkan', 'seria', 'kain', 'edric'] as HeroId[]).map((heroId) => {
...
const UPGRADE_COPY: ReadonlyArray<{ id: MetaUpgradeId; name: string; description: string; accent: string; maxLevel: number }> = [
...
export function lobbyUpgradeCards(profile: MetaProfile): LobbyUpgradeCard[] {
...
export class LobbyOverlay {
  ...
  private threatProfile: ThreatProfile | null = null;
  ...
  refresh(profile: MetaProfile): void {
  ...
  private render(profile: MetaProfile): void {
    this.root.replaceChildren();
    ...
    if (this.threatProfile) {
      ...
      for (const choice of lobbyThreatChoices(this.threatProfile)) {
        ...
        button.addEventListener('click', () => {
          if (!this.handlers?.onThreatChange) return;
