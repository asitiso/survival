  ...
  private drawBossArenaHazards(ctx: CanvasRenderingContext2D, motion: SecondaryCombatMotionPolicy): void {
    ...
    for (const hazard of this.bossArena.hazards) {
      ...
      if (shape === 'corridor' || shape === 'cross') {
        ...
        if (hazard.telegraph <= 0){ctx.globalAlpha=hazardFillAlpha;ctx.fill();}ctx.globalAlpha=hazard.telegraph>0?hazardBaseAlpha:hazardEdgeAlpha;ctx.stroke();
        if (shape === 'cross') {
        ...
        if (hazard.telegraph <= 0){ctx.globalAlpha=hazardFillAlpha;ctx.fill('evenodd');}ctx.globalAlpha=hazard.telegraph>0?hazardBaseAlpha:hazardEdgeAlpha;ctx.stroke();
      } else if (shape === 'orbit') {
        ctx.save(); ctx.translate(hazard.pos.x, hazard.pos.y); ctx.rotate(hazard.angle ?? 0); ctx.scale(1.45, .72);
        ctx.beginPath(); ctx.arc(0, 0, hazard.radius, 0, Math.PI * 2); if (hazard.telegraph <= 0){ctx.globalAlpha=hazardFillAlpha;ctx.fill();}ctx.globalAlpha=hazard.telegraph>0?hazardBaseAlpha:hazardEdgeAlpha;ctx.stroke(); ctx.restore();
      } else if (shape === 'clock') {
        ctx.translate(hazard.pos.x, hazard.pos.y); ctx.rotate(hazard.angle ?? 0);
        ctx.beginPath(); ctx.moveTo(0, 0); ctx.arc(0, 0, hazard.radius * 1.25, -.2, .2); ctx.closePath(); if (hazard.telegraph <= 0){ctx.globalAlpha=hazardFillAlpha;ctx.fill();}ctx.globalAlpha=hazard.telegraph>0?hazardBaseAlpha:hazardEdgeAlpha;ctx.stroke();
      } else {
