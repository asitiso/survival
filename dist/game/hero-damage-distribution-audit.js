...
function normalize(raw) {
    ...
    return Object.fromEntries(entries.map(([key, value]) => [key, round(value / Math.max(.0001, total))]));
}
function spread(values) { return round(Math.max(...values) / Math.max(.0001, Math.min(...values))); }
export function heroDamageDistributionSamples() {
    const samples = [];
...
export function auditHeroDamageDistribution() {
    ...
    maxHeroLossSpread = round(maxHeroLossSpread);
    maxCoreLossSpread = round(maxCoreLossSpread);
    ...
    for (const hero of HERO_PROFILES) {
        ...
        if (!(group[0].heroLossIndex < group[1].heroLossIndex && group[1].heroLossIndex < group[2].heroLossIndex && group[0].coreLossIndex < group[1].coreLossIndex && group[1].coreLossIndex < group[2].coreLossIndex))
