...
import { mythicLastLawProfile, type MythicLastLawProfile } from './mythic-last-law.js';

export type MythicLastLawId = 'solar-rupture' | 'brood-crown' | 'iron-verdict' | 'null-eclipse' | 'twin-cataclysm' | 'broken-hour' | 'none';

export interface MythicLastLawIdentityProfile extends MythicLastLawProfile {
  lawId: MythicLastLawId;
  accent: string;
}

interface IdentityBias {
  lawId: Exclude<MythicLastLawId, 'none'>;
  label: string;
  accent: string;
...
const BIASES: Record<BossArchetype, IdentityBias> = {
...
export function mythicLastLawIdentityProfile(
  profile: MythicBossProfile,
  archetype: BossArchetype,
  hpRatio: number,
  weakpointRatio: number,
): MythicLastLawIdentityProfile {
  const base = mythicLastLawProfile(profile, hpRatio, weakpointRatio);
  if (!base.active) return { ...base, lawId:'none', accent:'#ffffff' };
