    ...
    renderProjectiles(ctx, bossSpecialVfxAtlasImage = null, bossSpecialVfxAtlasReady = false, battlefieldEnvironmentReactionVfxAtlasImage = null, battlefieldEnvironmentReactionVfxAtlasReady = false, bossProjectileLifecycleVfxAtlasImage = null, bossProjectileLifecycleVfxAtlasReady = false, presentationQuality = 'high', reducedFlash = false, reducedMotion = false) {
        ...
        for (const projectile of this.projectiles) {
            ...
            if (bossTravelRelease && bossTravelRelease.visible && bossBridgeBudget.visible) {
                ctx.save();
                ctx.globalAlpha = Math.min(bossTravel?.alpha ?? 0, bossTravelRelease.alpha) * bossBridgeBudget.alphaScale * threatOwnership.travelAlphaScale;
            ...
            if (trail.owner === 'launch' && projectileResolutionBudget.visible) {
                ctx.save();
                ctx.globalAlpha = trail.alpha * threatOwnership.launchAlphaScale * directionCarryRecovery.tailAlphaScale * projectileResolution.transitionAlphaScale * projectileResolutionBudget.effectStrength;
                ...
                ctx.save();
                ctx.globalAlpha = travelThreatCarry.alphaScale * threatOwnership.travelAlphaScale * directionCarryRecovery.tailAlphaScale * projectileResolution.transitionAlphaScale * projectileResolutionBudget.effectStrength;
                ctx.strokeStyle = projectile.bossArchetype ? '#ffb26f' : '#ff7c86';
