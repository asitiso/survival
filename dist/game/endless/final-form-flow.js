...
export function createDefaultFinalFormFlowState() { return { formId: null, streak: 0, expiresAtMs: 0, lastCastAtMs: 0 }; }
export function recordFinalFormFlowCast(state, formId, moving, nowMs) {
    const now = Math.max(0, Number.isFinite(nowMs) ? nowMs : 0);
    if (!formId || !moving)
        return advanceFinalFormFlow(state, now);
...
export function advanceFinalFormFlow(state, nowMs) {
    const now = Math.max(0, Number.isFinite(nowMs) ? nowMs : 0);
    if (state.expiresAtMs <= now)
        return createDefaultFinalFormFlowState();
    return { ...state, streak: Math.max(0, Math.min(5, Math.floor(state.streak))) };
}
export function finalFormFlowModifiers(state, formId, nowMs) {
    const safe = advanceFinalFormFlow(state, nowMs);
    if (!formId || safe.formId !== formId || safe.streak <= 0)
        return { damageMultiplier: 1, cooldownMultiplier: 1, moveSpeedMultiplier: 1 };
    const ratio = clamp(safe.streak / 5, 0, 1);
    ...
    if (family === 'surge') {
        ...
        damage = 1 + .11 * ratio;
        cooldown = 1 - .075 * ratio;
    }
    else {
        damage = 1 + .13 * ratio;
        cooldown = 1 - .035 * ratio;
        move = 1 + .01 * ratio;
    }
