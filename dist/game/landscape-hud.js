...
function crop(text, max) { return text.length <= max ? text : `${text.slice(0, Math.max(1, max - 1))}…`; }
export function compactLandscapeStatusLine(input, maxChars = 64) {
...
export function hudNoTouchRects() { return NO_TOUCH.map((rect) => ({ ...rect })); }
export function isHudNoTouchPoint(point) {
    return NO_TOUCH.some((rect) => point.x >= rect.x && point.x <= rect.x + rect.width && point.y >= rect.y && point.y <= rect.y + rect.height);
}
export function safeJoystickOrigin(point, profile) {
...
export function shouldStartLandscapeJoystick(point, profile) {
...
function buildLabelPriority(label) {
    if (label.startsWith('SIGNATURE'))
        return 100;
    if (label.startsWith('CONTRACT'))
        return 84;
    if (label.startsWith('OVERDRIVE'))
        return 95;
    if (label.startsWith('FLOW'))
        return 93;
    if (label.startsWith('REPLAY'))
        return 90;
    if (label.startsWith('최종형'))
        return 86;
    if (label.startsWith('서약'))
        return 82;
    if (label.startsWith('RECOVER'))
        return 68;
    if (label.startsWith('융합'))
        return 62;
    if (label.startsWith('운명'))
        return 58;
    if (label.startsWith('시너지'))
        return 54;
    return 40;
}
export function prioritizeLandscapeBuildLabels(labels, context) {
