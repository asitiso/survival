...
function pointerSafetyChecks(): boolean[] {
  const once=(()=>{
    const tracker=new ActionHoldLeashTracker<string>();
...
export function auditActionHoldReliability(): ActionHoldReliabilityAudit {
  ...
  const issues:string[]=[];
  if(!jitterPassed)issues.push('jitter-hysteresis');
  if(!boundaryPassed)issues.push('leash-boundary');
  if(!foldablePassed)issues.push('foldable-touch-scale');
  if(!pointerSafetyPassed)issues.push('pointer-safety');
