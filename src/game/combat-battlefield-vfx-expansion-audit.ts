...
export interface CombatBattlefieldVfxExpansionAuditSample {
  id:string;
...
function add(samples:CombatBattlefieldVfxExpansionAuditSample[],id:string,expected:boolean|number|string,actual:boolean|number|string):void {
...
export function auditCombatBattlefieldVfxExpansion(){
  const samples:CombatBattlefieldVfxExpansionAuditSample[]=[];
  const enemyCells=new Set<string>();
  for(const type of ENEMY_COMBAT_VFX_TYPES){
    for(const kind of ['hit','death'] as const){
      const r=enemyCombatVfxSprite(type,kind); enemyCells.add(`${r.sx}:${r.sy}`);
  ...
  for(const archetype of BOSS_SPECIAL_COMBAT_VFX_ARCHETYPES){
    const projectile=bossSpecialProjectileVfxSprite(archetype); bossCells.add(`${projectile.sx}:${projectile.sy}`);
  ...
  for(const heroId of HERO_SPELL_SIGNATURE_VFX_HEROES){
    for(const channel of HERO_SPELL_SIGNATURE_VFX_CHANNELS){
      const r=heroSpellSignatureVfxSprite(heroId,channel); heroCells.add(`${r.sx}:${r.sy}`);
