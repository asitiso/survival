    ...
    renderProjectiles(ctx, bossSpecialVfxAtlasImage = null, bossSpecialVfxAtlasReady = false, battlefieldEnvironmentReactionVfxAtlasImage = null, battlefieldEnvironmentReactionVfxAtlasReady = false, bossProjectileLifecycleVfxAtlasImage = null, bossProjectileLifecycleVfxAtlasReady = false, presentationQuality = 'high', reducedFlash = false, reducedMotion = false) {
        ...
        for (const projectile of this.projectiles) {
            ...
            if (bossTravelRelease && bossTravelRelease.visible && bossBridgeBudget.visible) {
                ctx.save();
                ctx.globalAlpha = Math.min(bossTravel?.alpha ?? 0, bossTravelRelease.alpha) * bossBridgeBudget.alphaScale * threatOwnership.travelAlphaScale * projectilePriority.secondaryAlphaScale * projectileLayerBudget.projectileDecorationScale * projectileSpatial.trailAlphaScale * bossSpatialFocus.secondaryAlphaScale;
            ...
            if (hasBossVisual && bossSpecialVfxAtlasImage && projectile.bossArchetype) {
                ...
                ctx.rotate(angle);
                ctx.globalAlpha = 0.92 * projectileResolution.bodyAlphaScale;
            ...
            if (projectile.bossArchetype && bossProjectileLifecycleVfxAtlasReady && bossProjectileLifecycleVfxAtlasImage) {
                ...
                ctx.translate(visualPos.x, visualPos.y);
                ctx.rotate(angle);
                ctx.globalAlpha = 0.78 * projectileResolution.bodyAlphaScale;
            ...
            if (projectile.sourceType === 'archer' && battlefieldEnvironmentReactionVfxAtlasReady && battlefieldEnvironmentReactionVfxAtlasImage) {
                ...
                ctx.translate(visualPos.x, visualPos.y);
                ctx.rotate(angle);
                ctx.globalAlpha = 0.94 * projectileResolution.bodyAlphaScale;
