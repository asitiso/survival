...
export function equipmentGrade(rank) {
    return rank > 5 ? `전설 +${rank - 5}` : ['일반', '고급', '희귀', '영웅', '전설'][Math.max(0, rank - 1)] ?? '일반';
}
export function shopOfferPrice(state, offer) {
    if (offer.basePrice === undefined || offer.kind === 'potion')
        return offer.price;
...
export function legendaryEquipmentName(id) {
...
export function rerollCost(rerollsThisVisit) {
    const index = Math.max(0, Math.floor(rerollsThisVisit));
    return 50 * Math.pow(2, Math.min(index, 6));
}
export function purchaseOffer(state, offer, elapsedSeconds = Infinity) {
    ...
    if (existing)
        ...
        next.inventory.push({ ...purchased, count: 1 });
