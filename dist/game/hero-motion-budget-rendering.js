...
export function heroMotionBudgetPresentation(input, reducedMotion = false) {
    ...
    if (reducedMotion) {
        const compress = (value, protectedOwner) => clamp(value * (protectedOwner ? .86 : .7), .18, 1);
        movementScale = compress(movementScale, owner === 'movement');
        castScale = compress(castScale, owner === 'cast');
        ultimateScale = compress(ultimateScale, owner === 'ultimate');
        hitScale = compress(hitScale, owner === 'hit');
        transitionScale = compress(transitionScale, owner === 'evade');
    }
    ...
    if (rawLoad > budgetCap) {
        ...
        if (owner !== 'movement')
            movementScale *= compression;
        if (owner !== 'cast')
            castScale *= compression;
        if (owner !== 'ultimate')
            ultimateScale *= compression;
        if (owner !== 'hit')
            hitScale *= compression;
        if (owner !== 'evade')
            transitionScale *= compression;
