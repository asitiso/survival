...
export function relicDefinition(id) {
    const found = RELICS.find((entry) => entry.id === id);
    if (!found)
        throw new Error(`Unknown relic: ${id}`);
    return found;
}
export function relicDisplayName(id) {
    return id ? relicDefinition(id).name : '없음';
}
export function relicCandidates(heroId, activeRelic, rng = Math.random, bossArchetype = null, masteryLevel = 1) {
    const pool = RELICS
...
export function relicModifiers(id, heroId) {
    const out = { ...NEUTRAL };
    if (!id)
        return out;
    if (id === 'abyss-eye') {
