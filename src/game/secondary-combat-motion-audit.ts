...
import { secondaryCombatMotionPolicy, type CombatAttentionPrimary, type SecondaryCombatMotionInput, type SecondaryCombatMotionOwner } from './combat-cue-priority.js';
import { TACTICAL_STATUS_ICON_ATLAS, tacticalStatusIconPresentation } from './tactical-status-icon-assets.js';

export interface SecondaryCombatMotionAuditSample {
  feature: Exclude<SecondaryCombatMotionOwner,'none'>;
  context: string;
  expectedOwner: SecondaryCombatMotionOwner;
  actualOwner: SecondaryCombatMotionOwner;
  animatedOwners: number;
  maxAmplitude: number;
  passed: boolean;
}

export interface SecondaryCombatMotionAudit {
  samples: SecondaryCombatMotionAuditSample[];
...
const FEATURES: readonly Exclude<SecondaryCombatMotionOwner,'none'>[] = [
  'boss-hazard','priority-threat','supply-crate','field-node','freeze-status','core-ambient',
];

const CONTEXTS: readonly {name:string;combatPrimary:CombatAttentionPrimary;reducedFlash:boolean}[] = [
...
function featureInput(feature:Exclude<SecondaryCombatMotionOwner,'none'>,context:(typeof CONTEXTS)[number]):SecondaryCombatMotionInput{
  return {
...
function amplitudes(policy:ReturnType<typeof secondaryCombatMotionPolicy>):number[]{
  return [
...
export function auditSecondaryCombatMotion():SecondaryCombatMotionAudit{
  const samples:SecondaryCombatMotionAuditSample[]=[];
  ...
  if(allPresent.owner!=='boss-hazard'||allPresentOwners!==1)issues.push('single-owner-priority');
  if(maxAnimatedOwners>1)issues.push(`animated-owners:${maxAnimatedOwners}`);
  if(maxMotionAmplitude>.08+1e-9)issues.push(`motion-amplitude:${maxMotionAmplitude}`);
  if(reducedFlashMotionAmplitude!==0)issues.push(`reduced-flash:${reducedFlashMotionAmplitude}`);
  if(suppressedCombatMotionAmplitude!==0)issues.push(`attention-suppression:${suppressedCombatMotionAmplitude}`);
  if(!supplyIconReused)issues.push('supply-icon-reuse');
