...
export function auditBossEffectivePressureProjectionIdentity() {
    const samples = [];
    const seen = new Set();
    let managerStateMutation = false;
    let defensiveReadPassed = true;
    for (const id of EFFECTS) {
        for (const delta of DELTAS) {
            const p = projectBossEffectivePressure(withEffect(id, delta)), primary = p.primaryEffects[0];
            if (primary)
                seen.add(primary.effectId);
            samples.push({ id: `single:${id}:${delta}`, passed: primary?.effectId === id && primary.deltaPercent === Math.round(delta * 1000) / 10 && p.primaryEffects.length === 1 });
        }
    }
    for (let i = 0; i < EFFECTS.length; i++) {
        const dominant = EFFECTS[i];
        for (let offset = 1; offset <= 3; offset++) {
            const secondary = EFFECTS[(i + offset) % EFFECTS.length], m = withEffect(dominant, .25);
            if (secondary === 'special-cadence')
                m.specialCadenceMultiplier = .88;
            else if (secondary === 'summon-pressure')
                m.summonCountMultiplier = 1.12;
            else if (secondary === 'dash-distance')
                m.dashDistanceMultiplier = 1.12;
            else
                m.bossDamageTakenMultiplier = 1.12;
            const p = projectBossEffectivePressure(m), dominantThreat = dominant === 'summon-pressure' || dominant === 'dash-distance', secondaryThreat = secondary !== 'boss-vulnerability';
            const expectedFirst = dominantThreat ? dominant : secondaryThreat ? secondary : dominant, expectedSecond = expectedFirst === dominant ? secondary : dominant;
            samples.push({ id: `pair:${dominant}:${secondary}`, passed: p.primaryEffects.length === 2 && p.primaryEffects[0]?.effectId === expectedFirst && p.primaryEffects[1]?.effectId === expectedSecond });
        }
    }
    for (let i = 0; i < 12; i++) {
        const manager = new EnemyManager(), applied = { bossDamageTakenMultiplier: 1 + i * .01, specialCadenceMultiplier: 1 - i * .012, summonCountMultiplier: 1 + i * .008, dashDistanceMultiplier: 1 - i * .006 };
        manager.setBossEncounterModifiers(applied);
        const first = manager.getBossEncounterModifiers(), before = manager.getBossEncounterModifiers();
        first.specialCadenceMultiplier = 99;
        const after = manager.getBossEncounterModifiers(), ok = JSON.stringify(before) === JSON.stringify(after) && after.specialCadenceMultiplier === applied.specialCadenceMultiplier;
        defensiveReadPassed &&= ok;
        managerStateMutation ||= !ok;
        samples.push({ id: `manager:${i}`, passed: ok });
    }
    const atlas = auditMythicSafeZonePressureEffectIdentityAtlas();
    const neutralProjection = projectBossEffectivePressure(neutral());
    const neutralHiddenPassed = neutralProjection.primaryEffects.length === 0;
    ...
    const effectCoverageComplete = EFFECTS.every(id => seen.has(id)), atlasReusePassed = atlas.passed && EFFECTS.every(id => MYTHIC_SAFE_ZONE_PRESSURE_EFFECT_IDENTITY_IDS.includes(id));
    const issues = [];
    ...
    if (!effectCoverageComplete)
        issues.push('effect-coverage');
    if (!atlasReusePassed)
        issues.push('atlas-reuse');
    if (!defensiveReadPassed)
        issues.push('defensive-read');
    if (managerStateMutation)
        issues.push('manager-state-mutation');
    if (!neutralHiddenPassed)
        issues.push('neutral-visible');
