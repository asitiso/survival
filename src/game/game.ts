...
export class Game {
  ...
  private drawBossArenaHazards(ctx: CanvasRenderingContext2D, motion: SecondaryCombatMotionPolicy): void {
    ...
    if (safeLane) {
      ...
      ctx.save(); ctx.globalAlpha = safeLaneBaseAlpha*Math.max(safeLaneHazardRecovery.pathAlphaScale,safeLaneStackProtection.pathAlphaFloor,safeLaneCorridorProtection.pathAlphaFloor); ctx.strokeStyle = '#8fffd3'; ctx.fillStyle = '#8fffd3'; ctx.lineWidth = 2; ctx.setLineDash([7,7]);
