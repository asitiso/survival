...
const DECAY = { grunt: 3.8, hound: 4.7, brute: 2.7, archer: 4.2, bomber: 4.4, shaman: 3.4, shieldbearer: 2.35, assassin: 5.8, siegeGolem: 1.9, nullifier: 3.1, golden: 5.2, elite: 2.15, boss: 1.55 };
const WEIGHT = { grunt: .8, hound: .65, brute: 1.3, archer: .72, bomber: .82, shaman: .92, shieldbearer: 1.2, assassin: .58, siegeGolem: 1.65, nullifier: 1, golden: .55, elite: 1.5, boss: 1.9 };
export function advanceEnemyAttackResolveState(previous, type, didAttack, dt) {
    const prev = previous ?? { resolve: 0, settle: 0 };
    const safeDt = clamp(Number.isFinite(dt) ? dt : 0, 0, .1);
    if (didAttack)
        return { resolve: 1, settle: Math.max(prev.settle, .18) };
...
export function enemyAttackResolvePresentation(type, state, facingX, facingY, reducedMotion = false) {
