...
export interface MythicBossProfile {
  active: boolean;
  label: string;
  tier: number;
...
export interface MythicPressureModifiers {
...
function uniqueChannels(ordinal: number): BossArchetype[] {
  const channels: BossArchetype[] = [];
...
export function mythicBossProfile(elapsedSeconds: number, threatLevel: number, bossOrdinal: number): MythicBossProfile {
  const ordinal = Math.max(0, Math.floor(bossOrdinal));
  const active = elapsedSeconds >= 3600 && threatLevel >= 4 && ordinal % 4 === 3;
  if (!active) return { active: false, label: '', tier: 0, channels: [] };
...
export function mythicPressureModifiers(profile: MythicBossProfile): MythicPressureModifiers {
  if (!profile.active) return { healthMultiplier:1, damageMultiplier:1, specialCadenceMultiplier:1, projectileDensityMultiplier:1, summonCountMultiplier:1, rewardMultiplier:1 };
