...
const motionExpected = (settings) => !settings.reducedMotion;
const add = (samples, feature, settings, motionAmplitude, visible, shouldMove) => {
...
export function auditReducedMotionLiveCombat() {
    const samples = [];
    ...
    const secondaryOwners = ['boss-hazard', 'priority-threat', 'supply-crate', 'field-node', 'freeze-status', 'core-ambient'];
    const secondaryAmplitudes = (owner, reducedMotion) => {
    ...
    for (const owner of secondaryOwners) {
        const normalSettings = { name: 'owner-motion-on', reducedFlash: false, reducedMotion: false };
        add(samples, `secondary-${owner}`, normalSettings, secondaryAmplitudes(owner, false), true, true);
    }
    for (const owner of secondaryOwners) {
        const reducedSettings = { name: 'owner-motion-low', reducedFlash: false, reducedMotion: true };
        add(samples, `secondary-${owner}`, reducedSettings, secondaryAmplitudes(owner, true), true, false);
    }
    const residualOwners = ['black-hole-vortex', 'terrain-crystal', 'golden-enemy', 'bomber-body', 'final-form-flow'];
    const residualAmplitude = (owner, reducedMotion) => {
    ...
    for (const owner of ['black-hole-vortex', 'terrain-crystal', 'final-form-flow']) {
        const reducedSettings = { name: 'owner-motion-low', reducedFlash: false, reducedMotion: true };
    ...
    if (reducedMotionMaxAmplitude !== 0)
        issues.push(`motion-low:${reducedMotionMaxAmplitude}`);
    if (!flashOnlyMotionPreserved)
        issues.push('flash-motion-independence');
    if (steadyVisibilityRate !== 1)
        issues.push(`visibility:${steadyVisibilityRate}`);
    if (secondaryOwnerCoverage !== 1)
        issues.push(`secondary:${secondaryOwnerCoverage}`);
    if (residualOwnerCoverage !== 1)
        issues.push(`residual:${residualOwnerCoverage}`);
    if (actionCount !== 9)
