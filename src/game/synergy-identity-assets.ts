...
] as const satisfies readonly SynergyId[];

export type SynergyIdentityId = typeof SYNERGY_IDENTITY_IDS[number];

export const SYNERGY_IDENTITY_ATLAS = {
  src:'./assets/ui/synergy-icons.png', columns:4, rows:3, cellSize:96, width:384, height:288,
} as const;

const CELL_BY_ID:Readonly<Record<SynergyIdentityId,readonly [number,number]>>={
...
const LABEL:Readonly<Record<SynergyIdentityId,string>>={
...
const ACCENT:Readonly<Record<SynergyIdentityId,string>>={
...
export interface SynergyIdentityIcon { id:SynergyIdentityId; label:string; accent:string; sx:number; sy:number; sw:number; sh:number; animated:false; motionAmplitude:0; textFallbackPreserved:true; loadFailureBlocksGameplay:false; }
export function synergyIdentityIcon(id:SynergyIdentityId):SynergyIdentityIcon{
...
export function auditSynergyIdentityAtlas(){
  ...
  for(const id of SYNERGY_IDENTITY_IDS){const [c,r]=CELL_BY_ID[id];cells.add(`${c}:${r}`);if(c<0||r<0||c>=SYNERGY_IDENTITY_ATLAS.columns||r>=SYNERGY_IDENTITY_ATLAS.rows)outOfBounds.push(id);}
