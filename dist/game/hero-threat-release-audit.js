...
export function heroThreatPressureIndex(minute, threat) {
...
export function heroThreatReleaseCheckpoints() {
    const points = [];
...
export function auditHeroThreatReleaseBalance() {
    ...
    for (const hero of HERO_PROFILES) {
        for (const minute of MINUTES) {
            ...
            if (!(samples[0].pressureIndex < samples[1].pressureIndex && samples[1].pressureIndex < samples[2].pressureIndex))
                threatMonotonic = false;
            if (!(samples[0].releaseMargin > samples[1].releaseMargin && samples[1].releaseMargin > samples[2].releaseMargin))
                heroMarginsMonotonic = false;
        }
    }
