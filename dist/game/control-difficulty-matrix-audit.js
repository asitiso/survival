import { threatLevelModifiers } from '../domain/threat-level.js';
import { HERO_PROFILES } from './hero-profiles.js';
import { heroReleaseModel } from './hero-release-model.js';
export const CONTROL_EFFICIENCY = { averageManual: .86, auto: .90, skilledManual: 1.00 };
function round(v) { return Math.round(v * 10000) / 10000; }
export function controlThreatPressureIndex(threat) {
...
export function auditControlDifficultyMatrix() {
    const bands = [['average-manual', CONTROL_EFFICIENCY.averageManual], ['auto', CONTROL_EFFICIENCY.auto], ['skilled-manual', CONTROL_EFFICIENCY.skilledManual]];
    const samples = [];
    for (const hero of HERO_PROFILES) {
        const model = heroReleaseModel(hero.id);
        for (let t = 0; t <= 5; t++) {
            const threat = t, pressure = controlThreatPressureIndex(threat);
            for (const [control, eff] of bands)
                samples.push({ heroId: hero.id, threat, control, pressureIndex: pressure, controlEfficiency: eff, effectiveMargin: round(model.compositeIndex * eff / Math.pow(pressure, .42)) });
        }
    }
    const pressures = Array.from({ length: 6 }, (_, i) => controlThreatPressureIndex(i));
    const pressureMonotonic = pressures.every((value, index) => index === 0 || value > pressures[index - 1]);
    const controlOrderingPassed = HERO_PROFILES.every((hero) => Array.from({ length: 6 }, (_, i) => i).every((threat) => { const g = samples.filter((s) => s.heroId === hero.id && s.threat === threat); const avg = g.find((s) => s.control === 'average-manual'), auto = g.find((s) => s.control === 'auto'), skilled = g.find((s) => s.control === 'skilled-manual'); return avg.effectiveMargin < auto.effectiveMargin && auto.effectiveMargin < skilled.effectiveMargin; }));
    const threatTwoBaseline = Math.abs(controlThreatPressureIndex(2) - 1) <= .0001;
    const issues = [];
    if (samples.length !== 72)
        issues.push('matrix-size');
    if (!pressureMonotonic)
        issues.push('threat-pressure-monotonic');
    if (!controlOrderingPassed)
        issues.push('control-ordering');
    if (!threatTwoBaseline)
        issues.push('threat-two-baseline');
    if (CONTROL_EFFICIENCY.auto < .88 || CONTROL_EFFICIENCY.auto > .92)
        issues.push('auto-efficiency-band');
