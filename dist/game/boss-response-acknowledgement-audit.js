...
export function auditBossResponseAcknowledgement() {
    const samples = [];
    ...
    for (const archetype of ARCHETYPES) {
        ...
        const acknowledged = bossActionAssist({ archetype, specialTimer: .5, hpRatio: .8, potions: 0, readyActions: new Set([mapped[0], mapped[1]]), acknowledged: true });
        ackExpected++;
        ...
        const queued = bossActionAssist({ archetype, specialTimer: .5, hpRatio: .8, potions: 0, readyActions: new Set([mapped[0]]), queuedActions: new Set([mapped[2]]), previousCue: previous, previousCueAge: .1, previousArchetype: archetype });
        queuedExpected++;
        if (queued?.actionId === mapped[2])
            queuedHit++;
        add(samples, archetype, 'queued-cue-persistence', mapped[2], queued?.actionId ?? null);
    }
    const rescue = bossActionAssist({ archetype: 'summoner', specialTimer: .4, hpRatio: .25, potions: 1, readyActions: new Set(['potion', 'spell4']), acknowledged: true });
    add(samples, 'global', 'potion-rescue', 'potion', rescue?.actionId ?? null);
    const outside = bossActionAssist({ archetype: 'summoner', specialTimer: 1.06, hpRatio: .8, potions: 0, readyActions: new Set(['spell4']), acknowledged: true });
    add(samples, 'global', 'window-reset', null, outside?.actionId ?? null);
    const reprompt = bossActionAssist({ archetype: 'summoner', specialTimer: .5, hpRatio: .8, potions: 0, readyActions: new Set(['spell4']), queuedActions: new Set() });
    ...
    if (acknowledgementCoverage !== 1)
        issues.push('acknowledgement-coverage');
    if (alternativeResponseCoverage !== 1)
        issues.push('alternative-response-coverage');
    if (queuedCueCoverage !== 1)
        issues.push('queued-cue-coverage');
    if (potionRescueCoverage !== 1)
        issues.push('potion-rescue');
    if (windowResetCoverage !== 1)
        issues.push('window-reset');
    if (queuedCancelRepromptCoverage !== 1)
        issues.push('queued-cancel-reprompt');
