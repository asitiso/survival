...
import { DECISION_TRANSITION_BARRIER_MS, DecisionPickGuard, nextDecisionKind, type DecisionKind, type DecisionPendingState } from './decision-continuity.js';

export interface DecisionContinuityAudit {
  samples: number;
  prioritySamples: number;
  priorityOrder: Array<DecisionKind | null>;
  stackedLevelUpSamples: number;
  repeatedBossRewardSamples: number;
  exactlyOncePassed: boolean;
  transitionBarrierPassed: boolean;
  lifecycleResetPassed: boolean;
...
const pending = (overrides: Partial<DecisionPendingState> = {}): DecisionPendingState => ({
  fate: false,
  heroAscension: false,
  runContract: false,
  bossRewardCount: 0,
...
export function auditDecisionContinuity(): DecisionContinuityAudit {
  const priorityStates: DecisionPendingState[] = [
  ...
  const expectedPriority: Array<DecisionKind | null> = ['fate', 'heroAscension', 'runContract', 'bossReward', 'levelUp', null];
  ...
  const issues: string[] = [];
  if (!priorityPassed) issues.push('priority');
  if (!stackedLevelUpPassed) issues.push('stacked-level-up');
  if (!repeatedBossRewardPassed) issues.push('repeated-boss-reward');
  if (!exactlyOncePassed) issues.push('exactly-once');
  if (!transitionBarrierPassed) issues.push('transition-barrier');
  if (!lifecycleResetPassed) issues.push('lifecycle-reset');
  if (!pendingPreserved) issues.push('pending-preservation');
  if (actionCount !== 9) issues.push('action-surface');
