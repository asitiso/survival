...
export interface LongRunRewardDensityPolicy{
  goldMultiplier:number;
  xpMultiplier:number;
  baseMultiplier:number;
  targetGoldPerMinute:number;
  damping:number;
}
export interface LongRunRewardDensityCheckpoint{
  minute:number;
  goldMultiplier:number;
  xpMultiplier:number;
  targetGoldPerMinute:number;
}
export interface LongRunRewardDensityAudit{
  checkpoints:LongRunRewardDensityCheckpoint[];
  noDrought:boolean;
  noInflation:boolean;
  passed:boolean;
}

export type LongRunEconomyScenario='drought'|'healthy'|'saturated';
export interface LongRunEconomyCheckpoint{
  minute:number;
  scenario:LongRunEconomyScenario;
  recentGoldPerMinute:number;
  goldMultiplier:number;
  xpMultiplier:number;
}
export interface LongRunEconomyAudit{
  minutes:number[];
  checkpoints:LongRunEconomyCheckpoint[];
  bounded:boolean;
  saturatedNeutral:boolean;
  xpBalanced:boolean;
  noLateSpike:boolean;
  maxAdjacentGoldDelta:number;
  maxAdjacentXpDelta:number;
  passed:boolean;
}

function band(seconds:number):{base:number;target:number}{
  if(seconds<7200)return{base:1,target:0};
  if(seconds<14400)return{base:1.04,target:500};
  if(seconds<28800)return{base:1.06,target:650};
  return{base:1.08,target:800};
}

export function longRunRewardDensityPolicy(elapsedSeconds:number,recentGoldPerMinute:number):LongRunRewardDensityPolicy{
  ...
  if(selected.base===1)return{goldMultiplier:1,xpMultiplier:1,baseMultiplier:1,targetGoldPerMinute:0,damping:1};
...
export function auditLongRunRewardDensity():LongRunRewardDensityAudit{
  const checkpoints=[120,240,480,720].map((minute):LongRunRewardDensityCheckpoint=>{
...
export function auditLongRunEconomy():LongRunEconomyAudit{
  const minutes=[120,180,240,360,480,600,720];
  const scenarios:readonly LongRunEconomyScenario[]=['drought','healthy','saturated'];
  const checkpoints:LongRunEconomyCheckpoint[]=[];
