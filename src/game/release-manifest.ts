import type { ArchiveReproducibilityAudit } from './archive-reproducibility.js';
import type { PackageRuntimeSmokeAudit } from './package-runtime-smoke.js';
import type { PackageRunCycleSmokeAudit } from './package-run-cycle-smoke.js';
import type { ArchiveProvenanceAudit } from './archive-provenance.js';
export interface ReleaseCandidateManifestEvidence { ok:boolean;signature:string;issues:string[];summary?:string | undefined; }
export interface ReleaseManifestInput {
  sourceRevision:string;
  test:{ok:boolean;count:number};
  buildOk:boolean;
  raster:{ok:boolean;signature:string};
  release:{ok:boolean;signature:string;actionCount:number;profileCount:number};
  foldable:{ok:boolean;signature:string;reachableActionCount:number;maxLeftTravel:number;maxRightTravel:number;averageRightTravel:number;hingeClear:boolean};
  baselineMutation:boolean;
  candidateAudit?:ReleaseCandidateManifestEvidence | undefined;
  archiveReproducibility?:ArchiveReproducibilityAudit | undefined;
  archiveProvenance?:ArchiveProvenanceAudit | undefined;
  packageRuntime?:PackageRuntimeSmokeAudit | undefined;
  packageRunCycle?:PackageRunCycleSmokeAudit | undefined;
}
export interface ReleaseManifestJson {
  schemaVersion:1;
  sourceRevision:string;
  status:'PASS'|'REVIEW';
  signature:string;
  evidence:{
    tests:{ok:boolean;count:number};buildOk:boolean;raster:{ok:boolean;signature:string};release:{ok:boolean;signature:string;actionCount:number;profileCount:number};
    foldable:{ok:boolean;signature:string;reachableActionCount:number;maxLeftTravel:number;maxRightTravel:number;averageRightTravel:number;hingeClear:boolean};
    baselineMutation:boolean;
    candidateAudit?:ReleaseCandidateManifestEvidence | undefined;
    archiveReproducibility?:ArchiveReproducibilityAudit | undefined;
...
function finite(n:number):number{return Number.isFinite(n)?Math.round(n*10)/10:0;}
export function releaseManifest(input:ReleaseManifestInput):ReleaseManifest{
  ...
  const issues:string[]=[];
  if(!input.test.ok)issues.push('tests-failed');
  if(!input.buildOk)issues.push('build-failed');
  if(!input.raster.ok)issues.push('raster-review-required');
  if(!input.release.ok)issues.push('release-gate-failed');
  if(actionCount!==9)issues.push(`action-count:${actionCount}!=9`);
  if(input.release.profileCount!==5)issues.push(`raster-profile-count:${input.release.profileCount}!=5`);
  if(!input.foldable.ok)issues.push('foldable-thumb-travel');
  if(input.foldable.reachableActionCount!==9)issues.push(`foldable-actions:${input.foldable.reachableActionCount}!=9`);
  if(!input.foldable.hingeClear)issues.push('foldable-hinge-crossing');
  if(input.baselineMutation)issues.push('baseline-mutation-enabled');
  if(input.candidateAudit&&!input.candidateAudit.ok)issues.push(`candidate-audit:${input.candidateAudit.issues.join(',')||'failed'}`);
  if(input.archiveReproducibility&&!input.archiveReproducibility.passed)issues.push('archive-reproducibility');
  if(input.archiveProvenance&&!input.archiveProvenance.passed)issues.push('archive-provenance');
  if(input.packageRuntime&&!input.packageRuntime.passed)issues.push('package-runtime-smoke');
  if(input.packageRunCycle&&!input.packageRunCycle.passed)issues.push('package-run-cycle');
  const status=issues.length===0?'PASS' as const:'REVIEW' as const;
