...
export function openingWaveCeremony(elapsedSeconds) {
    ...
    for (const beat of BEATS) {
        ...
        if (delta >= 0 && delta < 7) {
            const fade = 1 - delta / 7;
            return { beatId: beat.id, label: beat.label, spawnPulse: 1 + (beat.spawn - 1) * fade, rewardPulse: 1 + (beat.reward - 1) * fade, telegraphPulse: 1 + (beat.telegraph - 1) * fade };
        }
    }
    return { beatId: null, label: '', spawnPulse: 1, rewardPulse: 1, telegraphPulse: 1 };
}
