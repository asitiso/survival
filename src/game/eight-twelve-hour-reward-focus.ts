...
export type EightTwelveHourRewardChoice<T extends BossRewardChoice=BossRewardChoice>=T&{badge?:string;hint?:string;best?:boolean};
export interface EightTwelveHourRewardContext { elapsedSeconds:number; completeBuild:boolean; finalFormActive:boolean; }
export interface EightTwelveHourRewardFocus<T extends BossRewardChoice=BossRewardChoice> {
  choices:EightTwelveHourRewardChoice<T>[];
  compact:boolean;
  preserveFullDetails:boolean;
  reduceAlternativeEmphasis:boolean;
  subtitle:string;
  autoSelect:false;
  estimatedReadReduction:number;
}

function finalFormRelevant<T extends BossRewardChoice>(choice:EightTwelveHourRewardChoice<T>):boolean {
...
export function focusEightTwelveHourBossRewards<T extends BossRewardChoice>(choices:readonly EightTwelveHourRewardChoice<T>[],context:EightTwelveHourRewardContext):EightTwelveHourRewardFocus<T> {
