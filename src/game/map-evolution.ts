import type { MapId, MapLayout, MapWall, MapSlowPool, MapCrystal } from './map-layouts.js';
import type { PresentationQuality } from './presentation-budget.js';

export type MapEvolutionStage = 0 | 1 | 2;

export function mapEvolutionStage(seconds: number): MapEvolutionStage {
...
function copyWalls(items: readonly MapWall[]): MapWall[] { return items.map((item) => ({ ...item })); }
function copyPools(items: readonly MapSlowPool[]): MapSlowPool[] { return items.map((item) => ({ ...item })); }
function copyCrystals(items: readonly MapCrystal[]): MapCrystal[] { return items.map((item) => ({ ...item })); }

export function evolveMapLayout(layout: MapLayout, stage: MapEvolutionStage): MapLayout {
  if (stage === 0) return { ...layout, walls: copyWalls(layout.walls), pools: copyPools(layout.pools), crystals: copyCrystals(layout.crystals) };
  ...
  if (layout.id === 'ruinedGate') {
    ...
    if (stage >= 1) {
      crystals.push(
        { x: 800, y: 210, threshold: 4, blastRadius: 165, blastDamage: 190 },
        { x: 800, y: 740, threshold: 4, blastRadius: 165, blastDamage: 190 },
      );
...
export function mapEvolutionLabel(mapId: MapId, stage: Exclude<MapEvolutionStage, 0>): string {
  if (mapId === 'ruinedGate') return stage === 1 ? '성벽 붕괴 · 서쪽 통로 개방' : '관문 붕괴 · 중앙 전선 개방';
  if (mapId === 'frozenFen') return stage === 1 ? '빙결 확산 · 늪지 확대' : '빙하 이동 · 안전지대 재편';
  return stage === 1 ? '수정 공명 · 연쇄망 활성화' : '마력 과포화 · 수정 폭주';
}


export type MapEnvironmentVfxMotif='embers'|'snow'|'shards';
export interface MapEnvironmentVfxDescriptor { motif:MapEnvironmentVfxMotif; color:string; particlesPerSecond:number; speed:number; size:number; evolutionPulseAlpha:number; }
export function mapEnvironmentVfxDescriptor(mapId:MapId,stage:MapEvolutionStage,quality:PresentationQuality):MapEnvironmentVfxDescriptor {
  const base=mapId==='ruinedGate'
    ? {motif:'embers' as const,color:'#f2a15c',rate:4.2,speed:34,size:2.6}
    : mapId==='frozenFen'
      ? {motif:'snow' as const,color:'#b8f1ff',rate:5.0,speed:24,size:2.8}
      : {motif:'shards' as const,color:'#cbb8ff',rate:4.5,speed:30,size:2.4};
...
export type EnvironmentDestructionKind='crystalBlast'|'evolutionCollapse';
export type EnvironmentDestructionMotion='burst'|'collapse';
export interface EnvironmentDestructionVfxDescriptor { motif:MapEnvironmentVfxMotif; color:string; motion:EnvironmentDestructionMotion; debrisCount:number; rayCount:number; waveCount:number; glowAlpha:number; debrisSpeed:number; }
export function environmentDestructionVfxDescriptor(mapId:MapId,kind:EnvironmentDestructionKind,stage:MapEvolutionStage,quality:PresentationQuality):EnvironmentDestructionVfxDescriptor {
  const env=mapEnvironmentVfxDescriptor(mapId,stage,quality); const q=quality==='high'?1:quality==='medium'?.68:.4; const crystal=kind==='crystalBlast';
...
export interface MapEvolutionMaterial {
  poolOuter: string;
  poolInner: string;
  poolGlow: string;
  poolRim: string;
...
export function resolveMapEvolutionMaterial(mapId: MapId, stage: MapEvolutionStage): MapEvolutionMaterial {
  if (mapId === 'ruinedGate') {
    return stage === 2
      ? {
