...
export interface FoldableThumbTravelAudit {
  applicable:boolean;
  ok:boolean;
  signature:string;
  leftAnchor:Vec2;
  rightAnchor:Vec2;
  maxLeftTravel:number;
  maxRightTravel:number;
  averageRightTravel:number;
  reachableActionCount:number;
  unreachableActions:ActionId[];
  crossHingeActions:ActionId[];
  hingeClear:boolean;
  issues:string[];
}

function distance(a:Vec2,b:Vec2):number{return Math.hypot(a.x-b.x,a.y-b.y);}
function rectContains(r:LandscapeSafeRect,p:Vec2):boolean{return p.x>=r.x&&p.x<=r.x+r.width&&p.y>=r.y&&p.y<=r.y+r.height;}
function fnv(text:string):string{let h=2166136261;for(let i=0;i<text.length;i++){h^=text.charCodeAt(i);h=Math.imul(h,16777619);}return(h>>>0).toString(16).toUpperCase().padStart(8,'0');}
function round1(value:number):number{return Math.round(value*10)/10;}

export function foldableThumbTravelAudit(safe:LandscapeSafeAreaProfile,buttons:readonly ActionButtonLayout[]):FoldableThumbTravelAudit{
  ...
  const issues:string[]=[];
  if(maxLeftTravel>370)issues.push('left-thumb-travel');
  if(maxRightTravel>560)issues.push('right-thumb-travel');
  if(averageRightTravel>360)issues.push('right-thumb-average');
  if(unreachableActions.length)issues.push(`unreachable:${unreachableActions.join(',')}`);
  if(crossHingeActions.length)issues.push(`hinge-cross:${crossHingeActions.join(',')}`);
