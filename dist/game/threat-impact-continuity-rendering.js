...
export function threatLaunchOwnershipPresentation(input, reducedFlash = false) {
    const launch = clamp01(input.launchLife), travel = clamp01(input.travelLife), threat = clamp01(input.threat);
    const owner = launch > .16 ? 'launch' : travel > .08 ? 'travel' : 'canonical';
...
export function projectileTravelThreatCarryPresentation(input, reducedMotion = false, reducedFlash = false) {
    ...
    return { visible, alphaScale: visible ? alpha : 0, lengthScale, minLength: Math.max(8, radius * 1.4), presentationOnly: true };
}
export function impactArrivalFootprintContinuityPresentation(input, reducedFlash = false) {
    ...
    return { fillAlphaScale, edgeAlphaScale, radiusScale, presentationOnly: true };
}
export function impactReactionCarryPresentation(input, reducedMotion = false) {
    ...
    return { aftermathAlphaScale, spriteAlphaScale, presentationOnly: true };
}
export function hazardExpiryEdgeContinuityPresentation(input, reducedFlash = false) {
    if ((Number.isFinite(input.telegraph) ? input.telegraph : 0) > 0)
        return { owner: 'telegraph', fillAlphaScale: 1, edgeAlphaScale: 1, presentationOnly: true };
    const life = clamp01((Number.isFinite(input.ttl) ? input.ttl : 0) / Math.max(.001, Number.isFinite(input.maxTtl) ? input.maxTtl : 1));
    if (life > .22)
        return { owner: 'active', fillAlphaScale: 1, edgeAlphaScale: 1, presentationOnly: true };
    const expiry = clamp01(life / .22), flash = reducedFlash ? .82 : 1;
    return { owner: 'expiry', fillAlphaScale: (.28 + .72 * expiry) * flash, edgeAlphaScale: (.9 + .1 * expiry) * flash, presentationOnly: true };
}
export function denseBattleSafeLaneContinuityPresentation(input, reducedMotion = false, reducedFlash = false) {
    ...
    return { pressure, safeLaneAlphaScale, hazardFillScale, hazardEdgeScale: 1, presentationOnly: true };
}
