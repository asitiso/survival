...
const FAMILIES = ['slayer', 'warden', 'arcane', 'hunter', 'survivor'];
const DURATIONS = { slayer: 45, warden: 30, arcane: 40, hunter: 60, survivor: 20 };
const SCHEDULE = [4, 9, 14, 19, 26];
const PACE_CASES = [{ p: .5, e: .58, id: 'onTrack' }, { p: .5, e: .59, id: 'catchUp' }, { p: .5, e: .75, id: 'catchUp' }, { p: .5, e: .76, id: 'critical' }, { p: 1, e: 1, id: 'onTrack' }];
function legacy(elapsedMs, guardianCoreHp = 1000) { return { heroId: 'arkan', elapsedMs, level: 30, threat: 5, kills: 500, bossesDefeated: 4, elitesDefeated: 25, gold: 5000, xp: 10000, guardianCoreHp, guardianCoreMaxHp: 1000, fate: 'none', spellFusionCount: 1, mapEvolutionRank: 2, masteryLevel: 12, deviceClass: 'high' }; }
function offerContaining(family) { for (let seed = 1; seed <= 128; seed++) {
    const result = createContractOffer(legacy(20 * 60_000), createDefaultContractState(), { seed, cursor: 0 });
    const option = result.offer.options.find(entry => entry.family === family);
    if (option)
        return { result, option };
} throw new Error(`offer family unavailable: ${family}`); }
function completionEvents(family, target) { if (family === 'slayer')
    return Array.from({ length: target }, () => ({ type: 'enemy_killed' })); if (family === 'arcane')
    return Array.from({ length: target }, () => ({ type: 'spell_cast', spellId: 'fire' })); if (family === 'hunter')
    return Array.from({ length: target }, () => ({ type: 'enemy_killed', elite: true })); return []; }
function completesWithNinetySecondBoon(family) { const { result, option } = offerContaining(family), startedAt = 1000; const accepted = acceptContract(result.state, option.optionId, startedAt, 1000); const timed = family === 'warden' || family === 'survivor'; const completeAt = timed ? startedAt + option.target : startedAt + 5000; const step = advanceContract(accepted, legacy(completeAt), completionEvents(family, option.target), timed ? option.target : 100); const boon = step.state.boons.at(-1); return !step.state.active && step.state.completedCount === 1 && step.effects.length === 1 && step.effects[0]?.type === 'contract_reward' && boon?.family === family && boon.expiresAtMs - completeAt === 90_000; }
function failsByFrozenRule(family) { const { result, option } = offerContaining(family), startedAt = 1000; const accepted = acceptContract(result.state, option.optionId, startedAt, 1000); if (family === 'warden') {
    const step = advanceContract(accepted, legacy(startedAt + 100, 790), [], 100);
    return step.effects[0]?.type === 'contract_failed' && !step.state.active;
} if (family === 'survivor') {
    const step = advanceContract(accepted, legacy(startedAt + 100), [{ type: 'hero_damaged', amount: 1 }], 100);
    return step.effects[0]?.type === 'contract_failed' && !step.state.active;
} const step = advanceContract(accepted, legacy(startedAt + option.durationMs), [], option.durationMs); return step.effects[0]?.type === 'contract_failed' && !step.state.active; }
function modifiersStable(family) { const state = { ...createDefaultContractState(), boons: [{ family, expiresAtMs: 100_000 }] }; const m = getContractModifiers(state, 1); if (family === 'slayer')
    return m.xpMultiplier === 1.12 && m.masteryMultiplier === 1.08; if (family === 'warden')
    return m.coreDamageTakenMultiplier === .88 && m.potionEfficiency === 1.1; if (family === 'arcane')
    return m.fusionPowerMultiplier === 1.1 && m.cooldownMultiplier === .92; if (family === 'hunter')
    return m.goldMultiplier === 1.15 && m.bossDamageMultiplier === 1.08; return m.coreDamageTakenMultiplier === .92 && m.potionEfficiency === 1.15; }
export function auditRunContractDecisionIdentityAssets() {
    const req = auditRunContractRequirementIdentityAtlas(), boon = auditRunContractBoonEffectIdentityAtlas(), samples = [];
    let gameplayContractMutation = false;
    const push = (caseId, family, passed) => { samples.push({ caseId, family, passed }); if (!passed)
    ...
    FAMILIES.forEach((family, index) => {
        ...
        const pace = PACE_CASES[index];
        push(`${family}:pace`, family, runMissionPaceIdentityForRatios(pace.p, pace.e) === pace.id);
    ...
    if (!req.passed)
        issues.push('requirement-atlas');
    if (!boon.passed)
        issues.push('boon-atlas');
