...
import { advanceAscension, createDefaultAscensionState, getAscensionTier } from './ascension.js';
import { ascensionMutatorRuntimeModifiers } from './ascension-mutator-runtime.js';
import { ASCENSION_MUTATOR_IDENTITY_IDS, auditAscensionMutatorIdentityAtlas, ascensionMutatorIdentityIcon } from './ascension-mutator-identity-assets.js';
...
export function auditAscensionMutatorIdentityAssets() {
    const atlas = auditAscensionMutatorIdentityAtlas();
    const samples = [];
    const push = (caseId, passed, id) => { samples.push({ caseId, passed, ...(id ? { id } : {}) }); };
    const toastSet = new Set(), recallSet = new Set(), fallbackSet = new Set();
    ...
    for (const id of ASCENSION_MUTATOR_IDENTITY_IDS) {
        ...
        const runtime = ascensionMutatorRuntimeModifiers([id]);
        const expected = RUNTIME_EXPECTED[id];
        ...
        push(`${id}:tier-boundaries`, tierOk, id);
        push(`${id}:tier-cap`, getAscensionTier(999 * 60_000) === 10, id);
        push(`${id}:rng-deterministic`, rngDeterministic, id);
        push(`${id}:rng-unique`, rngUnique, id);
        const ownRuntime = sameRuntime(runtime, expected);
        push(`${id}:runtime-own`, ownRuntime, id);
        const neutral = ascensionMutatorRuntimeModifiers([]);
        const neutralOk = sameRuntime(neutral, { projectileSpeedMultiplier: 1, eliteHealthMultiplier: 1, shopIntervalMultiplier: 1, volatileDeath: { enabled: false, radius: 0, damage: 0 } });
        push(`${id}:runtime-neutral`, neutralOk, id);
        ...
        tierContractMutation ||= !tierOk;
        rngContractMutation ||= !rngDeterministic || !rngUnique;
        runtimeModifierMutation ||= !ownRuntime || !neutralOk;
    }
    ...
    if (iconMotionAmplitude !== 0)
        issues.push('motion');
    if (tierContractMutation)
        issues.push('tier-contract-mutation');
    if (rngContractMutation)
        issues.push('rng-contract-mutation');
    if (runtimeModifierMutation)
        issues.push('runtime-modifier-mutation');
    if (actionCount !== 9)
