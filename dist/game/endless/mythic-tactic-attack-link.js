...
export function createMythicTacticAttackLink(archetype, nowMs, durationMs) {
...
export function activeMythicTacticAttackLink(state, nowMs, archetype) {
    if (!state || state.consumed || state.archetype !== archetype || !Number.isFinite(nowMs) || nowMs > state.expiresAtMs)
        return null;
    return state;
}
export function consumeMythicTacticAttackLink(state) { return state.consumed ? state : { ...state, consumed: true }; }
