...
import { combineEquipment, strengthenEquipment, strengthenRequirement } from '../domain/equipment-forge.js';
import { projectEquipmentSurvival } from '../game/equipment-survival-readiness.js';
...
const kindNames = { weapon: '무기', armor: '방어구', accessory: '장신구' };
const timeLabel = (seconds) => `${Math.floor(seconds / 60)}:${String(seconds % 60).padStart(2, '0')}`;
const survivalSummary = (model, state) => projectEquipmentSurvival(model, state).summary;
export function shopSetSummary(state) {
...
export function shopPurchaseView(model, offer) {
...
function equipmentEffectText(item) {
    const empty = { coins: 0, weapon: null, armor: null, accessory: null, healingPotions: 0, inventory: [], discoveredRecipes: [] };
    ...
    return Object.entries(labels).filter(([key]) => bonuses[key] !== 1)
        .map(([key, label]) => `${label} ${bonuses[key].toFixed(2)}×`).join(' · ') || '기본 효과';
}
export function shopSelectionView(model, key) {
    ...
    if (!item || !key)
        return null;
    const target = equippedKind ? { place: 'equipped', kind: equippedKind } : { place: 'inventory', stackKey: key };
...
export function shopRecipeView(model, recipe) {
...
export class ShopOverlay {
    ...
    constructor(parent) {
        ...
        this.root.addEventListener('keydown', event => {
            ...
            if (event.key === 'Tab') {
                const controls = [...this.root.querySelectorAll('button:not(:disabled), select, [tabindex="0"]')]
    ...
    render(model) {
        const focusedKey = this.root.contains(document.activeElement) ? document.activeElement?.dataset.shopFocus : undefined;
      ...
      <div class="shop-wallet"><span>보유 금화 · 물약 ${model.state.healingPotions}개</span><strong>🪙 ${model.state.coins.toLocaleString()}</strong></div>`;
        ...
        for (const [tab, label] of [['purchase', '구매'], ['forge', '대장간']]) {
            ...
            button.addEventListener('keydown', event => {
                ...
                this.root.querySelector(`#shop-tab-${next}`)?.focus();
            });
      ...
      `;
        ...
        const focusable = [...this.root.querySelectorAll('[data-shop-focus]')];
        const previous = focusable.find(element => element.dataset.shopFocus === focusedKey && !element.disabled);
        (previous ?? this.root.querySelector(`#shop-tab-${model.activeTab}`))?.focus({ preventScroll: true });
    }
    renderForge(body, model) {
        const equipped = document.createElement('div');
        ...
        if (!selected)
            actions.textContent = '장착 장비 또는 보관 장비를 클릭하면 장착·강화·판매 조건을 확인할 수 있습니다.';
        else {
        ...
        for (const recipe of model.recipes) {
            ...
            if (view.iconId)
                title.append(this.equipmentIcon({ id: view.iconId }, 'shop-recipe-result-icon'));
    ...
    itemButton(item, key, model, className, emptyText, count) {
        const button = document.createElement('button');
    ...
    actionButton(parent, label, key, view, action) {
        const row = document.createElement('div');
