...
import { nextFloat, pickWeighted, type RngState } from './rng.js';
import type { Effect, LegacyRunView } from './types.js';

export type WorldState = 'calm' | 'stormfront' | 'ruins' | 'mana_bloom' | 'blood_moon' | 'sanctuary';
export type FieldNodeKind = 'safe_corridor' | 'barricade' | 'mana_well' | 'volatile_zone' | 'sanctuary_zone';
...
export interface WorldModifiers {
  ...
  normalSpellCadenceMultiplier: number;
  siegePressureMultiplier: number;
}
...
const EVOLVABLE: Exclude<WorldState, 'calm'>[] = ['stormfront', 'ruins', 'mana_bloom', 'blood_moon', 'sanctuary'];

export function createDefaultWorldState(): WorldRuntimeState {
  return { current: 'calm', evolutionCount: 0, lastEvolutionAtMs: 0, nodes: [] };
}

export function shouldEvolveWorld(state: WorldRuntimeState, elapsedMs: number): boolean {
  return elapsedMs >= (state.evolutionCount + 1) * 8 * 60_000;
}

function worldWeight(world: Exclude<WorldState, 'calm'>, legacy: LegacyRunView): number {
  let weight = 1;
  if (legacy.fate === 'guardian' && world === 'sanctuary') weight += 1;
  if (legacy.fate === 'guardian' && world === 'ruins') weight += 0.5;
  if (legacy.fate === 'frenzy' && world === 'blood_moon') weight += 1;
  if (legacy.fate === 'gold' && world === 'blood_moon') weight += 0.6;
  if (legacy.spellFusionCount > 0 && world === 'mana_bloom') weight += 0.75;
  if (legacy.threat >= 4 && world === 'stormfront') weight += 0.4;
  return weight;
}

function nodePlan(world: Exclude<WorldState, 'calm'>): { kind: FieldNodeKind; count: number; radius: number; ttlMs: number } {
  switch (world) {
...
function makeNodes(world: Exclude<WorldState, 'calm'>, elapsedMs: number, evolutionCount: number, rng: RngState): { nodes: FieldNode[]; rng: RngState } {
  const plan = nodePlan(world);
  const nodes: FieldNode[] = [];
  let nextRng = rng;
  for (let i = 0; i < plan.count; i += 1) {
    const rx = nextFloat(nextRng); nextRng = rx.state;
    const ry = nextFloat(nextRng); nextRng = ry.state;
    nodes.push({
...
export function evolveWorld(legacy: LegacyRunView, state: WorldRuntimeState, rng: RngState): WorldEvolutionResult {
  ...
  const effects: Effect[] = [
    ...
    ...nodeResult.nodes.map((node): Effect => ({
...
export function getWorldModifiers(world: WorldState, threat: number): WorldModifiers {
  const t = clamp(threat, 0, 5);
  const base: WorldModifiers = {
    spawnMultiplier: 1,
...
export function pruneExpiredNodes(state: WorldRuntimeState, elapsedMs: number): WorldRuntimeState {
  return { ...state, nodes: state.nodes.filter((node) => node.expiresAtMs > elapsedMs) };
}


export function consumeFieldNode(state: WorldRuntimeState, nodeId: string): { state: WorldRuntimeState; consumed: FieldNode | undefined } {
  const consumed = state.nodes.find((node) => node.nodeId === nodeId);
  if (!consumed) return { state, consumed: undefined };
