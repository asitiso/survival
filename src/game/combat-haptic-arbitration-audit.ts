...
import { CombatHapticArbiter, arbitrateCombatHaptics, type CombatHapticIntent } from './combat-haptic-arbitration.js';
import { criticalHapticEvents, dangerUiState } from './danger-ui.js';

export interface CombatHapticArbitrationAuditSample {
  caseId:string;
...
export interface CombatHapticArbitrationAudit {
  passed:boolean;
  samples:CombatHapticArbitrationAuditSample[];
  maxDispatchPerFrame:number;
  criticalPriorityPreservationRate:number;
  dualCriticalMergeRate:number;
  suppressedStaleReplayCount:number;
  hapticDisabledDispatchCount:number;
  safeExitRearmRate:number;
  reachableActionCount:number;
  snapshotSchemaMutation:false;
  issues:string[];
}

const add=(samples:CombatHapticArbitrationAuditSample[],caseId:string,expected:CombatHapticArbitrationAuditSample['expected'],actual:CombatHapticArbitrationAuditSample['actual'])=>samples.push({caseId,expected,actual,passed:expected===actual});

export function auditCombatHapticArbitration():CombatHapticArbitrationAudit{
  const samples:CombatHapticArbitrationAuditSample[]=[];
  const decisions=[] as ReturnType<typeof arbitrateCombatHaptics>[];
  const decide=(intents:readonly CombatHapticIntent[],enabled=true)=>{const decision=arbitrateCombatHaptics(intents,enabled);decisions.push(decision);return decision;};
  ...
  arbiter.queue('bossCountdown');arbiter.queue('heroCritical');
  ...
  arbiter.queue('bossPhase3');arbiter.clear();
  ...
  if(maxDispatchPerFrame>1)issues.push('multi-dispatch-frame');
  if(criticalPriorityPreservationRate!==1)issues.push('critical-priority');
  if(dualCriticalMergeRate!==1)issues.push('dual-critical-merge');
  if(suppressedStaleReplayCount!==0)issues.push('stale-replay');
  if(hapticDisabledDispatchCount!==0)issues.push('haptic-disabled-dispatch');
  if(safeExitRearmRate!==1)issues.push('safe-exit-rearm');
