...
import { RELIC_RESONANCE_IMPACT_IDENTITY_IDS, auditRelicResonanceImpactIdentityAtlas, relicResonanceImpactIdentityIcon, } from './relic-resonance-impact-identity-assets.js';
import { projectRelicResonance } from './relic-resonance-projection.js';
import { RELIC_RESONANCE_TIER_IDENTITY_IDS, auditRelicResonanceTierIdentityAtlas, relicResonanceTierIdentityIcon, } from './relic-resonance-tier-identity-assets.js';
const MATCHING_RELIC = {
    arkan: 'ember-crown',
    seria: 'winter-heart',
    kain: 'storm-core',
    edric: 'oath-seal',
...
export function auditRelicResonanceProjectionIdentityAssets() {
    ...
    const push = (caseId, heroId, impactId, passed) => {
    ...
    for (const heroId of HEROES) {
        ...
        const scenarios = [
            { id: 'up', from: 'abyss-eye', to: matching, impact: 'tier-up', beforeTier: 0, afterTier: 1, delta: 1 },
            { id: 'steady', from: 'abyss-eye', to: 'chrono-shard', impact: 'steady', beforeTier: 0, afterTier: 0, delta: 0 },
            { id: 'down', from: matching, to: 'abyss-eye', impact: 'tier-down', beforeTier: 1, afterTier: 0, delta: -1 },
        ];
    ...
    if (!impactAtlas.passed)
        issues.push('impact-atlas');
    if (!tierAtlas.passed)
        issues.push('tier-atlas');
    if (!formulaStable)
        issues.push('resonance-formula');
    if (gameplayMutation)
