...
export type SpecialistLocomotionEvent='blink'|'brace'|'plant'|null;
export interface SpecialistLocomotionSignatureState { arrival:number; brace:number; plant:number; }
export interface SpecialistLocomotionSignaturePresentation {
  ...
  scaleX:number;
  scaleY:number;
  groundPulseAlpha:number;
  groundPulseRadius:number;
}
const clamp=(v:number,min:number,max:number)=>Math.max(min,Math.min(max,v));

export function advanceSpecialistLocomotionSignatureState(previous:SpecialistLocomotionSignatureState|undefined,type:SpecialistEnemyType,event:SpecialistLocomotionEvent,dt:number):SpecialistLocomotionSignatureState{
  ...
  if(event==='blink'&&type==='assassin')arrival=1;
  if(event==='brace'&&type==='shieldbearer')brace=1;
  if(event==='plant'&&type==='siegeGolem')plant=1;
  return{arrival,brace,plant};
}

export function specialistLocomotionSignaturePresentation(type:SpecialistEnemyType,state:SpecialistLocomotionSignatureState|undefined,motionBlend:number,recovery:number,facingX:number,facingY:number,reducedMotion=false):SpecialistLocomotionSignaturePresentation{
