...
export function logicalPointerPosition(clientX:number,clientY:number,rect:RectLike):Vec2{
...
export interface InputLifecycleResilienceAudit{profiles:number;actionCount:number;multitouchIsolation:boolean;transientResetCoverage:number;zeroRectSafe:boolean;hingeClear:boolean;finiteMappingCoverage:number;passed:boolean;}
export function auditInputLifecycleResilience():InputLifecycleResilienceAudit{
  const profiles:[number,number][]=[[1600,900],[2400,1080],[2208,1840],[0,0]];
  ...
  const joystickPointer=1;const actionPointers=new Map<number,string>([[2,'spell1'],[3,'ultimate1']]);
  const multitouchIsolation=![...actionPointers.keys()].includes(joystickPointer)&&new Set(actionPointers.keys()).size===2;
  const transientModel={joystickPointer:null as number|null,actionPointers:new Map<number,string>(),held:new Set<string>(),pressed:new Set<string>(),keys:new Set<string>(),move:{x:0,y:0}};
