...
import { BOSS_ASSIST_CUE_MEMORY_SECONDS, bossActionAssist, bossResponseActions, type BossActionAssistCue } from './boss-action-assist.js';

export interface BossAssistStabilityAuditSample {
...
export interface BossAssistStabilityAudit {
  passed: boolean;
  archetypeCount: number;
  samples: BossAssistStabilityAuditSample[];
  baselineSwitchCount: number;
...
const add = (samples: BossAssistStabilityAuditSample[], archetype: BossArchetype|'global', caseId:string, expected:BossAssistStabilityAuditSample['expected'], actual:BossAssistStabilityAuditSample['actual']) => {
  samples.push({ archetype, caseId, expected, actual, passed: actual === expected });
};

export function auditBossAssistStability(): BossAssistStabilityAudit {
  const samples: BossAssistStabilityAuditSample[] = [];
  ...
  for (const archetype of ARCHETYPES) {
    ...
    const ready = new Set<ActionId>([mapped[0]!, mapped[2]!]);
    ...
    if (baseline?.actionId !== previous.actionId) baselineSwitchCount++;
    if (stable?.actionId !== previous.actionId) stableSwitchCount++;
    responseExpected++;
    if (stable) responseHit++;
    add(samples, archetype, 'stable-ready-change', previous.actionId, stable?.actionId ?? null);

    const rescue = bossActionAssist({ archetype, specialTimer:.4, hpRatio:.25, potions:1, readyActions:new Set<ActionId>(['potion', previous.actionId]), previousCue:previous, previousCueAge:.1, previousArchetype:archetype });
    potionExpected++;
    if (rescue?.actionId === 'potion') potionHit++;
    add(samples, archetype, 'urgent-potion', 'potion', rescue?.actionId ?? null);

    const outside = bossActionAssist({ archetype, specialTimer:1.06, hpRatio:.8, potions:0, readyActions:new Set<ActionId>([previous.actionId]), previousCue:previous, previousCueAge:.1, previousArchetype:archetype });
    windowExpected++;
    if (outside === null) windowHit++;
    add(samples, archetype, 'window-exit', null, outside?.actionId ?? null);
  }

  const sampleArchetype: BossArchetype = 'summoner';
  const mapped = bossResponseActions(sampleArchetype);
  const previous = cueFor(mapped[2]!);
  const expired = bossActionAssist({ archetype:sampleArchetype, specialTimer:.5, hpRatio:.8, potions:0, readyActions:new Set<ActionId>([mapped[0]!, mapped[2]!]), previousCue:previous, previousCueAge:BOSS_ASSIST_CUE_MEMORY_SECONDS+.01, previousArchetype:sampleArchetype });
  responseExpected++; if (expired) responseHit++;
  add(samples,'global','memory-expiry',mapped[0]!,expired?.actionId??null);

  const unavailable = bossActionAssist({ archetype:sampleArchetype, specialTimer:.5, hpRatio:.8, potions:0, readyActions:new Set<ActionId>([mapped[0]!]), previousCue:previous, previousCueAge:.1, previousArchetype:sampleArchetype });
  responseExpected++; if (unavailable) responseHit++;
  add(samples,'global','previous-unavailable',mapped[0]!,unavailable?.actionId??null);

  const changed = bossActionAssist({ archetype:'juggernaut', specialTimer:.5, hpRatio:.8, potions:0, readyActions:new Set<ActionId>(['spell3','spell2']), previousCue:cueFor('spell2'), previousCueAge:.1, previousArchetype:'summoner' });
  responseExpected++; if (changed) responseHit++;
  ...
  if(!(stableSwitchCount<baselineSwitchCount))issues.push('switch-reduction');
  if(responseCoverage!==1)issues.push('response-coverage');
