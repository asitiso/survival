...
import {
  SPECIALIST_COMBAT_CONTRACT,
  assassinBlinkPosition,
  nullifierCooldownMultiplier,
  selectSpecialistEnemyType,
  specialistTarget,
} from './enemy-specialists.js';
import {
  SPECIALIST_INTENT_TYPES,
  auditSpecialistIntentAtlas,
  isSpecialistIntentType,
  specialistIntentEmphasis,
  specialistIntentIcon,
  specialistIntentOnBodyLayout,
  type SpecialistIntentType,
} from './specialist-intent-identity-assets.js';

export interface SpecialistIntentIdentityAssetSample {
  caseId:string;
  type?:SpecialistIntentType;
  surface:'body'|'auto'|'other';
  passed:boolean;
}

export interface SpecialistIntentIdentityAssetAudit {
  samples:SpecialistIntentIdentityAssetSample[];
  specialistCount:number;
  coverage:number;
  uniqueCellCount:number;
  outOfBounds:SpecialistIntentType[];
  onBodyCoverage:number;
  autoTargetCoverage:number;
  activeStateAccuracy:number;
  edgeClampCoverage:number;
  overlapPolicyViolations:number;
...
const EXPECTED_GEOMETRY:Readonly<Record<SpecialistIntentType,number>>={
  bomber:17,shaman:21,shieldbearer:23,assassin:16,siegeGolem:31,nullifier:22,
};

const same=(a:unknown,b:unknown):boolean=>JSON.stringify(a)===JSON.stringify(b);

function target(type:SpecialistIntentType,targetKind:'hero'|'core'='hero'):SpellTargetCandidate{
  return{id:7,type,pos:{x:300,y:300},target:targetKind,hp:50,maxHp:100,alive:true};
}

export function auditSpecialistIntentIdentityAssets():SpecialistIntentIdentityAssetAudit{
  const atlas=auditSpecialistIntentAtlas();
  const samples:SpecialistIntentIdentityAssetSample[]=[];
  const push=(caseId:string,passed:boolean,surface:SpecialistIntentIdentityAssetSample['surface']='other',type?:SpecialistIntentType):void=>{samples.push({caseId,passed,surface,...(type?{type}:{})});};
  const bodyTypes=new Set<SpecialistIntentType>();
  const autoTypes=new Set<SpecialistIntentType>();
  const activeTypes=new Set<SpecialistIntentType>();
  ...
  for(let index=0;index<SPECIALIST_INTENT_TYPES.length;index+=1){
    ...
    const base={guardHp:0,specialistTimer:99,target:'hero' as const,heroInsideNullifier:false};
    ...
    if(active)activeTypes.add(type);
  ...
  if(onBodyCoverage!==1)issues.push('on-body-coverage');
  if(autoTargetCoverage!==1)issues.push('auto-target-coverage');
  if(activeStateAccuracy!==1)issues.push('active-state-accuracy');
  if(edgeClampCoverage!==1)issues.push('edge-clamp');
  if(overlapPolicyViolations!==0)issues.push('overlap-policy');
  ...
  if(!specialistGameplayStable)issues.push('specialist-gameplay');
  if(!autoContractStable)issues.push('auto-target-contract');
  if(geometryMutation)issues.push('enemy-geometry');
