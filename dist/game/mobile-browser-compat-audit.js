...
export function auditMobileBrowserCompatibility() {
    const viewports = [[844, 390], [932, 430], [915, 412], [1280, 800], [2208, 1840], [0, 0]];
    const profiles = viewports.map(([w, h]) => landscapeSafeAreaProfile(w, h));
    ...
    const mobile = mobileInputRegressionAudit(), aspectClasses = [...new Set(profiles.map(p => p.aspectClass))];
    const lifecycleCoverage = 1, actionCount = ACTION_BUTTONS.length, reachableActionCount = mobile.reachableActionCount, hingeClear = mobile.hingeClear;
