...
import { heroAscensionCatalog, heroAscensionModifiers, advanceHeroAscension, createDefaultHeroAscensionState, selectHeroAscension, type HeroAscensionId } from './endless/hero-ascension.js';
...
export interface HeroAscensionProjectionIdentityAuditSample{id:string;passed:boolean;}
export interface HeroAscensionProjectionIdentityAudit{
  samples:HeroAscensionProjectionIdentityAuditSample[];
  modifierIdentityCount:number;directionIdentityCount:number;modifierCoverage:number;directionCoverage:number;modifierUniqueCellCount:number;directionUniqueCellCount:number;
  milestones:readonly[35,50,65];maxSelections:3;actionCount:number;snapshotSchemaMutation:false;gameplayMutation:false;issues:string[];passed:boolean;
}

const HEROES:readonly HeroId[]=['arkan','seria','kain','edric'];
const eq=(a:number,b:number)=>Math.abs(a-b)<1e-9;
const modifierEqual=(a:ReturnType<typeof heroAscensionModifiers>,b:ReturnType<typeof heroAscensionModifiers>)=>Object.keys(a).every(key=>eq(a[key as keyof typeof a],b[key as keyof typeof b]));

export function auditHeroAscensionProjectionIdentityAssets():HeroAscensionProjectionIdentityAudit{
  const samples:HeroAscensionProjectionIdentityAuditSample[]=[];
  ...
  const m35=advanceHeroAscension('arkan',35*60_000,state);state=m35.state;
  const p35=state.pendingOffer?.options[0]?.optionId; if(p35)state=selectHeroAscension(state,p35);
  const m50=advanceHeroAscension('arkan',50*60_000,state);state=m50.state;
  const p50=state.pendingOffer?.options[0]?.optionId; if(p50)state=selectHeroAscension(state,p50);
  const m65=advanceHeroAscension('arkan',65*60_000,state);state=m65.state;
  const p65=state.pendingOffer?.options[0]?.optionId; if(p65)state=selectHeroAscension(state,p65);
  ...
  if(!modifierAtlas.passed)issues.push('modifier-atlas');
  if(!directionAtlas.passed)issues.push('direction-atlas');
  if(HERO_ASCENSION_MODIFIER_IDENTITY_IDS.length!==8)issues.push('modifier-identity-contract');
  if(HERO_ASCENSION_BUILD_DIRECTION_IDS.length!==3)issues.push('direction-identity-contract');
