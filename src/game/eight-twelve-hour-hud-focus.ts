export interface EightTwelveHourHudFocusInput {
  elapsedSeconds:number;
  completeBuild:boolean;
  bossActive:boolean;
  mythicActive:boolean;
  finalFormActive:boolean;
  heroCritical:boolean;
  coreCritical:boolean;
}
export interface EightTwelveHourHudFocusPolicy {
  active:boolean;
  routineBuildLabelCap:0|2|4;
  statusMaxChars:number;
  showRoutineAutoText:boolean;
  preserveFinalForm:boolean;
  preserveMythic:boolean;
  preserveCritical:boolean;
  preserveBossDanger:boolean;
  maxProjectileCues:number;
  keepCriticalBars:true;
  keepDangerTelegraphs:true;
}
export function eightTwelveHourHudFocus(input:EightTwelveHourHudFocusInput):EightTwelveHourHudFocusPolicy {
  const elapsed=Number.isFinite(input.elapsedSeconds)?Math.max(0,input.elapsedSeconds):0;
  if(elapsed<8*3600)return{active:false,routineBuildLabelCap:4,statusMaxChars:64,showRoutineAutoText:true,preserveFinalForm:true,preserveMythic:true,preserveCritical:true,preserveBossDanger:true,maxProjectileCues:6,keepCriticalBars:true,keepDangerTelegraphs:true};
...
export function eightTwelveHourBuildLabels(labels:readonly string[],policy:EightTwelveHourHudFocusPolicy):string[] {
