...
export function projectNemesisAdaptationEffect(adaptation) {
...
export function projectNemesisAdaptationEffects(adaptations) {
...
export function nemesisAdaptationEffectHint(projection, limit = 2) {
    return projection.primaryEffects.slice(0, Math.max(0, Math.min(2, limit))).map(effect => effect.label).join(' · ');
}
export function nemesisAdaptationLearningToastLabel(count, projection) {
