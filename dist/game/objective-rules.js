...
export function objectiveAnchors(mapId, stage) {
...
export function chooseObjectiveAnchor(mapId, stage, hero) {
...
export function advanceRiftSeal(state, dt, heroInside, nearbyEnemies) {
...
export function advanceBeaconDefense(state, dt, nearbyEnemies) {
    const safeDt = Math.max(0, dt);
    const enemies = Math.max(0, Math.floor(nearbyEnemies));
...
export function advanceCursedAltar(state, dt) {
