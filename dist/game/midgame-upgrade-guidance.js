...
const NORMAL_SPELLS = new Set(['fireBolt', 'chainLightning', 'frostNova', 'flameField']);
function clampElapsed(value) { return Number.isFinite(value) ? Math.max(0, value) : 0; }
function nearestFusionTarget(context) {
    ...
    const spellId = (missingA >= missingB ? a : b);
    return { spellId, fusionName: best.def.name };
}
function choiceScore(choice, target) {
    if (target && choice.id === target.spellId)
        return /최종 진화/.test(choice.description) ? 118 : 108;
    if (NORMAL_SPELLS.has(choice.id) && /최종 진화/.test(choice.description))
        return 104;
    if (NORMAL_SPELLS.has(choice.id) && /1차 진화/.test(choice.description))
        return 92;
    if (choice.id === 'spellPower')
        return 78;
    if (choice.id === 'cooldown')
        return 75;
    if (NORMAL_SPELLS.has(choice.id))
        return 68;
    if (choice.id === 'maxHp')
        return 52;
...
function choiceHint(choice, target, context) {
    if (target && choice.id === target.spellId) {
    ...
    if (/최종 진화/.test(choice.description))
        return '최종 진화 · 빌드 핵심 완성';
    if (/1차 진화/.test(choice.description))
        return '1차 진화 · 주력 성장 연결';
    if (choice.id === 'spellPower')
        return '전체 화력 · 빌드 공백 보완';
    if (choice.id === 'cooldown')
        return '연사 주기 · 빌드 공백 보완';
    return '현재 빌드 성장 연결';
}
export function guideMidgameUpgradeChoices(choices, context) {
    ...
    choices.forEach((choice, index) => { const score = choiceScore(choice, target); if (score > bestScore) {
        bestScore = score;
        bestIndex = index;
    } });
