...
import { targetGuidanceMotionPolicy } from './combat-cue-priority.js';
const add = (samples, caseId, expected, actual) => samples.push({ caseId, expected, actual, passed: expected === actual });
const motion = (overrides) => targetGuidanceMotionPolicy({ combatPrimary: 'normal', reducedFlash: false, hasWeakpoint: true, hasAutoTarget: true, ...overrides });
export function auditTargetGuidanceAttention() {
    const samples = [];
    const dual = motion({});
    ...
    const criticalPrimaries = ['hero-critical', 'core-critical', 'damage-critical', 'boss-response', 'damage-heavy', 'boss-countdown'];
    ...
    const maxAnimatedOwners = Math.max(Number(dual.weakpointAnimated) + Number(dual.autoTargetAnimated), Number(autoOnly.weakpointAnimated) + Number(autoOnly.autoTargetAnimated), Number(weakOnly.weakpointAnimated) + Number(weakOnly.autoTargetAnimated), ...criticalPrimaries.map((primary) => { const p = motion({ combatPrimary: primary }); return Number(p.weakpointAnimated) + Number(p.autoTargetAnimated); }));
    ...
    if (maxAnimatedOwners > 1)
        issues.push('multiple-target-guidance-motion');
    if (criticalMotionAmplitude !== 0)
        issues.push('critical-target-guidance-motion');
    if (reducedFlashMotionAmplitude !== 0)
        issues.push('reduced-flash-target-guidance-motion');
    if (duplicateMotionCount !== 0)
        issues.push('duplicate-target-guidance-motion');
    if (staleMotionReplayCount !== 0)
        issues.push('stale-target-guidance-motion');
    if (targetVisibilityRate !== 1)
        issues.push('target-guidance-hidden');
