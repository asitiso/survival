...
const presentationControls = shell.querySelector('.presentation-controls');
let syncPresentationSettingsLayout = () => { };
if (presentationControls?.parentElement) {
    ...
    syncInstallAffordance();
    const isPhoneLandscapeSettings = () => window.matchMedia('(orientation: landscape) and (max-height: 520px) and (max-width: 1024px)').matches;
    let expanded = !isPhoneLandscapeSettings();
    let userExpandedOverride = null;
    ...
    syncPresentationSettingsLayout = () => {
        ...
        for (const button of presentationControls.querySelectorAll('button')) {
        ...
        if (userExpandedOverride === null)
            expanded = !phoneLandscape;
        syncExpandedState();
    };
...
gameHud.drawHud = (ctx) => {
    ...
    if (hudCanvas.width !== layout.logicalWidth)
        hudCanvas.width = layout.logicalWidth;
    if (hudCanvas.height !== layout.logicalHeight)
        hudCanvas.height = layout.logicalHeight;
