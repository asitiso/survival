...
export function runBossSafeResponseRelativeFollowAudit() {
    const samples = [];
    let passed = BOSS_SAFE_RESPONSE_SLOT_BOSS_DISPLACEMENT_GUARD >= 64;
    for (let i = 0; i < 64; i++) {
        ...
        const moved = { x: boss.x + 32, y: boss.y + 12 };
        r = bossSafeResponseSlotHysteresis({ previous: r.memory, current: placement('right', moved.x + 140, moved.y), placementInput: { ...base, bossPos: moved }, bossId: 40 + i, cycle: 3, now: 5.04 });
        const followed = r.placement.slot === 'above' && r.placement.pos.x === boss.x + 32 && r.placement.pos.y === boss.y - 88 && r.memory?.bossPos.x === boss.x;
        const ok = followed && r.presentationOnly;
        passed &&= ok;
        samples.push(`${i}:${followed ? 1 : 0}:${r.placement.pos.x.toFixed(1)}:${r.placement.pos.y.toFixed(1)}`);
    }
