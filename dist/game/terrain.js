...
import { MAP_LAYOUTS, selectMapLayout } from './map-layouts.js';
import { evolveMapLayout, mapEvolutionStage } from './map-evolution.js';
import { battlefieldTerrainMaterial } from './battlefield-environment-assets.js';
export function resolveCircleVsRect(pos, radius, rect) {
...
export class TerrainSystem {
    ...
    resolveHero(hero) {
        for (const wall of this.walls)
            hero.pos = resolveCircleVsRect(hero.pos, hero.radius, wall);
    }
    drainPresentationEvents() { const out = this.presentationEvents; this.presentationEvents = []; return out; }
    hitByMagic(pos, strength) {
        for (const crystal of this.crystals) {
            if (crystal.cooldown > 0 || distance(pos, crystal) > 100)
                continue;
            crystal.charge += strength;
            if (crystal.charge >= crystal.threshold)
                crystal.blastPending = true;
        }
    }
    get hasActiveCrystal() { return this.crystals.some((crystal) => crystal.cooldown <= 0); }
    update(dt, enemies) {
        ...
        for (const crystal of this.crystals) {
            ...
            for (const enemy of enemies.enemies) {
                if (enemy.alive && distance(enemy.pos, crystal) <= crystal.blastRadius + enemy.radius)
                    enemies.damage(enemy, crystal.blastDamage, crystal, 'explosion');
            }
        }
    }
    render(ctx, motion, readability) {
