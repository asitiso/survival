export interface BossDifficultyCurveProfile{
  healthMultiplier:number;
  damageMultiplier:number;
  rewardMultiplier:number;
  initialSpecialTimerMultiplier:number;
}
export interface BossDifficultyCurveAudit{
  profiles:BossDifficultyCurveProfile[];
  threatFiveProfiles:BossDifficultyCurveProfile[];
  monotonic:boolean;
  bounded:boolean;
  lateNeutral:boolean;
  passed:boolean;
...
const NEUTRAL:BossDifficultyCurveProfile={healthMultiplier:1,damageMultiplier:1,rewardMultiplier:1,initialSpecialTimerMultiplier:1};
const BASE:readonly BossDifficultyCurveProfile[]=[
...
function towardNeutral(value:number,factor:number):number{return round(1+(value-1)*factor);}

export function bossDifficultyCurve(bossOrdinal:number,_elapsedSeconds:number,threat:number):BossDifficultyCurveProfile{
  const ordinal=Math.max(0,Math.floor(Number.isFinite(bossOrdinal)?bossOrdinal:0));
  if(ordinal>=BASE.length)return{...NEUTRAL};
...
export function auditBossDifficultyCurve():BossDifficultyCurveAudit{
