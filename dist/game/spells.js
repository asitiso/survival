...
export function flameFieldVisualAlpha(ttl, maxTtl) {
...
export function persistentZoneExpirySeconds(kind) {
...
export class SpellSystem {
    ...
    renderPersistentReadabilityLayer(ctx, heroSpellSignatureAtlasImage, heroSpellSignatureAtlasReady = false, ultimateSignatureAtlasImage, ultimateSignatureAtlasReady = false, crowdControlPropagationVfxAtlasImage, crowdControlPropagationVfxAtlasReady = false, reducedFlash = false) {
                ...
                if (p.splashRadius > 0 && p.splashDamage > 0) {
                    for (const nearby of world.enemies.enemies) {
                        ...
                        if (distance(enemy.pos, nearby.pos) <= p.splashRadius + nearby.radius) {
                            ...
                            const splashKilled = world.enemies.damage(nearby, magicCriticalDamage(p.splashDamage, world.hero), enemy.pos, 'explosion');
    ...
    updateFields(dt, world) {
        ...
        for (const field of this.fields)
            if (field.ttl <= 0) {
                const maxTtl = persistentZoneExpirySeconds('flameField');
    ...
    updateBlackHoles(dt, world) {
        ...
        for (const hole of this.holes)
            if (hole.ttl <= 0) {
                const maxTtl = persistentZoneExpirySeconds('blackHole');
