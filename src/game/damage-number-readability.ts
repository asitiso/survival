export type DamageNumberTier='normal'|'heavy'|'critical';

export interface DamageNumberPresentationInput{
  tier:DamageNumberTier;
  battlefieldStress?:number;
  clusterIndex?:number;
  sameTargetIndex?:number;
  denseNeighborIndex?:number;
  anchorX?:number;
  anchorY?:number;
  viewportWidth?:number;
  viewportHeight?:number;
  resultPriorityNearby?:number;
  resultDistance?:number;
  resultVectorX?:number;
  resultVectorY?:number;
  sourceVectorX?:number;
  sourceVectorY?:number;
...
export function damageNumberPresentation(input:DamageNumberPresentationInput):DamageNumberPresentation{
  ...
  const avoidanceDirection=():{x:number;y:number}=>{
    const resultLength=Math.hypot(resultVectorX,resultVectorY);
    if(resultLength>1)return{x:resultVectorX/resultLength,y:resultVectorY/resultLength};
    const sourceLength=Math.hypot(sourceVectorX,sourceVectorY);
    if(sourceLength>1)return{x:sourceVectorX/sourceLength,y:sourceVectorY/sourceLength};
    return{x:0,y:-1};
  };
  const resultAvoidance=(magnitude:number):{x:number;y:number}=>{
    if(!resultOwnsCenter)return{x:0,y:0};
    const direction=avoidanceDirection();
    return{x:direction.x*magnitude*motionScale,y:direction.y*magnitude*motionScale};
  };
  const packingOffset=(step:number):number=>{
    if(sameTargetIndex===0)return 0;
    const ring=Math.ceil(sameTargetIndex/2);
    const sign=sameTargetIndex%2===1?1:-1;
    return sign*ring*step*motionScale;
  };
  const denseOffset=(step:number):number=>{
    if(denseNeighborIndex===0)return 0;
    const ring=Math.ceil(denseNeighborIndex/2);
    const sign=denseNeighborIndex%2===1?-1:1;
    return sign*ring*step*motionScale;
  };
  const clampToViewport=(offsetX:number,offsetY:number):{x:number;y:number}=>{
    ...
    if(!Number.isFinite(anchorX)||!Number.isFinite(anchorY)||width<=36||height<=40)return{x:offsetX,y:offsetY};
