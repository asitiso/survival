...
export class Game {
  ...
  private survivalResponseVfx: Array<{kind:SurvivalResponseVfxKind;x:number;y:number;ttl:number;maxTtl:number;worldGuardOwned?:boolean}> = [];
  ...
  private enemyFinisherVfx: Array<{source:EnemyDeathVisualSource;x:number;y:number;enemyType:EnemyType;ttl:number;maxTtl:number}> = [];
  ...
  private update(dt: number): void {
    ...
    this.enemies.update(dt, {
      ...
      onCoreDamage: (amount, source = 'contact') => {
        ...
        if (applied > 0) { this.frameEndlessEvents.push({ type: 'core_damaged', amount: applied }); this.queueSurvivalResponseVfx('coreHit'); }
        const prevented=Math.max(0,amount-applied);
  ...
  private handleSuccessfulCast(action: import('./config.js').ActionId, source: 'manual' | 'auto' = 'manual'): void {
    ...
    if (action === 'ultimate1' || action === 'ultimate2') this.heroUltimateBodyState = advanceHeroUltimateBodyState(this.heroUltimateBodyState, action === 'ultimate1' ? 'meteorStorm' : 'blackHole', 0, this.presentationSettings.reducedMotion);
  ...
  private drawEnemyDefeatBodyTransitions(ctx:CanvasRenderingContext2D):void{
    ...
    for(const cue of this.enemyDefeatBodyTransitions){
      ...
      ctx.save();ctx.translate(cue.death.x+body.offsetX,cue.death.y+body.offsetY);ctx.rotate(body.rotation);ctx.scale(body.scaleX,body.scaleY);ctx.globalAlpha=body.alpha*(this.presentationSettings.reducedFlash?.78:.92);
  ...
  private queueEnemyFinisherVfx(source:EnemyDeathVisualSource,x:number,y:number,enemyType:EnemyType):void{
    const maxTtl=source==='ultimate'||source==='finalForm'||source==='fusion'?.66:.52;this.enemyFinisherVfx.push({source,x,y,enemyType,ttl:maxTtl,maxTtl});if(this.enemyFinisherVfx.length>24)this.enemyFinisherVfx.splice(0,this.enemyFinisherVfx.length-24);
  }
  ...
  private drawBossHazardAftermathVfx(ctx:CanvasRenderingContext2D):void{
    ...
    for(const cue of this.bossHazardAftermathVfx){
      ...
      const progress=1-Math.max(0,cue.ttl/cue.maxTtl);const state=retirement.owner==='terrain'?'residual':progress<0.32?'detonate':'residual';const sprite=bossHazardAftermathVfxSprite(cue.kind,state);const size=Math.max(112,cue.radius*2.55)*(state==='detonate'?1+progress*.28:1.08+progress*.16)*retirement.sizeScale,cueAlpha=this.worldVfxCueAlpha('tactical',cue.x,cue.y,size*.5);if(cueAlpha<=0||retirement.owner==='retired')continue;const ownerAlpha=retirement.owner==='terrain'?retirement.terrainAlphaScale:retirement.aftermathAlphaScale;ctx.save();ctx.globalAlpha=Math.min(this.presentationSettings.reducedFlash?0.36:0.64,(1-progress)*.76+.06)*cueAlpha*lifecycle.aftermathAlphaScale*ownerAlpha;ctx.drawImage(this.bossHazardAftermathVfxAtlasImage,sprite.sx,sprite.sy,sprite.sw,sprite.sh,cue.x-size/2,cue.y-size/2,size,size);ctx.restore();
    }
  }
  ...
  private drawBossHazardClearedGroundMemory(ctx:CanvasRenderingContext2D):void{
    for(const cue of this.bossHazardClearedGroundMemory){
      ...
      if(memory.owner!=='cleared'||memory.clearedAlpha<=0)continue;
      const radius=cue.radius*memory.radiusScale,cueAlpha=this.worldVfxCueAlpha('informational',cue.x,cue.y,radius);if(cueAlpha<=0)continue;
      ...
      ctx.save();ctx.translate(cue.x,cue.y);ctx.rotate(geometry.angle);ctx.globalAlpha=geometry.alpha*cueAlpha*clearedAlphaScale;ctx.strokeStyle='#8fffd3';ctx.lineWidth=1.5;ctx.setLineDash([5,9]);ctx.beginPath();if(geometry.shape==='circle')ctx.arc(0,0,geometry.radius,0,Math.PI*2);else if(geometry.shape==='corridor')ctx.rect(-geometry.halfLength,-geometry.halfWidth,geometry.halfLength*2,geometry.halfWidth*2);else{ctx.moveTo(-geometry.halfLength,0);ctx.lineTo(geometry.halfLength,0);ctx.moveTo(0,-geometry.halfLength);ctx.lineTo(0,geometry.halfLength);}ctx.stroke();ctx.setLineDash([]);ctx.restore();
    }
  }

  private drawEnemyFinisherVfx(ctx:CanvasRenderingContext2D):void{
    if(!this.enemyFinisherVfxAtlasReady||!this.enemyFinisherVfxAtlasImage)return;for(const cue of this.enemyFinisherVfx){const progress=1-Math.max(0,cue.ttl/cue.maxTtl);const state=progress<0.38?'burst':'afterglow';const sprite=enemyFinisherVfxSprite(cue.source,state);const eliteScale=cue.enemyType==='boss'?1.55:cue.enemyType==='elite'?1.28:1;const size=(state==='burst'?92:108)*eliteScale*(1+progress*.18);ctx.save();ctx.globalAlpha=Math.min(this.presentationSettings.reducedFlash?0.42:0.72,(1-progress)*.82+.05);ctx.drawImage(this.enemyFinisherVfxAtlasImage,sprite.sx,sprite.sy,sprite.sw,sprite.sh,cue.x-size/2,cue.y-size/2,size,size);ctx.restore();}
  }
  ...
  private queueSurvivalResponseVfx(kind:SurvivalResponseVfxKind):void{
  ...
  private drawSurvivalResponseVfx(ctx:CanvasRenderingContext2D):void{
    ...
    for(const cue of this.survivalResponseVfx){
      ...
      ctx.drawImage(this.survivalResponseVfxAtlasImage,sprite.sx,sprite.sy,sprite.sw,sprite.sh,cue.x-size/2,cue.y-size/2,size,size);ctx.restore();
    }
  }
  ...
  private drawBossArenaHazards(ctx: CanvasRenderingContext2D, motion: SecondaryCombatMotionPolicy): void {
    ...
    if (safeLane) {
      ctx.save(); ctx.globalAlpha = .26 + safeLane.confidence * .18; ctx.strokeStyle = '#8fffd3'; ctx.fillStyle = '#8fffd3'; ctx.lineWidth = 2; ctx.setLineDash([7,7]);
      ctx.beginPath(); ctx.moveTo(this.hero.pos.x, this.hero.pos.y); ctx.lineTo(safeLane.target.x, safeLane.target.y); ctx.stroke(); ctx.setLineDash([]);
      ctx.beginPath(); ctx.arc(safeLane.target.x, safeLane.target.y, 14, 0, Math.PI*2); ctx.stroke();
      this.drawMapSafeLaneTransitionVfx(ctx,safeLane);
      if(forecast&&forecast.urgency>=.65){
        ctx.globalAlpha=.18+.22*forecast.urgency;ctx.strokeStyle='#7fd9ff';ctx.setLineDash([4,8]);
        ...
        if(!this.dangerState.heroCritical&&!this.dangerState.coreCritical&&!lawIdentity?.active){const direction=safeZoneTransitionDirectionFromVector(forecast.nextTarget.x-forecast.currentTarget.x,forecast.nextTarget.y-forecast.currentTarget.y);this.drawSafeZoneTransitionDirectionIcon(ctx,direction,forecast.nextTarget.x+16,forecast.nextTarget.y-34,20);}
      }
      ctx.globalAlpha=.78;ctx.fillStyle='#8fffd3';ctx.font='800 10px system-ui'; ctx.textAlign='center';
      ...
      if(lawTimeline?.lawStage==='active'&&lawIdentity&&lawIdentity.lawId!=='none')this.drawMythicLastLawSafeLaneIcon(ctx,safeLane.target.x-12,safeLane.target.y-54,24,lawIdentity.lawId);
      if(boss?.isMythic)this.drawMythicArenaGeometrySafeLaneIcon(ctx,boss,destroyedRatio,safeLane.target.x+18,safeLane.target.y-54,24,Boolean(lawIdentity?.active));
      ctx.fillText(`${safeLane.label}${forecastText}${timelineText}`, safeLane.target.x, safeLane.target.y-20);
      if(lawTimeline){const barW=72,barX=safeLane.target.x-barW/2,barY=safeLane.target.y-14;ctx.globalAlpha=.34;ctx.fillStyle='#172b34';ctx.fillRect(barX,barY,barW,4);ctx.globalAlpha=.86;ctx.fillStyle=lawTimeline.accent;ctx.fillRect(barX,barY,barW*Math.max(.06,1-lawTimeline.urgency*.72),4);}
      ctx.restore();
    }
    ...
    for (const hazard of this.bossArena.hazards) {
      ...
      const hazardHandoff=bossHazardTelegraphHandoffPresentation({telegraph:hazard.telegraph,launchTtl:hazard.launchTtl,launchMaxTtl:hazard.launchMaxTtl},this.presentationSettings.reducedFlash);const hazardLifecycle=bossHazardLifecycleOwnerPresentation({telegraph:hazard.telegraph,ttl:hazard.ttl,aftermathTtl:0,aftermathMaxTtl:0},this.presentationSettings.reducedFlash);
      const nearbyClearedMemory=this.bossHazardClearedGroundMemory.filter((cue)=>cue.ttl>0).reduce<(typeof this.bossHazardClearedGroundMemory)[number]|null>((best,cue)=>{const d=Math.hypot(cue.x-hazard.pos.x,cue.y-hazard.pos.y);return !best||d<Math.hypot(best.x-hazard.pos.x,best.y-hazard.pos.y)?cue:best;},null);const clearedHandoff=nearbyClearedMemory?bossHazardClearedGroundMemoryPresentation({memoryTtl:nearbyClearedMemory.ttl,memoryMaxTtl:nearbyClearedMemory.maxTtl,aftermathActive:this.bossHazardAftermathVfx.some((after)=>Math.hypot(after.x-nearbyClearedMemory.x,after.y-nearbyClearedMemory.y)<=Math.max(24,nearbyClearedMemory.radius*.35)&&after.ttl>0),nextHazardDistance:Math.hypot(nearbyClearedMemory.x-hazard.pos.x,nearbyClearedMemory.y-hazard.pos.y),nextHazardTelegraph:hazard.telegraph},this.presentationSettings.reducedFlash):null;const telegraphAlphaScale=clearedHandoff?.telegraphAlphaScale??1;
      if(hazard.launchOrigin&&hazardHandoff.launchCueAlpha>0){ctx.save();ctx.globalAlpha=hazardHandoff.launchCueAlpha;ctx.strokeStyle=color;ctx.lineWidth=2;ctx.setLineDash([6,9]);ctx.beginPath();ctx.moveTo(hazard.launchOrigin.x,hazard.launchOrigin.y);ctx.lineTo(hazard.pos.x,hazard.pos.y);ctx.stroke();ctx.setLineDash([]);ctx.restore();}
      ...
      if (shape === 'corridor' || shape === 'cross') {
        ...
        if (hazard.telegraph <= 0) ctx.fill(); ctx.stroke();
        if (shape === 'cross') {
        ...
        if (hazard.telegraph <= 0) ctx.fill('evenodd'); ctx.stroke();
      } else if (shape === 'orbit') {
        ctx.save(); ctx.translate(hazard.pos.x, hazard.pos.y); ctx.rotate(hazard.angle ?? 0); ctx.scale(1.45, .72);
        ctx.beginPath(); ctx.arc(0, 0, hazard.radius, 0, Math.PI * 2); if (hazard.telegraph <= 0) ctx.fill(); ctx.stroke(); ctx.restore();
      } else if (shape === 'clock') {
        ctx.translate(hazard.pos.x, hazard.pos.y); ctx.rotate(hazard.angle ?? 0);
        ctx.beginPath(); ctx.moveTo(0, 0); ctx.arc(0, 0, hazard.radius * 1.25, -.2, .2); ctx.closePath(); if (hazard.telegraph <= 0) ctx.fill(); ctx.stroke();
      } else {
  ...
  private drawMapSafeLaneTransitionVfx(ctx:CanvasRenderingContext2D,safeLane:NonNullable<ReturnType<typeof mythicSafeLaneHint>>):void{
    ...
    const transitionAlpha=this.presentationSettings.reducedFlash?0.24:0.44;
      ...
      const arrivalSize=86;ctx.save();ctx.globalAlpha=transitionAlpha+.08;ctx.drawImage(this.mapSafeLaneTransitionVfxAtlasImage,arrivalSprite.sx,arrivalSprite.sy,arrivalSprite.sw,arrivalSprite.sh,safeLane.target.x-arrivalSize/2,safeLane.target.y-arrivalSize/2,arrivalSize,arrivalSize);ctx.restore();
    }
  }
