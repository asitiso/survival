export interface TacticalRecap {
  objectivesCompleted:number;
  objectivesFailed:number;
  bestObjectiveStreak:number;
  bossNodesDestroyed:number;
  highestComboTier:number;
  highestComboName:string;
}
export function calculateTacticalScoreBonus(recap:TacticalRecap):number {
...
export function tacticalRecapLines(recap:TacticalRecap):string[] {
  ...
  if(recap.highestComboTier>0) lines.push(`ARCANE ${recap.highestComboTier>=3?'ASCENDANCY':recap.highestComboTier===2?'SURGE':'LINK'} · ${recap.highestComboName}`);
  if(recap.objectivesFailed>0) lines.push(`목표 실패 ${Math.max(0,Math.floor(recap.objectivesFailed))}회`);
