...
export function auditAutoTransitionLatency() {
    ...
    let preferred = null, last = null, switches = 0, unnecessarySwitches = 0, materialFrame = -1, switchFrame = -1;
    for (let frame = 0; frame < 120; frame++) {
        ...
        if (material && materialFrame < 0)
            materialFrame = frame;
        const chosen = chooseSpellTarget([one, two], hero, core, true, preferred);
    ...
    let corePreferred = 1, coreSwitch = -1, coreMaterial = 30;
    for (let frame = 0; frame < 60; frame++) {
        const one = enemy(1, 250), two = enemy(2, 430, frame >= coreMaterial ? 'core' : 'hero');
        const chosen = chooseSpellTarget([one, two], hero, core, true, corePreferred);
        corePreferred = chosen?.id ?? null;
        if (frame >= coreMaterial && corePreferred === 2 && coreSwitch < 0)
            coreSwitch = frame;
    }
    const coreThreatSwitchLatencyFrames = coreSwitch >= 0 ? coreSwitch - coreMaterial : 99;
    const boss = { id: 10, type: 'boss', pos: { x: 300, y: 0 } };
    let weakLastX = null, weakSwitch = -1;
    for (let frame = 0; frame < 60; frame++) {
        ...
        const aim = autoWeakpointAimPoint({ autoAim: true, target: boss, heroPos: hero, activeBossId: 10, nodes });
        if (weakLastX !== null && aim && aim.x !== weakLastX && weakSwitch < 0)
            weakSwitch = frame;
        weakLastX = aim?.x ?? null;
    }
    ...
    if (materialSwitchLatencyFrames > 1)
        issues.push('material-switch-latency');
    if (coreThreatSwitchLatencyFrames > 1)
        issues.push('core-switch-latency');
    if (weakpointSwitchLatencyFrames > 1)
        issues.push('weakpoint-switch-latency');
