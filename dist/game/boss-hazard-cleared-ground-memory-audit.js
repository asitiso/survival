...
export function runBossHazardClearedGroundMemoryAudit() {
    ...
    for (const reduced of [false, true])
        for (const aftermathActive of [false, true])
            for (const distance of [44, 120, 999])
                for (const ttl of [0, .24, .62, 1.18]) {
    ...
    return { samples, actionCount: ACTION_BUTTONS.length, presentationOnly: true, gameplayFormulaMutation: false, snapshotSchemaMutation: false, newAtlasCount: 0, passed: samples.length === 48 && samples.every(sample => sample.passed) && ACTION_BUTTONS.length === 9 };
}
