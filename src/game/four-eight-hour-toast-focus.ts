export interface FourEightHourToastFocusPolicy{show:boolean;critical:boolean;estimatedRoutineReduction:number;combatMutation:false;economyMutation:false;}
...
export function fourEightHourToastFocus(elapsedSeconds:number,message:string):FourEightHourToastFocusPolicy{
  const elapsed=Number.isFinite(elapsedSeconds)?Math.max(0,elapsedSeconds):0;
  const normalized=(message??'').trim().toLowerCase();
  const critical=PRESERVE.some(token=>normalized.includes(token));
  if(elapsed<14400||critical)return{show:true,critical,estimatedRoutineReduction:0,combatMutation:false,economyMutation:false};
