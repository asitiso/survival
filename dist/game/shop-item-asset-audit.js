import { SHOP_ITEM_ATLAS, SHOP_ITEM_IDS, auditShopItemAtlas, shopItemIconPresentation, shopItemIconSprite, } from './shop-item-assets.js';
const add = (samples, caseId, expected, actual) => {
    samples.push({ caseId, expected, actual, passed: expected === actual });
};
export function auditShopItemAssets() {
    const samples = [];
    ...
    if (atlas.coverage !== 1)
        issues.push('shop-item-coverage');
    if (atlas.uniqueCellCount !== 9)
        issues.push('shop-item-cell-collision');
    if (atlas.outOfBounds.length > 0)
        issues.push('shop-item-out-of-bounds');
    if (motionAmplitude !== 0)
        issues.push('shop-item-motion');
    if (!textFallbackPreserved)
        issues.push('shop-text-fallback');
