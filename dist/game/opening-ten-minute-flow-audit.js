...
export function auditOpeningTenMinuteFlow() {
    ...
    const issues = [];
    if (children.some(child => !child.passed))
        issues.push('opening-child-audit');
    if (maxCombatStatInflation > .001)
        issues.push('opening-stat-inflation');
    if (estimatedPauseReduction < .35)
        issues.push('opening-pause-reduction');
