import { relicDefinition, relicModifiers } from './relics.js';
...
const signed = (value, suffix = '%') => `${value >= 0 ? '+' : '-'}${fmt(Math.abs(value))}${suffix}`;
const ratioDelta = (before, after) => (after / Math.max(.0001, before) - 1) * 100;
const reductionDelta = (before, after) => (1 - after / Math.max(.0001, before)) * 100;
function relicDeltas(before, after) {
    const deltas = [];
    const addRatio = (label, a, b, reduction = false) => { const value = reduction ? reductionDelta(a, b) : ratioDelta(a, b); if (Math.abs(value) > .05)
        deltas.push({ label, text: `${label} ${reduction ? signed(-value) : signed(value)}`, salience: Math.abs(value) }); };
    ...
    const chance = (after.arkanExplosionChanceBonus - before.arkanExplosionChanceBonus) * 100;
    if (Math.abs(chance) > .05)
        deltas.push({ label: '폭발 확률', text: `폭발 확률 ${signed(chance, '%p')}`, salience: Math.abs(chance) });
    ...
    const overload = (after.kainOverloadMaxCooldownReduction - before.kainOverloadMaxCooldownReduction) * 100;
    if (Math.abs(overload) > .05)
        deltas.push({ label: '과부하 쿨감', text: `과부하 쿨감 ${signed(overload, '%p')}`, salience: Math.abs(overload) });
    const aura = after.edricAuraRadiusBonus - before.edricAuraRadiusBonus;
    if (Math.abs(aura) > .05)
        deltas.push({ label: '수호 오라', text: `수호 오라 ${aura >= 0 ? '+' : '-'}${fmt(Math.abs(aura))}`, salience: Math.abs(aura) / 4 });
...
function relicRole(id, heroId) {
    const def = relicDefinition(id);
    if (def.heroId !== null)
        return 'pivot';
    ...
    const ranked = [['economy', economy], ['survival', survival], ['offense', offense], ['growth', growth]];
    ranked.sort((a, b) => b[1] - a[1]);
    const top = ranked[0];
    return top && top[1] > 0 ? top[0] : 'growth';
}
function result(roleId, summary) { return { roleId, roleLabel: bossRewardImpactRoleIdentityIcon(roleId).label, summary }; }
export function projectBossRewardImpact(choice, context) {
    if (choice.kind === 'fusion') {
        const projection = projectFusionSelection(context.activeFusions, choice.fusionId, context.heroId);
        return result('pivot', `2마법 결합 · ${fusionProjectionHint(projection)}`);
    }
    ...
    if (choice.id === 'maxHp')
        return result('survival', '최대 HP +42 · 즉시 회복 +42');
    if (choice.id === 'spellPower')
        return result('offense', '전체 마법 피해 +12%');
    if (choice.id === 'cooldown')
        return result('growth', '전체 마법 쿨타임 -6%');
