...
import { FUSION_IDS, MAX_FUSIONS_PER_RUN, fusionDefinition } from './spell-fusions.js';
export function buildRecoveryGuidance(input) {
    if (!Number.isFinite(input.elapsedSeconds) || input.elapsedSeconds < 600)
        return null;
    if (!input.equipment.weapon)
        return { kind: 'equipment', label: 'RECOVER · 상점 무기 확보', detail: '빈 장비 슬롯 먼저 채우기' };
    if (!input.equipment.armor)
        return { kind: 'equipment', label: 'RECOVER · 상점 방어구 확보', detail: '빈 장비 슬롯 먼저 채우기' };
    if (!input.activeRelic)
        return { kind: 'relic', label: 'RECOVER · 다음 보스 유물', detail: '보스 보상에서 첫 유물 확보' };
    if (input.activeFusions.length >= MAX_FUSIONS_PER_RUN)
        return null;
    ...
    if (!best)
        return null;
    if (best.total === 0)
        return { kind: 'fusion', label: `RECOVER · 다음 보스 ${best.def.name}`, detail: '융합 조건 준비 완료' };
