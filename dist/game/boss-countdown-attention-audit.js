...
export function auditBossCountdownAttention() {
    const samples = [];
    const precedence = [
    ...
    for (const [id, input, expected] of precedence)
        add(samples, `priority-${id}`, expected, policy(input).primary);
    const countdowns = [8, 4, 1];
    let visible = 0;
    let maxAnimatedPrimaryWarnings = 0;
    for (const seconds of countdowns) {
        const p = policy({ bossCountdown: seconds });
        if (p.showBossCountdown)
            visible++;
        maxAnimatedPrimaryWarnings = Math.max(maxAnimatedPrimaryWarnings, Number(p.bossCountdownAnimated) + Number(p.heroWarningAnimated) + Number(p.coreWarningAnimated));
        add(samples, `countdown-${seconds}-visible`, true, p.showBossCountdown);
    }
    let criticalCountdownMotionAmplitude = 0;
    for (const [id, input] of [
        ['hero', { heroCritical: true, bossCountdown: 4 }],
        ['core', { coreCritical: true, bossCountdown: 4 }],
        ['damage', { damageSeverity: 'critical', bossCountdown: 4 }],
        ['heavy', { damageSeverity: 'heavy', bossCountdown: 4 }],
    ]) {
        const p = policy(input);
        criticalCountdownMotionAmplitude = Math.max(criticalCountdownMotionAmplitude, p.bossCountdownMotionAmplitude);
        add(samples, `secondary-${id}-steady`, 0, p.bossCountdownMotionAmplitude);
    }
    let reducedFlashCountdownMotionAmplitude = 0;
    for (const seconds of [8, 1]) {
        const p = policy({ bossCountdown: seconds, reducedFlash: true });
        reducedFlashCountdownMotionAmplitude = Math.max(reducedFlashCountdownMotionAmplitude, p.bossCountdownMotionAmplitude);
        add(samples, `reduced-${seconds}-steady`, 0, p.bossCountdownMotionAmplitude);
    }
    ...
    if (countdownVisibilityRate !== 1)
        issues.push('countdown-hidden');
    if (criticalCountdownMotionAmplitude !== 0)
        issues.push('critical-countdown-motion');
    if (reducedFlashCountdownMotionAmplitude !== 0)
        issues.push('reduced-flash-countdown-motion');
    if (staleCountdownReplayCount !== 0)
        issues.push('stale-countdown-replay');
    if (openingPrepDuplicateMotionCount !== 0)
        issues.push('opening-prep-duplicate-motion');
