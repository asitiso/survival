...
import { criticalHapticEvents, dangerUiState, type DangerUiState } from './danger-ui.js';

export interface CriticalDangerHysteresisAuditSample {
  caseId:string;
...
export interface CriticalDangerHysteresisAudit {
  passed:boolean;
  samples:CriticalDangerHysteresisAuditSample[];
  thresholdJitterToggleCount:number;
  duplicateHapticCount:number;
  safeExitRearmRate:number;
  criticalWarningVisibilityRate:number;
  maxAnimatedPrimaryWarnings:number;
  minHeroBandVignetteAlpha:number;
  reachableActionCount:number;
  snapshotSchemaMutation:false;
  issues:string[];
}

const add=(samples:CriticalDangerHysteresisAuditSample[],caseId:string,expected:CriticalDangerHysteresisAuditSample['expected'],actual:CriticalDangerHysteresisAuditSample['actual'])=>samples.push({caseId,expected,actual,passed:expected===actual});
const transitionCount=(states:readonly DangerUiState[],key:'heroCritical'|'coreCritical')=>states.slice(1).reduce((count,state,index)=>count+Number(state[key]!==states[index]![key]),0);

export function auditCriticalDangerHysteresis():CriticalDangerHysteresisAudit{
  const samples:CriticalDangerHysteresisAuditSample[]=[];
  ...
  if(thresholdJitterToggleCount!==0)issues.push('threshold-jitter-toggle');
  if(duplicateHapticCount!==0)issues.push('duplicate-haptic');
  if(normalizedRearmRate!==1)issues.push('haptic-rearm');
  if(criticalWarningVisibilityRate!==1)issues.push('warning-hidden');
  if(maxAnimatedPrimaryWarnings>1)issues.push('multiple-primary-warning-motion');
  if(minHeroBandVignetteAlpha<.18)issues.push('vignette-discontinuity');
