...
const familyRank = { projectile: 0, hazard: 1, footprint: 2, specialist: 3, memory: 3, forecast: 4, impact: 5, residue: 6, safeLane: -1, bossCue: -1 };
export function denseDashFamilyPresentation(input, reducedMotion = false, reducedFlash = false) {
    ...
    return { canonicalDashGapScale: 1, dashGapScale: protectedPattern ? 1 : (strongPattern ? 1 + crowd * .08 : demoted) * accessibility, protectedPattern, strongPattern, presentationOnly: true };
}
export function denseDashBudgetPresentation(input, reducedMotion = false, reducedFlash = false) {
    ...
    return { stress, strongPatternLimit, canonicalDashGapScale: 1, secondaryDashGapScale: (1 + stress * .34 + laneReserve) * accessibility, safeLaneDashGapScale: 1, presentationOnly: true };
}
