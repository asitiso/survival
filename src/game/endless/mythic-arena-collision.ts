import { clamp, normalize, type Vec2 } from '../../core/math.js';
import type { MythicArenaGeometryShape } from './mythic-arena-geometry.js';

export interface MythicArenaCollisionHazard {
  pos:Vec2; radius:number; damage:number; geometryShape?:MythicArenaGeometryShape;
  angle?:number; length?:number;
}
export interface MythicArenaHazardContact { hit:boolean; penetration:number; slowMultiplier:number; push:Vec2; }

function rotateInto(point:Vec2,center:Vec2,angle:number):Vec2{
  const dx=point.x-center.x,dy=point.y-center.y,c=Math.cos(-angle),s=Math.sin(-angle);
  return{x:dx*c-dy*s,y:dx*s+dy*c};
}
function rectPenetration(local:Vec2,halfLength:number,halfWidth:number,heroRadius:number):number{
...
function wedgeHit(local:Vec2,radius:number,heroRadius:number):number{
  const d=Math.hypot(local.x,local.y); if(d>radius*1.25+heroRadius)return 0;
...
export function mythicArenaHazardContact(hazard:MythicArenaCollisionHazard,point:Vec2,heroRadius:number):MythicArenaHazardContact{
  const r=Math.max(0,hazard.radius),hr=Math.max(0,heroRadius),shape=hazard.geometryShape;
  const angle=hazard.angle??0; let penetration=0; let local=rotateInto(point,hazard.pos,angle);
  if(shape==='ring'){
    ...
    const d=Math.hypot(local.x,local.y); penetration=d<=r+hr?Math.max(1,r+hr-d):0;
  }
  if(penetration<=0)return{hit:false,penetration:0,slowMultiplier:1,push:{x:0,y:0}};
