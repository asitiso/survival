...
export type SpellEvolutionTier = 0 | 1 | 2;

export interface SpellEvolutionProfile {
  tier: SpellEvolutionTier;
  name: string;
...
const BASE: Omit<SpellEvolutionProfile, 'tier' | 'name'> = {
  damageMultiplier: 1,
  areaMultiplier: 1,
  projectileBonus: 0,
  jumpBonus: 0,
...
const AWAKENED_NAMES: Record<HeroId, Record<SpellId, string>> = {
  arkan: {
...
const FINAL_NAMES: Record<HeroId, Record<SpellId, string>> = {
  arkan: {
...
export function spellEvolutionTier(level: number): SpellEvolutionTier {
...
export function spellEvolution(heroId: HeroId, spellId: SpellId, level: number): SpellEvolutionProfile {
  const tier = spellEvolutionTier(level);
  const profile: SpellEvolutionProfile = {
    ...BASE,
    tier,
  ...
  if (heroId === 'arkan') {
    ...
    if (spellId === 'meteorStorm') profile.projectileBonus += tier;
    ...
    if (spellId === 'chainLightning') profile.jumpBonus += tier;
    if (spellId === 'fireBolt') profile.projectileBonus += tier === 2 ? 1 : 0;
  } else {
    profile.areaMultiplier *= tier === 2 ? 1.08 : 1.04;
    profile.knockbackMultiplier *= tier === 2 ? 1.18 : 1.08;
    if (spellId === 'fireBolt') profile.pierceBonus += tier;
  }
