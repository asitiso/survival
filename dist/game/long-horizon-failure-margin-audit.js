...
function pressureAt(minute, threat) {
...
export function auditLongHorizonFailureMargin() {
    ...
    for (const hero of HERO_PROFILES)
        for (const minute of MINUTES) {
            ...
            if (!(group[0].heroReserveMargin > group[1].heroReserveMargin && group[1].heroReserveMargin > group[2].heroReserveMargin && group[0].coreReserveMargin > group[1].coreReserveMargin && group[1].coreReserveMargin > group[2].coreReserveMargin))
                threatMonotonic = false;
        }
    maxHeroSpread = round(maxHeroSpread);
    maxCoreSpread = round(maxCoreSpread);
