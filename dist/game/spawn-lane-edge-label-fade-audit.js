...
export function runSpawnLaneEdgeLabelFadeAudit() {
    const samples = [];
    let passed = SPAWN_LANE_EDGE_LABEL_FADE_SECONDS > .12 && SPAWN_LANE_EDGE_LABEL_FADE_SECONDS < .4;
    for (let i = 0; i < 64; i++) {
        const ttl = (i % 8) / 7 * SPAWN_LANE_EDGE_LABEL_FADE_SECONDS * 1.5;
        const alpha = .58;
        ...
        passed &&= ok;
        samples.push(`${i}:${ttl.toFixed(3)}:${r.labelAlpha.toFixed(3)}:${r.visible ? 1 : 0}`);
    }
