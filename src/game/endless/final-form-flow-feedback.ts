...
import type { FinalFormMobilityFamily } from './final-form-mobility.js';

export type FlowPresentationTier='full'|'reduced'|'minimal';
export interface FlowFeedbackProfile{auraAlpha:number;auraRadius:number;trailSegments:number;trailLength:number;pulseRadius:number;lineWidth:number;}
export function flowFeedbackProfile(streak:number,family:FinalFormMobilityFamily,tier:FlowPresentationTier):FlowFeedbackProfile{
...
export function shouldEmitFlowCue(previousStreak:number,nextStreak:number):boolean{
...
export interface FlowImpactProfile{freezeMs:number;shake:number;particleCount:number;ringRadius:number;soundKind:'flowImpact';}
export function flowImpactProfile(previousStreak:number,nextStreak:number,family:FinalFormMobilityFamily,tier:FlowPresentationTier):FlowImpactProfile|null{
  if(!shouldEmitFlowCue(previousStreak,nextStreak))return null;
