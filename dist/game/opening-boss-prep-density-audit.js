...
export function auditOpeningBossPrepDensity() {
    const samples = [];
    for (const countdown of [14, 12, 8, 2])
        for (const shopTokens of [0, 1])
            for (const hpRatio of [1, .6])
                for (const potions of [0, 1]) {
                    const cue = openingBossPrepAssist({ elapsedSeconds: 112, bossCountdown: countdown, shopTokens, hpRatio, potions });
                    samples.push({ countdown, shopTokens, hpRatio, potions, cue });
                }
    const prepared = [
    ...
    const cueActions = [...new Set(samples.flatMap(sample => sample.cue ? [sample.cue.actionId] : []))];
    const issues = [];
    if (maxConcurrentCues > 1)
        issues.push('prep-cue-density');
    if (falsePositiveCount > 0)
        issues.push('prep-early-false-positive');
    if (preparedSilenceCoverage < 1)
        issues.push('prepared-state-noise');
    if (cueActions.length !== 2)
        issues.push('prep-action-coverage');
