...
import { clampThreatLevel, threatLevelModifiers } from '../domain/threat-level.js';
export const BALANCE_CHECKPOINTS = [600, 1200, 1800, 2700];
function clamp(value, min, max) {
    return Math.max(min, Math.min(max, value));
}
function estimatedLevelAt(seconds) {
    const time = Math.max(0, seconds);
    return Math.max(1, Math.min(140, Math.floor(4 + Math.sqrt(time) * 1.18 + time / 190)));
}
function heroDpsBand(level) {
...
export function projectBalanceAt(seconds, threat) {
    const time = Math.max(0, seconds);
...
export function balanceProjectionSeries(threat) {
