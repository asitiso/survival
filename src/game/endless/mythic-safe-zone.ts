...
import type { LastLawSafeZoneLifecycle } from './last-law-safe-zone-lifecycle.js';

export type MythicSafeZonePhase='stable'|'collapse'|'collapsed'|'reform';
export interface MythicSafeZoneState {
  label:'SAFE ZONE';
...
function normalizedPoint(archetype:BossArchetype,index:number):Vec2{
  const i=Math.max(0,index);
  if(archetype==='inferno')return[{x:.5,y:.54},{x:.31,y:.62},{x:.69,y:.38}][i%3]!;
  if(archetype==='summoner')return[{x:.28,y:.34},{x:.72,y:.34},{x:.5,y:.7}][i%3]!;
  if(archetype==='juggernaut')return i%2===0?{x:.26,y:.52}:{x:.74,y:.48};
  if(archetype==='twinMaw')return[{x:.3,y:.34},{x:.7,y:.66},{x:.7,y:.34},{x:.3,y:.66}][i%4]!;
...
function arenaPoint(p:Vec2,width:number,height:number):Vec2{
  const w=Math.max(320,width),h=Math.max(240,height);
  return{x:clamp(p.x*w,80,w-80),y:clamp(p.y*h,120,h-80)};
}
function baseRadius(archetype:BossArchetype):number{
  if(archetype==='inferno')return 82;
  if(archetype==='summoner')return 96;
  if(archetype==='juggernaut')return 72;
  if(archetype==='abyssWitch')return 86;
  if(archetype==='twinMaw')return 78;
  return 88;
}
export function mythicSafeZoneState(archetype:BossArchetype,encounterElapsedMs:number,width:number,height:number,destroyedWeakpointRatio:number,lifecycle?:LastLawSafeZoneLifecycle|null):MythicSafeZoneState{
  ...
  const phase:MythicSafeZonePhase=local<timing.stableEndMs?'stable':local<timing.collapseEndMs?'collapse':local<timing.collapsedEndMs?'collapsed':'reform';
...
export function mythicSafeZoneDamageMultiplier(zone:MythicSafeZoneState|null,point:Vec2):number{
  if(!zone||!zone.active)return 1;
  if(Math.hypot(point.x-zone.center.x,point.y-zone.center.y)>zone.radius)return 1;
  if(zone.phase==='stable')return .18;
  if(zone.phase==='collapse')return .58;
  if(zone.phase==='reform')return .34;
  return 1;
}
