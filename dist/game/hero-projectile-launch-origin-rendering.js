...
export function heroProjectileLaunchOriginPresentation(input, reducedMotion = false) {
...
export function visualLaunchPosition(gameplayPos, offset, ttl, maxTtl) { const max = Math.max(.0001, Number.isFinite(maxTtl) ? maxTtl : 0), t = clamp((Number.isFinite(ttl) ? ttl : 0) / max, 0, 1), blend = t * t; return { x: gameplayPos.x + offset.x * blend, y: gameplayPos.y + offset.y * blend }; }
