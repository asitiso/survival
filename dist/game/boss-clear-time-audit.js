...
function expectedPlayerDps(seconds) { return 90 * Math.pow(1 + Math.max(0, seconds) / 300, .9); }
export function firstSixBossCheckpoints() {
    const points = [];
    let spawnSecond = 120;
...
export function auditFirstSixBosses() {
    ...
    maxClearTimeRatio = round(maxClearTimeRatio);
    maxDifficultyRatio = round(maxDifficultyRatio);
