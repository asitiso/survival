export type FatePathId = 'frenzy' | 'golden' | 'guardian';
export const FATE_CHECKPOINTS = [360, 720, 1080] as const;

export interface FateModifiers {
  spawnPressureMultiplier: number;
  enemySpeedMultiplier: number;
  eliteIntervalMultiplier: number;
  xpMultiplier: number;
  goldMultiplier: number;
  shopTokenMultiplier: number;
  coreDamageTakenMultiplier: number;
  bossVariantBonus: number;
  objectiveRewardMultiplier: number;
}
...
export interface FatePathDefinition {
  ...
  description: string;
  accent: string;
  modifiers: FateModifiers;
}

const NEUTRAL: FateModifiers = {
...
const PATHS: Record<FatePathId, FatePathDefinition> = {
  frenzy: {
...
export function fatePathDefinition(id: FatePathId): FatePathDefinition { return PATHS[id]; }
export function allFatePaths(): FatePathDefinition[] { return Object.values(PATHS); }

export function fateCheckpointIndex(elapsed: number, choicesMade: number): number {
  if (choicesMade < 0 || choicesMade >= FATE_CHECKPOINTS.length) return -1;
  return elapsed >= FATE_CHECKPOINTS[choicesMade]! ? choicesMade : -1;
}

export function composeFateModifiers(paths: readonly FatePathId[]): FateModifiers {
  const out = { ...NEUTRAL };
