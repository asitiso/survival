...
export function runBossSafeResponseDisplacementGuardAudit() {
    const samples = [];
    let passed = BOSS_SAFE_RESPONSE_SLOT_BOSS_DISPLACEMENT_GUARD >= 64 && BOSS_SAFE_RESPONSE_SLOT_BOSS_DISPLACEMENT_GUARD <= 140;
    for (let i = 0; i < 64; i++) {
        const basePos = { x: 500 + i, y: 360 };
        const base = { bossPos: basePos, bossRadius: 68, heroPos: { x: 180, y: 360 }, corePos: { x: 1080, y: 360 }, width: 1280, height: 800, extraProtected: [] };
        ...
        const ok = r.placement.pos.x === dash.x && r.memory?.bossPos.x === dash.x && r.presentationOnly;
        passed &&= ok;
        samples.push(`${i}:${r.placement.pos.x.toFixed(1)}:${r.memory?.bossPos.x.toFixed(1) ?? 'none'}`);
    }
