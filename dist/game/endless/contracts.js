...
export function contractOfferTimeMs(index) {
    if (index <= 3)
        return (4 + index * 5) * 60_000;
    return (19 + (index - 3) * 7) * 60_000;
}
export function shouldOfferContract(state, elapsedMs) {
    if (state.active || state.pendingOffer)
        return false;
    return elapsedMs >= contractOfferTimeMs(state.nextOfferIndex);
}
function familyWeight(family, legacy) {
    let weight = 1;
    if (legacy.fate === 'frenzy' && (family === 'slayer' || family === 'hunter'))
        weight += 0.8;
    if (legacy.fate === 'gold' && family === 'hunter')
        weight += 0.9;
    if (legacy.fate === 'guardian' && (family === 'warden' || family === 'survivor'))
        weight += 0.9;
    if (legacy.spellFusionCount > 0 && family === 'arcane')
        weight += 0.5;
    if (legacy.threat >= 4 && family === 'warden')
        weight += 0.35;
    if (legacy.masteryLevel >= 10 && family === 'survivor')
        weight += 0.25;
    return weight;
}
function contractShape(family, legacy, offerId) {
...
function blockedFamily(history) {
...
export function createContractOffer(legacy, state, rng) {
    ...
    while (chosen.length < 3) {
        const pick = pickWeighted(remaining
            ...
            .map((family) => ({ value: family, weight: familyWeight(family, legacy) })), nextRng);
        chosen.push(pick.value);
        nextRng = pick.state;
    }
    const options = chosen.map((family) => contractShape(family, legacy, offerId));
    const offer = { offerId, generatedAtMs: legacy.elapsedMs, options };
    return {
        offer,
        rng: nextRng,
        state: {
...
export function acceptContract(state, optionId, startedAtMs, guardianCoreHp) {
    const offer = state.pendingOffer;
    if (!offer)
        throw new Error('No pending contract offer');
    const option = offer.options.find((entry) => entry.optionId === optionId);
    if (!option)
        throw new Error(`Unknown contract option: ${optionId}`);
    return {
        ...state,
        pendingOffer: undefined,
...
export function advanceContract(state, legacy, events, deltaMs) {
    const working = { ...state, boons: state.boons.filter((boon) => boon.expiresAtMs > legacy.elapsedMs) };
    ...
    if (active.family === 'warden') {
        ...
        if (active.baselineCoreHp - legacy.guardianCoreHp > allowedLoss)
            return failContract(working, active.family);
    }
    let increment = 0;
    for (const event of events) {
        if (active.family === 'slayer' && event.type === 'enemy_killed')
            increment += 1;
        if (active.family === 'arcane' && event.type === 'spell_cast')
            increment += event.fusion ? 2 : 1;
        if (active.family === 'hunter' && event.type === 'enemy_killed' && event.elite)
            increment += 1;
        if (active.family === 'hunter' && event.type === 'boss_defeated')
            increment += 3;
    }
    if (active.family === 'survivor' || active.family === 'warden')
        increment += Math.max(0, deltaMs);
    const progress = clamp(active.progress + increment, 0, active.target);
    const next = { ...working, active: { ...active, progress } };
    if (progress >= active.target)
        return completeContract(next, active.family, legacy.elapsedMs);
    if (legacy.elapsedMs >= active.deadlineMs)
        return failContract(next, active.family);
    return { state: next, effects: [] };
}
const CONTRACT_HUD_LABELS = { slayer: 'SLAYER', warden: 'WARDEN', arcane: 'ARCANE', hunter: 'HUNTER', survivor: 'SURVIVOR' };
export function contractHudLine(state) {
    const active = state.active;
    if (!active)
        return '';
    const label = CONTRACT_HUD_LABELS[active.family];
    if (active.family === 'warden' || active.family === 'survivor')
        return `CONTRACT · ${label} ${Math.min(Math.floor(active.target / 1000), Math.floor(active.progress / 1000))}/${Math.floor(active.target / 1000)}s`;
    return `CONTRACT · ${label} ${Math.min(active.target, Math.floor(active.progress))}/${active.target}`;
}
export function getContractModifiers(state, elapsedMs) {
    const out = { xpMultiplier: 1, masteryMultiplier: 1, goldMultiplier: 1, coreDamageTakenMultiplier: 1, cooldownMultiplier: 1, bossDamageMultiplier: 1, fusionPowerMultiplier: 1, potionEfficiency: 1 };
    for (const boon of state.boons) {
        if (boon.expiresAtMs <= elapsedMs)
            continue;
        if (boon.family === 'slayer') {
            out.xpMultiplier *= 1.12;
            out.masteryMultiplier *= 1.08;
        }
        else if (boon.family === 'warden') {
            out.coreDamageTakenMultiplier *= 0.88;
            out.potionEfficiency *= 1.1;
        }
        else if (boon.family === 'arcane') {
            out.fusionPowerMultiplier *= 1.1;
            out.cooldownMultiplier *= 0.92;
        }
        else if (boon.family === 'hunter') {
            out.goldMultiplier *= 1.15;
            out.bossDamageMultiplier *= 1.08;
        }
        else if (boon.family === 'survivor') {
            out.coreDamageTakenMultiplier *= 0.92;
            out.potionEfficiency *= 1.15;
        }
    }
    return {
