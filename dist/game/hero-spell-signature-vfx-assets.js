...
export const HERO_SPELL_SIGNATURE_VFX_HEROES = ['arkan', 'seria', 'kain', 'edric'];
export const HERO_SPELL_SIGNATURE_VFX_CHANNELS = ['chainLightning', 'frostNova', 'flameField'];
const HERO_INDEX = new Map(HERO_SPELL_SIGNATURE_VFX_HEROES.map((id, index) => [id, index]));
const CHANNEL_INDEX = new Map(HERO_SPELL_SIGNATURE_VFX_CHANNELS.map((id, index) => [id, index]));
export function heroSpellSignatureVfxSprite(heroId, channel) {
    const column = HERO_INDEX.get(heroId);
    const row = CHANNEL_INDEX.get(channel);
    if (column === undefined || row === undefined)
        throw new Error(`Unknown hero spell VFX: ${heroId}:${channel}`);
    return {
...
export function auditHeroSpellSignatureVfxAtlas() {
    ...
    for (const heroId of HERO_SPELL_SIGNATURE_VFX_HEROES) {
        for (const channel of HERO_SPELL_SIGNATURE_VFX_CHANNELS) {
            ...
            if (sprite.sx < 0 || sprite.sy < 0 || sprite.sx + sprite.sw > HERO_SPELL_SIGNATURE_VFX_ATLAS.width || sprite.sy + sprite.sh > HERO_SPELL_SIGNATURE_VFX_ATLAS.height)
