...
import { enemyAttackMotionPresentation, type EnemyAttackMotionType } from './enemy-attack-motion-rendering.js';

export interface EnemyAttackMotionRenderAuditSample { id:string; expected:boolean|number|string; actual:boolean|number|string; passed:boolean; }
const add=(samples:EnemyAttackMotionRenderAuditSample[],id:string,expected:boolean|number|string,actual:boolean|number|string)=>samples.push({id,expected,actual,passed:Object.is(expected,actual)});
const TYPES:readonly EnemyAttackMotionType[]=['grunt','hound','brute','archer','bomber','shaman','shieldbearer','assassin','siegeGolem','nullifier','golden','elite','boss'];

export function runEnemyAttackMotionRenderAudit(){
  const samples:EnemyAttackMotionRenderAuditSample[]=[];
  ...
  return{samples,actionCount:ACTION_BUTTONS.length,presentationOnly:true as const,gameplayFormulaMutation:false as const,snapshotSchemaMutation:false as const,passed:samples.length===64&&samples.every(s=>s.passed)&&ACTION_BUTTONS.length===9};
}
