...
type PickupCollectState = Extract<PickupFlowVfxState,'collectSmall'|'collectLarge'>;
...
interface Pickup {
  ...
  radius: number;
  age: number;
  flowState: ActivePickupFlowState | null;
}

interface CollectionVfx {
  kind: PickupKind;
  pos: Vec2;
  state: PickupCollectState;
  ttl: number;
  maxTtl: number;
}

export interface PickupCallbacks {
  onXp: (value: number) => void;
  onCoin: (value: number) => void;
}

function isRichPickup(kind:PickupKind,value:number):boolean { return kind === 'coin' ? value >= 24 : value >= 36; }

export class PickupManager {
  private pickups: Pickup[] = [];
  private collectionVfx: CollectionVfx[] = [];
  private mergeTimer = 0;
  private globalMagnetTimer = 0;

  get globalMagnetRemaining(): number { return this.globalMagnetTimer; }

  reset(): void { this.pickups = []; this.collectionVfx = []; this.mergeTimer = 0; this.globalMagnetTimer = 0; }

  setGlobalMagnet(seconds: number): void {
    this.globalMagnetTimer = Math.max(this.globalMagnetTimer, Math.max(0, seconds));
  }

  spawnDeath(death: EnemyDeathEvent): void {
    this.spawn('xp', death.x, death.y, death.xp);
    if (death.gold > 0) this.spawn('coin', death.x, death.y, death.gold);
  }

  update(dt: number, hero: Hero, callbacks: PickupCallbacks): void {
    const step=Math.max(0,dt);
    this.globalMagnetTimer = Math.max(0, this.globalMagnetTimer - step);
    for (const cue of this.collectionVfx) cue.ttl -= step;
    ...
    for (const pickup of this.pickups) {
      if (distance(pickup.pos, hero.pos) <= hero.radius + pickup.radius + 6) {
        ...
        if (pickup.kind === 'xp') callbacks.onXp(pickup.value);
        else callbacks.onCoin(pickup.value);
  ...
  private queueCollectionVfx(pickup:Pickup):void {
    const state:PickupCollectState=isRichPickup(pickup.kind,pickup.value)||pickup.radius>=11?'collectLarge':'collectSmall';
    const maxTtl=state==='collectLarge'?.46:.32;
    this.collectionVfx.push({kind:pickup.kind,pos:{...pickup.pos},state,ttl:maxTtl,maxTtl});
    if(this.collectionVfx.length>32)this.collectionVfx.splice(0,this.collectionVfx.length-32);
  }
  ...
  private mergeDensePickups(): void {
    const next: Pickup[] = [];
    const buckets: Record<PickupKind, Pickup[]> = { xp: [], coin: [] };
    for (const p of this.pickups) buckets[p.kind].push(p);
    for (const kind of ['xp', 'coin'] as const) {
