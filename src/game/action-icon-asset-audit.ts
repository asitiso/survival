...
export interface ActionIconAssetSample {
  caseId: string;
...
export interface ActionIconAssetAudit {
  passed: boolean;
  samples: ActionIconAssetSample[];
  coverage: number;
...
const add = (samples: ActionIconAssetSample[], caseId: string, expected: ActionIconAssetSample['expected'], actual: ActionIconAssetSample['actual']): void => {
  samples.push({ caseId, expected, actual, passed: expected === actual });
};

export function auditActionIconAssets(): ActionIconAssetAudit {
  const samples: ActionIconAssetSample[] = [];
  ...
  if (atlas.coverage !== 1) issues.push('action-icon-coverage');
  if (atlas.uniqueCellCount !== 9) issues.push('action-icon-cell-collision');
  if (atlas.outOfBounds.length > 0) issues.push('action-icon-out-of-bounds');
  if (actionIds.length !== 9) issues.push('action-count');
  if (motionAmplitude !== 0) issues.push('action-icon-motion');
  if (!fallbackPreserved) issues.push('action-icon-fallback');
