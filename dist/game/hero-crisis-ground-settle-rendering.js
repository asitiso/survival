...
export function advanceHeroCrisisGroundSettleState(previous, hitIntensity, dt, reducedMotion = false) {
    ...
    if (hitIntensity !== null && hit >= .9) {
        impact = Math.max(impact, clamp((hit - .82) / .35, .48, 1));
        settle = 1;
    }
    const safeDt = clamp(Number.isFinite(dt) ? dt : 0, 0, .14);
    if (hitIntensity === null) {
        impact = Math.max(0, impact - safeDt / (reducedMotion ? .085 : .13));
        settle = Math.max(0, settle - safeDt / (reducedMotion ? .22 : .36));
    }
    return { impact, settle };
}
export function heroCrisisGroundSettlePresentation(state, movement, reducedMotion = false) {
    const impact = clamp(state?.impact ?? 0, 0, 1), settle = clamp(state?.settle ?? 0, 0, 1), move = clamp(movement, 0, 1);
    const owner = impact > .08 ? 'impact' : settle > .04 ? 'settle' : 'ground';
    if (owner === 'ground')
        return { owner, groundMotionScale: 1, movementRestartScale: 1, shadowFollowScale: 1, widthScale: 1, heightScale: 1, alphaScale: 1 };
    ...
    if (reducedMotion) {
        groundMotionScale = 1 - (1 - groundMotionScale) * .72;
        movementRestartScale = 1 - (1 - movementRestartScale) * .72;
        shadowFollowScale = 1 - (1 - shadowFollowScale) * .7;
        widthScale = 1 + (widthScale - 1) * .58;
        heightScale = 1 + (heightScale - 1) * .58;
        alphaScale = 1 - (1 - alphaScale) * .6;
    }
