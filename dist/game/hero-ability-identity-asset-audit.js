...
import { HERO_ABILITY_ACTIONS, HERO_ABILITY_HERO_IDS, HERO_ABILITY_IDENTITY_ATLAS, auditHeroAbilityIdentityAtlas, heroAbilityIdentityIcon, } from './hero-ability-identity-assets.js';
const SPELLS = ['fireBolt', 'chainLightning', 'frostNova', 'flameField', 'meteorStorm', 'blackHole'];
export function auditHeroAbilityIdentityAssets() {
    const atlas = auditHeroAbilityIdentityAtlas();
    const samples = [];
    const heroActionMismatch = [];
    ...
    if (atlas.coverage !== 1 || atlas.uniqueCellCount !== 24 || atlas.outOfBounds.length > 0)
        issues.push('atlas');
    if (heroActionMismatch.length > 0)
        issues.push('hero-action-mismatch');
    if (combatCoverage !== 1)
        issues.push('combat-coverage');
    if (decisionFallbackCoverage !== 1)
        issues.push('decision-fallback-coverage');
