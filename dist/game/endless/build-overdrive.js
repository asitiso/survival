...
export function createDefaultOverdriveState() { return { charge: 0, activeUntilMs: 0, activations: 0 }; }
export function resolveBuildArchetype(mods) {
    const scores = {
    ...
    const order = ['burst', 'cycle', 'domain', 'fortress'];
    return order.reduce((best, id) => scores[id] > scores[best] ? id : best, order[0]);
}
function chargeForEvent(event) {
    if (event.type === 'spell_cast')
        return event.fusion ? 5 : 2;
    if (event.type === 'enemy_killed')
        return event.elite ? 3 : 1;
    if (event.type === 'boss_defeated')
        return 20;
    return 0;
}
export function advanceBuildOverdrive(state, events, elapsedMs) {
    const now = Math.max(0, elapsedMs);
    const active = state.activeUntilMs > now;
    if (active)
        return { charge: clamp(state.charge, 0, 100), activeUntilMs: state.activeUntilMs, activations: Math.max(0, Math.floor(state.activations)) };
    let charge = state.activeUntilMs > 0 && state.activeUntilMs <= now ? 0 : clamp(state.charge, 0, 100);
    for (const event of events)
        charge = clamp(charge + chargeForEvent(event), 0, 100);
    if (charge >= 100)
        return { charge: 0, activeUntilMs: now + 12_000, activations: Math.max(0, Math.floor(state.activations)) + 1 };
...
export function overdriveModifiers(state, archetype, elapsedMs) {
    const active = state.activeUntilMs > Math.max(0, elapsedMs);
    const out = { active, spellPowerMultiplier: 1, cooldownMultiplier: 1, areaMultiplier: 1, heroDamageTakenMultiplier: 1, coreDamageTakenMultiplier: 1, bossDamageMultiplier: 1, fusionPowerMultiplier: 1 };
    if (!active)
        return out;
    if (archetype === 'burst') {
        out.spellPowerMultiplier = 1.2;
        out.bossDamageMultiplier = 1.18;
        out.fusionPowerMultiplier = 1.08;
    }
    else if (archetype === 'cycle') {
        out.cooldownMultiplier = .8;
        out.fusionPowerMultiplier = 1.12;
    }
    else if (archetype === 'domain') {
        out.areaMultiplier = 1.22;
        out.spellPowerMultiplier = 1.1;
    }
    else {
        out.heroDamageTakenMultiplier = .84;
        out.coreDamageTakenMultiplier = .82;
        out.spellPowerMultiplier = 1.06;
    }
    return {
        active: true,
