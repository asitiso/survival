...
export const ELITE_AFFIX_LIFECYCLE_VFX_AFFIXES = ELITE_AFFIXES;
export const ELITE_AFFIX_LIFECYCLE_VFX_STATES = ['active', 'response'];
...
export function eliteAffixLifecycleVfxSprite(affixId, state) {
    const column = ELITE_AFFIX_LIFECYCLE_VFX_AFFIXES.indexOf(affixId);
    const row = ELITE_AFFIX_LIFECYCLE_VFX_STATES.indexOf(state);
    if (column < 0 || row < 0)
        throw new Error(`Unknown elite affix lifecycle VFX: ${affixId}:${state}`);
...
export function auditEliteAffixLifecycleVfxAtlas() {
    ...
    for (const affixId of ELITE_AFFIX_LIFECYCLE_VFX_AFFIXES)
        for (const state of ELITE_AFFIX_LIFECYCLE_VFX_STATES) {
            const r = eliteAffixLifecycleVfxSprite(affixId, state);
            cells.add(`${r.sx}:${r.sy}`);
            if (r.sx < 0 || r.sy < 0 || r.sx + r.sw > ELITE_AFFIX_LIFECYCLE_VFX_ATLAS.width || r.sy + r.sh > ELITE_AFFIX_LIFECYCLE_VFX_ATLAS.height)
                outOfBounds.push(`${affixId}:${state}`);
        }
