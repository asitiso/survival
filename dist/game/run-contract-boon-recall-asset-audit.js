...
import { acceptContract, advanceContract, contractOfferTimeMs, createContractOffer, createDefaultContractState, getContractModifiers, } from './endless/contracts.js';
...
const legacy = (elapsedMs, overrides = {}) => ({
...
const active = (family, overrides = {}) => ({ contractId: `audit:${family}`, family, startedAtMs: 240_000, deadlineMs: 300_000, target: 1, progress: 0, baselineCoreHp: 1000, ...overrides });
export function auditRunContractBoonRecallAssets() {
    const atlas = auditRunContractBoonRecallAtlas();
    const samples = [];
    const push = (caseId, passed, family) => { samples.push({ caseId, passed, ...(family ? { family } : {}) }); };
    const acceptCoverage = new Set(), successCoverage = new Set(), failureCoverage = new Set(), activeCoverage = new Set(), fallbackCoverageSet = new Set(), countdownCoverageSet = new Set();
    ...
    for (const family of RUN_CONTRACT_RECALL_IDS) {
        ...
        const recall = activeRunContractBoonRecall([{ family, expiresAtMs: 190_000 }], 100_001);
        ...
        if (icon.acceptToastIdentitySupported)
            acceptCoverage.add(family);
        if (icon.outcomeToastIdentitySupported) {
            successCoverage.add(family);
            failureCoverage.add(family);
        }
        if (icon.activeBoonIdentitySupported)
            activeCoverage.add(family);
        if (icon.textFallbackPreserved)
            fallbackCoverageSet.add(family);
        if (recall?.remainingSeconds === 90)
            countdownCoverageSet.add(family);
    ...
    const scheduleOk = same([0, 1, 2, 3, 4, 5].map(contractOfferTimeMs), [4, 9, 14, 19, 26, 33].map(m => m * 60_000));
    push('contract:offer-schedule', scheduleOk);
    const offered = createContractOffer(legacy(240_000), createDefaultContractState(), { seed: 29, cursor: 0 });
    const threeChoiceOk = offered.offer.options.length === 3 && new Set(offered.offer.options.map(option => option.family)).size === 3;
    push('contract:three-unique-options', threeChoiceOk);
    ...
    const blockedOk = blockedOffer.offer.options.every(option => option.family !== 'slayer');
    push('contract:recent-family-block', blockedOk);
    const wardenBase = { ...createDefaultContractState(), active: active('warden', { target: 30_000, deadlineMs: 270_000, baselineCoreHp: 1000 }) };
    const wardenSafe = advanceContract(wardenBase, legacy(250_000, { guardianCoreHp: 800 }), [], 10_000);
    const wardenFail = advanceContract(wardenBase, legacy(250_000, { guardianCoreHp: 799 }), [], 10_000);
    const wardenOk = Boolean(wardenSafe.state.active) && !wardenSafe.effects.some(effect => effect.type === 'contract_failed') && !wardenFail.state.active && wardenFail.effects.some(effect => effect.type === 'contract_failed');
    push('contract:warden-20pct-core-loss', wardenOk);
    const survivorBase = { ...createDefaultContractState(), active: active('survivor', { target: 20_000, deadlineMs: 260_000 }) };
    const survivorFail = advanceContract(survivorBase, legacy(245_000), [{ type: 'hero_damaged', amount: 1 }], 1_000);
    const survivorOk = !survivorFail.state.active && survivorFail.effects.some(effect => effect.type === 'contract_failed');
    push('contract:survivor-no-hit', survivorOk);
    const slayerOption = { optionId: 'audit-offer:slayer', family: 'slayer', title: 'Slayer Contract', description: 'Defeat 1 enemy', target: 1, durationMs: 45_000 };
    ...
    const completed = advanceContract(accepted, legacy(250_000), [{ type: 'enemy_killed' }], 10_000);
    const boon = completed.state.boons.at(-1);
    const boonDurationOk = boon?.family === 'slayer' && boon.expiresAtMs === 340_000;
    push('contract:boon-90s', boonDurationOk);
    const modifierOk = same(getContractModifiers({ ...createDefaultContractState(), boons: [{ family: 'slayer', expiresAtMs: 1000 }] }, 0), { xpMultiplier: 1.12, masteryMultiplier: 1.08, goldMultiplier: 1, coreDamageTakenMultiplier: 1, cooldownMultiplier: 1, bossDamageMultiplier: 1, fusionPowerMultiplier: 1, potionEfficiency: 1 }) &&
    ...
    const expired = advanceContract(completed.state, legacy(340_000), [], 1);
    const expiryOk = expired.state.boons.length === 0 && same(getContractModifiers(expired.state, 340_000), { xpMultiplier: 1, masteryMultiplier: 1, goldMultiplier: 1, coreDamageTakenMultiplier: 1, cooldownMultiplier: 1, bossDamageMultiplier: 1, fusionPowerMultiplier: 1, potionEfficiency: 1 });
    push('contract:boon-expiry', expiryOk);
    const latestOnly = activeRunContractBoonRecall([{ family: 'slayer', expiresAtMs: 180_000 }, { family: 'hunter', expiresAtMs: 190_000 }], 100_000)?.family === 'hunter';
    push('presentation:max-one-active-boon', latestOnly);
    const extension = createDefaultExtensionState(17);
    extension.contracts = { ...createDefaultContractState(), boons: [{ family: 'arcane', expiresAtMs: 999_999 }] };
    const restored = restoreExtension(serializeExtension(extension), 1);
    const snapshotRoundTrip = restored.contracts.boons.length === 1 && restored.contracts.boons[0]?.family === 'arcane' && restored.contracts.boons[0]?.expiresAtMs === 999_999;
    ...
    if (!atlas.passed)
        issues.push('atlas');
    if (acceptToastCoverage !== 1)
        issues.push('accept-toast-coverage');
    if (successToastCoverage !== 1)
        issues.push('success-toast-coverage');
    if (failureToastCoverage !== 1)
        issues.push('failure-toast-coverage');
    if (activeBoonCoverage !== 1)
        issues.push('active-boon-coverage');
    if (fallbackCoverage !== 1)
        issues.push('fallback-coverage');
    if (countdownCoverage !== 1)
        issues.push('countdown-coverage');
    ...
    if (iconMotionAmplitude !== 0)
        issues.push('motion');
    if (offerScheduleMutation)
        issues.push('offer-schedule-mutation');
    if (offerChoiceMutation)
        issues.push('offer-choice-mutation');
    if (failureContractMutation)
        issues.push('failure-contract-mutation');
    if (boonDurationMutation)
        issues.push('boon-duration-mutation');
    if (modifierContractMutation)
        issues.push('modifier-contract-mutation');
    if (expiryContractMutation)
        issues.push('expiry-contract-mutation');
    if (actionCount !== 9)
