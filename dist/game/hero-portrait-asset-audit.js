...
export function auditHeroPortraitAssets() {
    const samples = [];
    ...
    for (let i = 0; i < HERO_PROFILES.length; i += 1)
        add(samples, `portrait-${HERO_PROFILES[i].id}-visible`, true, loaded[i].visible);
    ...
    if (atlas.coverage !== 1)
        issues.push('hero-portrait-coverage');
    if (atlas.uniqueCellCount !== 4)
        issues.push('hero-portrait-cell-collision');
    if (atlas.outOfBounds.length > 0)
        issues.push('hero-portrait-out-of-bounds');
    if (heroIds.length !== 4)
        issues.push('hero-count');
    if (motionAmplitude !== 0)
        issues.push('hero-portrait-motion');
    if (!fallbackPreserved)
        issues.push('hero-portrait-fallback');
