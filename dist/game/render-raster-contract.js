...
const ROLE_WEIGHT = { 'critical-hud': 5, interactive: 4, telegraph: 3, label: 2, decorative: 1 };
function covers(p, x, y) {
    if (p.kind === 'circle')
        return Math.hypot(x - p.x, y - p.y) <= p.radius;
    return x >= p.x && x <= p.x + p.width && y >= p.y && y <= p.y + p.height;
}
export function rasterizeRenderContract(contract) {
        ...
        for (let gy = 0; gy < GRID_H; gy++)
            for (let gx = 0; gx < GRID_W; gx++) {
                ...
                for (const p of frame.primitives)
                    if (covers(p, x, y))
                        weight = Math.max(weight, ROLE_WEIGHT[p.role]);
...
export function rasterContractSignature(contract) {
    return `RR-${fnv([contract.viewport.width, contract.viewport.height, ...contract.frames.flatMap((f) => [f.id, ...f.cells])].join('|'))}`;
}
export function rasterSimilarity(a, b) {
    const byId = new Map(b.frames.map((f) => [f.id, f]));
    let same = 0, total = 0;
    for (const fa of a.frames) {
        const fb = byId.get(fa.id);
        if (!fb) {
            total += fa.cells.reduce((s, v) => s + Math.max(1, v), 0);
            continue;
        }
        const n = Math.max(fa.cells.length, fb.cells.length);
        for (let i = 0; i < n; i++) {
            const av = fa.cells[i] ?? 0, bv = fb.cells[i] ?? 0;
            const weight = Math.max(1, av, bv);
            total += weight;
