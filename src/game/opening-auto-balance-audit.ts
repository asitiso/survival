...
export interface OpeningAutoBalanceAudit{
  passed:boolean;samples:number;spellCount:number;maxDamageMultiplier:number;maxCooldownBenefit:number;maxSurvivalMultiplier:number;manualOverrideCoverage:number;actionCount:number;snapshotMutation:boolean;issues:string[];
}
const SPELLS:readonly SpellId[]=['fireBolt','chainLightning','frostNova','flameField'];
const CHECKPOINTS=([1,3,5,8] as const);
export function auditOpeningAutoBalance():OpeningAutoBalanceAudit{
  ...
  const issues:string[]=[];
  if(samples.some(sample=>!sample.auto.autoTriggered))issues.push('auto-not-ready');
  if(maxDamageMultiplier>1)issues.push('auto-damage-inflation');
  if(maxCooldownBenefit>1)issues.push('auto-cooldown-inflation');
  if(maxSurvivalMultiplier>1)issues.push('auto-survival-inflation');
  if(manualOverrideCoverage<1)issues.push('manual-override');
