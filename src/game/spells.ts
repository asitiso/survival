...
export class SpellSystem {
  ...
  render(ctx: CanvasRenderingContext2D, motion?: ResidualCombatMotionPolicy, propVfxAtlasImage?: HTMLImageElement | null, propVfxAtlasReady = false, heroProjectileAtlasImage?: HTMLImageElement | null, heroProjectileAtlasReady = false, heroSpellSignatureAtlasImage?: HTMLImageElement | null, heroSpellSignatureAtlasReady = false, ultimateSignatureAtlasImage?: HTMLImageElement | null, ultimateSignatureAtlasReady = false, persistentSpellZoneVfxAtlasImage?: HTMLImageElement | null, persistentSpellZoneVfxAtlasReady = false, crowdControlPropagationVfxAtlasImage?: HTMLImageElement | null, crowdControlPropagationVfxAtlasReady = false, reducedFlash = false, ultimatePostImpactResidueVfxAtlasImage?: HTMLImageElement | null, ultimatePostImpactResidueVfxAtlasReady = false, reducedMotion = false, presentationQuality: PresentationQuality = 'high', primaryProjectileLabelBlockers:readonly Vec2[]=[]): void {
    ...
    for (const projectile of this.projectiles) {
      ...
      if(travelHandoff&&travelHandoff.visible&&travelBudget.visible){ctx.save();ctx.globalAlpha=Math.min(travel?.alpha??0,travelHandoff.alpha)*travelBudget.alphaScale;ctx.strokeStyle=projectile.secondary;ctx.lineWidth=Math.max(1.1,projectile.radius*.12);ctx.beginPath();ctx.moveTo(travelHandoff.start.x,travelHandoff.start.y);ctx.lineTo(travelHandoff.end.x,travelHandoff.end.y);ctx.stroke();ctx.restore();}
    ...
    if (heroProjectileAtlasReady && heroProjectileAtlasImage) {
      ...
      for (const impact of this.projectileImpactVisuals) {
        ...
        if(impactDirection?.visible&&impactDirectionOwner.owner!=='retired'&&impactDirectionBudget.visible&&impactResponseDensityBudget.visible){const cueLength=impactDirection.cueLength*impactDirectionBudget.lengthScale*impactResponsePriority.directionLengthScale*impactResponseDensityBudget.lengthScale;ctx.save();ctx.globalAlpha=impactDirection.alphaScale*impactDirectionOwner.alphaScale*impactDirectionBudget.alphaScale*impactResponsePriority.directionAlphaScale*impactResponseRelease.directionAlphaScale*impactResponseDensityBudget.alphaScale*(impact.alphaScale??1)*(budget?.alphaScale??1)*(reducedFlash?.62:1);ctx.strokeStyle=impact.secondaryKind==='splash'?'#ffe3b5':'#bcecff';ctx.lineWidth=1.7;ctx.beginPath();ctx.moveTo(impactVisualPos.x-impactDirection.facingX*cueLength,impactVisualPos.y-impactDirection.facingY*cueLength);ctx.lineTo(impactVisualPos.x,impactVisualPos.y);ctx.stroke();ctx.restore();}
