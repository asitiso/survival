...
import { MYTHIC_TACTIC_IDENTITY_IDS, auditMythicTacticIdentityAtlas, mythicTacticIdentityIcon, mythicTacticIdentityIdForArchetype } from './mythic-tactic-identity-assets.js';
const ARCHETYPE_BY_ID = { ember: 'inferno', brood: 'summoner', iron: 'juggernaut', void: 'abyssWitch', twin: 'twinMaw', time: 'timeEater' };
const ATTACK_EXPECTED = {
...
export function auditMythicTacticIdentityAssets() {
    const atlas = auditMythicTacticIdentityAtlas();
    const samples = [];
    const push = (caseId, passed, id) => { samples.push({ caseId, passed, ...(id ? { id } : {}) }); };
    const rewardSet = new Set(), primedSet = new Set(), consumedSet = new Set(), fallbackSet = new Set();
    ...
    for (const id of MYTHIC_TACTIC_IDENTITY_IDS) {
        ...
        const icon = mythicTacticIdentityIcon(id);
        const expected = ATTACK_EXPECTED[archetype];
        ...
        const rewardIdentityOk = icon.rewardIdentitySupported && mythicTacticIdentityIdForArchetype(archetype) === id;
        if (rewardIdentityOk)
            rewardSet.add(id);
        push(`${id}:reward-identity`, rewardIdentityOk, id);
        const primedOk = icon.primedIdentitySupported;
        if (primedOk)
            primedSet.add(id);
        push(`${id}:primed`, primedOk, id);
        const consumedOk = icon.consumedIdentitySupported;
        if (consumedOk)
            consumedSet.add(id);
        push(`${id}:consumed`, consumedOk, id);
        ...
        rewardContractMutation = rewardContractMutation || !rewardOk;
        push(`${id}:reward-contract`, rewardOk, id);
        ...
        attackLinkMutation = attackLinkMutation || !attackOk;
        push(`${id}:attack-contract`, attackOk, id);
        ...
        expiryConsumeMutation = expiryConsumeMutation || !expiryOk;
        push(`${id}:expiry-consume`, expiryOk, id);
    ...
    if (primedCoverage !== 1)
        issues.push('primed-coverage');
    if (consumedCoverage !== 1)
        issues.push('consumed-coverage');
    ...
    if (iconMotionAmplitude !== 0)
        issues.push('motion');
    if (rewardContractMutation)
        issues.push('reward-contract-mutation');
    if (attackLinkMutation)
        issues.push('attack-link-mutation');
    if (expiryConsumeMutation)
        issues.push('expiry-consume-mutation');
    if (actionCount !== 9)
