...
test('phase 4832 arena rendering keeps generated art underneath gameplay telegraphs with image failure fallback', () => {
  ...
  const drawArenaMethod = source.indexOf('private drawArena');
  const drawBackdrop = source.indexOf('battlefieldGameplayBackdropImage', drawArenaMethod);
  const renderArena = source.indexOf('this.drawArena(ctx);');
  const drawPropsCall = source.indexOf('this.drawBattlefieldGameplayProps(ctx);', renderArena);
  const drawTelegraphs = source.indexOf('this.drawDangerTelegraphs(ctx);', renderArena);
  assert.ok(drawArenaMethod >= 0 && drawBackdrop > drawArenaMethod);
  assert.ok(renderArena >= 0 && drawPropsCall > renderArena && drawPropsCall < drawTelegraphs, 'props must stay below gameplay warnings');
