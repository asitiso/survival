...
const NEUTRAL = { band: 'neutral', spawnPressureMultiplier: 1, eliteIntervalMultiplier: 1, rewardMultiplier: 1, shopIntervalMultiplier: 1, enemyBudgetMultiplier: 1 };
function lerp(a, b, t) { return a + (b - a) * Math.max(0, Math.min(1, t)); }
function round(value) { return Math.round(value * 10000) / 10000; }
function profile(band, s, start, end, from, to) {
...
export function firstThirtyMinuteProfile(elapsedSeconds) {
    const s = Math.max(0, Number.isFinite(elapsedSeconds) ? elapsedSeconds : 0);
    if (s < 600 || s >= 1800)
        return { ...NEUTRAL };
    if (s < 900)
        return profile('settle', s, 600, 900, [1.012, .994, 1.006], [1.024, .98, 1.014]);
    if (s < 1320)
        return profile('build_test', s, 900, 1320, [1.024, .98, 1.014], [1.036, .96, 1.022]);
    return profile('boss_ready', s, 1320, 1800, [1.036, .96, 1.022], [1.04, .94, 1.03]);
}
export function auditFirstThirtyMinutes() {
    const minutes = [2, 5, 10, 15, 20, 30];
    const checkpoints = minutes.map((minute) => {
