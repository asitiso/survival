...
const cluster=(x:number,y:number,vx:number,vy:number):ProjectileImpactCluster=>({impact:{x,y},incoming:{x:vx,y:vy},count:3,sourceClass:'archer',start:{x:x-28,y},end:{x,y},alpha:.5,accent:'#fff',animated:false,motionAmplitude:0});
const placement=(x:number,y:number):ProjectileImpactLabelPlacement=>({clusterIndex:0,sourceClass:'archer',pos:{x,y},visible:true,animated:false,motionAmplitude:0});
export interface ProjectileImpactLabelAnchorDirectionIdentityAudit{passed:boolean;samples:string[];actionCount:number;presentationOnly:true;gameplayFormulaMutation:false;snapshotSchemaMutation:false;}
export function runProjectileImpactLabelAnchorDirectionIdentityAudit():ProjectileImpactLabelAnchorDirectionIdentityAudit{
  const samples:string[]=[];let passed=PROJECTILE_IMPACT_LABEL_ANCHOR_DIRECTION_COSINE_MIN>.6&&PROJECTILE_IMPACT_LABEL_ANCHOR_DIRECTION_COSINE_MIN<.95;
  for(let i=0;i<64;i++){
    ...
    const ok=similarCompatible&&!crossCompatible&&placed.pos.x===341+i&&memory.every((entry)=>entry.presentationOnly);passed&&=ok;samples.push(`${i}:${similarCompatible?1:0}:${crossCompatible?1:0}:${placed.pos.x.toFixed(1)}`);
  }
