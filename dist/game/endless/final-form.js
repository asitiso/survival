...
function scoreForm(form, selected) {
    let score = 0;
    selected.forEach((id, index) => {
        if (form.affinity.includes(id))
            score += index === selected.length - 1 ? 3 : 2;
    });
    return score;
}
export function deriveHeroFinalForm(heroId, selected, elapsedMs) {
    if (elapsedMs < 80 * 60_000 || selected.length < 3)
        return null;
...
export function finalFormModifiers(form) {
    const out = {
    ...
    if (!form)
        return out;
    switch (form.id) {
        case 'solar-sovereign':
            out.spellPowerMultiplier = 1.18;
            out.bossDamageMultiplier = 1.18;
            out.fusionPowerMultiplier = 1.08;
            break;
        case 'phoenix-lord':
            out.cooldownMultiplier = .88;
            out.heroDamageTakenMultiplier = .88;
            out.moveSpeedMultiplier = 1.08;
            break;
        case 'volcanic-archon':
            out.areaMultiplier = 1.18;
            out.fusionPowerMultiplier = 1.18;
            out.spellPowerMultiplier = 1.08;
            break;
        case 'absolute-empress':
            out.areaMultiplier = 1.2;
            out.spellPowerMultiplier = 1.15;
            out.bossDamageMultiplier = 1.1;
            break;
        case 'winter-warden':
            out.cooldownMultiplier = .86;
            out.coreDamageTakenMultiplier = .82;
            out.heroDamageTakenMultiplier = .92;
            break;
        case 'crystal-oracle':
            out.fusionPowerMultiplier = 1.18;
            out.areaMultiplier = 1.12;
            out.moveSpeedMultiplier = 1.06;
            break;
        case 'thunder-tyrant':
            out.spellPowerMultiplier = 1.18;
            out.bossDamageMultiplier = 1.2;
            break;
        case 'tempest-runner':
            out.cooldownMultiplier = .84;
            out.moveSpeedMultiplier = 1.12;
            out.areaMultiplier = 1.06;
            break;
        case 'storm-oracle':
            out.fusionPowerMultiplier = 1.16;
            out.cooldownMultiplier = .9;
            out.heroDamageTakenMultiplier = .9;
            break;
        case 'radiant-king':
            out.bossDamageMultiplier = 1.18;
            out.spellPowerMultiplier = 1.12;
            out.areaMultiplier = 1.08;
            break;
        case 'oath-guardian':
            out.heroDamageTakenMultiplier = .84;
            out.coreDamageTakenMultiplier = .8;
            out.fusionPowerMultiplier = 1.06;
            break;
        case 'light-pilgrim':
            out.moveSpeedMultiplier = 1.1;
            out.areaMultiplier = 1.16;
            out.heroDamageTakenMultiplier = .9;
            break;
    }
    return {
...
export function finalFormCatalog(heroId) { return FORMS[heroId]; }
