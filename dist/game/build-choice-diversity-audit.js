...
function diversityScore(sample, minute) {
...
export function buildChoiceDiversitySamples() {
    ...
    for (const hero of HERO_PROFILES)
        for (const threat of THREATS)
            for (const minute of MINUTES) {
                ...
                if (top.length < 24)
                    top = group.slice(0, 24).map((entry) => entry.sample);
...
export function auditBuildChoiceDiversity() {
